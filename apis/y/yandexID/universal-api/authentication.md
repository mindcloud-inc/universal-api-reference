# Yandex ID Universal API Authentication

Every Yandex ID API request through MindCloud uses a MindCloud API key and the `connectionId` of a Yandex ID connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Yandex ID connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Yandex ID action](README.md#actions-1) with your API key and `connectionId`.

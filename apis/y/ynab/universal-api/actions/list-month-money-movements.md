# YNAB: List Month Money Movements

Retrieves money movements for a month in YNAB.

```
GET https://connect.mindcloud.co/v1/universal/ynab/latest/actions/list-month-money-movements
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/ynab/latest/actions/list-month-money-movements?connectionId=$CONNECTION_ID&planId=string&month=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/ynab/latest/actions/list-month-money-movements?${params}`, {
  headers: {
...
Through the native YNAB API, this operation is `GET /plans/:planId/months/:month/money_movements` (base URL `https://api.ynab.com/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-month-money-movements.md) for the provider-specific parameters and requirements.

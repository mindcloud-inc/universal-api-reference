...
```
GET https://connect.mindcloud.co/v1/universal/ynab/latest/actions/get-category
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/ynab/latest/actions/get-category?connectionId=$CONNECTION_ID&planId=string&categoryId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/ynab/latest/actions/get-category?${params}`, {
  headers: {
...
Through the native YNAB API, this operation is `GET /plans/:planId/categories/:categoryId` (base URL `https://api.ynab.com/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-category.md) for the provider-specific parameters and requirements.

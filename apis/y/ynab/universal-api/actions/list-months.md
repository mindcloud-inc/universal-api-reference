...
Retrieves months from a YNAB plan.

```
GET https://connect.mindcloud.co/v1/universal/ynab/latest/actions/list-months
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/ynab/latest/actions/list-months?connectionId=$CONNECTION_ID&planId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/ynab/latest/actions/list-months?${params}`, {
  headers: {
...
| `month` | date | The month represented by this budget summary. |
...
Through the native YNAB API, this operation is `GET /plans/:planId/months` (base URL `https://api.ynab.com/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-months.md) for the provider-specific parameters and requirements.

# Yutori: Bulk Subscribe to Scout

Creates email subscriptions for a scout in Yutori.

```
POST https://connect.mindcloud.co/v1/universal/yutori/latest/actions/bulk-subscribe-to-scout
```
...
Through the native Yutori API, this operation is `POST /v1/scouting/tasks/:scout_id/subscribe/bulk` (base URL `https://api.yutori.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/bulk-subscribe-to-scout.md) for the provider-specific parameters and requirements.

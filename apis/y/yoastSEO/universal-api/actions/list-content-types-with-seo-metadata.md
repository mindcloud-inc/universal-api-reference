# Yoast SEO: List Content Types With SEO Metadata

Lists content types with Yoast SEO metadata.

```
GET https://connect.mindcloud.co/v1/universal/yoastSEO/latest/actions/list-content-types-with-seo-metadata
```
...
Through the native Yoast SEO API, this operation is `GET /wp/v2/types` (base URL `{{credentials.siteUrl}}/wp-json`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-content-types-with-seo-metadata.md) for the provider-specific parameters and requirements.

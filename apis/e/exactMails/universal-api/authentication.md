# Exact Mails Universal API Authentication

Every Exact Mails API request through MindCloud uses a MindCloud API key and the `connectionId` of a Exact Mails connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Exact Mails connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Exact Mails action](README.md#actions-1) with your API key and `connectionId`.

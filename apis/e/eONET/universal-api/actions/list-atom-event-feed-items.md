# EONET: List Atom Event Feed Items

Retrieves Atom event feed items from EONET.

```
GET https://connect.mindcloud.co/v1/universal/eONET/latest/actions/list-atom-event-feed-items
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/eONET/latest/actions/list-atom-event-feed-items?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/eONET/latest/actions/list-atom-event-feed-items?${params}`, {
  headers: {
...
Through the native EONET API, this operation is `GET /events/atom` (base URL `https://eonet.gsfc.nasa.gov/api/v3`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-atom-event-feed-items.md) for the provider-specific parameters and requirements.

# Email Verifier Api Universal API Authentication

Every Email Verifier Api API request through MindCloud uses a MindCloud API key and the `connectionId` of a Email Verifier Api connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Email Verifier Api connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Email Verifier Api action](README.md#actions-2) with your API key and `connectionId`.

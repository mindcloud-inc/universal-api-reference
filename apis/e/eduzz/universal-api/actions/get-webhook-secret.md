# Eduzz: Get Webhook Secret

Retrieves the webhook signing secret from Eduzz.

```
GET https://connect.mindcloud.co/v1/universal/eduzz/latest/actions/get-webhook-secret
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/eduzz/latest/actions/get-webhook-secret?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/eduzz/latest/actions/get-webhook-secret?${params}`, {
  headers: {

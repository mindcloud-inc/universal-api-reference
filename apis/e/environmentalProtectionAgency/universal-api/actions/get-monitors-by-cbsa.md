# Environmental Protection Agency: Get Monitors By CBSA

Retrieves monitors for a CBSA from EPA AQS.

```
GET https://connect.mindcloud.co/v1/universal/environmentalProtectionAgency/latest/actions/get-monitors-by-cbsa
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/environmentalProtectionAgency/latest/actions/get-monitors-by-cbsa?connectionId=$CONNECTION_ID&bdate=20170101&edate=20171231&cbsa=16740" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/environmentalProtectionAgency/latest/actions/get-monitors-by-cbsa?${params}`, {
  headers: {
...
Through the native Environmental Protection Agency API, this operation is `GET /monitors/byCBSA` (base URL `https://aqs.epa.gov/data/api`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-monitors-by-cbsa.md) for the provider-specific parameters and requirements.

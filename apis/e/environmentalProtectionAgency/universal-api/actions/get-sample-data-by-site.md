# Environmental Protection Agency: Get Sample Data By Site

Retrieves sample data for a site from EPA AQS.

```
GET https://connect.mindcloud.co/v1/universal/environmentalProtectionAgency/latest/actions/get-sample-data-by-site
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/environmentalProtectionAgency/latest/actions/get-sample-data-by-site?connectionId=$CONNECTION_ID&param=44201&bdate=20170101&edate=20171231&state=37&county=183&site=0014" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/environmentalProtectionAgency/latest/actions/get-sample-data-by-site?${params}`, {
  headers: {
...
Through the native Environmental Protection Agency API, this operation is `GET /sampleData/bySite` (base URL `https://aqs.epa.gov/data/api`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-sample-data-by-site.md) for the provider-specific parameters and requirements.

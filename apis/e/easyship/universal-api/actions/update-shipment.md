# Easyship: Update Shipment

Updates an existing shipment in Easyship.

```
PUT https://connect.mindcloud.co/v1/universal/easyship/latest/actions/update-shipment
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/easyship/latest/actions/update-shipment" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/easyship/latest/actions/update-shipment', {
  method: 'PUT',
  headers: {
...
| `parcels[]` | array<object> | no | Shipment parcels array. |
...
Through the native Easyship API, this operation is `PATCH /shipments/:shipment_id` (base URL `https://public-api.easyship.com/2024-09`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-shipment.md) for the provider-specific parameters and requirements.

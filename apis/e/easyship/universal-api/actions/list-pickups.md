# Easyship: List Pickups

Retrieves a list of pickups from Easyship.

```
GET https://connect.mindcloud.co/v1/universal/easyship/latest/actions/list-pickups
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/easyship/latest/actions/list-pickups?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/easyship/latest/actions/list-pickups?${params}`, {
  headers: {
...
Through the native Easyship API, this operation is `GET /pickups` (base URL `https://public-api.easyship.com/2024-09`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-pickups.md) for the provider-specific parameters and requirements.

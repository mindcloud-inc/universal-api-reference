# Encodian - Convert: Convert - PDF to Word

Creates a Word file from a PDF in Encodian - Convert.

```
POST https://connect.mindcloud.co/v1/universal/encodianConvert/latest/actions/convert-pdf-to-word
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/encodianConvert/latest/actions/convert-pdf-to-word" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/encodianConvert/latest/actions/convert-pdf-to-word', {
  method: 'POST',
  headers: {
...
| --- | --- | --- | --- |
| `fileName` | string | yes | The filename of the source PDF file. |
| `outputFilename` | string | yes | The filename of the output DOCX document. |
| `finalOperation` | boolean | yes | Return the processed file instead of just an Operation ID. |

# Encodian - Convert: Convert - File to PDF

Creates a PDF file from another file in Encodian - Convert.

```
POST https://connect.mindcloud.co/v1/universal/encodianConvert/latest/actions/convert-file-to-pdf
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/encodianConvert/latest/actions/convert-file-to-pdf" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/encodianConvert/latest/actions/convert-file-to-pdf', {
  method: 'POST',
  headers: {
...
| `fileContent` | file | yes | Base64-encoded file content of the source file. |

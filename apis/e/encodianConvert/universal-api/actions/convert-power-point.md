# Encodian - Convert: Convert - PowerPoint

Creates a converted file from a PowerPoint document in Encodian - Convert.

```
POST https://connect.mindcloud.co/v1/universal/encodianConvert/latest/actions/convert-power-point
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/encodianConvert/latest/actions/convert-power-point" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/encodianConvert/latest/actions/convert-power-point', {
  method: 'POST',
  headers: {
...
| `outputFormat` | string | yes | Select the output file type format. |
| `fileName` | string | yes | The filename of the source file, including extension. |
| `fileContent` | file | yes | The file content of the source file. |
...
Through the native Encodian - Convert API, this operation is `POST /api/v1/Conversion/ConvertPowerPoint` (base URL `https://api.apps-encodian.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/convert-power-point.md) for the provider-specific parameters and requirements.

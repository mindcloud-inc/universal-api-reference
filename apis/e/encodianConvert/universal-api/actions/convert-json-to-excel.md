# Encodian - Convert: Convert - JSON to Excel

Creates an Excel file from JSON in Encodian - Convert.

```
POST https://connect.mindcloud.co/v1/universal/encodianConvert/latest/actions/convert-json-to-excel
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/encodianConvert/latest/actions/convert-json-to-excel" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/encodianConvert/latest/actions/convert-json-to-excel', {
  method: 'POST',
  headers: {
...
| --- | --- | --- | --- |
| `outputFilename` | string | yes | The filename of the output Excel workbook. |
| `jsonData` | string | no | Raw JSON content to convert. |
| `fileContent` | file | no | JSON file content to convert. |

# Emailable Universal API Authentication

Every Emailable API request through MindCloud uses a MindCloud API key and the `connectionId` of a Emailable connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Emailable connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Emailable action](README.md#actions-4) with your API key and `connectionId`.

# Expiration Reminder Universal API Authentication

Every Expiration Reminder API request through MindCloud uses a MindCloud API key and the `connectionId` of a Expiration Reminder connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Expiration Reminder connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Expiration Reminder action](README.md#actions-40) with your API key and `connectionId`.

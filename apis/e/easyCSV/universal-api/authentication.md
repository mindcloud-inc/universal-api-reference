# EasyCSV Universal API Authentication

Every EasyCSV API request through MindCloud uses a MindCloud API key and the `connectionId` of a EasyCSV connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your EasyCSV connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [EasyCSV action](README.md#actions-2) with your API key and `connectionId`.

# ExpoFP Universal API Authentication

Every ExpoFP API request through MindCloud uses a MindCloud API key and the `connectionId` of a ExpoFP connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your ExpoFP connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [ExpoFP action](README.md#actions-1) with your API key and `connectionId`.

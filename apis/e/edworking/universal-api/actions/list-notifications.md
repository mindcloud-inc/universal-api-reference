# Edworking: List Notifications

Retrieves notifications from Edworking, optionally filtered by project.

```
GET https://connect.mindcloud.co/v1/universal/edworking/latest/actions/list-notifications
```

Authenticate with `Authorization: Bearer $MINDCLOUD_API_KEY` and pass a Edworking `connectionId` ([setup](../authentication.md)).
...
The response envelope is `{ "success": true, "data": [...], "meta": {} }`. The `data` schema for this action is dynamic; it mirrors what the native Edworking API returns.

## Native endpoint

Through the native Edworking API, this operation is `POST /` (base URL `https://gateway.edworking.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-notifications.md) for the provider-specific parameters and requirements.

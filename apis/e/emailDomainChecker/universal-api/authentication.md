# Email Domain Checker Universal API Authentication

Every Email Domain Checker API request through MindCloud uses a MindCloud API key and the `connectionId` of a Email Domain Checker connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Email Domain Checker connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Email Domain Checker action](README.md#actions-1) with your API key and `connectionId`.

# EARLY: List available Webhook Events

Retrieves available webhook events from EARLY.

```
GET https://connect.mindcloud.co/v1/universal/eARLY/latest/actions/list-available-webhook-events
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/eARLY/latest/actions/list-available-webhook-events?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/eARLY/latest/actions/list-available-webhook-events?${params}`, {
  headers: {

# Exchange Rates API Universal API Authentication

Every Exchange Rates API API request through MindCloud uses a MindCloud API key and the `connectionId` of a Exchange Rates API connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Exchange Rates API connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Exchange Rates API action](README.md#actions-6) with your API key and `connectionId`.

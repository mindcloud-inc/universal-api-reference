# Eversign: List Template Drafts

Retrieves draft templates from your Eversign account.

```
GET https://connect.mindcloud.co/v1/universal/eversign/latest/actions/list-template-drafts
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/eversign/latest/actions/list-template-drafts?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/eversign/latest/actions/list-template-drafts?${params}`, {
  headers: {
...
Through the native Eversign API, this operation is `GET /document` (base URL `https://api.eversign.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-template-drafts.md) for the provider-specific parameters and requirements.

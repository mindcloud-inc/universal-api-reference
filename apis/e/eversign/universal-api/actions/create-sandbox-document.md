# Eversign: Create Sandbox Document

Creates a sandbox document in Eversign.

```
POST https://connect.mindcloud.co/v1/universal/eversign/latest/actions/create-sandbox-document
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/eversign/latest/actions/create-sandbox-document" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/eversign/latest/actions/create-sandbox-document', {
  method: 'POST',
  headers: {
...
Through the native Eversign API, this operation is `POST /document` (base URL `https://api.eversign.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-sandbox-document.md) for the provider-specific parameters and requirements.

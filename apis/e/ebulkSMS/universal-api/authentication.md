# EbulkSMS Universal API Authentication

Every EbulkSMS API request through MindCloud uses a MindCloud API key and the `connectionId` of a EbulkSMS connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your EbulkSMS connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [EbulkSMS action](README.md#actions-5) with your API key and `connectionId`.

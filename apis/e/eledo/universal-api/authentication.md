# Eledo Universal API Authentication

Every Eledo API request through MindCloud uses a MindCloud API key and the `connectionId` of a Eledo connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Eledo connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Eledo action](README.md#actions-6) with your API key and `connectionId`.

# Everbill: Update Offer

Updates an existing offer in Everbill.

```
PUT https://connect.mindcloud.co/v1/universal/everbill/latest/actions/update-offer
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/everbill/latest/actions/update-offer" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/everbill/latest/actions/update-offer', {
  method: 'PUT',
  headers: {
...
Through the native Everbill API, this operation is `PUT /offers/update/:id` (base URL `https://api.everbill.eu`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-offer.md) for the provider-specific parameters and requirements.

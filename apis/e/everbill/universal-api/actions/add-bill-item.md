# Everbill: Add Bill Item

Creates a new bill item in Everbill.

```
POST https://connect.mindcloud.co/v1/universal/everbill/latest/actions/add-bill-item
```
...
Through the native Everbill API, this operation is `POST /bills/add_item/:id` (base URL `https://api.everbill.eu`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/add-bill-item.md) for the provider-specific parameters and requirements.

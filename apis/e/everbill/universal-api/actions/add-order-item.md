# Everbill: Add Order Item

Creates a new order item in Everbill.

```
POST https://connect.mindcloud.co/v1/universal/everbill/latest/actions/add-order-item
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/everbill/latest/actions/add-order-item" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/everbill/latest/actions/add-order-item', {
  method: 'POST',
  headers: {
...
Through the native Everbill API, this operation is `POST /orders/add_item/:id` (base URL `https://api.everbill.eu`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/add-order-item.md) for the provider-specific parameters and requirements.

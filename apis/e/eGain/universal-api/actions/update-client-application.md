# eGain: Update Client Application

Updates an existing client application in eGain.

```
PUT https://connect.mindcloud.co/v1/universal/eGain/latest/actions/update-client-application
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/eGain/latest/actions/update-client-application" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/eGain/latest/actions/update-client-application', {
  method: 'PUT',
  headers: {
...
Through the native eGain API, this operation is `PUT /clientapplications/:id` (base URL `https://api.ai.egain.cloud/conversation/conversationmgr/v3`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-client-application.md) for the provider-specific parameters and requirements.

# EasyContent: Change Content Item Assignees And Due Dates

Updates assignees or due dates for an EasyContent item.

```
PUT https://connect.mindcloud.co/v1/universal/easyContent/latest/actions/change-content-item-assignees-and-due-dates
```
...
Through the native EasyContent API, this operation is `POST /zapier/actions/create/change_item_assignees_and_due_dates` (base URL `https://easycontent.io/api`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/change-content-item-assignees-and-due-dates.md) for the provider-specific parameters and requirements.

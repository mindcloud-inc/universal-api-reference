# Energy Information Administration: Query AEO Data

Retrieves AEO dataset records from EIA.

```
GET https://connect.mindcloud.co/v1/universal/energyInformationAdministration/latest/actions/query-aeo-data
```
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/energyInformationAdministration/latest/actions/query-aeo-data?${params}`, {
  headers: {
...
Through the native Energy Information Administration API, this operation is `GET /aeo/{route1}/data` (base URL `https://api.eia.gov/v2`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/query-aeo-data.md) for the provider-specific parameters and requirements.

# Energy Information Administration: Query Total Energy Data

Retrieves total energy data from EIA.

```
GET https://connect.mindcloud.co/v1/universal/energyInformationAdministration/latest/actions/query-total-energy-data
```
...
Through the native Energy Information Administration API, this operation is `GET /total-energy/data` (base URL `https://api.eia.gov/v2`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/query-total-energy-data.md) for the provider-specific parameters and requirements.

# Energy Information Administration: Query STEO Data

Retrieves STEO dataset records from EIA.

```
GET https://connect.mindcloud.co/v1/universal/energyInformationAdministration/latest/actions/query-steo-data
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/energyInformationAdministration/latest/actions/query-steo-data?connectionId=$CONNECTION_ID&limit=25&offset=0" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/energyInformationAdministration/latest/actions/query-steo-data?${params}`, {
  headers: {
...
Through the native Energy Information Administration API, this operation is `GET /steo/data` (base URL `https://api.eia.gov/v2`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/query-steo-data.md) for the provider-specific parameters and requirements.

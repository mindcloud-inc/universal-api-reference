# Energy Information Administration: Query SEDS Data

Retrieves SEDS dataset records from EIA.

```
GET https://connect.mindcloud.co/v1/universal/energyInformationAdministration/latest/actions/query-seds-data
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/energyInformationAdministration/latest/actions/query-seds-data?connectionId=$CONNECTION_ID&limit=25&offset=0" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/energyInformationAdministration/latest/actions/query-seds-data?${params}`, {
  headers: {
...
Through the native Energy Information Administration API, this operation is `GET /seds/data` (base URL `https://api.eia.gov/v2`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/query-seds-data.md) for the provider-specific parameters and requirements.

# EDEN AI: Get Image Generation Info

Retrieves feature information for image generation in EDEN AI.

```
GET https://connect.mindcloud.co/v1/universal/eDENAI/latest/actions/get-image-generation-info
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/eDENAI/latest/actions/get-image-generation-info?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/eDENAI/latest/actions/get-image-generation-info?${params}`, {
  headers: {
...
| `inputSchema` | object | Input schema for the image generation action. |
...
| `outputSchema` | object | Output schema for image generation results. |
| `outputSchema.fields` | array<object> | Top-level output fields. |
| `subfeature` | string | Specific Eden AI subfeature key. |

## Native endpoint

Through the native EDEN AI API, this operation is `GET /info/image/generation` (base URL `https://api.edenai.run/v3`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-image-generation-info.md) for the provider-specific parameters and requirements.

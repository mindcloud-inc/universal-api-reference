# Eagle Doc: Process Invoice

Creates an invoice extraction in Eagle Doc.

```
POST https://connect.mindcloud.co/v1/universal/eagleDoc/latest/actions/process-invoice
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/eagleDoc/latest/actions/process-invoice" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/eagleDoc/latest/actions/process-invoice', {
  method: 'POST',
  headers: {
...
| --- | --- | --- | --- |
| `file` | file | yes | Invoice file to upload |
...
| `signature` | boolean | no | Extract signature data from the invoice |
...
Through the native Eagle Doc API, this operation is `POST /api/invoice/v1/processing` (base URL `https://de.eagle-doc.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/process-invoice.md) for the provider-specific parameters and requirements.

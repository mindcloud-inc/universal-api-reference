# Eagle Doc: Split Document

Creates document split segments in Eagle Doc.

```
POST https://connect.mindcloud.co/v1/universal/eagleDoc/latest/actions/split-document
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/eagleDoc/latest/actions/split-document" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/eagleDoc/latest/actions/split-document', {
  method: 'POST',
  headers: {
...
| --- | --- | --- | --- |
| `file` | file | yes | Document file to split |

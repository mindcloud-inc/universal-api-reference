# Ellipsend Universal API Authentication

Every Ellipsend API request through MindCloud uses a MindCloud API key and the `connectionId` of a Ellipsend connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Ellipsend connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Ellipsend action](README.md#actions-21) with your API key and `connectionId`.

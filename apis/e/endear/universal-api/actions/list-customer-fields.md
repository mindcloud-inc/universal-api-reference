# Endear: List Customer Fields



```
GET https://connect.mindcloud.co/v1/universal/endear/latest/actions/list-customer-fields
```
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/endear/latest/actions/list-customer-fields?${params}`, {
  headers: {
...
| `variables.isCustom` | boolean | no | Is Custom for the Endear GraphQL operation. |
...
Through the native Endear API, this operation is `POST /graphql` (base URL `https://api.endearhq.com`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/list-customer-fields.md) for the provider-specific parameters and requirements.

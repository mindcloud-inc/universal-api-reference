# Endear: List Messages



```
GET https://connect.mindcloud.co/v1/universal/endear/latest/actions/list-messages
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/endear/latest/actions/list-messages?connectionId=$CONNECTION_ID&limit=25&offset=0&variables.conversationId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/endear/latest/actions/list-messages?${params}`, {
  headers: {
...
| `variables.conversationId` | string | yes | Conversation Id for the Endear GraphQL operation. |

# Endear: Assign Users To Task



```
PUT https://connect.mindcloud.co/v1/universal/endear/latest/actions/assign-users-to-task
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/endear/latest/actions/assign-users-to-task" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/endear/latest/actions/assign-users-to-task', {
  method: 'PUT',
  headers: {
...
Through the native Endear API, this operation is `POST /graphql` (base URL `https://api.endearhq.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/assign-users-to-task.md) for the provider-specific parameters and requirements.

# EmojiHub: List Emojis By Group

Lists emojis in a selected EmojiHub group.

```
GET https://connect.mindcloud.co/v1/universal/emojiHub/latest/actions/list-emojis-by-group
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/emojiHub/latest/actions/list-emojis-by-group?connectionId=$CONNECTION_ID&group=activities" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/emojiHub/latest/actions/list-emojis-by-group?${params}`, {
  headers: {
...
Through the native EmojiHub API, this operation is `GET /all/group/:group` (base URL `https://emojihub.yurace.pro/api`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-emojis-by-group.md) for the provider-specific parameters and requirements.

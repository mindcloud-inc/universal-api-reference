# Engage: Add Customer to Accounts

Adds a customer to one or more accounts in Engage.

```
PUT https://connect.mindcloud.co/v1/universal/engage/latest/actions/add-customer-to-accounts
```
...
| --- | --- | --- | --- |
| `accounts[]` | array<object> | yes | Array of account objects with id and optional role. |
...
Through the native Engage API, this operation is `POST /users/:uid/accounts` (base URL `https://api.engage.so/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/add-customer-to-accounts.md) for the provider-specific parameters and requirements.

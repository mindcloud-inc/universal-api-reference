# Envoy for Visitors Universal API Authentication

Every Envoy for Visitors API request through MindCloud uses a MindCloud API key and the `connectionId` of a Envoy for Visitors connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Envoy for Visitors connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Envoy for Visitors action](README.md#actions-24) with your API key and `connectionId`.

# Escrow.com: Perform Milestone Item Action

Performs a milestone item action in Escrow.com.

```
PUT https://connect.mindcloud.co/v1/universal/escrowcom/latest/actions/perform-milestone-item-action
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/escrowcom/latest/actions/perform-milestone-item-action" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/escrowcom/latest/actions/perform-milestone-item-action', {
  method: 'PUT',
  headers: {
...
| `action` | string | yes | Milestone item action to perform. |
...
Through the native Escrow.com API, this operation is `PATCH /transaction/:transaction_id/item/:item_id` (base URL `https://api.escrow-sandbox.com/2017-09-01`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/perform-milestone-item-action.md) for the provider-specific parameters and requirements.

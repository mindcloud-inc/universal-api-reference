# echowin: Add Contact Assignments

Adds contact assignments in echowin.

```
POST https://connect.mindcloud.co/v1/universal/echowin/latest/actions/add-contact-assignments
```
...
Through the native echowin API, this operation is `POST /contacts/:contactId/assignments` (base URL `https://echo.win/api/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/add-contact-assignments.md) for the provider-specific parameters and requirements.

# ExpertTexting: Send SMS

Creates an SMS message in ExpertTexting.

```
POST https://connect.mindcloud.co/v1/universal/expertTexting/latest/actions/send-sms
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/expertTexting/latest/actions/send-sms" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/expertTexting/latest/actions/send-sms', {
  method: 'POST',
  headers: {
...
| `text` | string | yes | SMS body text, UTF-8 and URL-encoded by the platform. |

...
```
GET https://connect.mindcloud.co/v1/universal/evenium/latest/actions/list-guests
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/evenium/latest/actions/list-guests?connectionId=$CONNECTION_ID&limit=25&offset=0&eventId=1" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/evenium/latest/actions/list-guests?${params}`, {
  headers: {
...
Through the native Evenium API, this operation is `GET /events/:eventId/guests` (base URL `https://evenium.com/api/1`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/list-guests.md) for the provider-specific parameters and requirements.

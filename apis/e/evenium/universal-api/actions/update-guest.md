# Evenium: Update Guest

Updates an existing guest in Evenium.

```
PUT https://connect.mindcloud.co/v1/universal/evenium/latest/actions/update-guest
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/evenium/latest/actions/update-guest" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/evenium/latest/actions/update-guest', {
  method: 'PUT',
  headers: {
...
Through the native Evenium API, this operation is `PUT /events/:eventId/guests/:contactId` (base URL `https://evenium.com/api/1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-guest.md) for the provider-specific parameters and requirements.

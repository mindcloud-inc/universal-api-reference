# ElevenLabs: Stream Speech To Speech

Streams audio transformed between voices in ElevenLabs.

```
POST https://connect.mindcloud.co/v1/universal/elevenLabs/latest/actions/stream-speech-to-speech
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/elevenLabs/latest/actions/stream-speech-to-speech" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/elevenLabs/latest/actions/stream-speech-to-speech', {
  method: 'POST',
  headers: {
...
Through the native ElevenLabs API, this operation is `POST /speech-to-speech/:voice_id/stream` (base URL `https://api.elevenlabs.io/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/stream-speech-to-speech.md) for the provider-specific parameters and requirements.

# ElevenLabs: Stream Dialogue

Streams dialogue audio from text in ElevenLabs.

```
POST https://connect.mindcloud.co/v1/universal/elevenLabs/latest/actions/stream-dialogue
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/elevenLabs/latest/actions/stream-dialogue" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/elevenLabs/latest/actions/stream-dialogue', {
  method: 'POST',
  headers: {
...
Through the native ElevenLabs API, this operation is `POST /text-to-dialogue/stream` (base URL `https://api.elevenlabs.io/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/stream-dialogue.md) for the provider-specific parameters and requirements.

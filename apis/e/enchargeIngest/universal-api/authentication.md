# Encharge Ingest Universal API Authentication

Every Encharge Ingest API request through MindCloud uses a MindCloud API key and the `connectionId` of a Encharge Ingest connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Encharge Ingest connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Encharge Ingest action](README.md#actions-4) with your API key and `connectionId`.

# Email List Validation Universal API Authentication

Every Email List Validation API request through MindCloud uses a MindCloud API key and the `connectionId` of a Email List Validation connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Email List Validation connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Email List Validation action](README.md#actions-1) with your API key and `connectionId`.

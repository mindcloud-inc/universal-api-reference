# Every.org Universal API Authentication

Every Every.org API request through MindCloud uses a MindCloud API key and the `connectionId` of a Every.org connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Every.org connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Every.org action](README.md#actions-6) with your API key and `connectionId`.

# Evalandgo: List Respondent Responses

Retrieves responses for a respondent from Evalandgo.

```
GET https://connect.mindcloud.co/v1/universal/evalandgo/latest/actions/list-respondent-responses
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/evalandgo/latest/actions/list-respondent-responses?connectionId=$CONNECTION_ID&respondentId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/evalandgo/latest/actions/list-respondent-responses?${params}`, {
  headers: {
...
Through the native Evalandgo API, this operation is `GET /api/v3/respondents/:respondentId/responses` (base URL `https://app.evalandgo.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-respondent-responses.md) for the provider-specific parameters and requirements.

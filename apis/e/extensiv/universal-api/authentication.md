# Extensiv Order Manager Universal API Authentication

Every Extensiv Order Manager API request through MindCloud uses a MindCloud API key and the `connectionId` of a Extensiv Order Manager connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Extensiv Order Manager connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Extensiv Order Manager action](README.md#actions-8) with your API key and `connectionId`.

# easybits Extractor Universal API Authentication

Every easybits Extractor API request through MindCloud uses a MindCloud API key and the `connectionId` of a easybits Extractor connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your easybits Extractor connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [easybits Extractor action](README.md#actions-2) with your API key and `connectionId`.

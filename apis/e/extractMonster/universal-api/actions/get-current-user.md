# Extract Monster: Get Current User

Retrieves the current user from Extract Monster.

```
GET https://connect.mindcloud.co/v1/universal/extractMonster/latest/actions/get-current-user
```

Authenticate with `Authorization: Bearer $MINDCLOUD_API_KEY` and pass a Extract Monster `connectionId` ([setup](../authentication.md)).

## Example request

```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/extractMonster/latest/actions/get-current-user?connectionId=$CONNECTION_ID" \
  -H "Authorization: Bearer $MINDCLOUD_API_KEY"
```

```js
const params = new URLSearchParams({
  connectionId
});

const response = await fetch(`https://connect.mindcloud.co/v1/universal/extractMonster/latest/actions/get-current-user?${params}`, {
  headers: {
    Authorization: `Bearer ${process.env.MINDCLOUD_API_KEY}`
  }
});

const { success, data } = await response.json();
```



## Response

```json
{
  "success": true,
  "data": [
    {
      "authenticated": true,
      "tokenId": "string",
      "tokenName": "Ava Chen",
      "userId": "string"
    }
  ],
  "meta": {}
}
```

### Response fields

| Key | Type | Description |
| --- | --- | --- |
| `authenticated` | boolean | Whether the request was authenticated. |
| `tokenId` | string | API token identifier. |
| `tokenName` | string | Human-readable API token name. |
| `userId` | string | Authenticated Extract Monster user ID. |

## Native endpoint

Through the native Extract Monster API, this operation is `GET /me` (base URL `https://api.extract.monster`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-current-user.md) for the provider-specific parameters and requirements.


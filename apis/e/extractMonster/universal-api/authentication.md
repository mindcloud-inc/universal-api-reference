# Extract Monster Universal API Authentication

Every Extract Monster API request through MindCloud uses a MindCloud API key and the `connectionId` of a Extract Monster connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Extract Monster connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Extract Monster action](README.md#actions-6) with your API key and `connectionId`.

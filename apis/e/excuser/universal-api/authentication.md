# Excuser Universal API Authentication

Every Excuser API request through MindCloud uses a MindCloud API key and the `connectionId` of a Excuser connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Excuser connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Excuser action](README.md#actions-5) with your API key and `connectionId`.

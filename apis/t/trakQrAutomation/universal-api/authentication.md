# Trak Qr Automation Universal API Authentication

Every Trak Qr Automation API request through MindCloud uses a MindCloud API key and the `connectionId` of a Trak Qr Automation connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Trak Qr Automation connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Trak Qr Automation action](README.md#actions-4) with your API key and `connectionId`.

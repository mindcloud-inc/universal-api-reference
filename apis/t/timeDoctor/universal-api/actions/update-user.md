# Time Doctor: Update User

Updates an existing user in Time Doctor.

```
PUT https://connect.mindcloud.co/v1/universal/timeDoctor/latest/actions/update-user
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/timeDoctor/latest/actions/update-user" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/timeDoctor/latest/actions/update-user', {
  method: 'PUT',
  headers: {
...
| `name` | string | no | User alias in the company. |
| `role` | string | no | User role in the company. |

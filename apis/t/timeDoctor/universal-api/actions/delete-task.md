# Time Doctor: Delete Task

Archives or restores a task in Time Doctor.

```
DELETE https://connect.mindcloud.co/v1/universal/timeDoctor/latest/actions/delete-task
```
...
```bash
curl -X DELETE "https://connect.mindcloud.co/v1/universal/timeDoctor/latest/actions/delete-task?connectionId=$CONNECTION_ID&taskId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/timeDoctor/latest/actions/delete-task?${params}`, {
  headers: {

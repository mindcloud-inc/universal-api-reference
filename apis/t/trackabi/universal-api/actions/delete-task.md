# Trackabi: Delete Task

Deletes an existing task from Trackabi.

```
DELETE https://connect.mindcloud.co/v1/universal/trackabi/latest/actions/delete-task
```
...
```bash
curl -X DELETE "https://connect.mindcloud.co/v1/universal/trackabi/latest/actions/delete-task?connectionId=$CONNECTION_ID&taskId=1" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/trackabi/latest/actions/delete-task?${params}`, {
  headers: {
...
| `recursive` | string | no | Delete task with subtasks recursively. |

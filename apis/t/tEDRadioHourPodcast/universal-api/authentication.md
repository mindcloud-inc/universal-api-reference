# TED Radio Hour Podcast Universal API Authentication

Every TED Radio Hour Podcast API request through MindCloud uses a MindCloud API key and the `connectionId` of a TED Radio Hour Podcast connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your TED Radio Hour Podcast connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [TED Radio Hour Podcast action](README.md#actions-1) with your API key and `connectionId`.

...
```
PUT https://connect.mindcloud.co/v1/universal/twist/latest/actions/update-thread
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/twist/latest/actions/update-thread" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/twist/latest/actions/update-thread', {
  method: 'PUT',
  headers: {
...
Through the native Twist API, this operation is `POST /threads/update` (base URL `https://api.twist.com/api/v3`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-thread.md) for the provider-specific parameters and requirements.

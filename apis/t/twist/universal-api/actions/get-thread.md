...
Retrieves a thread from Twist by ID.

```
GET https://connect.mindcloud.co/v1/universal/twist/latest/actions/get-thread
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/twist/latest/actions/get-thread?connectionId=$CONNECTION_ID&threadId=1" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/twist/latest/actions/get-thread?${params}`, {
  headers: {
...
Through the native Twist API, this operation is `GET /threads/getone` (base URL `https://api.twist.com/api/v3`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-thread.md) for the provider-specific parameters and requirements.

...
Retrieves cards on a board from Trello.

```
GET https://connect.mindcloud.co/v1/universal/trello/latest/actions/get-cards-on-a-board
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/trello/latest/actions/get-cards-on-a-board?connectionId=$CONNECTION_ID&id=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/trello/latest/actions/get-cards-on-a-board?${params}`, {
  headers: {
...
Through the native Trello API, this operation is `GET boards/:id/cards` (base URL `https://api.trello.com/1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-cards-on-a-board.md) for the provider-specific parameters and requirements.

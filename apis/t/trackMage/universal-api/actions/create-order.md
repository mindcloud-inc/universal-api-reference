# TrackMage: Create Order

Creates a new order in TrackMage.

```
POST https://connect.mindcloud.co/v1/universal/trackMage/latest/actions/create-order
```
...
| --- | --- | --- | --- |
| `workspace` | string | yes | The workspace reference to which the order belongs. |
| `orderNumber` | string | yes | A number associated with a purchase order. |
| `orderType` | string | yes | customer\|vendor |
...
| `externalSourceSyncId` | string | no | The id of the order in ecommerce store (WooCommerce, Shopify, etc.). |
| `externalSourceUrl` | string | no | The url of the order in ecommerce store. |

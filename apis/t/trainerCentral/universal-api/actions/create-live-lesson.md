# TrainerCentral: Create Live Lesson

Creates a live lesson in TrainerCentral.

```
POST https://connect.mindcloud.co/v1/universal/trainerCentral/latest/actions/create-live-lesson
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/trainerCentral/latest/actions/create-live-lesson" \
  ...
  "session.scheduledTime": 1,
  "session.scheduledEndTime": 1,
  "session.timezone": "UTC"
}'
```

```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/trainerCentral/latest/actions/create-live-lesson', {
  method: 'POST',
  headers: {
...
| --- | --- | --- | --- |
| `session.name` | string | yes | The name of the live lesson. |
| `session.description` | string | no | Optional description for the live lesson. Default: `Live validation session created by MindCloud wizard`. |
| `session.courseId` | string | yes | The course under which the live lesson should be created. |
| `session.sectionId` | string | yes | The chapter under which the live lesson should be created. |
| `session.scheduledTime` | number | yes | Live lesson start time in milliseconds. |
| `session.scheduledEndTime` | number | yes | Live lesson end time in milliseconds. |
| `session.timezone` | string | yes | IANA timezone for the scheduled start and end times. Default: `UTC`. |
  ...
  "data": [
    {
      ...
      "sessionScheduledEndTime": "string",
...
Through the native TrainerCentral API, this operation is `POST /sessions.json` (base URL `{{credentials.academyUrl}}/api/v4/{{credentials.orgId}}`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-live-lesson.md) for the provider-specific parameters and requirements.

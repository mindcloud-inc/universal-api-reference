# TrainerCentral: Create Live Workshop

Creates a live workshop in TrainerCentral.

```
POST https://connect.mindcloud.co/v1/universal/trainerCentral/latest/actions/create-live-workshop
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/trainerCentral/latest/actions/create-live-workshop" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/trainerCentral/latest/actions/create-live-workshop', {
  method: 'POST',
  headers: {
...
| --- | --- | --- | --- |
| `session.name` | string | yes | The name of the live workshop. |
| `session.description` | string | no | Optional description for the live workshop. Default: `Standalone live workshop created by MindCloud wizard`. |
| `session.scheduledTime` | number | yes | Live workshop start time in milliseconds. |
| `session.scheduledEndTime` | number | yes | Live workshop end time in milliseconds. |
...
Through the native TrainerCentral API, this operation is `POST /sessions.json` (base URL `{{credentials.academyUrl}}/api/v4/{{credentials.orgId}}`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-live-workshop.md) for the provider-specific parameters and requirements.

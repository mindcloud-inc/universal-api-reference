# TicketSource: List Customer Bookings

Retrieves bookings for a customer from TicketSource.

```
GET https://connect.mindcloud.co/v1/universal/ticketSource/latest/actions/list-customer-bookings
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/ticketSource/latest/actions/list-customer-bookings?connectionId=$CONNECTION_ID&limit=25&offset=0&customerId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/ticketSource/latest/actions/list-customer-bookings?${params}`, {
  headers: {
...
Through the native TicketSource API, this operation is `GET /customers/{CustomerId}/bookings` (base URL `https://api.ticketsource.io`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/list-customer-bookings.md) for the provider-specific parameters and requirements.

# TicketSource: Get Booking

Retrieves a booking from the TicketSource account.

```
GET https://connect.mindcloud.co/v1/universal/ticketSource/latest/actions/get-booking
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/ticketSource/latest/actions/get-booking?connectionId=$CONNECTION_ID&bookingId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/ticketSource/latest/actions/get-booking?${params}`, {
  headers: {
...
Through the native TicketSource API, this operation is `GET /bookings/{BookingId}` (base URL `https://api.ticketsource.io`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-booking.md) for the provider-specific parameters and requirements.

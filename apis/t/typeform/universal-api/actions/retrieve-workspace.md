...
```
GET https://connect.mindcloud.co/v1/universal/typeform/latest/actions/retrieve-workspace
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/typeform/latest/actions/retrieve-workspace?connectionId=$CONNECTION_ID&workspaceId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/typeform/latest/actions/retrieve-workspace?${params}`, {
  headers: {
...
Through the native Typeform API, this operation is `GET /workspaces/:workspaceId` (base URL `https://api.typeform.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/retrieve-workspace.md) for the provider-specific parameters and requirements.

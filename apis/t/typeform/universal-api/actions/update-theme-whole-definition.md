# Typeform: Update Theme (Whole Definition)



```
PUT https://connect.mindcloud.co/v1/universal/typeform/latest/actions/update-theme-whole-definition
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/typeform/latest/actions/update-theme-whole-definition" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/typeform/latest/actions/update-theme-whole-definition', {
  method: 'PUT',
  headers: {
...
Through the native Typeform API, this operation is `PUT /themes/:themeId` (base URL `https://api.typeform.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-theme-whole-definition.md) for the provider-specific parameters and requirements.

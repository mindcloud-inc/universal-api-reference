# Typeform: Update Form (Patch)



```
PUT https://connect.mindcloud.co/v1/universal/typeform/latest/actions/update-form-patch
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/typeform/latest/actions/update-form-patch" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/typeform/latest/actions/update-form-patch', {
  method: 'PUT',
  headers: {
...
Through the native Typeform API, this operation is `PATCH /forms/:formId` (base URL `https://api.typeform.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-form-patch.md) for the provider-specific parameters and requirements.

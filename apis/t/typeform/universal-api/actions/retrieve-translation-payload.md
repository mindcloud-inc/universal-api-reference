# Typeform: Retrieve Translation Payload



```
GET https://connect.mindcloud.co/v1/universal/typeform/latest/actions/retrieve-translation-payload
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/typeform/latest/actions/retrieve-translation-payload?connectionId=$CONNECTION_ID&formId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/typeform/latest/actions/retrieve-translation-payload?${params}`, {
  headers: {
...
| --- | --- | --- |
| `fields` | array<object> | Main-language translation payload fields. |
...
| `messages` | object | Main-language translation payload messages. |
...
| `thankyouScreens` | array<object> | Main-language thank-you screens. |
...
| `welcomeScreens` | array<object> | Main-language welcome screens. |
...
Through the native Typeform API, this operation is `GET /forms/:formId/translations/main` (base URL `https://api.typeform.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/retrieve-translation-payload.md) for the provider-specific parameters and requirements.

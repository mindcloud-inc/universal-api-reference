...
```
POST https://connect.mindcloud.co/v1/universal/typeform/latest/actions/create-theme
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/typeform/latest/actions/create-theme" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/typeform/latest/actions/create-theme', {
  method: 'POST',
  headers: {
...
Through the native Typeform API, this operation is `POST /themes` (base URL `https://api.typeform.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-theme.md) for the provider-specific parameters and requirements.

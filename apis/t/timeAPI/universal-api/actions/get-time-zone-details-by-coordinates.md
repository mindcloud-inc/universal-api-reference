# TimeAPI: Get Time Zone Details By Coordinates

Retrieves time zone details by coordinates from TimeAPI.

```
GET https://connect.mindcloud.co/v1/universal/timeAPI/latest/actions/get-time-zone-details-by-coordinates
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/timeAPI/latest/actions/get-time-zone-details-by-coordinates?connectionId=$CONNECTION_ID&latitude=52.3667&longitude=4.8945" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/timeAPI/latest/actions/get-time-zone-details-by-coordinates?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `latitude` | number | yes | Latitude for timezone lookup. Example: `52.3667`. |
| `longitude` | number | yes | Longitude for timezone lookup. Example: `4.8945`. |
...
| `timezone` | string | Resolved IANA timezone name. |
| `unix_timestamp` | number | Current Unix timestamp in seconds. |
| `utc_time` | date | Current UTC time. |

## Native endpoint

Through the native TimeAPI API, this operation is `GET /api/v1/timezone/coordinate` (base URL `https://www.timeapi.io`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-time-zone-details-by-coordinates.md) for the provider-specific parameters and requirements.

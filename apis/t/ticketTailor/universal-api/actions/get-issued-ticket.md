# Ticket Tailor: Get Issued Ticket

Retrieves an issued ticket from Ticket Tailor.

```
GET https://connect.mindcloud.co/v1/universal/ticketTailor/latest/actions/get-issued-ticket
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/ticketTailor/latest/actions/get-issued-ticket?connectionId=$CONNECTION_ID&issuedTicketId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/ticketTailor/latest/actions/get-issued-ticket?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `issuedTicketId` | string | yes | Ticket Tailor issued ticket ID. |
...
Through the native Ticket Tailor API, this operation is `GET /v1/issued_tickets/:issued_ticket_id` (base URL `https://api.tickettailor.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-issued-ticket.md) for the provider-specific parameters and requirements.

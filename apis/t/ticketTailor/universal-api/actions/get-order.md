# Ticket Tailor: Get Order

Retrieves an order from Ticket Tailor.

```
GET https://connect.mindcloud.co/v1/universal/ticketTailor/latest/actions/get-order
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/ticketTailor/latest/actions/get-order?connectionId=$CONNECTION_ID&orderId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/ticketTailor/latest/actions/get-order?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `orderId` | string | yes | Ticket Tailor order ID. |

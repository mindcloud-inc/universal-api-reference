# Ticket Tailor: Get Event

Retrieves an event from Ticket Tailor.

```
GET https://connect.mindcloud.co/v1/universal/ticketTailor/latest/actions/get-event
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/ticketTailor/latest/actions/get-event?connectionId=$CONNECTION_ID&eventId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/ticketTailor/latest/actions/get-event?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `eventId` | string | yes | Ticket Tailor event ID. |

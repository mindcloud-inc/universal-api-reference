# Ticket Tailor: List Event Occurrences

Retrieves event occurrences for a Ticket Tailor event series.

```
GET https://connect.mindcloud.co/v1/universal/ticketTailor/latest/actions/list-event-occurrences
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/ticketTailor/latest/actions/list-event-occurrences?connectionId=$CONNECTION_ID&limit=25&offset=0&eventSeriesId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/ticketTailor/latest/actions/list-event-occurrences?${params}`, {
  headers: {
...
Through the native Ticket Tailor API, this operation is `GET /v1/event_series/:event_series_id/events` (base URL `https://api.tickettailor.com`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/list-event-occurrences.md) for the provider-specific parameters and requirements.

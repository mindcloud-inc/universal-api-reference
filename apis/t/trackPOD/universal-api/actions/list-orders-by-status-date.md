# Track-POD: List Orders By Status Date

Retrieves orders from Track-POD by status date using UTC time.

```
GET https://connect.mindcloud.co/v1/universal/trackPOD/latest/actions/list-orders-by-status-date
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/trackPOD/latest/actions/list-orders-by-status-date?connectionId=$CONNECTION_ID&date=2026-03-22T15%3A30%3A00Z" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/trackPOD/latest/actions/list-orders-by-status-date?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `date` | string | yes | UTC date and time used to fetch orders by status change. Example: `2026-03-22T15:30:00Z`. |
...
Through the native Track-POD API, this operation is `GET /Order/Status/Date/:date` (base URL `https://api.track-pod.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-orders-by-status-date.md) for the provider-specific parameters and requirements.

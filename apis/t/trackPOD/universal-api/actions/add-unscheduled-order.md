# Track-POD: Add Unscheduled Order

Creates a new unscheduled order in Track-POD.

```
POST https://connect.mindcloud.co/v1/universal/trackPOD/latest/actions/add-unscheduled-order
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/trackPOD/latest/actions/add-unscheduled-order" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/trackPOD/latest/actions/add-unscheduled-order', {
  method: 'POST',
  headers: {
...
| `updateGoodsPrice` | boolean | no | Force-update existing goods prices from the payload. Default: `false`. |
...
Through the native Track-POD API, this operation is `POST /Order` (base URL `https://api.track-pod.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/add-unscheduled-order.md) for the provider-specific parameters and requirements.

# Track-POD: List Orders By Date

Retrieves orders from Track-POD by date.

```
GET https://connect.mindcloud.co/v1/universal/trackPOD/latest/actions/list-orders-by-date
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/trackPOD/latest/actions/list-orders-by-date?connectionId=$CONNECTION_ID&date=2026-03-23T00%3A00%3A00Z" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/trackPOD/latest/actions/list-orders-by-date?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `date` | string | yes | Date and time used to fetch orders. Example: `2026-03-23T00:00:00Z`. |
...
Through the native Track-POD API, this operation is `GET /Order/Date/:date` (base URL `https://api.track-pod.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-orders-by-date.md) for the provider-specific parameters and requirements.

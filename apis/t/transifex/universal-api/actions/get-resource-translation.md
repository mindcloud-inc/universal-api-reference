# Transifex: Get Resource Translation



```
GET https://connect.mindcloud.co/v1/universal/transifex/latest/actions/get-resource-translation
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/transifex/latest/actions/get-resource-translation?connectionId=$CONNECTION_ID&resourceTranslationId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/transifex/latest/actions/get-resource-translation?${params}`, {
  headers: {
...
Through the native Transifex API, this operation is `GET /resource_translations/:resource_translation_id` (base URL `https://rest.api.transifex.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-resource-translation.md) for the provider-specific parameters and requirements.

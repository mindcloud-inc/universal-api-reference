# Timekit: Update Project

Updates an existing project in Timekit.

```
PUT https://connect.mindcloud.co/v1/universal/timekit/latest/actions/update-project
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/timekit/latest/actions/update-project" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/timekit/latest/actions/update-project', {
  method: 'PUT',
  headers: {
...
Through the native Timekit API, this operation is `PUT /projects/:id` (base URL `https://api.timekit.io/v2`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-project.md) for the provider-specific parameters and requirements.

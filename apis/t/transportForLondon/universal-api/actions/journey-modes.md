# Transport for London: List Journey Modes

Retrieves journey planner modes from Transport for London.

```
GET https://connect.mindcloud.co/v1/universal/transportForLondon/latest/actions/journey-modes
```
...
Through the native Transport for London API, this operation is `GET /Journey/Meta/Modes` (base URL `https://api.tfl.gov.uk`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/journey-modes.md) for the provider-specific parameters and requirements.

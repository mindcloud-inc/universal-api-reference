# Transport for London Universal API Authentication

Every Transport for London API request through MindCloud uses a MindCloud API key and the `connectionId` of a Transport for London connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Transport for London connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Transport for London action](README.md#actions-23) with your API key and `connectionId`.

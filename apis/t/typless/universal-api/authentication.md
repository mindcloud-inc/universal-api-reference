# Typless Universal API Authentication

Every Typless API request through MindCloud uses a MindCloud API key and the `connectionId` of a Typless connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Typless connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Typless action](README.md#actions-9) with your API key and `connectionId`.

# Transcript Downloader Universal API Authentication

Every Transcript Downloader API request through MindCloud uses a MindCloud API key and the `connectionId` of a Transcript Downloader connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Transcript Downloader connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Transcript Downloader action](README.md#actions-11) with your API key and `connectionId`.

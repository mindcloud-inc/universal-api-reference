# Timely: Stop Time Entry Timer

Stops the timer for an existing time entry in Timely.

```
PUT https://connect.mindcloud.co/v1/universal/timely/latest/actions/stop-time-entry-timer
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/timely/latest/actions/stop-time-entry-timer" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/timely/latest/actions/stop-time-entry-timer', {
  method: 'PUT',
  headers: {
...
Through the native Timely API, this operation is `PUT /1.1/{account_id}/hours/{id}/stop` (base URL `https://api.timelyapp.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/stop-time-entry-timer.md) for the provider-specific parameters and requirements.

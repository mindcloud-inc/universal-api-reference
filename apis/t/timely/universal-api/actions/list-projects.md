...
```
GET https://connect.mindcloud.co/v1/universal/timely/latest/actions/list-projects
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/timely/latest/actions/list-projects?connectionId=$CONNECTION_ID&accountId=1" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/timely/latest/actions/list-projects?${params}`, {
  headers: {
...
| Key | Type | Required | Description |
| --- | --- | --- | --- |
| `offset` | number | no | Retrieve projects from offset Default: `0`. |
| `limit` | number | no | Retrieve number of projects Default: `50`. |
| `order` | string | no | Sorting order - desc, asc (Default desc) Default: `desc`. |
| `filter` | string | no | Deprecated: Filter projects - mine, active, archived, all (Default mine, ignored if state or relation parameter present) |
| `state` | string | no | Filter projects - active, archived, all |
| `relation` | string | no | Filter projects - assigned, created, all |
| `updatedAfter` | string | no | Retrieve records updated after a certain timestamp |
| `projectIds[]` | array<number> | no | Retrieve specific projects |
| `externalIds[]` | array<number> | no | Retrieve specific projects by external ID reference |

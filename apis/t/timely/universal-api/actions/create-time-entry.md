# Timely: Create Time Entry

Creates a time entry in Timely.

```
POST https://connect.mindcloud.co/v1/universal/timely/latest/actions/create-time-entry
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/timely/latest/actions/create-time-entry" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/timely/latest/actions/create-time-entry', {
  method: 'POST',
  headers: {
...
| --- | --- | --- | --- |
| `accountId` | number | yes | Account ID where the time entry will be created |
| `event` | object | yes |  |

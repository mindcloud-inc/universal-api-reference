...
```
GET https://connect.mindcloud.co/v1/universal/timely/latest/actions/list-users
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/timely/latest/actions/list-users?connectionId=$CONNECTION_ID&accountId=1" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/timely/latest/actions/list-users?${params}`, {
  headers: {
...
| `filter` | string | no | Filter for deleted users |

# turboSMTP Universal API Authentication

Every turboSMTP API request through MindCloud uses a MindCloud API key and the `connectionId` of a turboSMTP connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your turboSMTP connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [turboSMTP action](README.md#actions-30) with your API key and `connectionId`.

# Tolq Universal API Authentication

Every Tolq API request through MindCloud uses a MindCloud API key and the `connectionId` of a Tolq connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Tolq connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Tolq action](README.md#actions-9) with your API key and `connectionId`.

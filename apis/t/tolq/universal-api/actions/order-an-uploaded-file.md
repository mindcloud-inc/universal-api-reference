# Tolq: Order an Uploaded File

Orders an uploaded file in Tolq.

```
POST https://connect.mindcloud.co/v1/universal/tolq/latest/actions/order-an-uploaded-file
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/tolq/latest/actions/order-an-uploaded-file" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/tolq/latest/actions/order-an-uploaded-file', {
  method: 'POST',
  headers: {
...
Through the native Tolq API, this operation is `POST /translations/requests/files/:uid` (base URL `https://api.tolq.com/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/order-an-uploaded-file.md) for the provider-specific parameters and requirements.

# TouchBasePro: Get Validation Credit Balance

Retrieves validation credit balance from TouchBasePro.

```
GET https://connect.mindcloud.co/v1/universal/touchBasePro/latest/actions/get-validation-credit-balance
```
...
Through the native TouchBasePro API, this operation is `GET /validate/balance` (base URL `https://api.touchbasepro.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-validation-credit-balance.md) for the provider-specific parameters and requirements.

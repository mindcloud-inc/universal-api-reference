# TextingHouse: Send Commercial SMS

Creates a commercial SMS in TextingHouse.

```
POST https://connect.mindcloud.co/v1/universal/textingHouse/latest/actions/send-commercial-sms
```
...
| `txt` | string | yes | SMS content to send. Example: `ACME: 20% off today. Reply STOP to opt out.`. |
...
Through the native TextingHouse API, this operation is `POST /do` (base URL `https://api.textinghouse.com/http/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/send-commercial-sms.md) for the provider-specific parameters and requirements.

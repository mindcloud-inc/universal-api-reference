# Timing Universal API Authentication

Every Timing API request through MindCloud uses a MindCloud API key and the `connectionId` of a Timing connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Timing connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Timing action](README.md#actions-20) with your API key and `connectionId`.

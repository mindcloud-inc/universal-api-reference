# Tallyfy: Update Process

Updates an existing process in Tallyfy.

```
PUT https://connect.mindcloud.co/v1/universal/tallyfy/latest/actions/update-process
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/tallyfy/latest/actions/update-process" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/tallyfy/latest/actions/update-process', {
  method: 'PUT',
  headers: {
...
Through the native Tallyfy API, this operation is `PUT /organizations/:org/runs/:run_id` (base URL `https://api.tallyfy.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-process.md) for the provider-specific parameters and requirements.

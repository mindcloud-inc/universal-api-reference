# Tallyfy: Reopen Process Task

Reopens a completed process task in Tallyfy.

```
PUT https://connect.mindcloud.co/v1/universal/tallyfy/latest/actions/reopen-process-task
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/tallyfy/latest/actions/reopen-process-task" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/tallyfy/latest/actions/reopen-process-task', {
  method: 'PUT',
  headers: {
...
| `task_id` | string | yes | Completed task ID from the selected process. |
...
Through the native Tallyfy API, this operation is `DELETE /organizations/:org/runs/:run_id/completed-tasks/:task_id` (base URL `https://api.tallyfy.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/reopen-process-task.md) for the provider-specific parameters and requirements.

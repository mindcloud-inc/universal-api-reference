# Tallyfy: Complete Process Task

Completes a process task in Tallyfy.

```
PUT https://connect.mindcloud.co/v1/universal/tallyfy/latest/actions/complete-process-task
```
...
Through the native Tallyfy API, this operation is `POST /organizations/:org/runs/:run_id/completed-tasks` (base URL `https://api.tallyfy.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/complete-process-task.md) for the provider-specific parameters and requirements.

# Tallyfy: Get Process Task

Retrieves a process task from Tallyfy.

```
GET https://connect.mindcloud.co/v1/universal/tallyfy/latest/actions/get-process-task
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/tallyfy/latest/actions/get-process-task?connectionId=$CONNECTION_ID&id=string&run_id=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/tallyfy/latest/actions/get-process-task?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `id` | string | yes | Task ID from the selected process. |
...
Through the native Tallyfy API, this operation is `GET /organizations/:org/runs/:run_id/tasks/:id` (base URL `https://api.tallyfy.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-process-task.md) for the provider-specific parameters and requirements.

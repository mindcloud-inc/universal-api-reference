# Typlog: Update Episode



```
PUT https://connect.mindcloud.co/v1/universal/typlog/latest/actions/update-episode
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/typlog/latest/actions/update-episode" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/typlog/latest/actions/update-episode', {
  method: 'PUT',
  headers: {
...
Through the native Typlog API, this operation is `PUT /episodes/[:id]` (base URL `https://api.typlog.com/v3`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-episode.md) for the provider-specific parameters and requirements.

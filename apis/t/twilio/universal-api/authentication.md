# Twilio Universal API Authentication

Every Twilio API request through MindCloud uses a MindCloud API key and the `connectionId` of a Twilio connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Twilio connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Twilio action](README.md#actions-21) with your API key and `connectionId`.

# Trove Universal API Authentication

Every Trove API request through MindCloud uses a MindCloud API key and the `connectionId` of a Trove connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Trove connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Trove action](README.md#actions-4) with your API key and `connectionId`.

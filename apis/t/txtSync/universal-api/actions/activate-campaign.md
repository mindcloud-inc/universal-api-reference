# TxtSync: Activate Campaign

Activates an existing campaign in TxtSync.

```
PUT https://connect.mindcloud.co/v1/universal/txtSync/latest/actions/activate-campaign
```
...
Through the native TxtSync API, this operation is `POST /campaigns/:id/activate` (base URL `https://api.txtsync.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/activate-campaign.md) for the provider-specific parameters and requirements.

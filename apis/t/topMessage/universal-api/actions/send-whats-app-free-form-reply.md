# TopMessage: Send WhatsApp Free-form Reply

Creates a WhatsApp free-form reply in TopMessage.

```
POST https://connect.mindcloud.co/v1/universal/topMessage/latest/actions/send-whats-app-free-form-reply
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/topMessage/latest/actions/send-whats-app-free-form-reply" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/topMessage/latest/actions/send-whats-app-free-form-reply', {
  method: 'POST',
  headers: {
...
| `text` | string | yes | The free-form WhatsApp reply content to send. |

# The Odds: Get Historical Event Odds

Retrieves historical odds for a specific event from The Odds API.

```
GET https://connect.mindcloud.co/v1/universal/theOddsAPI/latest/actions/get-historical-event-odds
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/theOddsAPI/latest/actions/get-historical-event-odds?connectionId=$CONNECTION_ID&date=string&eventId=string&regions=string&sport=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/theOddsAPI/latest/actions/get-historical-event-odds?${params}`, {
  headers: {
...
| `eventId` | string | yes | The event id returned by List Historical Events or the provider snapshot. |

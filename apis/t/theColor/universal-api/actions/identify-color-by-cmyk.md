# The Color: Identify Color By CMYK

Identifies a color in The Color by CMYK value.

```
GET https://connect.mindcloud.co/v1/universal/theColor/latest/actions/identify-color-by-cmyk
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/theColor/latest/actions/identify-color-by-cmyk?connectionId=$CONNECTION_ID&cmyk=100%2C58%2C0%2C33" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/theColor/latest/actions/identify-color-by-cmyk?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `cmyk` | string | yes | Valid CMYK color, such as 100,58,0,33 or cmyk(100,58,0,33). Default: `100,58,0,33`. |
...
Through the native The Color API, this operation is `GET /id` (base URL `https://www.thecolorapi.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/identify-color-by-cmyk.md) for the provider-specific parameters and requirements.

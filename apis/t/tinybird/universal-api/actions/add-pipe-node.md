# Tinybird: Add Pipe Node



```
POST https://connect.mindcloud.co/v1/universal/tinybird/latest/actions/add-pipe-node
```
...
| --- | --- | --- | --- |
| `description` | string | no | Optional node description |
| `name` | string | yes | The pipe name to target. |
| `nodeName` | string | no | New node name |
| `sql` | string | yes | SQL for the new transformation node |
...
Through the native Tinybird API, this operation is `POST v0/pipes/:name/nodes` (base URL `{{credentials.apiHost}}`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/add-pipe-node.md) for the provider-specific parameters and requirements.

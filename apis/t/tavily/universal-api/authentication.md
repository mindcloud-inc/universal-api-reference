# Tavily Universal API Authentication

Every Tavily API request through MindCloud uses a MindCloud API key and the `connectionId` of a Tavily connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Tavily connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Tavily action](README.md#actions-7) with your API key and `connectionId`.

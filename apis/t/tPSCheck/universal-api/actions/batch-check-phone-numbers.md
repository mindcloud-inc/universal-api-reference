# TPSCheck: Batch check phone numbers



```
GET https://connect.mindcloud.co/v1/universal/tPSCheck/latest/actions/batch-check-phone-numbers
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/tPSCheck/latest/actions/batch-check-phone-numbers?connectionId=$CONNECTION_ID&phones%5B%5D=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/tPSCheck/latest/actions/batch-check-phone-numbers?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `phones[]` | array<string> | yes | Array of UK phone numbers to verify. TPSCheck documents a maximum of 100 numbers per request on this endpoint. |
...
Through the native TPSCheck API, this operation is `POST /batch` (base URL `https://api.tpscheck.uk`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/batch-check-phone-numbers.md) for the provider-specific parameters and requirements.

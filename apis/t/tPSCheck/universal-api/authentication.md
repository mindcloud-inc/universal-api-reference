# TPSCheck Universal API Authentication

Every TPSCheck API request through MindCloud uses a MindCloud API key and the `connectionId` of a TPSCheck connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your TPSCheck connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [TPSCheck action](README.md#actions-4) with your API key and `connectionId`.

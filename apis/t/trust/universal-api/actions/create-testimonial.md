...
Creates a new testimonial in Trust.

```
POST https://connect.mindcloud.co/v1/universal/trust/latest/actions/create-testimonial
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/trust/latest/actions/create-testimonial" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/trust/latest/actions/create-testimonial', {
  method: 'POST',
  headers: {
...
Through the native Trust API, this operation is `POST /testimonial` (base URL `https://api.usetrust.app/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-testimonial.md) for the provider-specific parameters and requirements.

# timeBuzzer: List Tiles



```
GET https://connect.mindcloud.co/v1/universal/timeBuzzer/latest/actions/list-tiles
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/timeBuzzer/latest/actions/list-tiles?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/timeBuzzer/latest/actions/list-tiles?${params}`, {
  headers: {
...
Through the native timeBuzzer API, this operation is `GET /open-api/tiles` (base URL `https://my.timebuzzer.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-tiles.md) for the provider-specific parameters and requirements.

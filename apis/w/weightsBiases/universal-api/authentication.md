# Weights & Biases Universal API Authentication

Every Weights & Biases API request through MindCloud uses a MindCloud API key and the `connectionId` of a Weights & Biases connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Weights & Biases connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Weights & Biases action](README.md#actions-24) with your API key and `connectionId`.

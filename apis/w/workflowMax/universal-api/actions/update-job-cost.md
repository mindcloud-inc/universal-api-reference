# WorkflowMax: Update Job Cost



```
PUT https://connect.mindcloud.co/v1/universal/workflowMax/latest/actions/update-job-cost
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/workflowMax/latest/actions/update-job-cost" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/workflowMax/latest/actions/update-job-cost', {
  method: 'PUT',
  headers: {
...
| --- | --- | --- | --- |
| `jobCostId` | string | yes | The WorkflowMax job cost UUID. |
...
Through the native WorkflowMax API, this operation is `PUT v2/jobs/costs/{jobCostUUID}` (base URL `https://api.workflowmax.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-job-cost.md) for the provider-specific parameters and requirements.

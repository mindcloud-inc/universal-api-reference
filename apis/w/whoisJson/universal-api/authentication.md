# WhoisJson Universal API Authentication

Every WhoisJson API request through MindCloud uses a MindCloud API key and the `connectionId` of a WhoisJson connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your WhoisJson connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [WhoisJson action](README.md#actions-1) with your API key and `connectionId`.

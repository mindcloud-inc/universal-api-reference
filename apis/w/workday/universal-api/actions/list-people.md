...
```
GET https://connect.mindcloud.co/v1/universal/workday/latest/actions/list-people
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/workday/latest/actions/list-people?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/workday/latest/actions/list-people?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `universalId` | string | no | Optional universal ID filter for the person you want to retrieve. |
...
Through the native Workday API, this operation is `GET people` (base URL `{{credentials.restAPIBaseURL}}/`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-people.md) for the provider-specific parameters and requirements.

# Woodpecker.co: Add Campaign Step

Adds a campaign step in Woodpecker.

```
POST https://connect.mindcloud.co/v1/universal/woodpeckerco/latest/actions/add-campaign-step
```
...
Through the native Woodpecker.co API, this operation is `POST /rest/v2/campaigns/[:campaign_id]/steps` (base URL `https://api.woodpecker.co`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/add-campaign-step.md) for the provider-specific parameters and requirements.

# Weberlo Universal API Pagination

Paginated list actions accept `limit` and `offset` as query parameters. MindCloud translates them into whatever pagination model Weberlo expects, so the request shape stays the same even when the native API uses pages or cursors.
...
## Weberlo actions that support pagination

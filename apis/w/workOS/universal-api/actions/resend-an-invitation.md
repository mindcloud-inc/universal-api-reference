# WorkOS: Resend an invitation

Resends an invitation in your WorkOS environment.

```
POST https://connect.mindcloud.co/v1/universal/workOS/latest/actions/resend-an-invitation
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/workOS/latest/actions/resend-an-invitation" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/workOS/latest/actions/resend-an-invitation', {
  method: 'POST',
  headers: {
...
Through the native WorkOS API, this operation is `POST /user_management/invitations/{id}/resend` (base URL `https://api.workos.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/resend-an-invitation.md) for the provider-specific parameters and requirements.

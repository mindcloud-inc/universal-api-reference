# WorkOS: Create an Organization Domain

Creates an organization domain in your WorkOS environment.

```
POST https://connect.mindcloud.co/v1/universal/workOS/latest/actions/create-an-organization-domain
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/workOS/latest/actions/create-an-organization-domain" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/workOS/latest/actions/create-an-organization-domain', {
  method: 'POST',
  headers: {
...
Through the native WorkOS API, this operation is `POST /organization_domains` (base URL `https://api.workos.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-an-organization-domain.md) for the provider-specific parameters and requirements.

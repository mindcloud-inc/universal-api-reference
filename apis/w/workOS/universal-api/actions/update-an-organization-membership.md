# WorkOS: Update an organization membership

Updates an organization membership in your WorkOS environment.

```
PUT https://connect.mindcloud.co/v1/universal/workOS/latest/actions/update-an-organization-membership
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/workOS/latest/actions/update-an-organization-membership" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/workOS/latest/actions/update-an-organization-membership', {
  method: 'PUT',
  headers: {
...
Through the native WorkOS API, this operation is `PUT /user_management/organization_memberships/{id}` (base URL `https://api.workos.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-an-organization-membership.md) for the provider-specific parameters and requirements.

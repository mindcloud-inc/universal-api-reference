...
Retrieves users from your WorkOS environment.

```
GET https://connect.mindcloud.co/v1/universal/workOS/latest/actions/list-users
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/workOS/latest/actions/list-users?connectionId=$CONNECTION_ID&limit=25&offset=0" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/workOS/latest/actions/list-users?${params}`, {
  headers: {
...
| `organization_id` | string | no | Filter users by the organization they are a member of. |
| `email` | string | no | Filter users by their email address. |

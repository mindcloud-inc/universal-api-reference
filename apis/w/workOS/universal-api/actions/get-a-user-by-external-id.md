# WorkOS: Get a user by external ID

Retrieves a user by external ID from your WorkOS environment.

```
GET https://connect.mindcloud.co/v1/universal/workOS/latest/actions/get-a-user-by-external-id
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/workOS/latest/actions/get-a-user-by-external-id?connectionId=$CONNECTION_ID&external_id=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/workOS/latest/actions/get-a-user-by-external-id?${params}`, {
  headers: {
...
Through the native WorkOS API, this operation is `GET /user_management/users/external_id/{external_id}` (base URL `https://api.workos.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-a-user-by-external-id.md) for the provider-specific parameters and requirements.

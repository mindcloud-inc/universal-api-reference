# WorkOS: List sessions

Retrieves sessions from your WorkOS environment.

```
GET https://connect.mindcloud.co/v1/universal/workOS/latest/actions/list-sessions
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/workOS/latest/actions/list-sessions?connectionId=$CONNECTION_ID&limit=25&offset=0&id=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/workOS/latest/actions/list-sessions?${params}`, {
  headers: {
...
Through the native WorkOS API, this operation is `GET /user_management/users/{id}/sessions` (base URL `https://api.workos.com`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/list-sessions.md) for the provider-specific parameters and requirements.

# Weaviate Vector Store Universal API Authentication

Every Weaviate Vector Store API request through MindCloud uses a MindCloud API key and the `connectionId` of a Weaviate Vector Store connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Weaviate Vector Store connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Weaviate Vector Store action](README.md#actions-160) with your API key and `connectionId`.

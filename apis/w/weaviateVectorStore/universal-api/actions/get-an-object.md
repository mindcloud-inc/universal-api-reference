...
```
GET https://connect.mindcloud.co/v1/universal/weaviateVectorStore/latest/actions/get-an-object
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/weaviateVectorStore/latest/actions/get-an-object?connectionId=$CONNECTION_ID&classname=Ava%20Chen&id=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/weaviateVectorStore/latest/actions/get-an-object?${params}`, {
  headers: {
...
Through the native Weaviate Vector Store API, this operation is `GET /objects/:className/:id` (base URL `https://tl3apaxxsoiuwhpnsdv19a.c0.us-west3.gcp.weaviate.cloud`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-an-object.md) for the provider-specific parameters and requirements.

# Weaviate Vector Store: Activate a user

Activates a user in Weaviate.

```
PUT https://connect.mindcloud.co/v1/universal/weaviateVectorStore/latest/actions/activate-a-user
```
...
Through the native Weaviate Vector Store API, this operation is `POST /users/db/:user_id/activate` (base URL `https://tl3apaxxsoiuwhpnsdv19a.c0.us-west3.gcp.weaviate.cloud`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/activate-a-user.md) for the provider-specific parameters and requirements.

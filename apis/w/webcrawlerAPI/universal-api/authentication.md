# Webcrawler API Universal API Authentication

Every Webcrawler API API request through MindCloud uses a MindCloud API key and the `connectionId` of a Webcrawler API connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Webcrawler API connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Webcrawler API action](README.md#actions-20) with your API key and `connectionId`.

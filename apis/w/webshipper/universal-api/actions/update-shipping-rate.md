# Webshipper: Update Shipping Rate

Updates a shipping rate in Webshipper.

```
PUT https://connect.mindcloud.co/v1/universal/webshipper/latest/actions/update-shipping-rate
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/webshipper/latest/actions/update-shipping-rate" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/webshipper/latest/actions/update-shipping-rate', {
  method: 'PUT',
  headers: {
...
| `attributes.customMessage` | string |  |
...
Through the native Webshipper API, this operation is `PATCH /shipping_rates/:id` (base URL `https://{{credentials.accountName}}.api.webshipper.io/v2`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-shipping-rate.md) for the provider-specific parameters and requirements.

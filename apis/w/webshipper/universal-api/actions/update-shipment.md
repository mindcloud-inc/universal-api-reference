# Webshipper: Update Shipment



```
PUT https://connect.mindcloud.co/v1/universal/webshipper/latest/actions/update-shipment
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/webshipper/latest/actions/update-shipment" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/webshipper/latest/actions/update-shipment', {
  method: 'PUT',
  headers: {
...
Through the native Webshipper API, this operation is `PATCH /shipments/:id` (base URL `https://{{credentials.accountName}}.api.webshipper.io/v2`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-shipment.md) for the provider-specific parameters and requirements.

# Webshipper: Get Order Channel

Retrieves an order channel from Webshipper.

```
GET https://connect.mindcloud.co/v1/universal/webshipper/latest/actions/get-order-channel
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/webshipper/latest/actions/get-order-channel?connectionId=$CONNECTION_ID&id=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/webshipper/latest/actions/get-order-channel?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `id` | string | yes | Order channel ID. |
...
Through the native Webshipper API, this operation is `GET /order_channels/:id` (base URL `https://{{credentials.accountName}}.api.webshipper.io/v2`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-order-channel.md) for the provider-specific parameters and requirements.

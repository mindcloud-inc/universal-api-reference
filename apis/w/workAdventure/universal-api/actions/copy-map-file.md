# WorkAdventure: Copy map file

Copies a file in WorkAdventure map storage.

```
POST https://connect.mindcloud.co/v1/universal/workAdventure/latest/actions/copy-map-file
```
...
| --- | --- | --- | --- |
| `source` | string | yes | Existing map-storage file path to copy from. |
| `destination` | string | yes | New map-storage file path to copy to. |
...
Through the native WorkAdventure API, this operation is `POST https://mindcloud-34294.map-storage.workadventu.re/copy` (base URL `https://admin.workadventu.re`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/copy-map-file.md) for the provider-specific parameters and requirements.

...
```
PUT https://connect.mindcloud.co/v1/universal/webex/latest/actions/update-meeting
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/webex/latest/actions/update-meeting" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/webex/latest/actions/update-meeting', {
  method: 'PUT',
  headers: {
...
| `title` | string | no | Updated meeting title. Example: `Updated MindCloud Meeting`. |
| `start` | date | no | Updated meeting start time in ISO-8601 format. Example: `2026-04-20T16:00:00Z`. |
| `end` | date | no | Updated meeting end time in ISO-8601 format. Example: `2026-04-20T16:30:00Z`. |
...
Through the native Webex API, this operation is `PUT /meetings/:meetingId` (base URL `https://webexapis.com/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-meeting.md) for the provider-specific parameters and requirements.

...
Retrieves a specific meeting from Webex.

```
GET https://connect.mindcloud.co/v1/universal/webex/latest/actions/get-meeting
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/webex/latest/actions/get-meeting?connectionId=$CONNECTION_ID&meetingId=123456789" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/webex/latest/actions/get-meeting?${params}`, {
  headers: {
...
Through the native Webex API, this operation is `GET /meetings/:meetingId` (base URL `https://webexapis.com/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-meeting.md) for the provider-specific parameters and requirements.

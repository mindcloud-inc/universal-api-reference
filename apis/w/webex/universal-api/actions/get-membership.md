...
Retrieves a specific membership from Webex.

```
GET https://connect.mindcloud.co/v1/universal/webex/latest/actions/get-membership
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/webex/latest/actions/get-membership?connectionId=$CONNECTION_ID&membershipId=Y2lzY29zcGFyazovL3VzL01FTUJFUlNISVAv..." \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/webex/latest/actions/get-membership?${params}`, {
  headers: {
...
Through the native Webex API, this operation is `GET /memberships/:membershipId` (base URL `https://webexapis.com/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-membership.md) for the provider-specific parameters and requirements.

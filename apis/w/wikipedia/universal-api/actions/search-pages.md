...
```
GET https://connect.mindcloud.co/v1/universal/wikipedia/latest/actions/search-pages
```
...
Through the native Wikipedia API, this operation is `GET /w/api.php?action=query&list=search&format=json&formatversion=2` (base URL `https://en.wikipedia.org`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/search-pages.md) for the provider-specific parameters and requirements.

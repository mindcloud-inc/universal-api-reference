# Walmart Universal API Authentication

Every Walmart API request through MindCloud uses a MindCloud API key and the `connectionId` of a Walmart connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Walmart connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Walmart action](README.md#actions-90) with your API key and `connectionId`.

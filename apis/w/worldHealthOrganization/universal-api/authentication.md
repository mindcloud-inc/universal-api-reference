# World Health Organization Universal API Authentication

Every World Health Organization API request through MindCloud uses a MindCloud API key and the `connectionId` of a World Health Organization connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your World Health Organization connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [World Health Organization action](README.md#actions-16) with your API key and `connectionId`.

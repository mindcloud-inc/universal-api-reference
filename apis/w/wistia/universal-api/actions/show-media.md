...
Retrieves a media item from Wistia.

```
GET https://connect.mindcloud.co/v1/universal/wistia/latest/actions/show-media
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/wistia/latest/actions/show-media?connectionId=$CONNECTION_ID&mediaHashedId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/wistia/latest/actions/show-media?${params}`, {
  headers: {
...
Through the native Wistia API, this operation is `GET /modern/medias/:mediaHashedId` (base URL `https://api.wistia.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/show-media.md) for the provider-specific parameters and requirements.

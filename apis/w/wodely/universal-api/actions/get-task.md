...
```
GET https://connect.mindcloud.co/v1/universal/wodely/latest/actions/get-task
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/wodely/latest/actions/get-task?connectionId=$CONNECTION_ID&taskGuid=your-task-guid" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/wodely/latest/actions/get-task?${params}`, {
  headers: {
...
Through the native Wodely API, this operation is `POST /v2/tasks/get` (base URL `https://api.wodely.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-task.md) for the provider-specific parameters and requirements.

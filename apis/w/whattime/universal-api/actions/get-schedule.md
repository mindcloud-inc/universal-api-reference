# Whattime: Get Schedule



```
GET https://connect.mindcloud.co/v1/universal/whattime/latest/actions/get-schedule
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/whattime/latest/actions/get-schedule?connectionId=$CONNECTION_ID&code=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/whattime/latest/actions/get-schedule?${params}`, {
  headers: {
...
Through the native Whattime API, this operation is `GET /schedules/:code` (base URL `https://api.whattime.co.kr/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-schedule.md) for the provider-specific parameters and requirements.

# Whattime: Reschedule Schedule



```
PUT https://connect.mindcloud.co/v1/universal/whattime/latest/actions/reschedule-schedule
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/whattime/latest/actions/reschedule-schedule" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/whattime/latest/actions/reschedule-schedule', {
  method: 'PUT',
  headers: {
...
| `rescheduleReason` | string | no | 변경 사유 |
...
Through the native Whattime API, this operation is `POST /schedules/:code/reschedule` (base URL `https://api.whattime.co.kr/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/reschedule-schedule.md) for the provider-specific parameters and requirements.

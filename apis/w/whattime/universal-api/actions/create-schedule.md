# Whattime: Create Schedule



```
POST https://connect.mindcloud.co/v1/universal/whattime/latest/actions/create-schedule
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/whattime/latest/actions/create-schedule" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/whattime/latest/actions/create-schedule', {
  method: 'POST',
  headers: {
...
| --- | --- | --- | --- |
| `calendar` | string | no | Calendar uri |
| `date` | date | yes | 변경 날짜 |
| `time` | string | yes | 변경 시간 |
| `timeZone` | string | no | 타임존 |
| `name` | string | yes | 이름 |
| `email` | string | no | 이메일 |
| `phone` | string | no | 전화번호, 국가번호 포함되어도 무관 |
| `questions[]` | array<object> | no | 설문 답변 |
...
Through the native Whattime API, this operation is `POST /schedules` (base URL `https://api.whattime.co.kr/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-schedule.md) for the provider-specific parameters and requirements.

# Whattime: Cancel Schedule



```
PUT https://connect.mindcloud.co/v1/universal/whattime/latest/actions/cancel-schedule
```
...
| `cancelReason` | string | no | 취소 사유 |
| `cancelGuestAlarm` | boolean | no | 게스트에게 알림을 보낼지 여부 |
...
Through the native Whattime API, this operation is `POST /schedules/:code/cancel` (base URL `https://api.whattime.co.kr/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/cancel-schedule.md) for the provider-specific parameters and requirements.

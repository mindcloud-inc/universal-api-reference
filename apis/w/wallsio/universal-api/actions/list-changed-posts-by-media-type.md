# Walls.io: List Changed Posts By Media Type

Finds updated posts in Walls.io by media type.

```
GET https://connect.mindcloud.co/v1/universal/wallsio/latest/actions/list-changed-posts-by-media-type
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/wallsio/latest/actions/list-changed-posts-by-media-type?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/wallsio/latest/actions/list-changed-posts-by-media-type?${params}`, {
  headers: {

# Walls.io: List Changed Posts By Type

Finds updated posts in Walls.io by post type.

```
GET https://connect.mindcloud.co/v1/universal/wallsio/latest/actions/list-changed-posts-by-type
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/wallsio/latest/actions/list-changed-posts-by-type?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/wallsio/latest/actions/list-changed-posts-by-type?${params}`, {
  headers: {
...
| `types` | string | no | Comma-separated list of post types to include. |

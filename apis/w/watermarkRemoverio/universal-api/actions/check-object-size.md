# WatermarkRemover.io: Check Object Size

Checks object size in a file with WatermarkRemover.io.

```
GET https://connect.mindcloud.co/v1/universal/watermarkRemoverio/latest/actions/check-object-size
```
...
Through the native WatermarkRemover.io API, this operation is `GET /v2/[:cloudName]/[:zone]/cos.detect()/[:filePath]` (base URL `https://cdn.pixelbin.io`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/check-object-size.md) for the provider-specific parameters and requirements.

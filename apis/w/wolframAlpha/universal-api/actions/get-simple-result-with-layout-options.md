# Wolfram Alpha: Get Simple Result with Layout Options

Retrieves a simple Wolfram Alpha result image with layout options.

```
GET https://connect.mindcloud.co/v1/universal/wolframAlpha/latest/actions/get-simple-result-with-layout-options
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/wolframAlpha/latest/actions/get-simple-result-with-layout-options?connectionId=$CONNECTION_ID&i=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/wolframAlpha/latest/actions/get-simple-result-with-layout-options?${params}`, {
  headers: {

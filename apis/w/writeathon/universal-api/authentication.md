# Writeathon Universal API Authentication

Every Writeathon API request through MindCloud uses a MindCloud API key and the `connectionId` of a Writeathon connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Writeathon connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Writeathon action](README.md#actions-11) with your API key and `connectionId`.

# White Swan: Start Personal Plan Request

Starts a personal plan request in White Swan.

```
POST https://connect.mindcloud.co/v1/universal/whiteSwan/latest/actions/start-personal-plan-request
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/whiteSwan/latest/actions/start-personal-plan-request" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/whiteSwan/latest/actions/start-personal-plan-request', {
  method: 'POST',
  headers: {
...
| `paidUpPeriod` | string | no | Paid-up period for permanent policy requests. |
| `termDuration` | string | no | Requested term duration. |
| `premiumBudget` | number | no | Monthly or scheduled premium budget. |
...
| `associatedPerson` | string | no | White Swan user email to associate with the request. |
| `requestComment` | string | no | Internal comment to attach to the request. |
...
Through the native White Swan API, this operation is `POST /new_request` (base URL `https://app.whiteswan.io/api/1.1/wf`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/start-personal-plan-request.md) for the provider-specific parameters and requirements.

# Where the ISS at Universal API Authentication

Every Where the ISS at API request through MindCloud uses a MindCloud API key and the `connectionId` of a Where the ISS at connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Where the ISS at connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Where the ISS at action](README.md#actions-5) with your API key and `connectionId`.

# WakaTime Universal API Authentication

Every WakaTime API request through MindCloud uses a MindCloud API key and the `connectionId` of a WakaTime connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your WakaTime connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [WakaTime action](README.md#actions-87) with your API key and `connectionId`.

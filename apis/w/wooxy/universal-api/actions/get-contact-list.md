# Wooxy: Get Contact List

Retrieves a contact list from Wooxy.

```
GET https://connect.mindcloud.co/v1/universal/wooxy/latest/actions/get-contact-list
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/wooxy/latest/actions/get-contact-list?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/wooxy/latest/actions/get-contact-list?${params}`, {
  headers: {

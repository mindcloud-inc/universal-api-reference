# Worksnaps: Get users (i.e., all user assignments) in a project

Retrieves user assignments from a Worksnaps project.

```
GET https://connect.mindcloud.co/v1/universal/worksnaps/latest/actions/get-users-ie-all-user-assignments-in-a-project
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/worksnaps/latest/actions/get-users-ie-all-user-assignments-in-a-project?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/worksnaps/latest/actions/get-users-ie-all-user-assignments-in-a-project?${params}`, {
  headers: {
...
Through the native Worksnaps API, this operation is `GET /projects/{project_id}/user_assignments.xml` (base URL `https://api.worksnaps.com/api`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-users-ie-all-user-assignments-in-a-project.md) for the provider-specific parameters and requirements.

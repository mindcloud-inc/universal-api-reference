# Humanitix Universal API Authentication

Every Humanitix API request through MindCloud uses a MindCloud API key and the `connectionId` of a Humanitix connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Humanitix connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Humanitix action](README.md#actions-4) with your API key and `connectionId`.

# Happy SMS: Search Messages



```
GET https://connect.mindcloud.co/v1/universal/happySMS/latest/actions/search-messages
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/happySMS/latest/actions/search-messages?connectionId=$CONNECTION_ID&fullText=%2B687999999" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/happySMS/latest/actions/search-messages?${params}`, {
  headers: {
...
| `fullText` | string | yes | Required text to search across messages. Default: `+687999999`. |

### Advanced

| Key | Type | Required | Description |
| --- | --- | --- | --- |
| `sort` | string | no | Sort expression such as lastStatusDate;DESC. |
...
Through the native Happy SMS API, this operation is `GET /api/v1/protected/domain/sms/messages/lookup` (base URL `https://www.api.nc`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/search-messages.md) for the provider-specific parameters and requirements.

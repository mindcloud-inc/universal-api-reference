# Happy SMS: List Messages



```
GET https://connect.mindcloud.co/v1/universal/happySMS/latest/actions/list-messages
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/happySMS/latest/actions/list-messages?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/happySMS/latest/actions/list-messages?${params}`, {
  headers: {
...
| `queryFilter` | string | no | RSQL filter expression for narrowing messages. |
...
Through the native Happy SMS API, this operation is `GET /api/v1/protected/domain/sms/messages` (base URL `https://www.api.nc`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-messages.md) for the provider-specific parameters and requirements.

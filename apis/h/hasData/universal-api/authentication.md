# HasData Universal API Authentication

Every HasData API request through MindCloud uses a MindCloud API key and the `connectionId` of a HasData connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your HasData connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [HasData action](README.md#actions-26) with your API key and `connectionId`.

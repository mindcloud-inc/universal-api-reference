# HasData: Search Google SERP

Retrieves Google SERP results from HasData.

```
GET https://connect.mindcloud.co/v1/universal/hasData/latest/actions/search-google-serp
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/hasData/latest/actions/search-google-serp?connectionId=$CONNECTION_ID&q=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/hasData/latest/actions/search-google-serp?${params}`, {
  headers: {

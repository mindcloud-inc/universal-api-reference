# HITL Platform Universal API Authentication

Every HITL Platform API request through MindCloud uses a MindCloud API key and the `connectionId` of a HITL Platform connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your HITL Platform connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [HITL Platform action](README.md#actions-14) with your API key and `connectionId`.

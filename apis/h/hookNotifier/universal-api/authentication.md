# Hook.Notifier Universal API Authentication

Every Hook.Notifier API request through MindCloud uses a MindCloud API key and the `connectionId` of a Hook.Notifier connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Hook.Notifier connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Hook.Notifier action](README.md#actions-2) with your API key and `connectionId`.

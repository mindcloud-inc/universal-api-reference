# Hebcal: List Candle-lighting Calendar Events

Retrieves candle-lighting calendar events from Hebcal.

```
GET https://connect.mindcloud.co/v1/universal/hebcal/latest/actions/list-candle-lighting-calendar-events
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/hebcal/latest/actions/list-candle-lighting-calendar-events?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/hebcal/latest/actions/list-candle-lighting-calendar-events?${params}`, {
  headers: {
...
Through the native Hebcal API, this operation is `GET /hebcal` (base URL `https://www.hebcal.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-candle-lighting-calendar-events.md) for the provider-specific parameters and requirements.

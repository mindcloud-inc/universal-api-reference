# Hebcal: Check Assur Melacha Now

Checks whether work is forbidden now in Hebcal.

```
GET https://connect.mindcloud.co/v1/universal/hebcal/latest/actions/check-assur-melacha-now
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/hebcal/latest/actions/check-assur-melacha-now?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/hebcal/latest/actions/check-assur-melacha-now?${params}`, {
  headers: {
...
Through the native Hebcal API, this operation is `GET /zmanim` (base URL `https://www.hebcal.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/check-assur-melacha-now.md) for the provider-specific parameters and requirements.

# Hamsa: Clone Voice Agent

Clones a voice agent in Hamsa.

```
POST https://connect.mindcloud.co/v1/universal/hamsa/latest/actions/clone-voice-agent
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/hamsa/latest/actions/clone-voice-agent" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/hamsa/latest/actions/clone-voice-agent', {
  method: 'POST',
  headers: {
...
Through the native Hamsa API, this operation is `POST /v1/voice-agents/clone` (base URL `https://api.tryhamsa.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/clone-voice-agent.md) for the provider-specific parameters and requirements.

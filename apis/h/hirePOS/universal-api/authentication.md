# HirePOS Universal API Authentication

Every HirePOS API request through MindCloud uses a MindCloud API key and the `connectionId` of a HirePOS connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your HirePOS connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [HirePOS action](README.md#actions-13) with your API key and `connectionId`.

# Habitica: Add Checklist Item

Adds a checklist item to a Habitica task.

```
POST https://connect.mindcloud.co/v1/universal/habitica/latest/actions/add-checklist-item
```
...
Through the native Habitica API, this operation is `POST /tasks/:taskId/checklist` (base URL `https://habitica.com/api/v3`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/add-checklist-item.md) for the provider-specific parameters and requirements.

# HappyFox: Update Ticket Properties

Updates ticket properties in HappyFox.

```
PUT https://connect.mindcloud.co/v1/universal/happyFox/latest/actions/update-ticket-properties
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/happyFox/latest/actions/update-ticket-properties" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/happyFox/latest/actions/update-ticket-properties', {
  method: 'PUT',
  headers: {
...
| `staff` | number | yes |  |
| `priority` | number | no | HappyFox priority ID for the ticket. |
| `assignee` | number | no | Optional HappyFox staff user ID to assign the ticket to. |
| `status` | number | no | Optional HappyFox status ID for the ticket. |
...
Through the native HappyFox API, this operation is `POST /ticket/:ticket_number/staff_update/` (base URL `https://{{credentials.accountDomain}}/api/1.1/json`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-ticket-properties.md) for the provider-specific parameters and requirements.

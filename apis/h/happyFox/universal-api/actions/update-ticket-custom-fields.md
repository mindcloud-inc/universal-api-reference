# HappyFox: Update Ticket Custom Fields

Updates a ticket's custom fields in HappyFox.

```
PUT https://connect.mindcloud.co/v1/universal/happyFox/latest/actions/update-ticket-custom-fields
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/happyFox/latest/actions/update-ticket-custom-fields" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/happyFox/latest/actions/update-ticket-custom-fields', {
  method: 'PUT',
  headers: {
...
Through the native HappyFox API, this operation is `POST /ticket/:ticket_number/update_custom_fields/` (base URL `https://{{credentials.accountDomain}}/api/1.1/json`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-ticket-custom-fields.md) for the provider-specific parameters and requirements.

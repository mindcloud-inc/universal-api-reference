# HappyFox: Update Ticket Tags

Updates a ticket's tags in HappyFox.

```
PUT https://connect.mindcloud.co/v1/universal/happyFox/latest/actions/update-ticket-tags
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/happyFox/latest/actions/update-ticket-tags" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/happyFox/latest/actions/update-ticket-tags', {
  method: 'PUT',
  headers: {
...
| `staffId` | number | yes |  |
| `add` | string | no | Comma-separated ticket tags. |
| `remove` | string | no |  |
...
Through the native HappyFox API, this operation is `POST /ticket/:ticket_number/update_tags/` (base URL `https://{{credentials.accountDomain}}/api/1.1/json`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-ticket-tags.md) for the provider-specific parameters and requirements.

# Hippo Video: Get Viewer Profile by Video

Retrieves viewer profiles for a Hippo Video video.

```
GET https://connect.mindcloud.co/v1/universal/hippoVideo/latest/actions/get-viewer-profile-by-video
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/hippoVideo/latest/actions/get-viewer-profile-by-video?connectionId=$CONNECTION_ID&videoId=1" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/hippoVideo/latest/actions/get-viewer-profile-by-video?${params}`, {
  headers: {

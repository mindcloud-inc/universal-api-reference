# Harvestr.io: List Message Feedback



```
GET https://connect.mindcloud.co/v1/universal/harvestr/latest/actions/list-message-feedback
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/harvestr/latest/actions/list-message-feedback?connectionId=$CONNECTION_ID&limit=25&offset=0&id=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/harvestr/latest/actions/list-message-feedback?${params}`, {
  headers: {
...
| `discoveryId` | string | no | Filter feedback by discovery ID |
...
Through the native Harvestr.io API, this operation is `GET /message/{id}/feedback` (base URL `https://rest.harvestr.io/v1`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/list-message-feedback.md) for the provider-specific parameters and requirements.

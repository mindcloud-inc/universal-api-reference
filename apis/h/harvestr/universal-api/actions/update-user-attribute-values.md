# Harvestr.io: Update User Attribute Values



```
PUT https://connect.mindcloud.co/v1/universal/harvestr/latest/actions/update-user-attribute-values
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/harvestr/latest/actions/update-user-attribute-values" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/harvestr/latest/actions/update-user-attribute-values', {
  method: 'PUT',
  headers: {
...
| `attributeId` | string | yes | ID of the attribute to retrieve for the user |
...
Through the native Harvestr.io API, this operation is `PATCH /user/{id}/attribute/{attributeId}` (base URL `https://rest.harvestr.io/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-user-attribute-values.md) for the provider-specific parameters and requirements.

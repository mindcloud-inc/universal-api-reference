# Harvestr.io: List User Attribute Values



```
GET https://connect.mindcloud.co/v1/universal/harvestr/latest/actions/list-user-attribute-values
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/harvestr/latest/actions/list-user-attribute-values?connectionId=$CONNECTION_ID&limit=25&offset=0&id=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/harvestr/latest/actions/list-user-attribute-values?${params}`, {
  headers: {
...
Through the native Harvestr.io API, this operation is `GET /user/{id}/attribute-value` (base URL `https://rest.harvestr.io/v1`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/list-user-attribute-values.md) for the provider-specific parameters and requirements.

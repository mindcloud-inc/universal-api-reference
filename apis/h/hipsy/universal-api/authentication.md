# Hipsy Universal API Authentication

Every Hipsy API request through MindCloud uses a MindCloud API key and the `connectionId` of a Hipsy connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Hipsy connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Hipsy action](README.md#actions-3) with your API key and `connectionId`.

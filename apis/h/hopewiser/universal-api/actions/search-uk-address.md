# Hopewiser: Search UK Address



```
GET https://connect.mindcloud.co/v1/universal/hopewiser/latest/actions/search-uk-address
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/hopewiser/latest/actions/search-uk-address?connectionId=$CONNECTION_ID&maf=uk-rm-paf-mr&query=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/hopewiser/latest/actions/search-uk-address?${params}`, {
  headers: {
...
| `query` | string | yes | Address search criteria, postcode, partial address, or a Hopewiser Sid returned by a previous lookup. |
...
Through the native Hopewiser API, this operation is `GET /atlaslive/json/:maf` (base URL `https://cloud.hopewiser.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/search-uk-address.md) for the provider-specific parameters and requirements.

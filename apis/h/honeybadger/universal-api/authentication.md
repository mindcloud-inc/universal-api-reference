# Honeybadger Universal API Authentication

Every Honeybadger API request through MindCloud uses a MindCloud API key and the `connectionId` of a Honeybadger connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Honeybadger connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Honeybadger action](README.md#actions-7) with your API key and `connectionId`.

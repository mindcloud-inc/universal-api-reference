# HiDrive: List Home Directory

Retrieves the home directory from HiDrive.

```
GET https://connect.mindcloud.co/v1/universal/hidrive/latest/actions/list-home-directory
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/hidrive/latest/actions/list-home-directory?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/hidrive/latest/actions/list-home-directory?${params}`, {
  headers: {
...
Through the native HiDrive API, this operation is `GET /dir/home` (base URL `https://api.hidrive.strato.com/2.1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-home-directory.md) for the provider-specific parameters and requirements.

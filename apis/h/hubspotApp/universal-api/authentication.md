# HubSpot Universal API Authentication

Every HubSpot API request through MindCloud uses a MindCloud API key and the `connectionId` of a HubSpot connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your HubSpot connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [HubSpot action](README.md#actions-93) with your API key and `connectionId`.

# HubSpot: Get Shipping Method

Retrieves the shipping method property from HubSpot.

```
GET https://connect.mindcloud.co/v1/universal/hubspotApp/latest/actions/get-shipping-method
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/hubspotApp/latest/actions/get-shipping-method?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/hubspotApp/latest/actions/get-shipping-method?${params}`, {
  headers: {
...
Through the native HubSpot API, this operation is `GET crm/v3/properties/deals/shipping_method` (base URL `https://api.hubapi.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-shipping-method.md) for the provider-specific parameters and requirements.

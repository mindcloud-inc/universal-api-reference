# Hex Universal API Authentication

Every Hex API request through MindCloud uses a MindCloud API key and the `connectionId` of a Hex connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Hex connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Hex action](README.md#actions-19) with your API key and `connectionId`.

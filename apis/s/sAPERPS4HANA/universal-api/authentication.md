# SAP ERP (S/4HANA) Universal API Authentication

Every SAP ERP (S/4HANA) API request through MindCloud uses a MindCloud API key and the `connectionId` of a SAP ERP (S/4HANA) connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your SAP ERP (S/4HANA) connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [SAP ERP (S/4HANA) action](README.md#actions-40) with your API key and `connectionId`.

...
```
GET https://connect.mindcloud.co/v1/universal/shareFile/latest/actions/get-user
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/shareFile/latest/actions/get-user?connectionId=$CONNECTION_ID&id=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/shareFile/latest/actions/get-user?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `id` | string | yes | The ShareFile user identifier. |
...
Through the native ShareFile API, this operation is `GET /Users({{id}})` (base URL `https://{{credentials.accessTokenRequest.subdomain}}.{{credentials.accessTokenRequest.apicp}}/sf/v3`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-user.md) for the provider-specific parameters and requirements.

# SendApp Universal API Authentication

Every SendApp API request through MindCloud uses a MindCloud API key and the `connectionId` of a SendApp connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your SendApp connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [SendApp action](README.md#actions-5) with your API key and `connectionId`.

# Status Hero Universal API Authentication

Every Status Hero API request through MindCloud uses a MindCloud API key and the `connectionId` of a Status Hero connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Status Hero connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Status Hero action](README.md#actions-16) with your API key and `connectionId`.

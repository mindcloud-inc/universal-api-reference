# Samply Universal API Authentication

Every Samply API request through MindCloud uses a MindCloud API key and the `connectionId` of a Samply connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Samply connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Samply action](README.md#actions-34) with your API key and `connectionId`.

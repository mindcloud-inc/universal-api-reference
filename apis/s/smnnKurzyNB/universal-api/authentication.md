# Směnné kurzy ČNB Universal API Authentication

Every Směnné kurzy ČNB API request through MindCloud uses a MindCloud API key and the `connectionId` of a Směnné kurzy ČNB connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Směnné kurzy ČNB connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Směnné kurzy ČNB action](README.md#actions-22) with your API key and `connectionId`.

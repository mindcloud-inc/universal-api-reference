# Sellty Universal API Authentication

Every Sellty API request through MindCloud uses a MindCloud API key and the `connectionId` of a Sellty connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Sellty connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Sellty action](README.md#actions-10) with your API key and `connectionId`.

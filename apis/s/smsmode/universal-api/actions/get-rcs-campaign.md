# smsmode: Get RCS Campaign



```
GET https://connect.mindcloud.co/v1/universal/smsmode/latest/actions/get-rcs-campaign
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/smsmode/latest/actions/get-rcs-campaign?connectionId=$CONNECTION_ID&channelId=string&campaignId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/smsmode/latest/actions/get-rcs-campaign?${params}`, {
  headers: {
...
Through the native smsmode API, this operation is `GET rcs/v1/channels/:channelId/campaigns/:campaignId` (base URL `https://rest.smsmode.com/`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-rcs-campaign.md) for the provider-specific parameters and requirements.

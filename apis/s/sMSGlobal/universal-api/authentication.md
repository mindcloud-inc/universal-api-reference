# SMSGlobal Universal API Authentication

Every SMSGlobal API request through MindCloud uses a MindCloud API key and the `connectionId` of a SMSGlobal connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your SMSGlobal connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [SMSGlobal action](README.md#actions-17) with your API key and `connectionId`.

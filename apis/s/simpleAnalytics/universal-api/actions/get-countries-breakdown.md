# Simple Analytics: Get Countries Breakdown

Retrieves a country breakdown from Simple Analytics.

```
GET https://connect.mindcloud.co/v1/universal/simpleAnalytics/latest/actions/get-countries-breakdown
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/simpleAnalytics/latest/actions/get-countries-breakdown?connectionId=$CONNECTION_ID&hostname=Ava%20Chen" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/simpleAnalytics/latest/actions/get-countries-breakdown?${params}`, {
  headers: {
...
| --- | --- | --- |
| `countries` | array<object> | Top countries with aggregated pageviews and visitors. |
...
| `ok` | boolean | Whether the countries breakdown response is valid. |
...
Through the native Simple Analytics API, this operation is `GET /{{hostname}}.json` (base URL `https://simpleanalytics.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-countries-breakdown.md) for the provider-specific parameters and requirements.

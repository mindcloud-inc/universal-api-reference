# Simple Analytics: Get Referrers Breakdown

Retrieves a referrer breakdown from Simple Analytics.

```
GET https://connect.mindcloud.co/v1/universal/simpleAnalytics/latest/actions/get-referrers-breakdown
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/simpleAnalytics/latest/actions/get-referrers-breakdown?connectionId=$CONNECTION_ID&hostname=Ava%20Chen" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/simpleAnalytics/latest/actions/get-referrers-breakdown?${params}`, {
  headers: {
...
| `ok` | boolean | Whether the referrers breakdown response is valid. |
| `path` | string | Path segment covered by the report. |
| `referrers` | array<object> | Top referrers with aggregated pageviews and visitors. |
...
Through the native Simple Analytics API, this operation is `GET /{{hostname}}.json` (base URL `https://simpleanalytics.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-referrers-breakdown.md) for the provider-specific parameters and requirements.

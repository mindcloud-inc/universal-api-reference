# Streak: Get multiple boxes

Retrieves multiple boxes from a Streak pipeline.

```
GET https://connect.mindcloud.co/v1/universal/streak/latest/actions/get-multiple-boxes
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/streak/latest/actions/get-multiple-boxes?connectionId=$CONNECTION_ID&pipelineKey=string&boxKeys%5B%5D=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/streak/latest/actions/get-multiple-boxes?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `pipelineKey` | list<string> | yes | The key of the pipeline containing the boxes to retrieve. |
| `boxKeys[]` | array<string> | yes | The box keys to retrieve. |
...
Through the native Streak API, this operation is `POST /api/v1/pipelines/:pipelineKey/boxes/batch/` (base URL `https://api.streak.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-multiple-boxes.md) for the provider-specific parameters and requirements.

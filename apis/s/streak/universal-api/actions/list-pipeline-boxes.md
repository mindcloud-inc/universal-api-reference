# Streak: List Pipeline Boxes

Retrieves boxes from a Streak pipeline.

```
GET https://connect.mindcloud.co/v1/universal/streak/latest/actions/list-pipeline-boxes
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/streak/latest/actions/list-pipeline-boxes?connectionId=$CONNECTION_ID&pipelineKey=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/streak/latest/actions/list-pipeline-boxes?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `pipelineKey` | list<string> | yes | The key of the pipeline whose boxes to list. |

### Advanced

| Key | Type | Required | Description |
| --- | --- | --- | --- |
| `sortBy` | list<string> | no | What order to sort the boxes by. Valid values are creationTimestamp and lastUpdatedTimestamp. One of: `creationTimestamp`, `lastUpdatedTimestamp`. |
| `stageKey` | string | no | Stage to search. Returns boxes in only the specified stage. |
...
Through the native Streak API, this operation is `GET /api/v1/pipelines/:pipelineKey/boxes` (base URL `https://api.streak.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-pipeline-boxes.md) for the provider-specific parameters and requirements.

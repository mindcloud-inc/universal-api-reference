...
```
GET https://connect.mindcloud.co/v1/universal/smugMug/latest/actions/get-user
```
...
Through the native SmugMug API, this operation is `GET /user/:nickname` (base URL `https://api.smugmug.com/api/v2`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-user.md) for the provider-specific parameters and requirements.

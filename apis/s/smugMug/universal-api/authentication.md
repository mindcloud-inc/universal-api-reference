# SmugMug Universal API Authentication

Every SmugMug API request through MindCloud uses a MindCloud API key and the `connectionId` of a SmugMug connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your SmugMug connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [SmugMug action](README.md#actions-20) with your API key and `connectionId`.

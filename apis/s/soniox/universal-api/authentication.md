# Soniox Universal API Authentication

Every Soniox API request through MindCloud uses a MindCloud API key and the `connectionId` of a Soniox connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Soniox connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Soniox action](README.md#actions-9) with your API key and `connectionId`.

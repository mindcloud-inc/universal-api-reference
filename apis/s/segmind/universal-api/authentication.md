# Segmind Universal API Authentication

Every Segmind API request through MindCloud uses a MindCloud API key and the `connectionId` of a Segmind connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Segmind connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Segmind action](README.md#actions-130) with your API key and `connectionId`.

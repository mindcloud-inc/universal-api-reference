# Stable Diffusion: Outpaint Image

Outpaints an image in Stable Diffusion.

```
POST https://connect.mindcloud.co/v1/universal/stableDiffusion/latest/actions/outpaint-image
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/stableDiffusion/latest/actions/outpaint-image" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/stableDiffusion/latest/actions/outpaint-image', {
  method: 'POST',
  headers: {
...
| --- | --- | --- | --- |
| `image` | string | yes | Source image to extend. |

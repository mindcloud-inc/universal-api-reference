# screenshot.fyi Universal API Authentication

Every screenshot.fyi API request through MindCloud uses a MindCloud API key and the `connectionId` of a screenshot.fyi connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your screenshot.fyi connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [screenshot.fyi action](README.md#actions-1) with your API key and `connectionId`.

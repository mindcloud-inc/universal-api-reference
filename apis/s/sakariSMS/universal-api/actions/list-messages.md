# Sakari SMS: List Messages

Retrieves account messages from Sakari SMS.

```
GET https://connect.mindcloud.co/v1/universal/sakariSMS/latest/actions/list-messages
```
...
| --- | --- | --- | --- |
| `contactId` | string | no | ID of contact |
| `conversationId` | string | no | ID of conversation |
...
Through the native Sakari SMS API, this operation is `GET /v1/accounts/:accountId/messages` (base URL `https://api.sakari.io`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-messages.md) for the provider-specific parameters and requirements.

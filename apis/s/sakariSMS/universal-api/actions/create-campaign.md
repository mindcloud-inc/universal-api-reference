# Sakari SMS: Create Campaign

Creates a new campaign in Sakari SMS.

```
POST https://connect.mindcloud.co/v1/universal/sakariSMS/latest/actions/create-campaign
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/sakariSMS/latest/actions/create-campaign" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/sakariSMS/latest/actions/create-campaign', {
  method: 'POST',
  headers: {
...
| `name` | string | no |  |
| `trigger` | object | no |  |
| `trigger.code` | string | no | Campaign type specifies how it sources contacts and what event triggers its execution Sort order * `M` - Manual * `S` - Scheduled * `FU` - File Upload * `V2` - V2 |
| `filters` | object | no |  |
| `filters.contacts[]` | array<string> | no |  |
| `filters.tags[]` | array<string> | no |  |
| `filters.attributes[]` | array<string> | no |  |
| `template` | string | no |  |
| `reporting` | object | no |  |
| `reporting.when` | string | no |  |
...
Through the native Sakari SMS API, this operation is `POST /v1/accounts/:accountId/campaigns` (base URL `https://api.sakari.io`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-campaign.md) for the provider-specific parameters and requirements.

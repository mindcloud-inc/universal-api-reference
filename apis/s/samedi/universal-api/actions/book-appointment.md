# Samedi: Book Appointment

Books an appointment in Samedi.

```
POST https://connect.mindcloud.co/v1/universal/samedi/latest/actions/book-appointment
```
...
| `allowEmailNotifications` | boolean | no | Consent flag for email notifications. |
| `allowSmsNotifications` | boolean | no | Consent flag for SMS notifications. |
| `ref` | string | no | Optional alphanumeric source identifier for the booking. |
...
Through the native Samedi API, this operation is `POST /booking/v3/book` (base URL `https://patient.samedi.de/api`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/book-appointment.md) for the provider-specific parameters and requirements.

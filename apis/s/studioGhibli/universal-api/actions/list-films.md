# Studio Ghibli: List Films



```
GET https://connect.mindcloud.co/v1/universal/studioGhibli/latest/actions/list-films
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/studioGhibli/latest/actions/list-films?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/studioGhibli/latest/actions/list-films?${params}`, {
  headers: {
...
Through the native Studio Ghibli API, this operation is `GET /films` (base URL `https://ghibliapi.vercel.app`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-films.md) for the provider-specific parameters and requirements.

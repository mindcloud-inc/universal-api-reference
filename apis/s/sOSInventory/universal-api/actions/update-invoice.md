# SOS Inventory: Update Invoice

Updates an existing invoice in SOS Inventory.

```
PUT https://connect.mindcloud.co/v1/universal/sOSInventory/latest/actions/update-invoice
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/sOSInventory/latest/actions/update-invoice" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/sOSInventory/latest/actions/update-invoice', {
  method: 'PUT',
  headers: {
...
| --- | --- | --- | --- |
| `id` | number | yes | Invoice ID in the request path. |
| `number` | string | yes | Current invoice number. |

# SOS Inventory: List Purchase Orders

Retrieves purchase orders from SOS Inventory.

```
GET https://connect.mindcloud.co/v1/universal/sOSInventory/latest/actions/list-purchase-orders
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/sOSInventory/latest/actions/list-purchase-orders?connectionId=$CONNECTION_ID&limit=25&offset=0" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/sOSInventory/latest/actions/list-purchase-orders?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `status` | string | no | Filter by open or closed purchase orders. |
| `query` | string | no | Match order number, comment, vendor name, or customer name. |
| `archived` | string | no | Use yes for archived only or no for active only. |
| `from` | string | no | Beginning transaction date in YYYY-MM-DDTHH:MM:SS format. |
| `to` | string | no | Ending transaction date in YYYY-MM-DDTHH:MM:SS format. |
| `createdsince` | string | no | Return transactions created since this timestamp. |
| `updatedsince` | string | no | Return transactions updated since this timestamp. |

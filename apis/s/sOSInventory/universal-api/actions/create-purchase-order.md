# SOS Inventory: Create Purchase Order

Creates a purchase order in SOS Inventory.

```
POST https://connect.mindcloud.co/v1/universal/sOSInventory/latest/actions/create-purchase-order
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/sOSInventory/latest/actions/create-purchase-order" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/sOSInventory/latest/actions/create-purchase-order', {
  method: 'POST',
  headers: {
...
| --- | --- | --- | --- |
| `number` | string | no | Purchase order number. Use `auto` for SOS automatic numbering. Default: `auto`. |

# Svix Universal API Authentication

Every Svix API request through MindCloud uses a MindCloud API key and the `connectionId` of a Svix connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Svix connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Svix action](README.md#actions-128) with your API key and `connectionId`.

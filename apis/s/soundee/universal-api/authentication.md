# Soundee Universal API Authentication

Every Soundee API request through MindCloud uses a MindCloud API key and the `connectionId` of a Soundee connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Soundee connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Soundee action](README.md#actions-10) with your API key and `connectionId`.

# Speech is Cheap Universal API Authentication

Every Speech is Cheap API request through MindCloud uses a MindCloud API key and the `connectionId` of a Speech is Cheap connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Speech is Cheap connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Speech is Cheap action](README.md#actions-4) with your API key and `connectionId`.

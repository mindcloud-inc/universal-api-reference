...
```
POST https://connect.mindcloud.co/v1/universal/sVAHNAR/latest/actions/run-agent
```
...
| `message` | string | yes | The message or command to send to the agent. |
...
| Key | Type | Required | Description |
| --- | --- | --- | --- |
| `agentHistory[]` | array<object> | no | Optional list of prior messages. |
...
| `response` | array<object> | Conversation response blocks returned by the agent run. |
| `usage` | object | Credit usage summary for the run. |

## Native endpoint

Through the native SVAHNAR API, this operation is `POST /v1/agents/run` (base URL `https://api.svahnar.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/run-agent.md) for the provider-specific parameters and requirements.

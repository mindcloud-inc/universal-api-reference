# Supabase Universal API Authentication

Every Supabase API request through MindCloud uses a MindCloud API key and the `connectionId` of a Supabase connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Supabase connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Supabase action](README.md#actions-39) with your API key and `connectionId`.

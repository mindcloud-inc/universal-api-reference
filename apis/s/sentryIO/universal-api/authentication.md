# Sentry IO Universal API Authentication

Every Sentry IO API request through MindCloud uses a MindCloud API key and the `connectionId` of a Sentry IO connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Sentry IO connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Sentry IO action](README.md#actions-22) with your API key and `connectionId`.

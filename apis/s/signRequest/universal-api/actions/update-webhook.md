# SignRequest: Update Webhook



```
PUT https://connect.mindcloud.co/v1/universal/signRequest/latest/actions/update-webhook
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/signRequest/latest/actions/update-webhook" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/signRequest/latest/actions/update-webhook', {
  method: 'PUT',
  headers: {
...
Through the native SignRequest API, this operation is `PATCH /webhooks/:uuid/` (base URL `https://signrequest.com/api/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-webhook.md) for the provider-specific parameters and requirements.

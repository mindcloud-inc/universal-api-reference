# Sumtracker: List Purchase Order Lines

Retrieves purchase order lines from Sumtracker.

```
GET https://connect.mindcloud.co/v1/universal/sumtracker/latest/actions/list-po-lines
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/sumtracker/latest/actions/list-po-lines?connectionId=$CONNECTION_ID&limit=25&offset=0&document_type=string&po_id=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/sumtracker/latest/actions/list-po-lines?${params}`, {
  headers: {
...
| `results[].deliveredQuantity` | number |  |
| `results[].packSize` | number |  |
| `results[].moq` | number |  |
| `results[].cost` | string |  |
| `results[].shippingCost` | string |  |
| `results[].lineSubtotal` | string |  |
| `results[].lineTax` | string |  |
| `results[].lineTotal` | string |  |
| `results[].taxRate` | string |  |
...
Through the native Sumtracker API, this operation is `GET /api/version/2025-03/purchases/:document_type/:po_id/lines/` (base URL `https://inventory-api.sumtracker.com`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/list-po-lines.md) for the provider-specific parameters and requirements.

# Sumtracker: Create Goods Receipt Note

Creates a goods receipt note in Sumtracker.

```
POST https://connect.mindcloud.co/v1/universal/sumtracker/latest/actions/create-grn
```
...
Through the native Sumtracker API, this operation is `POST /api/version/2025-03/purchases/:document_type/:po_id/grns/` (base URL `https://inventory-api.sumtracker.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-grn.md) for the provider-specific parameters and requirements.

# Skribble: Get Signature Requests By Bulk

Retrieves multiple signature requests from Skribble by ID.

```
GET https://connect.mindcloud.co/v1/universal/skribble/latest/actions/get-signature-requests-by-bulk
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/skribble/latest/actions/get-signature-requests-by-bulk?connectionId=$CONNECTION_ID&signatureRequestIds%5B%5D=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/skribble/latest/actions/get-signature-requests-by-bulk?${params}`, {
  headers: {

# Skribble: Download Signature Request Attachment

Retrieves an attachment from a signature request in Skribble.

```
GET https://connect.mindcloud.co/v1/universal/skribble/latest/actions/download-signature-request-attachment
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/skribble/latest/actions/download-signature-request-attachment?connectionId=$CONNECTION_ID&attachmentId=string&signatureRequestId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/skribble/latest/actions/download-signature-request-attachment?${params}`, {
  headers: {

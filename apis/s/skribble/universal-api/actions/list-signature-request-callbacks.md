# Skribble: List Signature Request Callbacks

Retrieves callbacks for a signature request in Skribble.

```
GET https://connect.mindcloud.co/v1/universal/skribble/latest/actions/list-signature-request-callbacks
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/skribble/latest/actions/list-signature-request-callbacks?connectionId=$CONNECTION_ID&signatureRequestId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/skribble/latest/actions/list-signature-request-callbacks?${params}`, {
  headers: {

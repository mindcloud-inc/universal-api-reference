# SalesViewer: Search Sessions by Form Data

Finds sessions in SalesViewer by form data.

```
GET https://connect.mindcloud.co/v1/universal/salesViewer/latest/actions/search-sessions-by-form-data
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/salesViewer/latest/actions/search-sessions-by-form-data?connectionId=$CONNECTION_ID&limit=25&offset=0" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/salesViewer/latest/actions/search-sessions-by-form-data?${params}`, {
  headers: {
...
Through the native SalesViewer API, this operation is `POST /sessions.json` (base URL `https://api.salesviewer.com/`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/search-sessions-by-form-data.md) for the provider-specific parameters and requirements.

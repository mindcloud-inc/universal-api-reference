# Split CSV Universal API Authentication

Every Split CSV API request through MindCloud uses a MindCloud API key and the `connectionId` of a Split CSV connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Split CSV connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Split CSV action](README.md#actions-3) with your API key and `connectionId`.

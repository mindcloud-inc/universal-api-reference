# SpreadsheetWeb Hub: Create Tag

Creates a new tag in SpreadsheetWeb Hub.

```
POST https://connect.mindcloud.co/v1/universal/spreadsheetWebHub/latest/actions/create-tag
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/spreadsheetWebHub/latest/actions/create-tag" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/spreadsheetWebHub/latest/actions/create-tag', {
  method: 'POST',
  headers: {
...
| `request.type` | number | no | Tag type enum value. |

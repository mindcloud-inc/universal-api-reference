# ServiceTitan: Get Invoices By Job Id

Retrieves invoices from ServiceTitan by job ID.

```
GET https://connect.mindcloud.co/v1/universal/servicetitan/latest/actions/get-invoices-by-job-id
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/servicetitan/latest/actions/get-invoices-by-job-id?connectionId=$CONNECTION_ID&limit=25&offset=0" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/servicetitan/latest/actions/get-invoices-by-job-id?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `jobId` | string | no | Default: `156057101`. |
...
Through the native ServiceTitan API, this operation is `GET accounting/v2/tenant/{{credentials.tenant}}/invoices` (base URL `https://{{credentials.baseUrl}}/`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/get-invoices-by-job-id.md) for the provider-specific parameters and requirements.

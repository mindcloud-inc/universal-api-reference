# SelectPdf Universal API Authentication

Every SelectPdf API request through MindCloud uses a MindCloud API key and the `connectionId` of a SelectPdf connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your SelectPdf connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [SelectPdf action](README.md#actions-9) with your API key and `connectionId`.

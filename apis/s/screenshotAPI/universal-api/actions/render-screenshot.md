# ScreenshotAPI: Render Screenshot

Creates a new PNG screenshot in ScreenshotAPI.

```
POST https://connect.mindcloud.co/v1/universal/screenshotAPI/latest/actions/render-screenshot
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/screenshotAPI/latest/actions/render-screenshot" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/screenshotAPI/latest/actions/render-screenshot', {
  method: 'POST',
  headers: {
...
Through the native ScreenshotAPI API, this operation is `GET /screenshot` (base URL `https://shot.screenshotapi.net/v3`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/render-screenshot.md) for the provider-specific parameters and requirements.

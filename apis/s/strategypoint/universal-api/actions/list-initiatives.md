# Strategypoint: List Initiatives

Retrieves initiatives from Strategypoint.

```
GET https://connect.mindcloud.co/v1/universal/strategypoint/latest/actions/list-initiatives
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/strategypoint/latest/actions/list-initiatives?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/strategypoint/latest/actions/list-initiatives?${params}`, {
  headers: {
...
Through the native Strategypoint API, this operation is `GET /initiatives` (base URL `https://app.clearpointstrategy.com/api/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-initiatives.md) for the provider-specific parameters and requirements.

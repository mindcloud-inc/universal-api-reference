# Swell Universal API Pagination

Paginated list actions accept `limit` and `offset` as query parameters. MindCloud translates them into whatever pagination model Swell expects, so the request shape stays the same even when the native API uses pages or cursors.
...
## Swell actions that support pagination

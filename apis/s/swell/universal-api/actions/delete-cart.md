...
```
DELETE https://connect.mindcloud.co/v1/universal/swell/latest/actions/delete-cart
```
...
```bash
curl -X DELETE "https://connect.mindcloud.co/v1/universal/swell/latest/actions/delete-cart?connectionId=$CONNECTION_ID&id=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/swell/latest/actions/delete-cart?${params}`, {
  headers: {
...
Through the native Swell API, this operation is `DELETE /carts/:id` (base URL `https://api.swell.store`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/delete-cart.md) for the provider-specific parameters and requirements.

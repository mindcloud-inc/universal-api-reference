# Shorten.REST Universal API Authentication

Every Shorten.REST API request through MindCloud uses a MindCloud API key and the `connectionId` of a Shorten.REST connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Shorten.REST connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Shorten.REST action](README.md#actions-6) with your API key and `connectionId`.

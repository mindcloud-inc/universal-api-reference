# Solace PubSub+ Universal API Authentication

Every Solace PubSub+ API request through MindCloud uses a MindCloud API key and the `connectionId` of a Solace PubSub+ connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Solace PubSub+ connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Solace PubSub+ action](README.md#actions-30) with your API key and `connectionId`.

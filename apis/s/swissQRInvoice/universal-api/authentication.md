# Swiss QR Invoice Universal API Authentication

Every Swiss QR Invoice API request through MindCloud uses a MindCloud API key and the `connectionId` of a Swiss QR Invoice connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Swiss QR Invoice connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Swiss QR Invoice action](README.md#actions-1) with your API key and `connectionId`.

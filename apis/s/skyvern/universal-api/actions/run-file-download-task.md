# Skyvern: Run File Download Task

Runs a file download task in Skyvern.

```
POST https://connect.mindcloud.co/v1/universal/skyvern/latest/actions/run-file-download-task
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/skyvern/latest/actions/run-file-download-task" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/skyvern/latest/actions/run-file-download-task', {
  method: 'POST',
  headers: {
...
| --- | --- | --- | --- |
| `navigationGoal` | string | yes | Instructions for navigating to and downloading the file. |
| `url` | string | no | Website URL for the download task. |
...
Through the native Skyvern API, this operation is `POST /v1/run/tasks/download_files` (base URL `https://api.skyvern.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/run-file-download-task.md) for the provider-specific parameters and requirements.

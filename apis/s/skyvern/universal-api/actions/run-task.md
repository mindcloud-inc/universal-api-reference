...
```
POST https://connect.mindcloud.co/v1/universal/skyvern/latest/actions/run-task
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/skyvern/latest/actions/run-task" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/skyvern/latest/actions/run-task', {
  method: 'POST',
  headers: {
...
| --- | --- | --- | --- |
| `prompt` | string | yes | The goal or task description for Skyvern to accomplish. |
| `url` | string | no | The starting URL for the task. |
...
Through the native Skyvern API, this operation is `POST /v1/run/tasks` (base URL `https://api.skyvern.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/run-task.md) for the provider-specific parameters and requirements.

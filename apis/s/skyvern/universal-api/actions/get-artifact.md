# Skyvern: Get Artifact

Retrieves a run artifact from Skyvern.

```
GET https://connect.mindcloud.co/v1/universal/skyvern/latest/actions/get-artifact
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/skyvern/latest/actions/get-artifact?connectionId=$CONNECTION_ID&artifactId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/skyvern/latest/actions/get-artifact?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `artifactId` | string | yes | The artifact ID to retrieve. |
...
Through the native Skyvern API, this operation is `GET /v1/artifacts/:artifact_id` (base URL `https://api.skyvern.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-artifact.md) for the provider-specific parameters and requirements.

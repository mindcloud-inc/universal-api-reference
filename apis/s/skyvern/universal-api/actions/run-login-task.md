# Skyvern: Run Login Task

Runs a login automation task in Skyvern.

```
POST https://connect.mindcloud.co/v1/universal/skyvern/latest/actions/run-login-task
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/skyvern/latest/actions/run-login-task" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/skyvern/latest/actions/run-login-task', {
  method: 'POST',
  headers: {
...
| --- | --- | --- | --- |
| `credentialId` | string | no | ID of the Skyvern credential to use for login |
| `credentialType` | string | yes | Credential provider to use for the login flow. |
| `url` | string | no | Website URL for the login task. |
...
Through the native Skyvern API, this operation is `POST /v1/run/tasks/login` (base URL `https://api.skyvern.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/run-login-task.md) for the provider-specific parameters and requirements.

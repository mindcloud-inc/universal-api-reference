# Skyvern: Create Workflow

Creates a new workflow in Skyvern.

```
POST https://connect.mindcloud.co/v1/universal/skyvern/latest/actions/create-workflow
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/skyvern/latest/actions/create-workflow" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/skyvern/latest/actions/create-workflow', {
  method: 'POST',
  headers: {
...
| --- | --- | --- | --- |
| `folderId` | string | no | Optional folder ID to assign the workflow to |

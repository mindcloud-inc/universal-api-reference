# Skyvern: List Workflow Versions

Retrieves all versions of a workflow from Skyvern.

```
GET https://connect.mindcloud.co/v1/universal/skyvern/latest/actions/list-workflow-versions
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/skyvern/latest/actions/list-workflow-versions?connectionId=$CONNECTION_ID&workflowPermanentId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/skyvern/latest/actions/list-workflow-versions?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `template` | boolean | no |  |
| `workflowPermanentId` | string | yes |  |
...
Through the native Skyvern API, this operation is `GET /v1/workflows/:workflow_permanent_id/versions` (base URL `https://api.skyvern.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-workflow-versions.md) for the provider-specific parameters and requirements.

# Sipuni Universal API Authentication

Every Sipuni API request through MindCloud uses a MindCloud API key and the `connectionId` of a Sipuni connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Sipuni connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Sipuni action](README.md#actions-6) with your API key and `connectionId`.

# Stripe: Create Variable-Amount Checkout Session

Creates a new checkout session in Stripe.

```
POST https://connect.mindcloud.co/v1/universal/stripe/latest/actions/create-checkout-session-copy
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/stripe/latest/actions/create-checkout-session-copy" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/stripe/latest/actions/create-checkout-session-copy', {
  method: 'POST',
  headers: {
...
| `priceDataCurrency` | string | yes | Three-letter lowercase currency code for the inline Stripe Price, for example usd. |
| `unitAmount` | number | yes | Positive integer amount in the currency's smallest unit (cents for USD). |
| `productName` | string | yes | Customer-visible product or payment-request name. |

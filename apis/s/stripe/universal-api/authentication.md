# Stripe Universal API Authentication

Every Stripe API request through MindCloud uses a MindCloud API key and the `connectionId` of a Stripe connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Stripe connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Stripe action](README.md#actions-28) with your API key and `connectionId`.

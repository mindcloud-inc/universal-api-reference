# Swipe One: List Contacts



```
GET https://connect.mindcloud.co/v1/universal/swipeOne/latest/actions/list-contacts
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/swipeOne/latest/actions/list-contacts?connectionId=$CONNECTION_ID&workspaceId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/swipeOne/latest/actions/list-contacts?${params}`, {
  headers: {
...
| `data.contacts[].customerLifetimeValue` | object |  |
| `data.contacts[].email` | string |  |
| `data.contacts[].emailVerificationStatus` | string |  |
| `data.contacts[].eventsSummary.taskCreated.count` | number |  |
| `data.contacts[].eventsSummary.taskCreated.latestTimestamp` | string |  |
...
Through the native Swipe One API, this operation is `GET /workspaces/:workspaceId/contacts` (base URL `https://api.swipeone.com/api`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-contacts.md) for the provider-specific parameters and requirements.

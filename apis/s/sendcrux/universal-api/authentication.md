# Sendcrux Universal API Authentication

Every Sendcrux API request through MindCloud uses a MindCloud API key and the `connectionId` of a Sendcrux connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Sendcrux connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Sendcrux action](README.md#actions-29) with your API key and `connectionId`.

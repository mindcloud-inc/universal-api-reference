# SuperSend: Create Campaign

Creates a new campaign in SuperSend.

```
POST https://connect.mindcloud.co/v1/universal/superSend/latest/actions/create-campaign
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/superSend/latest/actions/create-campaign" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/superSend/latest/actions/create-campaign', {
  method: 'POST',
  headers: {
...
| `name` | string | yes |  |
| `teamId` | string | yes |  |
| `trackDomain` | string | no | Tracking domain for links (team default used if omitted) |
| `nodes[]` | array | no |  |
| `edges[]` | array | no |  |
| `isDraft` | boolean | no | Default: true. |
| `version` | number | no | Allowed values: 1, 2, 3. Default: 2. |
| `customVariables[]` | array<object> | no |  |
| `customVariables[].name` | string | no |  |
| `customVariables[].type` | string | no | Allowed values: text, boolean. Default: text. |
| `templateId` | string | no |  |
| `track` | boolean | no | Enable open and click tracking. When false, no pixels or link rewrites. |
| `timezone` | string | no |  |
| `days` | object | no |  |
...
| `blacklistIfUnsubscribe` | boolean | no | Default: true. |
| `blacklistIfBounced` | boolean | no | Default: true. |
| `blacklistDomainOnReply` | boolean | no | Default: true. |
...
Through the native SuperSend API, this operation is `POST /campaigns` (base URL `https://api.supersend.io/v2`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-campaign.md) for the provider-specific parameters and requirements.

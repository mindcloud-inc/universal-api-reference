...
```
GET https://connect.mindcloud.co/v1/universal/simpro/latest/actions/get-quote
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/simpro/latest/actions/get-quote?connectionId=$CONNECTION_ID&companyId=0&quoteId=1" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/simpro/latest/actions/get-quote?${params}`, {
  headers: {
...
Through the native Simpro API, this operation is `GET /companies/:companyId/quotes/:quoteId` (base URL `https://mindcloud.simprosuite.com/api/v1.0`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-quote.md) for the provider-specific parameters and requirements.

# SportsData: NFL Players

Retrieves NFL player details from SportsData.

```
GET https://connect.mindcloud.co/v1/universal/sportsData/latest/actions/nfl-players
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/sportsData/latest/actions/nfl-players?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/sportsData/latest/actions/nfl-players?${params}`, {
  headers: {
...
Through the native SportsData API, this operation is `GET /v3/nfl/scores/json/Players` (base URL `https://api.sportsdata.io`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/nfl-players.md) for the provider-specific parameters and requirements.

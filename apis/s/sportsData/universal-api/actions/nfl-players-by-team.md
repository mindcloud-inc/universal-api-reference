# SportsData: NFL Players By Team

Retrieves NFL player details from SportsData by team.

```
GET https://connect.mindcloud.co/v1/universal/sportsData/latest/actions/nfl-players-by-team
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/sportsData/latest/actions/nfl-players-by-team?connectionId=$CONNECTION_ID&team=WAS" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/sportsData/latest/actions/nfl-players-by-team?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `team` | string | yes | NFL team abbreviation. Example: `WAS`. |
...
Through the native SportsData API, this operation is `GET /v3/nfl/scores/json/Players/:team` (base URL `https://api.sportsdata.io`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/nfl-players-by-team.md) for the provider-specific parameters and requirements.

# SportsData: NBA Games By Date

Retrieves NBA games from SportsData by date.

```
GET https://connect.mindcloud.co/v1/universal/sportsData/latest/actions/nba-games-by-date
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/sportsData/latest/actions/nba-games-by-date?connectionId=$CONNECTION_ID&date=2026-APR-09" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/sportsData/latest/actions/nba-games-by-date?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `date` | string | yes | Game date for the NBA games feed. Example: `2026-APR-09`. |
...
Through the native SportsData API, this operation is `GET /v3/nba/scores/json/GamesByDate/:date` (base URL `https://api.sportsdata.io`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/nba-games-by-date.md) for the provider-specific parameters and requirements.

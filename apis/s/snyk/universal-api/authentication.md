# Snyk Universal API Authentication

Every Snyk API request through MindCloud uses a MindCloud API key and the `connectionId` of a Snyk connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Snyk connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Snyk action](README.md#actions-60) with your API key and `connectionId`.

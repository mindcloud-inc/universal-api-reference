# SAIA EDI: Get Shipment by Bill of Lading Number

Retrieves a shipment from SAIA EDI by bill of lading number.

```
GET https://connect.mindcloud.co/v1/universal/sAIAEDI/latest/actions/get-shipment-by-bill-of-lading-number
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/sAIAEDI/latest/actions/get-shipment-by-bill-of-lading-number?connectionId=$CONNECTION_ID&BLNumber=string&OriginZipcode=string&DestinationZipcode=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/sAIAEDI/latest/actions/get-shipment-by-bill-of-lading-number?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `BLNumber` | string | yes | Customer bill of lading number. |
...
Through the native SAIA EDI API, this operation is `POST /webservice/shipment/xml.aspx` (base URL `https://www.saiasecure.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-shipment-by-bill-of-lading-number.md) for the provider-specific parameters and requirements.

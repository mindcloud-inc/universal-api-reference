# SAIA EDI Universal API Authentication

Every SAIA EDI API request through MindCloud uses a MindCloud API key and the `connectionId` of a SAIA EDI connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your SAIA EDI connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [SAIA EDI action](README.md#actions-5) with your API key and `connectionId`.

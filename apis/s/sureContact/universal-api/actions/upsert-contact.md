# SureContact: Upsert Contact

Creates or updates a contact in SureContact by email.

```
PUT https://connect.mindcloud.co/v1/universal/sureContact/latest/actions/upsert-contact
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/sureContact/latest/actions/upsert-contact" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/sureContact/latest/actions/upsert-contact', {
  method: 'PUT',
  headers: {
...
| `primaryFields.email` | string | yes | The contact email used for upsert matching. |

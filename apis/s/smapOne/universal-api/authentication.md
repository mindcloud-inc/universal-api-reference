# smapOne Universal API Authentication

Every smapOne API request through MindCloud uses a MindCloud API key and the `connectionId` of a smapOne connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your smapOne connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [smapOne action](README.md#actions-23) with your API key and `connectionId`.

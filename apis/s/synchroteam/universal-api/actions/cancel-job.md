# Synchroteam: Cancel Job

Cancels a job in Synchroteam by supported identifier.

```
PUT https://connect.mindcloud.co/v1/universal/synchroteam/latest/actions/cancel-job
```
...
Through the native Synchroteam API, this operation is `PUT /Api/v2/Jobs/Cancel` (base URL `https://ws.synchroteam.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/cancel-job.md) for the provider-specific parameters and requirements.

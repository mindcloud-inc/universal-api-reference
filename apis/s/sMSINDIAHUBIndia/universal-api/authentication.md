# SMSINDIAHUB (India) Universal API Authentication

Every SMSINDIAHUB (India) API request through MindCloud uses a MindCloud API key and the `connectionId` of a SMSINDIAHUB (India) connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your SMSINDIAHUB (India) connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [SMSINDIAHUB (India) action](README.md#actions-11) with your API key and `connectionId`.

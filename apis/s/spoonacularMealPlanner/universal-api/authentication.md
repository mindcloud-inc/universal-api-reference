# Spoonacular Meal Planner Universal API Authentication

Every Spoonacular Meal Planner API request through MindCloud uses a MindCloud API key and the `connectionId` of a Spoonacular Meal Planner connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Spoonacular Meal Planner connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Spoonacular Meal Planner action](README.md#actions-24) with your API key and `connectionId`.

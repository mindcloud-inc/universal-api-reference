# SmartRoutes: Update Vehicle



```
PUT https://connect.mindcloud.co/v1/universal/smartRoutes/latest/actions/update-vehicle
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/smartRoutes/latest/actions/update-vehicle" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/smartRoutes/latest/actions/update-vehicle', {
  method: 'PUT',
  headers: {
...
| --- | --- | --- | --- |
| `id` | number | yes | ID of the vehicle to update. |
...
Through the native SmartRoutes API, this operation is `PUT /vehicles/:id` (base URL `https://api.smartroutes.io/v2`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-vehicle.md) for the provider-specific parameters and requirements.

...
Updates and sends a draft document in SignWell.

```
PUT https://connect.mindcloud.co/v1/universal/signWell/latest/actions/update-and-send-document
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/signWell/latest/actions/update-and-send-document" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/signWell/latest/actions/update-and-send-document', {
  method: 'PUT',
  headers: {
...
Through the native SignWell API, this operation is `POST /documents/:id/send` (base URL `https://www.signwell.com/api/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-and-send-document.md) for the provider-specific parameters and requirements.

# SigParser: List Updated Meetings



```
GET https://connect.mindcloud.co/v1/universal/sigParser/latest/actions/list-updated-meetings
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/sigParser/latest/actions/list-updated-meetings?connectionId=$CONNECTION_ID&lastModifiedAfter=1" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/sigParser/latest/actions/list-updated-meetings?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `lastModifiedAfter` | number | yes | Return meetings updated after this timestamp key. |
...
Through the native SigParser API, this operation is `GET /api/Meetings/Distinct` (base URL `https://ipaas.sigparser.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-updated-meetings.md) for the provider-specific parameters and requirements.

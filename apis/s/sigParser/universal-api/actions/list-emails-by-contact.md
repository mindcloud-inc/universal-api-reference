# SigParser: List Emails By Contact



```
GET https://connect.mindcloud.co/v1/universal/sigParser/latest/actions/list-emails-by-contact
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/sigParser/latest/actions/list-emails-by-contact?connectionId=$CONNECTION_ID&emailAddress=ava%40example.com" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/sigParser/latest/actions/list-emails-by-contact?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `emailAddress` | string | yes | Contact email address to filter by. |
...
Through the native SigParser API, this operation is `GET /api/Emails/Distinct` (base URL `https://ipaas.sigparser.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-emails-by-contact.md) for the provider-specific parameters and requirements.

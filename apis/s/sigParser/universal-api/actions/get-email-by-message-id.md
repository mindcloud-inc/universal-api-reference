# SigParser: Get Email By Message ID



```
GET https://connect.mindcloud.co/v1/universal/sigParser/latest/actions/get-email-by-message-id
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/sigParser/latest/actions/get-email-by-message-id?connectionId=$CONNECTION_ID&id=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/sigParser/latest/actions/get-email-by-message-id?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `id` | string | yes | SigParser email record ID. |
...
Through the native SigParser API, this operation is `GET /api/Emails/Distinct` (base URL `https://ipaas.sigparser.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-email-by-message-id.md) for the provider-specific parameters and requirements.

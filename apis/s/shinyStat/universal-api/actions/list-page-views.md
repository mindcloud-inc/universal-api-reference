# ShinyStat: List Page Views

Retrieves page view report data from ShinyStat.

```
GET https://connect.mindcloud.co/v1/universal/shinyStat/latest/actions/list-page-views
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/shinyStat/latest/actions/list-page-views?connectionId=$CONNECTION_ID&href=%2F" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/shinyStat/latest/actions/list-page-views?${params}`, {
  headers: {
...
Through the native ShinyStat API, this operation is `POST /ajax` (base URL `https://report.shinystat.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-page-views.md) for the provider-specific parameters and requirements.

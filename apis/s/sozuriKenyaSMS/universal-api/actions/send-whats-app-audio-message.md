# Sozuri (Kenya) SMS: Send WhatsApp Audio Message

Sends a WhatsApp audio message through Sozuri.

```
POST https://connect.mindcloud.co/v1/universal/sozuriKenyaSMS/latest/actions/send-whats-app-audio-message
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/sozuriKenyaSMS/latest/actions/send-whats-app-audio-message" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/sozuriKenyaSMS/latest/actions/send-whats-app-audio-message', {
  method: 'POST',
  headers: {
...
| `audio` | object | yes | The WhatsApp audio payload object, including a publicly accessible link. |

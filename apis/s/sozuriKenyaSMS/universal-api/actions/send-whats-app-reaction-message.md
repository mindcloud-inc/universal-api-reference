# Sozuri (Kenya) SMS: Send WhatsApp Reaction Message

Sends a WhatsApp reaction through Sozuri.

```
POST https://connect.mindcloud.co/v1/universal/sozuriKenyaSMS/latest/actions/send-whats-app-reaction-message
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/sozuriKenyaSMS/latest/actions/send-whats-app-reaction-message" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/sozuriKenyaSMS/latest/actions/send-whats-app-reaction-message', {
  method: 'POST',
  headers: {
...
| `reaction` | object | yes | The WhatsApp reaction payload object, including the original message_id and emoji. |

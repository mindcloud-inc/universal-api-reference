# Sozuri (Kenya) SMS: Send WhatsApp Location Message

Sends a WhatsApp location message through Sozuri.

```
POST https://connect.mindcloud.co/v1/universal/sozuriKenyaSMS/latest/actions/send-whats-app-location-message
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/sozuriKenyaSMS/latest/actions/send-whats-app-location-message" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/sozuriKenyaSMS/latest/actions/send-whats-app-location-message', {
  method: 'POST',
  headers: {
...
| `location` | object | yes | The WhatsApp location payload object with longitude, latitude, name, and address. |

# Sozuri (Kenya) SMS: Send WhatsApp Contacts Message

Sends WhatsApp contact cards through Sozuri.

```
POST https://connect.mindcloud.co/v1/universal/sozuriKenyaSMS/latest/actions/send-whats-app-contacts-message
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/sozuriKenyaSMS/latest/actions/send-whats-app-contacts-message" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/sozuriKenyaSMS/latest/actions/send-whats-app-contacts-message', {
  method: 'POST',
  headers: {
...
| `contacts[]` | array<object> | yes | The WhatsApp contacts payload array. |

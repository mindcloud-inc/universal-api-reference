# Sozuri (Kenya) SMS: Send WhatsApp Video Message

Sends a WhatsApp video message through Sozuri.

```
POST https://connect.mindcloud.co/v1/universal/sozuriKenyaSMS/latest/actions/send-whats-app-video-message
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/sozuriKenyaSMS/latest/actions/send-whats-app-video-message" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/sozuriKenyaSMS/latest/actions/send-whats-app-video-message', {
  method: 'POST',
  headers: {
...
| `video` | object | yes | The WhatsApp video payload object, including a publicly accessible link. |

# Sozuri (Kenya) SMS: Send WhatsApp Image Message

Sends a WhatsApp image message through Sozuri.

```
POST https://connect.mindcloud.co/v1/universal/sozuriKenyaSMS/latest/actions/send-whats-app-image-message
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/sozuriKenyaSMS/latest/actions/send-whats-app-image-message" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/sozuriKenyaSMS/latest/actions/send-whats-app-image-message', {
  method: 'POST',
  headers: {
...
| `image` | object | yes | The WhatsApp image payload object, including a publicly accessible link and optional caption. |

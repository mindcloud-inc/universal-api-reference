# Sozuri (Kenya) SMS: Send WhatsApp Document Message

Sends a WhatsApp document message through Sozuri.

```
POST https://connect.mindcloud.co/v1/universal/sozuriKenyaSMS/latest/actions/send-whats-app-document-message
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/sozuriKenyaSMS/latest/actions/send-whats-app-document-message" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/sozuriKenyaSMS/latest/actions/send-whats-app-document-message', {
  method: 'POST',
  headers: {
...
| `document` | object | yes | The WhatsApp document payload object, including a publicly accessible link and optional filename/caption. |

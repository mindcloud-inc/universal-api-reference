# Sozuri (Kenya) SMS Universal API Authentication

Every Sozuri (Kenya) SMS API request through MindCloud uses a MindCloud API key and the `connectionId` of a Sozuri (Kenya) SMS connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Sozuri (Kenya) SMS connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Sozuri (Kenya) SMS action](README.md#actions-22) with your API key and `connectionId`.

# SweetProcess: List Teams



```
GET https://connect.mindcloud.co/v1/universal/sweetProcess/latest/actions/list-teams
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/sweetProcess/latest/actions/list-teams?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/sweetProcess/latest/actions/list-teams?${params}`, {
  headers: {
...
| `canEdit` | boolean | Whether the current actor can edit the team. |
...
| `numberOfFolders` | number | The number of folders in the team. |
...
| `numberOfRoles` | number | The number of roles in the team. |
...
Through the native SweetProcess API, this operation is `GET /teams/` (base URL `https://www.sweetprocess.com/api/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-teams.md) for the provider-specific parameters and requirements.

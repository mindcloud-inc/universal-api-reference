# Spike.sh Universal API Authentication

Every Spike.sh API request through MindCloud uses a MindCloud API key and the `connectionId` of a Spike.sh connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Spike.sh connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Spike.sh action](README.md#actions-22) with your API key and `connectionId`.

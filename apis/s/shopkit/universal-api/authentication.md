# Shopkit Universal API Authentication

Every Shopkit API request through MindCloud uses a MindCloud API key and the `connectionId` of a Shopkit connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Shopkit connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Shopkit action](README.md#actions-40) with your API key and `connectionId`.

# Spoonacular: Autocomplete Ingredient Search

Autocompletes ingredient names in Spoonacular.

```
GET https://connect.mindcloud.co/v1/universal/spoonacular/latest/actions/autocomplete-ingredient-search
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/spoonacular/latest/actions/autocomplete-ingredient-search?connectionId=$CONNECTION_ID&query=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/spoonacular/latest/actions/autocomplete-ingredient-search?${params}`, {
  headers: {

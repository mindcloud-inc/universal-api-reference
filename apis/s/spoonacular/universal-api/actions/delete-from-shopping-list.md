# Spoonacular: Delete from Shopping List

Deletes an item from a Spoonacular shopping list.

```
DELETE https://connect.mindcloud.co/v1/universal/spoonacular/latest/actions/delete-from-shopping-list
```
...
```bash
curl -X DELETE "https://connect.mindcloud.co/v1/universal/spoonacular/latest/actions/delete-from-shopping-list?connectionId=$CONNECTION_ID&hash=string&id=string&username=Ava%20Chen" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/spoonacular/latest/actions/delete-from-shopping-list?${params}`, {
  headers: {

# Spoonacular: Delete Meal Plan Template

Deletes a meal plan template from Spoonacular.

```
DELETE https://connect.mindcloud.co/v1/universal/spoonacular/latest/actions/delete-meal-plan-template
```
...
```bash
curl -X DELETE "https://connect.mindcloud.co/v1/universal/spoonacular/latest/actions/delete-meal-plan-template?connectionId=$CONNECTION_ID&hash=string&id=string&username=Ava%20Chen" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/spoonacular/latest/actions/delete-meal-plan-template?${params}`, {
  headers: {

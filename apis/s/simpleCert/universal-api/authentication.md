# SimpleCert Universal API Authentication

Every SimpleCert API request through MindCloud uses a MindCloud API key and the `connectionId` of a SimpleCert connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your SimpleCert connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [SimpleCert action](README.md#actions-9) with your API key and `connectionId`.

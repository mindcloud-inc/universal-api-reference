# Sendloop Universal API Authentication

Every Sendloop API request through MindCloud uses a MindCloud API key and the `connectionId` of a Sendloop connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Sendloop connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Sendloop action](README.md#actions-20) with your API key and `connectionId`.

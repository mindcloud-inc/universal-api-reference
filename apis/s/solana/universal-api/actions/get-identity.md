...
Retrieves the node identity from Solana.

```
GET https://connect.mindcloud.co/v1/universal/solana/latest/actions/get-identity
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/solana/latest/actions/get-identity?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/solana/latest/actions/get-identity?${params}`, {
  headers: {

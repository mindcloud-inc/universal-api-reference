...
```
GET https://connect.mindcloud.co/v1/universal/streamtime/latest/actions/get-user
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/streamtime/latest/actions/get-user?connectionId=$CONNECTION_ID&userId=185869" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/streamtime/latest/actions/get-user?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `userId` | number | yes | User ID Example: `185869`. |
...
Through the native Streamtime API, this operation is `GET /users/:user_id` (base URL `https://api.streamtime.net/v2`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-user.md) for the provider-specific parameters and requirements.

# Streamtime: Update Job



```
PUT https://connect.mindcloud.co/v1/universal/streamtime/latest/actions/update-job
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/streamtime/latest/actions/update-job" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/streamtime/latest/actions/update-job', {
  method: 'PUT',
  headers: {
...
Through the native Streamtime API, this operation is `PUT /jobs/:job_id` (base URL `https://api.streamtime.net/v2`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-job.md) for the provider-specific parameters and requirements.

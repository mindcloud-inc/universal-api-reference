# Starburst Galaxy: List clusters



```
GET https://connect.mindcloud.co/v1/universal/starburstGalaxy/latest/actions/list-clusters
```
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/starburstGalaxy/latest/actions/list-clusters?${params}`, {
  headers: {
...
| `result[].cloudRegionId` | string | Cloud region ID. |
| `result[].clusterId` | string | Cluster ID. |
| `result[].clusterState` | string | Cluster state. |
| `result[].name` | string | Cluster name. |
| `result[].trinoUri` | string | Trino connection URL. |

## Native endpoint

Through the native Starburst Galaxy API, this operation is `GET /public/api/v1/cluster` (base URL `https://mindcloud.galaxy.starburst.io`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-clusters.md) for the provider-specific parameters and requirements.

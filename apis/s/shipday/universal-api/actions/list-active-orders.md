# Shipday: List Active Orders

Retrieves active orders from Shipday.

```
GET https://connect.mindcloud.co/v1/universal/shipday/latest/actions/list-active-orders
```
...
Through the native Shipday API, this operation is `GET /orders` (base URL `https://api.shipday.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-active-orders.md) for the provider-specific parameters and requirements.

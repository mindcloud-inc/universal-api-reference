# SavvyCal: Update Scheduling Link



```
PUT https://connect.mindcloud.co/v1/universal/savvyCal/latest/actions/update-scheduling-link
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/savvyCal/latest/actions/update-scheduling-link" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/savvyCal/latest/actions/update-scheduling-link', {
  method: 'PUT',
  headers: {
...
Through the native SavvyCal API, this operation is `PATCH /v1/links/:link_id` (base URL `https://api.savvycal.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-scheduling-link.md) for the provider-specific parameters and requirements.

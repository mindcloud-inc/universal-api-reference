# SavvyCal: Get Time Zone



```
GET https://connect.mindcloud.co/v1/universal/savvyCal/latest/actions/get-time-zone
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/savvyCal/latest/actions/get-time-zone?connectionId=$CONNECTION_ID&segments=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/savvyCal/latest/actions/get-time-zone?${params}`, {
  headers: {
...
Through the native SavvyCal API, this operation is `GET /v1/time_zones/:segments` (base URL `https://api.savvycal.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-time-zone.md) for the provider-specific parameters and requirements.

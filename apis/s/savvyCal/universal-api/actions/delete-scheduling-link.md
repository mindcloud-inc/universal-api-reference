# SavvyCal: Delete Scheduling Link



```
DELETE https://connect.mindcloud.co/v1/universal/savvyCal/latest/actions/delete-scheduling-link
```
...
```bash
curl -X DELETE "https://connect.mindcloud.co/v1/universal/savvyCal/latest/actions/delete-scheduling-link?connectionId=$CONNECTION_ID&linkId=https%3A%2F%2Fexample.com" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/savvyCal/latest/actions/delete-scheduling-link?${params}`, {
  headers: {
...
Through the native SavvyCal API, this operation is `DELETE /v1/links/:link_id` (base URL `https://api.savvycal.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/delete-scheduling-link.md) for the provider-specific parameters and requirements.

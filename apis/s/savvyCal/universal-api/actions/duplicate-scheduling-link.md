# SavvyCal: Duplicate Scheduling Link



```
POST https://connect.mindcloud.co/v1/universal/savvyCal/latest/actions/duplicate-scheduling-link
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/savvyCal/latest/actions/duplicate-scheduling-link" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/savvyCal/latest/actions/duplicate-scheduling-link', {
  method: 'POST',
  headers: {
...
Through the native SavvyCal API, this operation is `POST /v1/links/:link_id/duplicate` (base URL `https://api.savvycal.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/duplicate-scheduling-link.md) for the provider-specific parameters and requirements.

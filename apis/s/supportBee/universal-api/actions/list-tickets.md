# SupportBee: List Tickets

Retrieves tickets from SupportBee.

```
GET https://connect.mindcloud.co/v1/universal/supportBee/latest/actions/list-tickets
```
...
| --- | --- | --- | --- |
| `archived` | string | no | If true, retrieves archived tickets only. If false, archived tickets are excluded. If any, archived tickets are included. |
| `spam` | boolean | no | If true, retrieves tickets marked as spam. |
| `trash` | boolean | no | If true, retrieves tickets that are trashed. |
| `replies` | boolean | no | Filter to tickets with replies or without replies. |
| `maxReplies` | number | no | Retrieve tickets with a specific number of replies. Cannot be used with replies=false. |
| `assignedUser` | string | no | Use me, any, none, or a SupportBee agent ID. |
| `assignedTeam` | string | no | Use mine, none, or a SupportBee team ID. |
| `label` | string | no | Retrieve only tickets with the specified label name. |
| `since` | date | no | Retrieve tickets whose last activity is after the provided date, date-time, or timestamp. |
| `until` | date | no | Retrieve tickets whose last activity is before the provided date, date-time, or timestamp. |
| `sortBy` | string | no | Sort tickets by last_activity or creation_time. |
| `requesterEmails` | string | no | Comma-separated requester email addresses. |
| `totalOnly` | boolean | no | Return only the total number of matching tickets. |

# ServiceTrade: Update Location

Updates an existing location in ServiceTrade.

```
PUT https://connect.mindcloud.co/v1/universal/serviceTrade/latest/actions/update-location
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/serviceTrade/latest/actions/update-location" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/serviceTrade/latest/actions/update-location', {
  method: 'PUT',
  headers: {
...
| --- | --- | --- | --- |
| `locationId` | number | yes | Location to update. |
| `name` | string | no | Updated name for the location. |
| `phoneNumber` | string | no | Updated phone number for the location. |
...
Through the native ServiceTrade API, this operation is `PUT location/:locationId` (base URL `https://api.servicetrade.com/api`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-location.md) for the provider-specific parameters and requirements.

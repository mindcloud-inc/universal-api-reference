...
```
POST https://connect.mindcloud.co/v1/universal/sMSup/latest/actions/send-sms
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/sMSup/latest/actions/send-sms" \
  ...
  "messages[].text": "Hello from SMSup"
}'
```

```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/sMSup/latest/actions/send-sms', {
  method: 'POST',
  headers: {
...
| `messages[].text` | string | yes | Body of the text message. Example: `Hello from SMSup`. |
| `reportUrl` | string | no | URL where delivery reports should be sent. Example: `https://example.com/dlr`. |

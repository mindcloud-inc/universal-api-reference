# Shippo - Legacy Universal API Authentication

Every Shippo - Legacy API request through MindCloud uses a MindCloud API key and the `connectionId` of a Shippo - Legacy connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Shippo - Legacy connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Shippo - Legacy action](README.md#actions-8) with your API key and `connectionId`.

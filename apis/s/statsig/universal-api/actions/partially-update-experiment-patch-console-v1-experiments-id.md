# Statsig: Partially Update Experiment

Updates an experiment in Statsig.

```
PUT https://connect.mindcloud.co/v1/universal/statsig/latest/actions/partially-update-experiment-patch-console-v1-experiments-id
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/statsig/latest/actions/partially-update-experiment-patch-console-v1-experiments-id" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/statsig/latest/actions/partially-update-experiment-patch-console-v1-experiments-id', {
  method: 'PUT',
  headers: {
...
Through the native Statsig API, this operation is `PATCH /console/v1/experiments/{id}` (base URL `https://statsigapi.net`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/partially-update-experiment-patch-console-v1-experiments-id.md) for the provider-specific parameters and requirements.

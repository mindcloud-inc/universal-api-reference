# Statsig Universal API Authentication

Every Statsig API request through MindCloud uses a MindCloud API key and the `connectionId` of a Statsig connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Statsig connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Statsig action](README.md#actions-279) with your API key and `connectionId`.

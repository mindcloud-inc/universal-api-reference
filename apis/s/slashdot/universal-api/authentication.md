# Slashdot Universal API Authentication

Every Slashdot API request through MindCloud uses a MindCloud API key and the `connectionId` of a Slashdot connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Slashdot connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Slashdot action](README.md#actions-7) with your API key and `connectionId`.

...
```
PUT https://connect.mindcloud.co/v1/universal/softr/latest/actions/update-table
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/softr/latest/actions/update-table" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/softr/latest/actions/update-table', {
  method: 'PUT',
  headers: {
...
Through the native Softr API, this operation is `PUT /databases/:databaseId/tables/:tableId` (base URL `https://tables-api.softr.io/api/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-table.md) for the provider-specific parameters and requirements.

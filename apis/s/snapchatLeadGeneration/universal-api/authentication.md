# Snapchat Lead Generation Universal API Authentication

Every Snapchat Lead Generation API request through MindCloud uses a MindCloud API key and the `connectionId` of a Snapchat Lead Generation connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Snapchat Lead Generation connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Snapchat Lead Generation action](README.md#actions-9) with your API key and `connectionId`.

# Socket: Get Threat Feed Items

Retrieves deprecated threat feed items from Socket.

```
GET https://connect.mindcloud.co/v1/universal/socket/latest/actions/get-threat-feed-items
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/socket/latest/actions/get-threat-feed-items?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/socket/latest/actions/get-threat-feed-items?${params}`, {
  headers: {
...
Through the native Socket API, this operation is `GET /threat-feed` (base URL `https://api.socket.dev/v0`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-threat-feed-items.md) for the provider-specific parameters and requirements.

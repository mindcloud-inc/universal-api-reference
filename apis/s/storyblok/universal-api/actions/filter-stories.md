# Storyblok: Filter Stories

Retrieves Storyblok stories using a filter query.

```
GET https://connect.mindcloud.co/v1/universal/storyblok/latest/actions/filter-stories
```
...
const params = new URLSearchParams({
  ...
  limit: '25',
  offset: '0',
  "filterQuery": "component[in]=page"
});

const response = await fetch(`https://connect.mindcloud.co/v1/universal/storyblok/latest/actions/filter-stories?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `filterQuery` | string | yes | A serialized Storyblok filter_query expression. Default: `component[in]=page`. |
...
| `stories` | array<object> | Stories matching the filter query. |
...
Through the native Storyblok API, this operation is `GET /stories` (base URL `https://api.storyblok.com/v2/cdn`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/filter-stories.md) for the provider-specific parameters and requirements.

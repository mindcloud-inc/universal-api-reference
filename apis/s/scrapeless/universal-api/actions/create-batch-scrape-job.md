# Scrapeless: Create Batch Scrape Job

Creates a new batch scrape job in Scrapeless.

```
POST https://connect.mindcloud.co/v1/universal/scrapeless/latest/actions/create-batch-scrape-job
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/scrapeless/latest/actions/create-batch-scrape-job" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/scrapeless/latest/actions/create-batch-scrape-job', {
  method: 'POST',
  headers: {
...
| --- | --- | --- |
| `id` | string |  |
| `invalidURLs` | array<string> | If ignoreInvalidURLs is true, this is an array containing the invalid URLs that were specified in the request. If there were no invalid URLs, this will be an empty array. If ignoreInvalidURLs is false, this field will be undefined. |
| `success` | boolean |  |

## Native endpoint

Through the native Scrapeless API, this operation is `POST /api/v2/crawler/scrape/batch` (base URL `https://api.scrapeless.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-batch-scrape-job.md) for the provider-specific parameters and requirements.

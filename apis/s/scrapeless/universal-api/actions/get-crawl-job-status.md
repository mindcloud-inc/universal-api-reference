# Scrapeless: Get Crawl Job Status

Retrieves a crawl job status from Scrapeless.

```
GET https://connect.mindcloud.co/v1/universal/scrapeless/latest/actions/get-crawl-job-status
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/scrapeless/latest/actions/get-crawl-job-status?connectionId=$CONNECTION_ID&id=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/scrapeless/latest/actions/get-crawl-job-status?${params}`, {
  headers: {
...
| `data` | array<object> | The data of the crawl. |
...
Through the native Scrapeless API, this operation is `GET /api/v1/crawler/crawl/:id` (base URL `https://api.scrapeless.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-crawl-job-status.md) for the provider-specific parameters and requirements.

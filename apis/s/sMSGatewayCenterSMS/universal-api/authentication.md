# SMSGatewayCenter SMS Universal API Authentication

Every SMSGatewayCenter SMS API request through MindCloud uses a MindCloud API key and the `connectionId` of a SMSGatewayCenter SMS connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your SMSGatewayCenter SMS connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [SMSGatewayCenter SMS action](README.md#actions-20) with your API key and `connectionId`.

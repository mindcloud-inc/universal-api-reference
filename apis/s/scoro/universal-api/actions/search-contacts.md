# Scoro: Search Contacts

Finds contacts in Scoro.

```
GET https://connect.mindcloud.co/v1/universal/scoro/latest/actions/search-contacts
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/scoro/latest/actions/search-contacts?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/scoro/latest/actions/search-contacts?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `filter.name` | string | no | Exact contact name filter. |
| `filter.is_client` | boolean | no | Filter contacts by client flag. |
| `filter.modified_date.start` | date | no | Start of modified date range. |
| `filter.modified_date.end` | date | no | End of modified date range. |
...
Through the native Scoro API, this operation is `POST contacts` (base URL `{{credentials.subdomain}}`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/search-contacts.md) for the provider-specific parameters and requirements.

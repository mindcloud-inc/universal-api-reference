# Scoro Universal API Authentication

Every Scoro API request through MindCloud uses a MindCloud API key and the `connectionId` of a Scoro connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Scoro connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Scoro action](README.md#actions-44) with your API key and `connectionId`.

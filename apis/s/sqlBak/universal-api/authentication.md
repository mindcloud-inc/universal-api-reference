# SqlBak Universal API Authentication

Every SqlBak API request through MindCloud uses a MindCloud API key and the `connectionId` of a SqlBak connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your SqlBak connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [SqlBak action](README.md#actions-5) with your API key and `connectionId`.

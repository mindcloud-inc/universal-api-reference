# Stackby Universal API Authentication

Every Stackby API request through MindCloud uses a MindCloud API key and the `connectionId` of a Stackby connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Stackby connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Stackby action](README.md#actions-17) with your API key and `connectionId`.

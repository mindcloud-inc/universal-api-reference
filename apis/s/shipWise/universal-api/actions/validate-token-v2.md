# ShipWise: Validate Token V2

Validates an API token in ShipWise.

```
GET https://connect.mindcloud.co/v1/universal/shipWise/latest/actions/validate-token-v2
```
...
Through the native ShipWise API, this operation is `GET /api/v2/Authorize/Validate` (base URL `https://api.shipwise.com/`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/validate-token-v2.md) for the provider-specific parameters and requirements.

# Stockpilot: Cancel Order

Updates an order as canceled in Stockpilot.

```
PUT https://connect.mindcloud.co/v1/universal/stockpilot/latest/actions/cancel-order
```
...
| --- | --- | --- | --- |
| `orderPk` | number | yes |  |
| `reasonCode` | string | no | Default: `REQUESTED_BY_CUSTOMER`. |
...
Through the native Stockpilot API, this operation is `PUT /orders/cancel-order` (base URL `https://api.stockpilot.dev`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/cancel-order.md) for the provider-specific parameters and requirements.

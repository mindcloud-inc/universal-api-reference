# ShortPixel Universal API Authentication

Every ShortPixel API request through MindCloud uses a MindCloud API key and the `connectionId` of a ShortPixel connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your ShortPixel connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [ShortPixel action](README.md#actions-5) with your API key and `connectionId`.

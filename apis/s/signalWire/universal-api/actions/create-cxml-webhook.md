# SignalWire: Create cXML Webhook

Creates a new cXML webhook in SignalWire.

```
POST https://connect.mindcloud.co/v1/universal/signalWire/latest/actions/create-cxml-webhook
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/signalWire/latest/actions/create-cxml-webhook" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/signalWire/latest/actions/create-cxml-webhook', {
  method: 'POST',
  headers: {
...
| `primaryRequestUrl` | string | yes | Primary request url of the CXML Webhook. |
...
Through the native SignalWire API, this operation is `POST /fabric/resources/cxml_webhooks` (base URL `https://mindcloud.signalwire.com/api`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-cxml-webhook.md) for the provider-specific parameters and requirements.

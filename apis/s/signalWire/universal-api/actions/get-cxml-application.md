# SignalWire: Get cXML Application

Retrieves a cXML application from SignalWire.

```
GET https://connect.mindcloud.co/v1/universal/signalWire/latest/actions/get-cxml-application
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/signalWire/latest/actions/get-cxml-application?connectionId=$CONNECTION_ID&id=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/signalWire/latest/actions/get-cxml-application?${params}`, {
  headers: {
...
Through the native SignalWire API, this operation is `GET /fabric/resources/cxml_applications/{id}` (base URL `https://mindcloud.signalwire.com/api`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-cxml-application.md) for the provider-specific parameters and requirements.

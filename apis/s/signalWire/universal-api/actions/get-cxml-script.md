# SignalWire: Get cXML Script

Retrieves a cXML script from SignalWire.

```
GET https://connect.mindcloud.co/v1/universal/signalWire/latest/actions/get-cxml-script
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/signalWire/latest/actions/get-cxml-script?connectionId=$CONNECTION_ID&id=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/signalWire/latest/actions/get-cxml-script?${params}`, {
  headers: {
...
Through the native SignalWire API, this operation is `GET /fabric/resources/cxml_scripts/{id}` (base URL `https://mindcloud.signalwire.com/api`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-cxml-script.md) for the provider-specific parameters and requirements.

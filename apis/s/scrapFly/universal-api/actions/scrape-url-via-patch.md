# ScrapFly: Scrape URL via PATCH

Updates an existing page scrape in ScrapFly.

```
PUT https://connect.mindcloud.co/v1/universal/scrapFly/latest/actions/scrape-url-via-patch
```
...
| `url` | string | yes | Target URL to scrape with a PATCH request. Example: `https://httpbin.dev/anything`. |
...
Through the native ScrapFly API, this operation is `PATCH /scrape` (base URL `https://api.scrapfly.io`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/scrape-url-via-patch.md) for the provider-specific parameters and requirements.

# SafetyCulture: Update Issue Status

Updates an issue status in SafetyCulture.

```
PUT https://connect.mindcloud.co/v1/universal/safetyCulture/latest/actions/update-issue-status
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/safetyCulture/latest/actions/update-issue-status" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/safetyCulture/latest/actions/update-issue-status', {
  method: 'PUT',
  headers: {
...
Through the native SafetyCulture API, this operation is `PUT /tasks/v1/incidents/{task_id}/status` (base URL `https://api.safetyculture.io`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-issue-status.md) for the provider-specific parameters and requirements.

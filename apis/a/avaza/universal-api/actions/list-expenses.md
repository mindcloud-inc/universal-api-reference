...
```
GET https://connect.mindcloud.co/v1/universal/avaza/latest/actions/list-expenses
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/avaza/latest/actions/list-expenses?connectionId=$CONNECTION_ID&limit=25&offset=0" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/avaza/latest/actions/list-expenses?${params}`, {
  headers: {
...
| `expensedatefrom` | date | no |  |
| `expensedateto` | date | no |  |
| `useremail` | string | no |  |
| `userid` | number | no |  |
...
| `expensereimbursementidfk` | number | no |  |
| `expensepaymentmethodidfk` | number | no |  |
| `expenseapprovalstatuscode` | string | no |  |
| `search` | string | no |  |

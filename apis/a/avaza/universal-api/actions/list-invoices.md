...
```
GET https://connect.mindcloud.co/v1/universal/avaza/latest/actions/list-invoices
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/avaza/latest/actions/list-invoices?connectionId=$CONNECTION_ID&limit=25&offset=0" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/avaza/latest/actions/list-invoices?${params}`, {
  headers: {
...
| `transactionstatuscode` | string | no |  |
| `issuedatefrom` | date | no |  |
| `issuedateto` | date | no |  |
| `duedatefrom` | date | no |  |
| `duedateto` | date | no |  |

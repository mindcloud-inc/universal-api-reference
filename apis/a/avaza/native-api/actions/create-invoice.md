...
- **Official documentation:** [Create Invoice](https://api.avaza.com/#!/Invoice/Invoice_Post)
...
| `InvoiceNumber` | body | `string` | no | Pass any string. If left blank it will use the next number in the auto incrementing sequence. If an integer is passed then the largest integer will be use as the seed to auto generate the next invoice number in the sequence. |
...
| `InvoiceTemplateIDFK` | body | `number` | no | If left blank the account default invoice template will be used. |

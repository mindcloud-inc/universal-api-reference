# Appzi: List Portal Survey Configurations

Retrieves portal survey configurations from Appzi.

```
GET https://connect.mindcloud.co/v1/universal/appzi/latest/actions/list-portal-survey-configurations
```
...
| --- | --- | --- | --- |
| `portalToken` | string | yes | Appzi portal token from Portal Settings > Installation Code. Example: `fYbQ6`. |
...
Through the native Appzi API, this operation is `GET /api/probe/:portalToken` (base URL `https://api.appzi.io`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-portal-survey-configurations.md) for the provider-specific parameters and requirements.

# Appzi: Add Custom Feedback Data

Generates Appzi custom feedback data settings.

```
PUT https://connect.mindcloud.co/v1/universal/appzi/latest/actions/add-custom-feedback-data
```
...
| --- | --- | --- | --- |
| `dataJson` | string | no | Optional JSON object to merge into the Appzi feedback data payload. |
...
Through the native Appzi API, this operation is `GET https://docs.appzi.io/integration/add-data/` (base URL `https://api.appzi.io`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/add-custom-feedback-data.md) for the provider-specific parameters and requirements.

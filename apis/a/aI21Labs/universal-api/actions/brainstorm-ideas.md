# AI21 Labs: Brainstorm Ideas

Creates an idea brainstorming run in AI21 Labs.

```
POST https://connect.mindcloud.co/v1/universal/aI21Labs/latest/actions/brainstorm-ideas
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/aI21Labs/latest/actions/brainstorm-ideas" \
  -H "Authorization: Bearer $MINDCLOUD_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
  "connectionId": "$CONNECTION_ID",
  "input": "Describe the problem, goal, or theme you want ideas for."
}'
```

```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/aI21Labs/latest/actions/brainstorm-ideas', {
  method: 'POST',
  headers: {
...
| --- | --- | --- | --- |
| `input` | string | yes | The topic, problem, or goal to brainstorm around. Example: `Describe the problem, goal, or theme you want ideas for.`. |
...
Through the native AI21 Labs API, this operation is `POST /maestro/runs` (base URL `https://api.ai21.com/studio/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/brainstorm-ideas.md) for the provider-specific parameters and requirements.

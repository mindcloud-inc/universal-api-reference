# AI21 Labs: Compare Texts

Creates a text comparison run in AI21 Labs.

```
POST https://connect.mindcloud.co/v1/universal/aI21Labs/latest/actions/compare-texts
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/aI21Labs/latest/actions/compare-texts" \
  -H "Authorization: Bearer $MINDCLOUD_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
  "connectionId": "$CONNECTION_ID",
  "input": "Paste both texts and indicate that they should be compared."
}'
```

```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/aI21Labs/latest/actions/compare-texts', {
  method: 'POST',
  headers: {
...
| --- | --- | --- | --- |
| `input` | string | yes | The passages, versions, or drafts to compare. Example: `Paste both texts and indicate that they should be compared.`. |
...
Through the native AI21 Labs API, this operation is `POST /maestro/runs` (base URL `https://api.ai21.com/studio/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/compare-texts.md) for the provider-specific parameters and requirements.

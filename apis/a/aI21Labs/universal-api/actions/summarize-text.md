# AI21 Labs: Summarize Text

Creates a text summarization run in AI21 Labs.

```
POST https://connect.mindcloud.co/v1/universal/aI21Labs/latest/actions/summarize-text
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/aI21Labs/latest/actions/summarize-text" \
  -H "Authorization: Bearer $MINDCLOUD_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
  "connectionId": "$CONNECTION_ID",
  "input": "Paste the text you want summarized."
}'
```

```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/aI21Labs/latest/actions/summarize-text', {
  method: 'POST',
  headers: {
...
| --- | --- | --- | --- |
| `input` | string | yes | The source text, notes, or content to summarize. Example: `Paste the text you want summarized.`. |
...
Through the native AI21 Labs API, this operation is `POST /maestro/runs` (base URL `https://api.ai21.com/studio/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/summarize-text.md) for the provider-specific parameters and requirements.

# Aqara Home for KR Universal API Authentication

Every Aqara Home for KR API request through MindCloud uses a MindCloud API key and the `connectionId` of a Aqara Home for KR connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Aqara Home for KR connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Aqara Home for KR action](README.md#actions-9) with your API key and `connectionId`.

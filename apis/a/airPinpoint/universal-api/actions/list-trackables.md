# AirPinpoint: List Trackables

Retrieves trackable devices linked to AirPinpoint.

```
GET https://connect.mindcloud.co/v1/universal/airPinpoint/latest/actions/list-trackables
```
...
Through the native AirPinpoint API, this operation is `GET /trackables` (base URL `https://api.airpinpoint.com/v1`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/list-trackables.md) for the provider-specific parameters and requirements.

# AddEvent: Update calendar



```
PUT https://connect.mindcloud.co/v1/universal/addEvent/latest/actions/update-calendar
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/addEvent/latest/actions/update-calendar" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/addEvent/latest/actions/update-calendar', {
  method: 'PUT',
  headers: {
...
| `timezone` | string | no | Default timezone for events created on this calendar. |
| `weekdayBegin` | string | no | Day of the week that the calendar starts on. |
| `description` | string | no | Calendar description shown on the landing page. |
| `internalName` | string | no | Internal-only calendar name. |
| `calendarColor` | number | no | Calendar color value from the account palette. |
| `landingPageTemplateId` | string | no | Custom landing page template ID or default. |
| `embeddableCalendarTemplateId` | string | no | Custom embeddable calendar template ID or default. |
...
Through the native AddEvent API, this operation is `PATCH /calendars/:calendar_id` (base URL `https://api.addevent.com/calevent/v2`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-calendar.md) for the provider-specific parameters and requirements.

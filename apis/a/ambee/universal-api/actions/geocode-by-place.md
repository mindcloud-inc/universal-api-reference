# Ambee: Geocode By Place

Retrieves location coordinates in Ambee by place name.

```
GET https://connect.mindcloud.co/v1/universal/ambee/latest/actions/geocode-by-place
```
...
| --- | --- | --- | --- |
| `place` | string | yes | Name of the location to search. Example: `new york`. |
...
Through the native Ambee API, this operation is `GET /geocode/by-place` (base URL `https://api.ambeedata.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/geocode-by-place.md) for the provider-specific parameters and requirements.

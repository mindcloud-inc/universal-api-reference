# Acumatica: Get Payment



```
GET https://connect.mindcloud.co/v1/universal/acumatica/latest/actions/list-payment-by-id
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/acumatica/latest/actions/list-payment-by-id?connectionId=$CONNECTION_ID&id=671e3b2c-d87f-e411-beca-00b56d0561c2" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/acumatica/latest/actions/list-payment-by-id?${params}`, {
  headers: {
...
Through the native Acumatica API, this operation is `GET /entity/{{credentials.endpointName}}/{{credentials.endpointVersion}}/Payment/:id` (base URL `{{credentials.uRL}}`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-payment-by-id.md) for the provider-specific parameters and requirements.

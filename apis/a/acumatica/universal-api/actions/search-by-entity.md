...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/acumatica/latest/actions/search-by-entity?connectionId=$CONNECTION_ID&limit=25&offset=0&entity=Contacts&endpointVersion=Default&webServiceEndpoint=Default" \
...
| `endpointVersion` | list<string> | yes | Example: `Default`. |
...
| `webServiceEndpoint` | list<string> | yes | Which Web Service Endpoint configured at Acumatica will be used. "Default" exists in every account, but you can create custom endpoints One of: `Default`. Example: `Default`. |
...
Through the native Acumatica API, this operation is `GET /entity/:webServiceEndpoint/:endpointVersion/:entity` (base URL `{{credentials.uRL}}`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/search-by-entity.md) for the provider-specific parameters and requirements.

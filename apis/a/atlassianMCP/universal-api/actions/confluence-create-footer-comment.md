# Atlassian MCP: Confluence - Create Footer Comment

Creates a Confluence footer comment in Atlassian MCP.

```
POST https://connect.mindcloud.co/v1/universal/atlassianMCP/latest/actions/confluence-create-footer-comment
```
...
Through the native Atlassian MCP API, this operation is `POST /mcp` (base URL `https://mcp.atlassian.com/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/confluence-create-footer-comment.md) for the provider-specific parameters and requirements.

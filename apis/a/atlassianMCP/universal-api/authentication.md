# Atlassian MCP Universal API Authentication

Every Atlassian MCP API request through MindCloud uses a MindCloud API key and the `connectionId` of a Atlassian MCP connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Atlassian MCP connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Atlassian MCP action](README.md#actions-27) with your API key and `connectionId`.

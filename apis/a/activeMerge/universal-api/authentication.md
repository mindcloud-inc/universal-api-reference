# ActiveMerge Universal API Authentication

Every ActiveMerge API request through MindCloud uses a MindCloud API key and the `connectionId` of a ActiveMerge connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your ActiveMerge connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [ActiveMerge action](README.md#actions-7) with your API key and `connectionId`.

...
```
GET https://connect.mindcloud.co/v1/universal/acronis/latest/actions/list-tenants
```
...
Through the native Acronis API, this operation is `GET /api/2/tenants` (base URL `{{credentials.dataCenterUrl}}`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-tenants.md) for the provider-specific parameters and requirements.

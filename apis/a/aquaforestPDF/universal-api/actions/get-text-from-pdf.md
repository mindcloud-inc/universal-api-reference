# Aquaforest PDF: Get text from PDF

Retrieves matched text from PDFs in Aquaforest PDF.

```
GET https://connect.mindcloud.co/v1/universal/aquaforestPDF/latest/actions/get-text-from-pdf
```

Authenticate with `Authorization: Bearer $MINDCLOUD_API_KEY` and pass a Aquaforest PDF `connectionId` ([setup](../authentication.md)).

## Example request

```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/aquaforestPDF/latest/actions/get-text-from-pdf?connectionId=$CONNECTION_ID&fileContent=%2BPiA%2BPiAvQ29udGVudHMgNSAwIFIgPj4KZW5kb2JqCjQgMCBvYmoKPDwgL1R5cGUgL0ZvbnQgL1N1YnR5cGUgL1R5cGUxIC9CYXNlRm9udCAvSGVsdmV0aWNhID4%2BCmVuZG9iago1IDAgb2JqCjw8IC9MZW5ndGggMTM0ID4%%2BCnN0YXJ0eHJlZgo0OTUKJSVFT0YK&sourceFileName=aquaforest-runtime.pdf&textResultTemplate=%25VALUE1%25&noTextTemplate=NoText_%25FILENAME%25" \
  -H "Authorization: Bearer $MINDCLOUD_API_KEY"
```

```js
const params = new URLSearchParams({
  connectionId,
  "fileContent": "",
  "sourceFileName": "aquaforest-runtime.pdf",
  "textResultTemplate": "%VALUE1%",
  "noTextTemplate": "NoText_%FILENAME%"
});

const response = await fetch(`https://connect.mindcloud.co/v1/universal/aquaforestPDF/latest/actions/get-text-from-pdf?${params}`, {
  headers: {
    Authorization: `Bearer ${process.env.MINDCLOUD_API_KEY}`
  }
});

const { success, data } = await response.json();
```

## Inputs

Arguments are sent as query string parameters ([conventions](../arguments.md)).

| Key | Type | Required | Description |
| --- | --- | --- | --- |
| `fileContent` | file | yes | The content of the source PDF file. Default: ``. |
| `sourceFileName` | string | yes | The name of the source file. Default: `aquaforest-runtime.pdf`. |
| `textResultTemplate` | string | yes | Template for the text returned when a match is found. Default: `%VALUE1%`. |
| `noTextTemplate` | string | yes | Template for the text returned when no match is found. Default: `NoText_%FILENAME%`. |

### Advanced

| Key | Type | Required | Description |
| --- | --- | --- | --- |
| `pagerange` | string | no | Page range to extract text from, such as 1,2,4-7. Default: `1`. |
| `zones[]` | array<object> | no | Text extraction zone objects with location, expression, position, and optional regex. Default: `[{"location":"all","position":"all text in line after value","expression":["Invoice Number"]}]`. |

## Response

```json
{
  "success": true,
  "data": [
    {
      "ErrorMessage": "string",
      "IsSuccessful": true,
      "LicenceInfo": "string",
      "TextResult": "string",
      "TextResults": [
        {}
      ]
    }
  ],
  "meta": {}
}
```

### Response fields

| Key | Type | Description |
| --- | --- | --- |
| `ErrorMessage` | string | Error details when IsSuccessful is false. |
| `IsSuccessful` | boolean | Whether text was matched successfully. |
| `LicenceInfo` | string | API subscription key information. |
| `TextResult` | string | Generated text result after applying the result template. |
| `TextResults` | array<object> | Per-page extracted text values. |

## Native endpoint

Through the native Aquaforest PDF API, this operation is `POST /GetTextValue` (base URL `https://aquaforest-pdf.azure-api.net/AquaforestPDFAPIV2`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-text-from-pdf.md) for the provider-specific parameters and requirements.


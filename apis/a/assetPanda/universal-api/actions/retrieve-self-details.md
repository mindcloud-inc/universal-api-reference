# Asset Panda: Retrieve Self Details

Retrieves your user details from Asset Panda.

```
GET https://connect.mindcloud.co/v1/universal/assetPanda/latest/actions/retrieve-self-details
```
...
Through the native Asset Panda API, this operation is `GET /v3/users/me` (base URL `https://api.assetpanda.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/retrieve-self-details.md) for the provider-specific parameters and requirements.

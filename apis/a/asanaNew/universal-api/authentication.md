# Asana Universal API Authentication

Every Asana API request through MindCloud uses a MindCloud API key and the `connectionId` of a Asana connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Asana connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Asana action](README.md#actions-168) with your API key and `connectionId`.

# Ashby Job Postings Universal API Authentication

Every Ashby Job Postings API request through MindCloud uses a MindCloud API key and the `connectionId` of a Ashby Job Postings connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Ashby Job Postings connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Ashby Job Postings action](README.md#actions-1) with your API key and `connectionId`.

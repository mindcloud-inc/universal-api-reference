# Aspera on Cloud Universal API Authentication

Every Aspera on Cloud API request through MindCloud uses a MindCloud API key and the `connectionId` of a Aspera on Cloud connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Aspera on Cloud connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Aspera on Cloud action](README.md#actions-24) with your API key and `connectionId`.

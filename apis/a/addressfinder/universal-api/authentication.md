# Addressfinder Universal API Authentication

Every Addressfinder API request through MindCloud uses a MindCloud API key and the `connectionId` of a Addressfinder connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Addressfinder connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Addressfinder action](README.md#actions-5) with your API key and `connectionId`.

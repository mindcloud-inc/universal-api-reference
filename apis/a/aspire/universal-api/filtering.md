# Aspire Universal API Filtering

Filterable list actions accept a `where` query parameter written as an RSQL expression. MindCloud translates it into the filtering format Aspire expects, and each action page lists the fields available to filter.
...
## Aspire actions that support filtering

# Aspire Universal API Authentication

Every Aspire API request through MindCloud uses a MindCloud API key and the `connectionId` of a Aspire connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Aspire connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Aspire action](README.md#actions-153) with your API key and `connectionId`.

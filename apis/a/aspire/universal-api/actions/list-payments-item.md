# Aspire: List Payments Item



```
GET https://connect.mindcloud.co/v1/universal/aspire/latest/actions/list-payments-item
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/aspire/latest/actions/list-payments-item?connectionId=$CONNECTION_ID&limit=25&offset=0" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/aspire/latest/actions/list-payments-item?${params}`, {
  headers: {
...
Through the native Aspire API, this operation is `GET Payments` (base URL `https://{{credentials.environment}}.youraspire.com/`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/list-payments-item.md) for the provider-specific parameters and requirements.

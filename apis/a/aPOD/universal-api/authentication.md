# APOD Universal API Authentication

Every APOD API request through MindCloud uses a MindCloud API key and the `connectionId` of a APOD connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your APOD connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [APOD action](README.md#actions-4) with your API key and `connectionId`.

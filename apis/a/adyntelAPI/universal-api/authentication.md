# Adyntel Universal API Authentication

Every Adyntel API request through MindCloud uses a MindCloud API key and the `connectionId` of a Adyntel connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Adyntel connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Adyntel action](README.md#actions-10) with your API key and `connectionId`.

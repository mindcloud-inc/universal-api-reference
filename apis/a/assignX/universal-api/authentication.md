# AssignX Universal API Authentication

Every AssignX API request through MindCloud uses a MindCloud API key and the `connectionId` of a AssignX connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your AssignX connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [AssignX action](README.md#actions-12) with your API key and `connectionId`.

# actiTIME: List Customer Comments

Retrieves comments for a customer in actiTIME.

```
GET https://connect.mindcloud.co/v1/universal/actiTIME/latest/actions/list-customer-comments
```
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/actiTIME/latest/actions/list-customer-comments?${params}`, {
  headers: {
...
Through the native actiTIME API, this operation is `GET /customers/:id/comments` (base URL `{{credentials.instanceUrl}}/api/v1`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/list-customer-comments.md) for the provider-specific parameters and requirements.

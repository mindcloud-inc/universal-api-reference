# AntsRoute: Get Customer by External ID

Retrieves a customer from AntsRoute by external ID.

```
GET https://connect.mindcloud.co/v1/universal/antsRoute/latest/actions/get-customer-by-external-id
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/antsRoute/latest/actions/get-customer-by-external-id?connectionId=$CONNECTION_ID&externalId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/antsRoute/latest/actions/get-customer-by-external-id?${params}`, {
  headers: {

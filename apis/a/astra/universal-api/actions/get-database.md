...
Retrieves one Astra database by ID.

```
GET https://connect.mindcloud.co/v1/universal/astra/latest/actions/get-database
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/astra/latest/actions/get-database?connectionId=$CONNECTION_ID&databaseId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/astra/latest/actions/get-database?${params}`, {
  headers: {
...
| `info` | object | Database metadata including name, region, tier, keyspaces, and datacenters. |

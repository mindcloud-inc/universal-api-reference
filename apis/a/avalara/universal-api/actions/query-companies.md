# Avalara AvaTax: Query Companies



```
GET https://connect.mindcloud.co/v1/universal/avalara/latest/actions/query-companies
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/avalara/latest/actions/query-companies?connectionId=$CONNECTION_ID&limit=25&offset=0" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/avalara/latest/actions/query-companies?${params}`, {
  headers: {
...
Through the native Avalara AvaTax API, this operation is `GET companies` (base URL `{{credentials.environment}}`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/query-companies.md) for the provider-specific parameters and requirements.

# Apideck: Import connection

Imports an authorized connection into Apideck Vault.

```
POST https://connect.mindcloud.co/v1/universal/apideck/latest/actions/connectionsimport
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/apideck/latest/actions/connectionsimport" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/apideck/latest/actions/connectionsimport', {
  method: 'POST',
  headers: {
...
Through the native Apideck API, this operation is `POST /vault/connections/:unified_api/:service_id/import` (base URL `https://unify.apideck.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/connectionsimport.md) for the provider-specific parameters and requirements.

# Apideck: Update settings

Updates connection resource settings in Apideck Vault.

```
PUT https://connect.mindcloud.co/v1/universal/apideck/latest/actions/connectionsettingsupdate
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/apideck/latest/actions/connectionsettingsupdate" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/apideck/latest/actions/connectionsettingsupdate', {
  method: 'PUT',
  headers: {
...
Through the native Apideck API, this operation is `PATCH /vault/connections/:unified_api/:service_id/:resource/config` (base URL `https://unify.apideck.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/connectionsettingsupdate.md) for the provider-specific parameters and requirements.

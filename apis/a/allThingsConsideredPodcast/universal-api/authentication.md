# All Things Considered Podcast Universal API Authentication

Every All Things Considered Podcast API request through MindCloud uses a MindCloud API key and the `connectionId` of a All Things Considered Podcast connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your All Things Considered Podcast connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [All Things Considered Podcast action](README.md#actions-8) with your API key and `connectionId`.

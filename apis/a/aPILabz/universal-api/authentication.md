# API Labz Universal API Authentication

Every API Labz API request through MindCloud uses a MindCloud API key and the `connectionId` of a API Labz connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your API Labz connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [API Labz action](README.md#actions-5) with your API key and `connectionId`.

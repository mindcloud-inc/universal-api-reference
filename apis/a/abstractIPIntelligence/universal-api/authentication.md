# Abstract IP Intelligence Universal API Authentication

Every Abstract IP Intelligence API request through MindCloud uses a MindCloud API key and the `connectionId` of a Abstract IP Intelligence connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Abstract IP Intelligence connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Abstract IP Intelligence action](README.md#actions-1) with your API key and `connectionId`.

# Aqara Home for RU: Update Device Position

Updates a device position in Aqara Home for RU.

```
PUT https://connect.mindcloud.co/v1/universal/aqaraHomeForRU/latest/actions/config-device-position
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/aqaraHomeForRU/latest/actions/config-device-position" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/aqaraHomeForRU/latest/actions/config-device-position', {
  method: 'PUT',
  headers: {

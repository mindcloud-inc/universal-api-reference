# Aqara Home for RU: Update Position Time Zone

Updates a position time zone in Aqara Home for RU.

```
PUT https://connect.mindcloud.co/v1/universal/aqaraHomeForRU/latest/actions/config-position-time-zone
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/aqaraHomeForRU/latest/actions/config-position-time-zone" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/aqaraHomeForRU/latest/actions/config-position-time-zone', {
  method: 'PUT',
  headers: {

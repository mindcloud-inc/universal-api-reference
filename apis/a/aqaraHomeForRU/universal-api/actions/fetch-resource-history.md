# Aqara Home for RU: Fetch Resource History

Retrieves resource history from Aqara Home for RU.

```
GET https://connect.mindcloud.co/v1/universal/aqaraHomeForRU/latest/actions/fetch-resource-history
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/aqaraHomeForRU/latest/actions/fetch-resource-history?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/aqaraHomeForRU/latest/actions/fetch-resource-history?${params}`, {
  headers: {

# Aqara Home for RU: Run Scene

Runs a scene in Aqara Home for RU.

```
POST https://connect.mindcloud.co/v1/universal/aqaraHomeForRU/latest/actions/config-scene-run
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/aqaraHomeForRU/latest/actions/config-scene-run" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/aqaraHomeForRU/latest/actions/config-scene-run', {
  method: 'POST',
  headers: {

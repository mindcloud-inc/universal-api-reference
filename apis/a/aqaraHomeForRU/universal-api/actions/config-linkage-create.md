# Aqara Home for RU: Create Linkage

Creates a linkage in Aqara Home for RU.

```
POST https://connect.mindcloud.co/v1/universal/aqaraHomeForRU/latest/actions/config-linkage-create
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/aqaraHomeForRU/latest/actions/config-linkage-create" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/aqaraHomeForRU/latest/actions/config-linkage-create', {
  method: 'POST',
  headers: {

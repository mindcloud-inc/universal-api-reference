# Aqara Home for RU: Update Scene

Updates a scene in Aqara Home for RU.

```
PUT https://connect.mindcloud.co/v1/universal/aqaraHomeForRU/latest/actions/config-scene-update
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/aqaraHomeForRU/latest/actions/config-scene-update" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/aqaraHomeForRU/latest/actions/config-scene-update', {
  method: 'PUT',
  headers: {

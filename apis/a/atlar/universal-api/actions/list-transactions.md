# Atlar: List transactions

Retrieves transactions from Atlar.

```
GET https://connect.mindcloud.co/v1/universal/atlar/latest/actions/list-transactions
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/atlar/latest/actions/list-transactions?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/atlar/latest/actions/list-transactions?${params}`, {
  headers: {
...
| `reconciliationStatus` | string<string> | no |  |

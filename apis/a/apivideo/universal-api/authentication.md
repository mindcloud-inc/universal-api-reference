# api.video Universal API Authentication

Every api.video API request through MindCloud uses a MindCloud API key and the `connectionId` of a api.video connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your api.video connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [api.video action](README.md#actions-36) with your API key and `connectionId`.

# Abyssale: Duplicate Workspace Template With Custom Name

Duplicates a workspace template into an Abyssale project with a custom name.

```
POST https://connect.mindcloud.co/v1/universal/abyssale/latest/actions/duplicate-workspace-template-with-custom-name
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/abyssale/latest/actions/duplicate-workspace-template-with-custom-name" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/abyssale/latest/actions/duplicate-workspace-template-with-custom-name', {
  method: 'POST',
  headers: {
...
| `name` | string | yes | Optional custom name for the duplicated template Example: `My Custom Template`. |

# Abstract IBAN Validator Universal API Authentication

Every Abstract IBAN Validator API request through MindCloud uses a MindCloud API key and the `connectionId` of a Abstract IBAN Validator connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Abstract IBAN Validator connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Abstract IBAN Validator action](README.md#actions-1) with your API key and `connectionId`.

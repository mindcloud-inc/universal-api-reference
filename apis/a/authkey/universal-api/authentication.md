# Authkey Universal API Authentication

Every Authkey API request through MindCloud uses a MindCloud API key and the `connectionId` of a Authkey connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Authkey connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Authkey action](README.md#actions-22) with your API key and `connectionId`.

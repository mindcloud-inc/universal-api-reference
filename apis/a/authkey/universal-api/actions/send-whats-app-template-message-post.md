# Authkey: Send WhatsApp Template Message (POST)

Sends a WhatsApp template message through Authkey.

```
POST https://connect.mindcloud.co/v1/universal/authkey/latest/actions/send-whats-app-template-message-post
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/authkey/latest/actions/send-whats-app-template-message-post" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/authkey/latest/actions/send-whats-app-template-message-post', {
  method: 'POST',
  headers: {

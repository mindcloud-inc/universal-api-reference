# Authkey: Send WhatsApp Template Message

Sends a WhatsApp template message through Authkey.

```
POST https://connect.mindcloud.co/v1/universal/authkey/latest/actions/send-whats-app-template-message
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/authkey/latest/actions/send-whats-app-template-message" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/authkey/latest/actions/send-whats-app-template-message', {
  method: 'POST',
  headers: {

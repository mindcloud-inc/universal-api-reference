# Automate Sales CRM Universal API Authentication

Every Automate Sales CRM API request through MindCloud uses a MindCloud API key and the `connectionId` of a Automate Sales CRM connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Automate Sales CRM connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Automate Sales CRM action](README.md#actions-1) with your API key and `connectionId`.

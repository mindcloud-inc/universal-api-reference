# Appwrite: Update authenticator (confirmation)

Completes MFA authenticator setup in Appwrite.

```
PUT https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/account-update-mfa-authenticator
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/account-update-mfa-authenticator" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/account-update-mfa-authenticator', {
  method: 'PUT',
  headers: {
...
Through the native Appwrite API, this operation is `PUT /account/mfa/authenticators/{type}` (base URL `https://cloud.appwrite.io/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/account-update-mfa-authenticator.md) for the provider-specific parameters and requirements.

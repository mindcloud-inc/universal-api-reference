# Appwrite: Update site's deployment

Updates a site deployment in your Appwrite project.

```
PUT https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/sites-update-site-deployment
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/sites-update-site-deployment" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/sites-update-site-deployment', {
  method: 'PUT',
  headers: {
...
Through the native Appwrite API, this operation is `PATCH /sites/{siteId}/deployment` (base URL `https://cloud.appwrite.io/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/sites-update-site-deployment.md) for the provider-specific parameters and requirements.

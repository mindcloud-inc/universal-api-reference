...
```
PUT https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/sites-update-variable
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/sites-update-variable" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/sites-update-variable', {
  method: 'PUT',
  headers: {
...
Through the native Appwrite API, this operation is `PUT /sites/{siteId}/variables/{variableId}` (base URL `https://cloud.appwrite.io/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/sites-update-variable.md) for the provider-specific parameters and requirements.

# Appwrite: Update MFA challenge (confirmation)

Completes an MFA challenge in Appwrite.

```
PUT https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/account-update-mfa-challenge
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/account-update-mfa-challenge" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/account-update-mfa-challenge', {
  method: 'PUT',
  headers: {
...
Through the native Appwrite API, this operation is `PUT /account/mfa/challenges` (base URL `https://cloud.appwrite.io/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/account-update-mfa-challenge.md) for the provider-specific parameters and requirements.

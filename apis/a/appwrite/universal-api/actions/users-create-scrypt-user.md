# Appwrite: Create user with Scrypt password

Creates a new user with Scrypt password in your Appwrite project.

```
POST https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/users-create-scrypt-user
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/users-create-scrypt-user" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/users-create-scrypt-user', {
  method: 'POST',
  headers: {
...
| `passwordCpu` | number | yes | Optional CPU cost used to hash password. |
| `passwordMemory` | number | yes | Optional memory cost used to hash password. |
| `passwordParallel` | number | yes | Optional parallelization cost used to hash password. |
| `passwordLength` | number | yes | Optional hash length used to hash password. |
...
Through the native Appwrite API, this operation is `POST /users/scrypt` (base URL `https://cloud.appwrite.io/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/users-create-scrypt-user.md) for the provider-specific parameters and requirements.

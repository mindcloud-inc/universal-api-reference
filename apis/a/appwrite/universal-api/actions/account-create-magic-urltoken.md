# Appwrite: Create magic URL token

Creates a new magic URL token in your Appwrite project.

```
POST https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/account-create-magic-urltoken
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/account-create-magic-urltoken" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/account-create-magic-urltoken', {
  method: 'POST',
  headers: {
...
Through the native Appwrite API, this operation is `POST /account/tokens/magic-url` (base URL `https://cloud.appwrite.io/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/account-create-magic-urltoken.md) for the provider-specific parameters and requirements.

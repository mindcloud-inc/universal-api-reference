# Appwrite: Update team membership status

Updates the team membership status in your Appwrite project.

```
PUT https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/teams-update-membership-status
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/teams-update-membership-status" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/teams-update-membership-status', {
  method: 'PUT',
  headers: {
...
Through the native Appwrite API, this operation is `PATCH /teams/{teamId}/memberships/{membershipId}/status` (base URL `https://cloud.appwrite.io/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/teams-update-membership-status.md) for the provider-specific parameters and requirements.

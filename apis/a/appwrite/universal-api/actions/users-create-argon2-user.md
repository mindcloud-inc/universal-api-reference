# Appwrite: Create user with Argon2 password

Creates a new user with Argon2 password in your Appwrite project.

```
POST https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/users-create-argon2-user
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/users-create-argon2-user" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/users-create-argon2-user', {
  method: 'POST',
  headers: {
...
Through the native Appwrite API, this operation is `POST /users/argon2` (base URL `https://cloud.appwrite.io/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/users-create-argon2-user.md) for the provider-specific parameters and requirements.

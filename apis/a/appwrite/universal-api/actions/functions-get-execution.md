# Appwrite: Get execution

Retrieves the execution from your Appwrite project.

```
GET https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/functions-get-execution
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/functions-get-execution?connectionId=$CONNECTION_ID&functionId=string&executionId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/functions-get-execution?${params}`, {
  headers: {
...
Through the native Appwrite API, this operation is `GET /functions/{functionId}/executions/{executionId}` (base URL `https://cloud.appwrite.io/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/functions-get-execution.md) for the provider-specific parameters and requirements.

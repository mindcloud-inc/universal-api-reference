...
Retrieves account details from Appwrite.

```
GET https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/account-get
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/account-get?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/account-get?${params}`, {
  headers: {
...
Through the native Appwrite API, this operation is `GET /account` (base URL `https://cloud.appwrite.io/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/account-get.md) for the provider-specific parameters and requirements.

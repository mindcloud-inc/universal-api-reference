# Appwrite: Create Vonage provider

Creates a new vonage provider in your Appwrite project.

```
POST https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/messaging-create-vonage-provider
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/messaging-create-vonage-provider" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/messaging-create-vonage-provider', {
  method: 'POST',
  headers: {
...
Through the native Appwrite API, this operation is `POST /messaging/providers/vonage` (base URL `https://cloud.appwrite.io/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/messaging-create-vonage-provider.md) for the provider-specific parameters and requirements.

...
```
GET https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/sites-get-deployment
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/sites-get-deployment?connectionId=$CONNECTION_ID&siteId=string&deploymentId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/sites-get-deployment?${params}`, {
  headers: {
...
Through the native Appwrite API, this operation is `GET /sites/{siteId}/deployments/{deploymentId}` (base URL `https://cloud.appwrite.io/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/sites-get-deployment.md) for the provider-specific parameters and requirements.

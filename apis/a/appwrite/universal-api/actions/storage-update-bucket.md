# Appwrite: Update bucket

Updates the bucket in your Appwrite project.

```
PUT https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/storage-update-bucket
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/storage-update-bucket" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/storage-update-bucket', {
  method: 'PUT',
  headers: {
...
Through the native Appwrite API, this operation is `PUT /storage/buckets/{bucketId}` (base URL `https://cloud.appwrite.io/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/storage-update-bucket.md) for the provider-specific parameters and requirements.

# Appwrite: Update magic URL session

Updates the magic URL session in your Appwrite project.

```
PUT https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/account-update-magic-urlsession
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/account-update-magic-urlsession" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/account-update-magic-urlsession', {
  method: 'PUT',
  headers: {
...
Through the native Appwrite API, this operation is `PUT /account/sessions/magic-url` (base URL `https://cloud.appwrite.io/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/account-update-magic-urlsession.md) for the provider-specific parameters and requirements.

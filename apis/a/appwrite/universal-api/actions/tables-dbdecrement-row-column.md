# Appwrite: Decrement row column

Decrements the row column in your Appwrite project.

```
PUT https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/tables-dbdecrement-row-column
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/tables-dbdecrement-row-column" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/tables-dbdecrement-row-column', {
  method: 'PUT',
  headers: {
...
Through the native Appwrite API, this operation is `PATCH /tablesdb/{databaseId}/tables/{tableId}/rows/{rowId}/{column}/decrement` (base URL `https://cloud.appwrite.io/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/tables-dbdecrement-row-column.md) for the provider-specific parameters and requirements.

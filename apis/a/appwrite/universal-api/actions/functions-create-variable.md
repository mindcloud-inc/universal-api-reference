...
```
POST https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/functions-create-variable
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/functions-create-variable" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/functions-create-variable', {
  method: 'POST',
  headers: {
...
Through the native Appwrite API, this operation is `POST /functions/{functionId}/variables` (base URL `https://cloud.appwrite.io/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/functions-create-variable.md) for the provider-specific parameters and requirements.

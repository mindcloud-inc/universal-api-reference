...
```
POST https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/sites-create-deployment
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/sites-create-deployment" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/sites-create-deployment', {
  method: 'POST',
  headers: {
...
Through the native Appwrite API, this operation is `POST /sites/{siteId}/deployments` (base URL `https://cloud.appwrite.io/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/sites-create-deployment.md) for the provider-specific parameters and requirements.

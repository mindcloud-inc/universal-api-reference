# Appwrite: Create user with bcrypt password

Creates a new user with bcrypt password in your Appwrite project.

```
POST https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/users-create-bcrypt-user
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/users-create-bcrypt-user" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/users-create-bcrypt-user', {
  method: 'POST',
  headers: {
...
Through the native Appwrite API, this operation is `POST /users/bcrypt` (base URL `https://cloud.appwrite.io/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/users-create-bcrypt-user.md) for the provider-specific parameters and requirements.

...
```
PUT https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/sites-update-deployment-status
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/sites-update-deployment-status" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/sites-update-deployment-status', {
  method: 'PUT',
  headers: {
...
Through the native Appwrite API, this operation is `PATCH /sites/{siteId}/deployments/{deploymentId}/status` (base URL `https://cloud.appwrite.io/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/sites-update-deployment-status.md) for the provider-specific parameters and requirements.

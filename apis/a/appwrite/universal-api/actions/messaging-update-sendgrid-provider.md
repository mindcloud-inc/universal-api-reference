# Appwrite: Update Sendgrid provider

Updates the sendgrid provider in your Appwrite project.

```
PUT https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/messaging-update-sendgrid-provider
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/messaging-update-sendgrid-provider" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/messaging-update-sendgrid-provider', {
  method: 'PUT',
  headers: {
...
Through the native Appwrite API, this operation is `PATCH /messaging/providers/sendgrid/{providerId}` (base URL `https://cloud.appwrite.io/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/messaging-update-sendgrid-provider.md) for the provider-specific parameters and requirements.

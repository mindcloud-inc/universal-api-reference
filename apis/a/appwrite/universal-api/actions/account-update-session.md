# Appwrite: Update session

Updates the session in your Appwrite project.

```
PUT https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/account-update-session
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/account-update-session" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/account-update-session', {
  method: 'PUT',
  headers: {
...
Through the native Appwrite API, this operation is `PATCH /account/sessions/{sessionId}` (base URL `https://cloud.appwrite.io/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/account-update-session.md) for the provider-specific parameters and requirements.

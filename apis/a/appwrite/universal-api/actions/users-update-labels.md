# Appwrite: Update user labels

Updates the user labels in your Appwrite project.

```
PUT https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/users-update-labels
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/users-update-labels" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/users-update-labels', {
  method: 'PUT',
  headers: {
...
| --- | --- | --- | --- |
| `labels` | string | no | Array of user labels. Replaces the previous labels. Maximum of 1000 labels are allowed, each up to 36 alphanumeric characters long. |
...
Through the native Appwrite API, this operation is `PUT /users/{userId}/labels` (base URL `https://cloud.appwrite.io/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/users-update-labels.md) for the provider-specific parameters and requirements.

...
```
POST https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/account-create-session
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/account-create-session" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/account-create-session', {
  method: 'POST',
  headers: {
...
Through the native Appwrite API, this operation is `POST /account/sessions/token` (base URL `https://cloud.appwrite.io/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/account-create-session.md) for the provider-specific parameters and requirements.

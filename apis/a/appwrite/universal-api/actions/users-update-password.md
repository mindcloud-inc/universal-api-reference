...
```
PUT https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/users-update-password
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/users-update-password" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/users-update-password', {
  method: 'PUT',
  headers: {
...
Through the native Appwrite API, this operation is `PATCH /users/{userId}/password` (base URL `https://cloud.appwrite.io/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/users-update-password.md) for the provider-specific parameters and requirements.

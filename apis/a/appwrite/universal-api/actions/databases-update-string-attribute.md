# Appwrite: Update string attribute

Updates the string attribute in your Appwrite project.

```
PUT https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/databases-update-string-attribute
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/databases-update-string-attribute" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/databases-update-string-attribute', {
  method: 'PUT',
  headers: {
...
| `size` | number | no | Maximum size of the string attribute. |
...
Through the native Appwrite API, this operation is `PATCH /databases/{databaseId}/collections/{collectionId}/attributes/string/{key}` (base URL `https://cloud.appwrite.io/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/databases-update-string-attribute.md) for the provider-specific parameters and requirements.

# Appwrite: Upsert a document

Upserts a document in your Appwrite project.

```
PUT https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/databases-upsert-document
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/databases-upsert-document" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/appwrite/latest/actions/databases-upsert-document', {
  method: 'PUT',
  headers: {
...
Through the native Appwrite API, this operation is `PUT /databases/{databaseId}/collections/{collectionId}/documents/{documentId}` (base URL `https://cloud.appwrite.io/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/databases-upsert-document.md) for the provider-specific parameters and requirements.

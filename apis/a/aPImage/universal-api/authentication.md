# APImage Universal API Authentication

Every APImage API request through MindCloud uses a MindCloud API key and the `connectionId` of a APImage connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your APImage connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [APImage action](README.md#actions-4) with your API key and `connectionId`.

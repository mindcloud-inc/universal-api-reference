# Aqara Home for CH Universal API Authentication

Every Aqara Home for CH API request through MindCloud uses a MindCloud API key and the `connectionId` of a Aqara Home for CH connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Aqara Home for CH connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Aqara Home for CH action](README.md#actions-8) with your API key and `connectionId`.

# Api2Convert: List Job Inputs

Retrieves input files for a job from Api2Convert.

```
GET https://connect.mindcloud.co/v1/universal/api2Convert/latest/actions/list-job-inputs
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/api2Convert/latest/actions/list-job-inputs?connectionId=$CONNECTION_ID&jobId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/api2Convert/latest/actions/list-job-inputs?${params}`, {
  headers: {
...
Through the native Api2Convert API, this operation is `GET /jobs/:job_id/input` (base URL `https://api.api2convert.com/v2`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-job-inputs.md) for the provider-specific parameters and requirements.

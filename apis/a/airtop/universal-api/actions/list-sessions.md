# Airtop: List Sessions

Finds sessions in Airtop by ID or status.

```
GET https://connect.mindcloud.co/v1/universal/airtop/latest/actions/list-sessions
```
...
Through the native Airtop API, this operation is `GET /sessions` (base URL `https://api.airtop.ai/api/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-sessions.md) for the provider-specific parameters and requirements.

...
```
GET https://connect.mindcloud.co/v1/universal/agentMail/latest/actions/get-inbox-thread
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/agentMail/latest/actions/get-inbox-thread?connectionId=$CONNECTION_ID&inboxId=string&threadId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/agentMail/latest/actions/get-inbox-thread?${params}`, {
  headers: {
...
Through the native Agent Mail API, this operation is `GET /inboxes/{inbox_id}/threads/{thread_id}` (base URL `https://api.agentmail.to/v0`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-inbox-thread.md) for the provider-specific parameters and requirements.

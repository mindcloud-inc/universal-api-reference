# Aqara Home for SG Universal API Authentication

Every Aqara Home for SG API request through MindCloud uses a MindCloud API key and the `connectionId` of a Aqara Home for SG connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Aqara Home for SG connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Aqara Home for SG action](README.md#actions-3) with your API key and `connectionId`.

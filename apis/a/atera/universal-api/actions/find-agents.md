...
Finds agents in Atera.

```
GET https://connect.mindcloud.co/v1/universal/atera/latest/actions/find-agents
```
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/atera/latest/actions/find-agents?${params}`, {
  headers: {
...
Through the native Atera API, this operation is `GET /api/v3/agents` (base URL `https://app.atera.com`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/find-agents.md) for the provider-specific parameters and requirements.

...
Retrieves an agent from Atera by ID.

```
GET https://connect.mindcloud.co/v1/universal/atera/latest/actions/get-agent
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/atera/latest/actions/get-agent?connectionId=$CONNECTION_ID&agentId=1" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/atera/latest/actions/get-agent?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `agentId` | number | yes | System agent ID. |

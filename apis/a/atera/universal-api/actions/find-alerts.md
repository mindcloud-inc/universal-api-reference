...
Finds alerts in Atera.

```
GET https://connect.mindcloud.co/v1/universal/atera/latest/actions/find-alerts
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/atera/latest/actions/find-alerts?connectionId=$CONNECTION_ID&limit=25&offset=0" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/atera/latest/actions/find-alerts?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `alertStatus` | string | no | Filter alerts by status. Example: `Open`. |
...
Through the native Atera API, this operation is `GET /api/v3/alerts` (base URL `https://app.atera.com`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/find-alerts.md) for the provider-specific parameters and requirements.

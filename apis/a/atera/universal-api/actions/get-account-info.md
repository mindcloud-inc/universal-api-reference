# Atera: Get account info

Retrieves account details from Atera.

```
GET https://connect.mindcloud.co/v1/universal/atera/latest/actions/get-account-info
```

Authenticate with `Authorization: Bearer $MINDCLOUD_API_KEY` and pass a Atera `connectionId` ([setup](../authentication.md)).

## Example request

```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/atera/latest/actions/get-account-info?connectionId=$CONNECTION_ID" \
  -H "Authorization: Bearer $MINDCLOUD_API_KEY"
```

```js
const params = new URLSearchParams({
  connectionId
});

const response = await fetch(`https://connect.mindcloud.co/v1/universal/atera/latest/actions/get-account-info?${params}`, {
  headers: {
    Authorization: `Bearer ${process.env.MINDCLOUD_API_KEY}`
  }
});

const { success, data } = await response.json();
```



## Response

```json
{
  "success": true,
  "data": [
    {
      "accountID": "string",
      "companyName": "Ava Chen",
      "country": "string",
      "createdOn": "string",
      "isITDepartment": true,
      "plan": "string",
      "timeZoneName": "Ava Chen"
    }
  ],
  "meta": {}
}
```

### Response fields

| Key | Type | Description |
| --- | --- | --- |
| `accountID` | string | Atera account ID. |
| `companyName` | string | Company name. |
| `country` | string | Account country. |
| `createdOn` | string | Account creation timestamp returned by Atera. |
| `isITDepartment` | boolean | Whether the account is configured as an IT department. |
| `plan` | string | Current Atera plan name. |
| `timeZoneName` | string | Configured account time zone. |

## Native endpoint

Through the native Atera API, this operation is `GET /api/v3/account` (base URL `https://app.atera.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-account-info.md) for the provider-specific parameters and requirements.


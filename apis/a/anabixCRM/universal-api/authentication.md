# Anabix CRM Universal API Authentication

Every Anabix CRM API request through MindCloud uses a MindCloud API key and the `connectionId` of a Anabix CRM connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Anabix CRM connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Anabix CRM action](README.md#actions-25) with your API key and `connectionId`.

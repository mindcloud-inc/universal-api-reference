# Apple News and Music: Lookup Catalog Item by iTunes ID

Retrieves a catalog item from Apple's catalog by iTunes ID.

```
GET https://connect.mindcloud.co/v1/universal/appleNewsAndMusic/latest/actions/lookup-catalog-item-by-itunes-id
```
...
| --- | --- | --- | --- |
| `id` | string | yes | Apple iTunes identifier to look up, such as an artist, album, or track ID. Example: `e.g. 909253`. |
...
Through the native Apple News and Music API, this operation is `GET https://itunes.apple.com/lookup` (base URL `https://itunes.apple.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/lookup-catalog-item-by-itunes-id.md) for the provider-specific parameters and requirements.

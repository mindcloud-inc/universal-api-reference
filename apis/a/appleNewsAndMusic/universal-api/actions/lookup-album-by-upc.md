# Apple News and Music: Lookup Album by UPC

Retrieves an album from Apple's catalog by UPC.

```
GET https://connect.mindcloud.co/v1/universal/appleNewsAndMusic/latest/actions/lookup-album-by-upc
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/appleNewsAndMusic/latest/actions/lookup-album-by-upc?connectionId=$CONNECTION_ID&upc=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/appleNewsAndMusic/latest/actions/lookup-album-by-upc?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `upc` | string | yes | Universal Product Code for an album in Apple's catalog. |
...
Through the native Apple News and Music API, this operation is `GET https://itunes.apple.com/lookup` (base URL `https://itunes.apple.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/lookup-album-by-upc.md) for the provider-specific parameters and requirements.

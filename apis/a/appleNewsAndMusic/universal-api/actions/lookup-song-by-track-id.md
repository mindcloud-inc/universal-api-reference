# Apple News and Music: Lookup Song by Track ID

Retrieves a song from Apple's catalog by track ID.

```
GET https://connect.mindcloud.co/v1/universal/appleNewsAndMusic/latest/actions/lookup-song-by-track-id
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/appleNewsAndMusic/latest/actions/lookup-song-by-track-id?connectionId=$CONNECTION_ID&id=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/appleNewsAndMusic/latest/actions/lookup-song-by-track-id?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `id` | string | yes | Apple song track identifier to look up. |
...
Through the native Apple News and Music API, this operation is `GET https://itunes.apple.com/lookup` (base URL `https://itunes.apple.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/lookup-song-by-track-id.md) for the provider-specific parameters and requirements.

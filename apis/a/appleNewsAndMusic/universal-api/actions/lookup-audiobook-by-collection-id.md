# Apple News and Music: Lookup Audiobook by Collection ID

Retrieves an audiobook from Apple's catalog by collection ID.

```
GET https://connect.mindcloud.co/v1/universal/appleNewsAndMusic/latest/actions/lookup-audiobook-by-collection-id
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/appleNewsAndMusic/latest/actions/lookup-audiobook-by-collection-id?connectionId=$CONNECTION_ID&id=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/appleNewsAndMusic/latest/actions/lookup-audiobook-by-collection-id?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `id` | string | yes | Apple audiobook collection identifier to look up. |
...
Through the native Apple News and Music API, this operation is `GET https://itunes.apple.com/lookup` (base URL `https://itunes.apple.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/lookup-audiobook-by-collection-id.md) for the provider-specific parameters and requirements.

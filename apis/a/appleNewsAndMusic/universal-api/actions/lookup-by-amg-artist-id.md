# Apple News and Music: Lookup by AMG Artist ID

Retrieves a catalog item from Apple's catalog by AMG artist ID.

```
GET https://connect.mindcloud.co/v1/universal/appleNewsAndMusic/latest/actions/lookup-by-amg-artist-id
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/appleNewsAndMusic/latest/actions/lookup-by-amg-artist-id?connectionId=$CONNECTION_ID&amgArtistId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/appleNewsAndMusic/latest/actions/lookup-by-amg-artist-id?${params}`, {
  headers: {
...
Through the native Apple News and Music API, this operation is `GET https://itunes.apple.com/lookup` (base URL `https://itunes.apple.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/lookup-by-amg-artist-id.md) for the provider-specific parameters and requirements.

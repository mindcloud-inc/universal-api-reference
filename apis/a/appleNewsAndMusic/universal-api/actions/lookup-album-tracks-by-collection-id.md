# Apple News and Music: Lookup Album Tracks by Collection ID

Retrieves an album's tracks from Apple's catalog by collection ID.

```
GET https://connect.mindcloud.co/v1/universal/appleNewsAndMusic/latest/actions/lookup-album-tracks-by-collection-id
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/appleNewsAndMusic/latest/actions/lookup-album-tracks-by-collection-id?connectionId=$CONNECTION_ID&id=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/appleNewsAndMusic/latest/actions/lookup-album-tracks-by-collection-id?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `id` | string | yes | Apple album collection identifier to expand into track results. |
| `entity` | string | no | Fixed Apple lookup entity. Default: `song`. |
| `limit` | string | no | Maximum number of track results to return. Default: `10`. |
...
Through the native Apple News and Music API, this operation is `GET https://itunes.apple.com/lookup` (base URL `https://itunes.apple.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/lookup-album-tracks-by-collection-id.md) for the provider-specific parameters and requirements.

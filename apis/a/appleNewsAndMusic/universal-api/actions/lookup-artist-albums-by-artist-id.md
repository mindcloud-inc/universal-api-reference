# Apple News and Music: Lookup Artist Albums by Artist ID

Retrieves an artist's albums from Apple's catalog by artist ID.

```
GET https://connect.mindcloud.co/v1/universal/appleNewsAndMusic/latest/actions/lookup-artist-albums-by-artist-id
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/appleNewsAndMusic/latest/actions/lookup-artist-albums-by-artist-id?connectionId=$CONNECTION_ID&id=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/appleNewsAndMusic/latest/actions/lookup-artist-albums-by-artist-id?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `id` | string | yes | Apple iTunes artist identifier to expand into album results. |
...
Through the native Apple News and Music API, this operation is `GET https://itunes.apple.com/lookup` (base URL `https://itunes.apple.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/lookup-artist-albums-by-artist-id.md) for the provider-specific parameters and requirements.

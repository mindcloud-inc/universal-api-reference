# Apple News and Music: Lookup AMG Artist Albums

Retrieves an artist's albums from Apple's catalog by AMG artist ID.

```
GET https://connect.mindcloud.co/v1/universal/appleNewsAndMusic/latest/actions/lookup-amg-artist-albums
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/appleNewsAndMusic/latest/actions/lookup-amg-artist-albums?connectionId=$CONNECTION_ID&amgArtistId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/appleNewsAndMusic/latest/actions/lookup-amg-artist-albums?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `amgArtistId` | string | yes | AllMusic artist identifier accepted by Apple lookup. |
...
Through the native Apple News and Music API, this operation is `GET https://itunes.apple.com/lookup` (base URL `https://itunes.apple.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/lookup-amg-artist-albums.md) for the provider-specific parameters and requirements.

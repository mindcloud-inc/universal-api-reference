# Apple News and Music: Lookup App by Bundle ID

Retrieves an app from Apple's catalog by bundle ID.

```
GET https://connect.mindcloud.co/v1/universal/appleNewsAndMusic/latest/actions/lookup-app-by-bundle-id
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/appleNewsAndMusic/latest/actions/lookup-app-by-bundle-id?connectionId=$CONNECTION_ID&bundleId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/appleNewsAndMusic/latest/actions/lookup-app-by-bundle-id?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `bundleId` | string | yes | Apple app bundle identifier to look up. |
...
Through the native Apple News and Music API, this operation is `GET https://itunes.apple.com/lookup` (base URL `https://itunes.apple.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/lookup-app-by-bundle-id.md) for the provider-specific parameters and requirements.

# Apple News and Music: Lookup App by App Store ID

Retrieves an app from Apple's catalog by App Store ID.

```
GET https://connect.mindcloud.co/v1/universal/appleNewsAndMusic/latest/actions/lookup-app-by-app-store-id
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/appleNewsAndMusic/latest/actions/lookup-app-by-app-store-id?connectionId=$CONNECTION_ID&id=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/appleNewsAndMusic/latest/actions/lookup-app-by-app-store-id?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `id` | string | yes | Apple App Store track identifier to look up. |
| `entity` | string | no | Fixed Apple lookup entity. Default: `software`. |
...
Through the native Apple News and Music API, this operation is `GET https://itunes.apple.com/lookup` (base URL `https://itunes.apple.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/lookup-app-by-app-store-id.md) for the provider-specific parameters and requirements.

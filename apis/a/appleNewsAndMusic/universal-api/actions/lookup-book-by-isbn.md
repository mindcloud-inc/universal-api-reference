# Apple News and Music: Lookup Book by ISBN

Retrieves a book from Apple's catalog by ISBN.

```
GET https://connect.mindcloud.co/v1/universal/appleNewsAndMusic/latest/actions/lookup-book-by-isbn
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/appleNewsAndMusic/latest/actions/lookup-book-by-isbn?connectionId=$CONNECTION_ID&isbn=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/appleNewsAndMusic/latest/actions/lookup-book-by-isbn?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `isbn` | string | yes | ISBN for a book in Apple's catalog. |
...
Through the native Apple News and Music API, this operation is `GET https://itunes.apple.com/lookup` (base URL `https://itunes.apple.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/lookup-book-by-isbn.md) for the provider-specific parameters and requirements.

# Apple News and Music: Lookup Podcast Episodes by Podcast ID

Retrieves podcast episodes from Apple's catalog by podcast ID.

```
GET https://connect.mindcloud.co/v1/universal/appleNewsAndMusic/latest/actions/lookup-podcast-episodes-by-podcast-id
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/appleNewsAndMusic/latest/actions/lookup-podcast-episodes-by-podcast-id?connectionId=$CONNECTION_ID&id=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/appleNewsAndMusic/latest/actions/lookup-podcast-episodes-by-podcast-id?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `id` | string | yes | Apple podcast identifier to expand into episode results. |
| `entity` | string | no | Fixed Apple lookup entity. Default: `podcastEpisode`. |
| `limit` | string | no | Maximum number of episode results to return. Default: `10`. |

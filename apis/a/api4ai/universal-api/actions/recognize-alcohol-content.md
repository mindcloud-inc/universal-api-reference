# api4ai: Recognize Alcohol Content

Recognizes alcohol labels from an image URL in api4ai.

```
GET https://connect.mindcloud.co/v1/universal/api4ai/latest/actions/recognize-alcohol-content
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/api4ai/latest/actions/recognize-alcohol-content?connectionId=$CONNECTION_ID&url=https%3A%2F%2Fexample.com" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/api4ai/latest/actions/recognize-alcohol-content?${params}`, {
  headers: {
...
Through the native api4ai API, this operation is `POST /alco-rec/v1/results` (base URL `https://api4ai.cloud`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/recognize-alcohol-content.md) for the provider-specific parameters and requirements.

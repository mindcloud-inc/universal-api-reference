# api4ai: Anonymize Image from File

Anonymizes an image from a file in api4ai.

```
GET https://connect.mindcloud.co/v1/universal/api4ai/latest/actions/anonymize-image-from-file
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/api4ai/latest/actions/anonymize-image-from-file?connectionId=$CONNECTION_ID&image=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/api4ai/latest/actions/anonymize-image-from-file?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `image` | file | yes | Image file to anonymize. |
...
Through the native api4ai API, this operation is `POST /img-anonymization/v1/results` (base URL `https://api4ai.cloud`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/anonymize-image-from-file.md) for the provider-specific parameters and requirements.

# api4ai: Anonymize Image

Anonymizes an image from a URL in api4ai.

```
GET https://connect.mindcloud.co/v1/universal/api4ai/latest/actions/anonymize-image
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/api4ai/latest/actions/anonymize-image?connectionId=$CONNECTION_ID&url=https%3A%2F%2Fexample.com" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/api4ai/latest/actions/anonymize-image?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `url` | string | yes | Publicly reachable image URL to anonymize. |
...
Through the native api4ai API, this operation is `POST /img-anonymization/v1/results` (base URL `https://api4ai.cloud`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/anonymize-image.md) for the provider-specific parameters and requirements.

# api4ai: Remove People Background

Removes a person's background from a URL in api4ai.

```
GET https://connect.mindcloud.co/v1/universal/api4ai/latest/actions/remove-people-background
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/api4ai/latest/actions/remove-people-background?connectionId=$CONNECTION_ID&url=https%3A%2F%2Fexample.com" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/api4ai/latest/actions/remove-people-background?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `url` | string | yes | Publicly reachable image URL to process. |
...
Through the native api4ai API, this operation is `POST /img-bg-removal/v1/people/results` (base URL `https://api4ai.cloud`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/remove-people-background.md) for the provider-specific parameters and requirements.

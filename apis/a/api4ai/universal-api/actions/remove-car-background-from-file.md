# api4ai: Remove Car Background from File

Removes a car background from a file in api4ai.

```
GET https://connect.mindcloud.co/v1/universal/api4ai/latest/actions/remove-car-background-from-file
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/api4ai/latest/actions/remove-car-background-from-file?connectionId=$CONNECTION_ID&image=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/api4ai/latest/actions/remove-car-background-from-file?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `image` | file | yes | Car image file to process. |
...
Through the native api4ai API, this operation is `POST /img-bg-removal/v1/cars/results` (base URL `https://api4ai.cloud`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/remove-car-background-from-file.md) for the provider-specific parameters and requirements.

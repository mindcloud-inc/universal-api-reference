# api4ai: Get OCR API Version

Retrieves the OCR API version from api4ai.

```
GET https://connect.mindcloud.co/v1/universal/api4ai/latest/actions/get-ocrapi-version
```
...
Through the native api4ai API, this operation is `GET /ocr/v1/version` (base URL `https://api4ai.cloud`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-ocrapi-version.md) for the provider-specific parameters and requirements.

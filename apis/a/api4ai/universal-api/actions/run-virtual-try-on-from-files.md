# api4ai: Run Virtual Try-On from Files

Runs virtual try-on from person and apparel files in api4ai.

```
GET https://connect.mindcloud.co/v1/universal/api4ai/latest/actions/run-virtual-try-on-from-files
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/api4ai/latest/actions/run-virtual-try-on-from-files?connectionId=$CONNECTION_ID&image=string&imageApparel=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/api4ai/latest/actions/run-virtual-try-on-from-files?${params}`, {
  headers: {
...
Through the native api4ai API, this operation is `POST /virtual-try-on/v1/results` (base URL `https://api4ai.cloud`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/run-virtual-try-on-from-files.md) for the provider-specific parameters and requirements.

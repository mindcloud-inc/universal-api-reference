...
```
GET https://connect.mindcloud.co/v1/universal/api4ai/latest/actions/analyze-face
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/api4ai/latest/actions/analyze-face?connectionId=$CONNECTION_ID&url=https%3A%2F%2Fexample.com" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/api4ai/latest/actions/analyze-face?${params}`, {
  headers: {
...
Through the native api4ai API, this operation is `POST /face-analyzer/v1/results` (base URL `https://api4ai.cloud`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/analyze-face.md) for the provider-specific parameters and requirements.

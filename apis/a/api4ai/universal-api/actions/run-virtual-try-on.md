# api4ai: Run Virtual Try-On

Runs virtual try-on from person and apparel URLs in api4ai.

```
GET https://connect.mindcloud.co/v1/universal/api4ai/latest/actions/run-virtual-try-on
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/api4ai/latest/actions/run-virtual-try-on?connectionId=$CONNECTION_ID&url=https%3A%2F%2Fexample.com&urlApparel=https%3A%2F%2Fexample.com" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/api4ai/latest/actions/run-virtual-try-on?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `url` | string | yes | Publicly reachable image URL to use for the try-on request. |
...
Through the native api4ai API, this operation is `POST /virtual-try-on/v1/results` (base URL `https://api4ai.cloud`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/run-virtual-try-on.md) for the provider-specific parameters and requirements.

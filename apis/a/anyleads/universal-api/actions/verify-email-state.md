# Anyleads: Verify Email State

Retrieves email verification status from Anyleads.

```
GET https://connect.mindcloud.co/v1/universal/anyleads/latest/actions/verify-email-state
```
...
Through the native Anyleads API, this operation is `POST /api-product/incoming-webhook/verify-email-state` (base URL `https://myapiconnect.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/verify-email-state.md) for the provider-specific parameters and requirements.

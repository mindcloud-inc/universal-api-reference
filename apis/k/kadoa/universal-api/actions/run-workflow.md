...
```
PUT https://connect.mindcloud.co/v1/universal/kadoa/latest/actions/run-workflow
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/kadoa/latest/actions/run-workflow" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/kadoa/latest/actions/run-workflow', {
  method: 'PUT',
  headers: {
...
| Key | Type | Required | Description |
| --- | --- | --- | --- |
| `variables` | string | no | JSON object of runtime variables |

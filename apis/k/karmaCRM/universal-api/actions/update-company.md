# Karma CRM: Update Company

Updates an existing company in Karma CRM.

```
PUT https://connect.mindcloud.co/v1/universal/karmaCRM/latest/actions/update-company
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/karmaCRM/latest/actions/update-company" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/karmaCRM/latest/actions/update-company', {
  method: 'PUT',
  headers: {
...
| `company` | object | yes | Company payload object with the fields to update. |

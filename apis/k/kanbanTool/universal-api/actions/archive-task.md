# Kanban Tool: Archive Task



```
PUT https://connect.mindcloud.co/v1/universal/kanbanTool/latest/actions/archive-task
```
...
Through the native Kanban Tool API, this operation is `PATCH /tasks/:task_id.json` (base URL `https://{{credentials.domain}}.kanbantool.com/api/v3`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/archive-task.md) for the provider-specific parameters and requirements.

# Kite Suite: Get task by serial number.



```
GET https://connect.mindcloud.co/v1/universal/kiteSuiteCustom/latest/actions/get-task-by-serial-number
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/kiteSuiteCustom/latest/actions/get-task-by-serial-number?connectionId=$CONNECTION_ID&sn=string&key=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/kiteSuiteCustom/latest/actions/get-task-by-serial-number?${params}`, {
  headers: {
...
Through the native Kite Suite API, this operation is `GET /api/v1/task/sn/:sn?key=` (base URL `https://api.kitesuite.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-task-by-serial-number.md) for the provider-specific parameters and requirements.

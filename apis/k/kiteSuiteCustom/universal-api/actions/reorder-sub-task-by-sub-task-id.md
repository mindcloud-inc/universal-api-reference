# Kite Suite: Reorder sub task by sub task Id



```
PUT https://connect.mindcloud.co/v1/universal/kiteSuiteCustom/latest/actions/reorder-sub-task-by-sub-task-id
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/kiteSuiteCustom/latest/actions/reorder-sub-task-by-sub-task-id" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/kiteSuiteCustom/latest/actions/reorder-sub-task-by-sub-task-id', {
  method: 'PUT',
  headers: {
...
Through the native Kite Suite API, this operation is `PATCH /api/v1/task/sub-task/:id` (base URL `https://api.kitesuite.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/reorder-sub-task-by-sub-task-id.md) for the provider-specific parameters and requirements.

# Kite Suite: update multiple tasks



```
PUT https://connect.mindcloud.co/v1/universal/kiteSuiteCustom/latest/actions/update-multiple-tasks
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/kiteSuiteCustom/latest/actions/update-multiple-tasks" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/kiteSuiteCustom/latest/actions/update-multiple-tasks', {
  method: 'PUT',
  headers: {
...
Through the native Kite Suite API, this operation is `POST /api/v1/task/multiple` (base URL `https://api.kitesuite.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-multiple-tasks.md) for the provider-specific parameters and requirements.

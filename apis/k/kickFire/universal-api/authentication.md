# KickFire Universal API Authentication

Every KickFire API request through MindCloud uses a MindCloud API key and the `connectionId` of a KickFire connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your KickFire connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [KickFire action](README.md#actions-4) with your API key and `connectionId`.

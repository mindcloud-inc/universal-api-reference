...
```
POST https://connect.mindcloud.co/v1/universal/kommo/latest/actions/create-task
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/kommo/latest/actions/create-task" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/kommo/latest/actions/create-task', {
  method: 'POST',
  headers: {
...
| `completeTill` | number | yes | Task deadline timestamp. |
...
Through the native Kommo API, this operation is `POST /tasks` (base URL `https://{{credentials.authorizeRequest.referer}}/api/v4`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-task.md) for the provider-specific parameters and requirements.

# Kommo: Create Company



```
POST https://connect.mindcloud.co/v1/universal/kommo/latest/actions/create-company
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/kommo/latest/actions/create-company" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/kommo/latest/actions/create-company', {
  method: 'POST',
  headers: {
...
Through the native Kommo API, this operation is `POST /companies` (base URL `https://{{credentials.authorizeRequest.referer}}/api/v4`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-company.md) for the provider-specific parameters and requirements.

# Key Value Storage Universal API Authentication

Every Key Value Storage API request through MindCloud uses a MindCloud API key and the `connectionId` of a Key Value Storage connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Key Value Storage connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Key Value Storage action](README.md#actions-6) with your API key and `connectionId`.

# Kimi Universal API Authentication

Every Kimi API request through MindCloud uses a MindCloud API key and the `connectionId` of a Kimi connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Kimi connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Kimi action](README.md#actions-3) with your API key and `connectionId`.

# mfr Field Service Management Universal API Authentication

Every mfr Field Service Management API request through MindCloud uses a MindCloud API key and the `connectionId` of a mfr Field Service Management connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your mfr Field Service Management connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [mfr Field Service Management action](README.md#actions-24) with your API key and `connectionId`.

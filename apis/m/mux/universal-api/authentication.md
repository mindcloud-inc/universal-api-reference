# Mux Universal API Authentication

Every Mux API request through MindCloud uses a MindCloud API key and the `connectionId` of a Mux connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Mux connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Mux action](README.md#actions-59) with your API key and `connectionId`.

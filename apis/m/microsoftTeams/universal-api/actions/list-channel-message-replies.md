# Microsoft Teams: List Channel Message Replies

Retrieves replies to a Microsoft Teams channel message.

```
GET https://connect.mindcloud.co/v1/universal/microsoftTeams/latest/actions/list-channel-message-replies
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/microsoftTeams/latest/actions/list-channel-message-replies?connectionId=$CONNECTION_ID&teamId=string&channelId=string&messageId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/microsoftTeams/latest/actions/list-channel-message-replies?${params}`, {
  headers: {
...
Through the native Microsoft Teams API, this operation is `GET /v1.0/teams/:teamId/channels/:channelId/messages/:messageId/replies` (base URL `https://graph.microsoft.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-channel-message-replies.md) for the provider-specific parameters and requirements.

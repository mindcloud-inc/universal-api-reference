# Meme API Universal API Authentication

Every Meme API API request through MindCloud uses a MindCloud API key and the `connectionId` of a Meme API connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Meme API connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Meme API action](README.md#actions-4) with your API key and `connectionId`.

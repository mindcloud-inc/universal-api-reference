# Memix: Get Subscription Status

Retrieves current subscription status from Memix.

```
GET https://connect.mindcloud.co/v1/universal/memix/latest/actions/get-subscription-status
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/memix/latest/actions/get-subscription-status?connectionId=$CONNECTION_ID&installation_id=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/memix/latest/actions/get-subscription-status?${params}`, {
  headers: {

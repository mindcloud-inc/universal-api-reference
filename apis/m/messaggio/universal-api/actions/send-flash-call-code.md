# Messaggio: Send Flash Call Code

Creates a flash call code in Messaggio.

```
POST https://connect.mindcloud.co/v1/universal/messaggio/latest/actions/send-flash-call-code
```
...
| `senderCode` | string | yes | Flash Call sender code from the Messaggio project. Example: `FLASHID`. |
| `verificationCode` | string | yes | Four-digit numeric code to deliver through Flash Call. Example: `1234`. |
...
Through the native Messaggio API, this operation is `POST /send` (base URL `https://msg.messaggio.com/api/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/send-flash-call-code.md) for the provider-specific parameters and requirements.

# Messaggio: Send Viber Text + Image + Button

Creates a Viber image message with a button in Messaggio.

```
POST https://connect.mindcloud.co/v1/universal/messaggio/latest/actions/send-viber-text-image-button
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/messaggio/latest/actions/send-viber-text-image-button" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/messaggio/latest/actions/send-viber-text-image-button', {
  method: 'POST',
  headers: {

# Mem Universal API Authentication

Every Mem API request through MindCloud uses a MindCloud API key and the `connectionId` of a Mem connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Mem connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Mem action](README.md#actions-11) with your API key and `connectionId`.

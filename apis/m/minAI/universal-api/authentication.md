# 1minAI Universal API Authentication

Every 1minAI API request through MindCloud uses a MindCloud API key and the `connectionId` of a 1minAI connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your 1minAI connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [1minAI action](README.md#actions-24) with your API key and `connectionId`.

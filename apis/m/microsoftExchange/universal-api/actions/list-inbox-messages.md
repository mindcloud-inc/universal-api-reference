# Microsoft Exchange: List Inbox Messages

Finds inbox messages in Microsoft Exchange by newest first.

```
GET https://connect.mindcloud.co/v1/universal/microsoftExchange/latest/actions/list-inbox-messages
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/microsoftExchange/latest/actions/list-inbox-messages?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/microsoftExchange/latest/actions/list-inbox-messages?${params}`, {
  headers: {

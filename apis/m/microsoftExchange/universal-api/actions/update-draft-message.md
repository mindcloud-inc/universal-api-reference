# Microsoft Exchange: Update Draft Message

Updates an existing draft message in Microsoft Exchange.

```
PUT https://connect.mindcloud.co/v1/universal/microsoftExchange/latest/actions/update-draft-message
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/microsoftExchange/latest/actions/update-draft-message" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/microsoftExchange/latest/actions/update-draft-message', {
  method: 'PUT',
  headers: {
...
| --- | --- | --- | --- |
| `messageId` | string | yes | The ID of the Exchange message to update. Example: `AAMkAG...`. |

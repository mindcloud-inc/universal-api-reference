# Microsoft Exchange: Get User Folder Name

Finds messages in a user's mailbox.

```
GET https://connect.mindcloud.co/v1/universal/microsoftExchange/latest/actions/get-user-folder-name
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/microsoftExchange/latest/actions/get-user-folder-name?connectionId=$CONNECTION_ID&userIdOrPrincipalName=user%40company.com%20or%20Entra%20user%20id&parentFolderId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/microsoftExchange/latest/actions/get-user-folder-name?${params}`, {
  headers: {
...
Through the native Microsoft Exchange API, this operation is `GET https://graph.microsoft.com/v1.0/users/:userIdOrPrincipalName/mailFolders/:parentFolderId` (base URL `https://graph.microsoft.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-user-folder-name.md) for the provider-specific parameters and requirements.

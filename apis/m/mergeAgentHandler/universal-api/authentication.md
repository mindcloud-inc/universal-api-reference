# Merge Agent Handler Universal API Authentication

Every Merge Agent Handler API request through MindCloud uses a MindCloud API key and the `connectionId` of a Merge Agent Handler connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Merge Agent Handler connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Merge Agent Handler action](README.md#actions-40) with your API key and `connectionId`.

# Merge Agent Handler: List Tool Packs

Retrieves tool packs from Merge Agent Handler.

```
GET https://connect.mindcloud.co/v1/universal/mergeAgentHandler/latest/actions/list-tool-packs
```
...
Through the native Merge Agent Handler API, this operation is `GET /tool-packs/` (base URL `https://ah-api.merge.dev/api/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-tool-packs.md) for the provider-specific parameters and requirements.

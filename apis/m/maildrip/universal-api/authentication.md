# Maildrip Universal API Authentication

Every Maildrip API request through MindCloud uses a MindCloud API key and the `connectionId` of a Maildrip connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Maildrip connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Maildrip action](README.md#actions-167) with your API key and `connectionId`.

# Mindee: Start Classification Job From URL

Creates a new classification job in Mindee from a URL.

```
POST https://connect.mindcloud.co/v1/universal/mindee/latest/actions/start-classification-job-from-url
```
...
Through the native Mindee API, this operation is `POST /v2/products/classification/enqueue` (base URL `https://api-v2.mindee.net`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/start-classification-job-from-url.md) for the provider-specific parameters and requirements.

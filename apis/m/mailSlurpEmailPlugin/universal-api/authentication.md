# MailSlurp Email Plugin Universal API Authentication

Every MailSlurp Email Plugin API request through MindCloud uses a MindCloud API key and the `connectionId` of a MailSlurp Email Plugin connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your MailSlurp Email Plugin connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [MailSlurp Email Plugin action](README.md#actions-35) with your API key and `connectionId`.

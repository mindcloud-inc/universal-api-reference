# Microsoft Power BI: Dashboards GenerateTokenInGroup



```
POST https://connect.mindcloud.co/v1/universal/microsoftPowerBI/latest/actions/embed-token-dashboards-generatetoken-in-group
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/microsoftPowerBI/latest/actions/embed-token-dashboards-generatetoken-in-group" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/microsoftPowerBI/latest/actions/embed-token-dashboards-generatetoken-in-group', {
  method: 'POST',
  headers: {
...
Through the native Microsoft Power BI API, this operation is `POST groups/[:groupId]/dashboards/[:dashboardId]/GenerateToken` (base URL `https://api.powerbi.com/v1.0/myorg`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/embed-token-dashboards-generatetoken-in-group.md) for the provider-specific parameters and requirements.

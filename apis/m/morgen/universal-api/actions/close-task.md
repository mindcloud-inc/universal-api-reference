...
```
PUT https://connect.mindcloud.co/v1/universal/morgen/latest/actions/close-task
```
...
Through the native Morgen API, this operation is `POST /v3/tasks/close` (base URL `https://api.morgen.so`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/close-task.md) for the provider-specific parameters and requirements.

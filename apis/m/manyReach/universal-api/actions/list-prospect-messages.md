# ManyReach: List Prospect Messages

Retrieves messages for a prospect from ManyReach.

```
GET https://connect.mindcloud.co/v1/universal/manyReach/latest/actions/list-prospect-messages
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/manyReach/latest/actions/list-prospect-messages?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/manyReach/latest/actions/list-prospect-messages?${params}`, {
  headers: {
...
Through the native ManyReach API, this operation is `GET https://api.manyreach.com/api/v2/prospects/:id/messages` (base URL `https://api.manyreach.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-prospect-messages.md) for the provider-specific parameters and requirements.

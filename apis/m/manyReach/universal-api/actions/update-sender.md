# ManyReach: Update Sender

Updates an existing sender in ManyReach.

```
PUT https://connect.mindcloud.co/v1/universal/manyReach/latest/actions/update-sender
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/manyReach/latest/actions/update-sender" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/manyReach/latest/actions/update-sender', {
  method: 'PUT',
  headers: {
...
| --- | --- | --- | --- |
| `customImapPass` | string | no | IMAP password. |
| `customImapPort` | string | no | IMAP server port. |
| `customImapServer` | string | no | IMAP server hostname. |
| `customImapUsername` | string | no | IMAP username. |
| `customSmtpPass` | string | no | SMTP password. |
| `customSmtpPort` | number | no | SMTP server port. |
| `customSmtpServer` | string | no | SMTP server hostname. |
| `customSmtpUsername` | string | no | SMTP username. |
| `customWarmupTag` | string | no | Custom tag applied to warmup behavior. |
| `dailyLimit` | number | no | Maximum emails the sender can send per day. |
...
| `id` | string | yes | The ID of the sender to update. |
...
| `trackingDomain` | string | no | Custom tracking domain. |

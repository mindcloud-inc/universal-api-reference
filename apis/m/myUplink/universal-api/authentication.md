# myUplink Universal API Authentication

Every myUplink API request through MindCloud uses a MindCloud API key and the `connectionId` of a myUplink connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your myUplink connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [myUplink action](README.md#actions-3) with your API key and `connectionId`.

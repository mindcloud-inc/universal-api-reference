# Mailchimp: Add E-commerce Store

Creates a new e-commerce store in Mailchimp.

```
POST https://connect.mindcloud.co/v1/universal/mailChimp/latest/actions/add-e-commerce-store
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/mailChimp/latest/actions/add-e-commerce-store" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/mailChimp/latest/actions/add-e-commerce-store', {
  method: 'POST',
  headers: {
...
Through the native Mailchimp API, this operation is `POST ecommerce/stores` (base URL `https://{{credentials.serverPrefix}}.api.mailchimp.com/3.0/`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/add-e-commerce-store.md) for the provider-specific parameters and requirements.

# Mailchimp Universal API Authentication

Every Mailchimp API request through MindCloud uses a MindCloud API key and the `connectionId` of a Mailchimp connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Mailchimp connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Mailchimp action](README.md#actions-41) with your API key and `connectionId`.

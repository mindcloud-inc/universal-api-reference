# Mailcoach Universal API Authentication

Every Mailcoach API request through MindCloud uses a MindCloud API key and the `connectionId` of a Mailcoach connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Mailcoach connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Mailcoach action](README.md#actions-20) with your API key and `connectionId`.

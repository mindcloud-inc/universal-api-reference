# MojoTxt: List Phone Number Subscribers

Retrieves subscribers for a MojoTxt phone number.

```
GET https://connect.mindcloud.co/v1/universal/mojoTxt/latest/actions/list-phone-number-subscribers
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/mojoTxt/latest/actions/list-phone-number-subscribers?connectionId=$CONNECTION_ID&limit=25&offset=0&phoneNumber=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/mojoTxt/latest/actions/list-phone-number-subscribers?${params}`, {
  headers: {
...
Through the native MojoTxt API, this operation is `GET /:phoneNumber/subscribers/list` (base URL `https://app.mojotxt.com/api/v1`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/list-phone-number-subscribers.md) for the provider-specific parameters and requirements.

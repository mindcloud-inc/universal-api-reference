# MailRook Email Validation Universal API Authentication

Every MailRook Email Validation API request through MindCloud uses a MindCloud API key and the `connectionId` of a MailRook Email Validation connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your MailRook Email Validation connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [MailRook Email Validation action](README.md#actions-6) with your API key and `connectionId`.

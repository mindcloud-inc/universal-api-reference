# Matomo: AbTesting get Metric Details



```
GET https://connect.mindcloud.co/v1/universal/matomo/latest/actions/ab-testing-get-metric-details
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/matomo/latest/actions/ab-testing-get-metric-details?connectionId=$CONNECTION_ID&idSite=1&period=day&date=yesterday&idExperiment=string&successMetric=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/matomo/latest/actions/ab-testing-get-metric-details?${params}`, {
  headers: {
...
| `successMetric` | string | yes | Matomo API parameter. |

# Matomo: CustomReports update Custom Report



```
PUT https://connect.mindcloud.co/v1/universal/matomo/latest/actions/custom-reports-update-custom-report
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/matomo/latest/actions/custom-reports-update-custom-report" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/matomo/latest/actions/custom-reports-update-custom-report', {
  method: 'PUT',
  headers: {
...
| `subCategoryReportIds` | string | no | Matomo API parameter. Default: `Array`. |

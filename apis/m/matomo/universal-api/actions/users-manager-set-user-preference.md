# Matomo: UsersManager set User Preference



```
PUT https://connect.mindcloud.co/v1/universal/matomo/latest/actions/users-manager-set-user-preference
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/matomo/latest/actions/users-manager-set-user-preference" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/matomo/latest/actions/users-manager-set-user-preference', {
  method: 'PUT',
  headers: {
...
| `preferenceValue` | string | yes | Matomo API parameter. |

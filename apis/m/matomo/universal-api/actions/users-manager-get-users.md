# Matomo: UsersManager get Users



```
GET https://connect.mindcloud.co/v1/universal/matomo/latest/actions/users-manager-get-users
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/matomo/latest/actions/users-manager-get-users?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/matomo/latest/actions/users-manager-get-users?${params}`, {
  headers: {
...
| Key | Type | Required | Description |
| --- | --- | --- | --- |
| `userLogins` | string | no | Matomo API parameter. |

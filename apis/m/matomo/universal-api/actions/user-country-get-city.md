...
```
GET https://connect.mindcloud.co/v1/universal/matomo/latest/actions/user-country-get-city
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/matomo/latest/actions/user-country-get-city?connectionId=$CONNECTION_ID&idSite=1&period=day&date=yesterday" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/matomo/latest/actions/user-country-get-city?${params}`, {
  headers: {
...
Through the native Matomo API, this operation is `POST /index.php` (base URL `https://mindcloud.matomo.cloud`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/user-country-get-city.md) for the provider-specific parameters and requirements.

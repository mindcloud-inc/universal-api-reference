# Matomo: UsersManager update User



```
PUT https://connect.mindcloud.co/v1/universal/matomo/latest/actions/users-manager-update-user
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/matomo/latest/actions/users-manager-update-user" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/matomo/latest/actions/users-manager-update-user', {
  method: 'PUT',
  headers: {
...
| Key | Type | Required | Description |
| --- | --- | --- | --- |
| `password` | string | no | Matomo API parameter. |
| `email` | string | no | Matomo API parameter. |

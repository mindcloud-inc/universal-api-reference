# Matomo: Get Search Keywords with No Results



```
GET https://connect.mindcloud.co/v1/universal/matomo/latest/actions/actions-get-site-search-no-result-keywords
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/matomo/latest/actions/actions-get-site-search-no-result-keywords?connectionId=$CONNECTION_ID&idSite=1&period=day&date=yesterday" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/matomo/latest/actions/actions-get-site-search-no-result-keywords?${params}`, {
  headers: {

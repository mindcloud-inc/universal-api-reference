# Matomo: Get Device model



```
GET https://connect.mindcloud.co/v1/universal/matomo/latest/actions/devices-detection-get-model
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/matomo/latest/actions/devices-detection-get-model?connectionId=$CONNECTION_ID&idSite=1&period=day&date=yesterday" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/matomo/latest/actions/devices-detection-get-model?${params}`, {
  headers: {

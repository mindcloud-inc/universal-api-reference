# Matomo: ScheduledReports update Report



```
PUT https://connect.mindcloud.co/v1/universal/matomo/latest/actions/scheduled-reports-update-report
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/matomo/latest/actions/scheduled-reports-update-report" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/matomo/latest/actions/scheduled-reports-update-report', {
  method: 'PUT',
  headers: {
...
Through the native Matomo API, this operation is `POST /index.php` (base URL `https://mindcloud.matomo.cloud`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/scheduled-reports-update-report.md) for the provider-specific parameters and requirements.

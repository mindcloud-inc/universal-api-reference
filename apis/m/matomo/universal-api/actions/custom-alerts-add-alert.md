# Matomo: CustomAlerts add Alert



```
POST https://connect.mindcloud.co/v1/universal/matomo/latest/actions/custom-alerts-add-alert
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/matomo/latest/actions/custom-alerts-add-alert" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/matomo/latest/actions/custom-alerts-add-alert', {
  method: 'POST',
  headers: {
...
Through the native Matomo API, this operation is `POST /index.php` (base URL `https://mindcloud.matomo.cloud`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/custom-alerts-add-alert.md) for the provider-specific parameters and requirements.

# Matomo: HeatmapSessionRecording update Heatmap



```
PUT https://connect.mindcloud.co/v1/universal/matomo/latest/actions/heatmap-session-recording-update-heatmap
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/matomo/latest/actions/heatmap-session-recording-update-heatmap" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/matomo/latest/actions/heatmap-session-recording-update-heatmap', {
  method: 'PUT',
  headers: {
...
Through the native Matomo API, this operation is `POST /index.php` (base URL `https://mindcloud.matomo.cloud`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/heatmap-session-recording-update-heatmap.md) for the provider-specific parameters and requirements.

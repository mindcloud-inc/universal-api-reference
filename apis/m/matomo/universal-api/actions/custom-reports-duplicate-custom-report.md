# Matomo: CustomReports duplicate Custom Report



```
GET https://connect.mindcloud.co/v1/universal/matomo/latest/actions/custom-reports-duplicate-custom-report
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/matomo/latest/actions/custom-reports-duplicate-custom-report?connectionId=$CONNECTION_ID&idSite=1&idCustomReport=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/matomo/latest/actions/custom-reports-duplicate-custom-report?${params}`, {
  headers: {

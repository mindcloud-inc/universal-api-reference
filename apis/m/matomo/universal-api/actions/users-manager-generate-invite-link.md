# Matomo: UsersManager generate Invite Link



```
GET https://connect.mindcloud.co/v1/universal/matomo/latest/actions/users-manager-generate-invite-link
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/matomo/latest/actions/users-manager-generate-invite-link?connectionId=$CONNECTION_ID&userLogin=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/matomo/latest/actions/users-manager-generate-invite-link?${params}`, {
  headers: {
...
| Key | Type | Required | Description |
| --- | --- | --- | --- |
| `expiryInDays` | string | no | Matomo API parameter. Default: `7`. |

# Matomo: Get Entry page titles



```
GET https://connect.mindcloud.co/v1/universal/matomo/latest/actions/actions-get-entry-page-titles
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/matomo/latest/actions/actions-get-entry-page-titles?connectionId=$CONNECTION_ID&idSite=1&period=day&date=yesterday" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/matomo/latest/actions/actions-get-entry-page-titles?${params}`, {
  headers: {
  ...
  "data": [
    {
      ...
      "label": "string",
      "nb_conversions": 1,
      "nb_conversions_entry": 1,
      "nb_conversions_entry_rate": 1,
      "revenue_entry": 1,
      "revenue_per_entry": 1,
      "revenue_per_visit": 1
    }
...
| `entry_bounce_count` | number | Matomo metric entry_bounce_count |
| `entry_nb_visits` | number | Matomo metric entry_nb_visits |
| `exit_rate` | number | Matomo metric exit_rate |
| `label` | string | Entry Page title |
| `nb_conversions` | number | Matomo metric nb_conversions |
| `nb_conversions_entry` | number | Matomo metric nb_conversions_entry |
| `nb_conversions_entry_rate` | number | Matomo metric nb_conversions_entry_rate |
| `revenue_entry` | number | Matomo metric revenue_entry |
| `revenue_per_entry` | number | Matomo metric revenue_per_entry |
| `revenue_per_visit` | number | Matomo metric revenue_per_visit |

## Native endpoint

Through the native Matomo API, this operation is `POST /index.php` (base URL `https://mindcloud.matomo.cloud`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/actions-get-entry-page-titles.md) for the provider-specific parameters and requirements.

# Matomo: CustomReports get Configured Report



```
GET https://connect.mindcloud.co/v1/universal/matomo/latest/actions/custom-reports-get-configured-report
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/matomo/latest/actions/custom-reports-get-configured-report?connectionId=$CONNECTION_ID&idSite=1&idCustomReport=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/matomo/latest/actions/custom-reports-get-configured-report?${params}`, {
  headers: {
...
| `idCustomReport` | string | yes | Matomo API parameter. |

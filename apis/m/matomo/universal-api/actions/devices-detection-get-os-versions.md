# Matomo: Get Operating System versions



```
GET https://connect.mindcloud.co/v1/universal/matomo/latest/actions/devices-detection-get-os-versions
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/matomo/latest/actions/devices-detection-get-os-versions?connectionId=$CONNECTION_ID&idSite=1&period=day&date=yesterday" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/matomo/latest/actions/devices-detection-get-os-versions?${params}`, {
  headers: {
...
| `label` | string | Operating system version |

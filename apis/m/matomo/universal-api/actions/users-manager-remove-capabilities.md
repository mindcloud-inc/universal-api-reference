# Matomo: UsersManager remove Capabilities



```
DELETE https://connect.mindcloud.co/v1/universal/matomo/latest/actions/users-manager-remove-capabilities
```
...
```bash
curl -X DELETE "https://connect.mindcloud.co/v1/universal/matomo/latest/actions/users-manager-remove-capabilities?connectionId=$CONNECTION_ID&userLogin=string&capabilities=string&idSites=1" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/matomo/latest/actions/users-manager-remove-capabilities?${params}`, {
  headers: {

# Matomo Universal API Authentication

Every Matomo API request through MindCloud uses a MindCloud API key and the `connectionId` of a Matomo connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Matomo connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Matomo action](README.md#actions-584) with your API key and `connectionId`.

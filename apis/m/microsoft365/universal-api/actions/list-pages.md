# Microsoft 365: List Pages

Retrieves OneNote pages from Microsoft 365.

```
GET https://connect.mindcloud.co/v1/universal/microsoft365/latest/actions/list-pages
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/microsoft365/latest/actions/list-pages?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/microsoft365/latest/actions/list-pages?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `maxResults` | number | no | Maximum number of pages to return. Default: `10`. |
...
Through the native Microsoft 365 API, this operation is `GET /v1.0/me/onenote/pages` (base URL `https://graph.microsoft.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-pages.md) for the provider-specific parameters and requirements.

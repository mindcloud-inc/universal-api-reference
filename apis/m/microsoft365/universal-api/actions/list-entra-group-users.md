# Microsoft 365: List Entra Group Users

Retrieves Entra group members from Microsoft 365.

```
GET https://connect.mindcloud.co/v1/universal/microsoft365/latest/actions/list-entra-group-users
```
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/microsoft365/latest/actions/list-entra-group-users?${params}`, {
  headers: {
...
Through the native Microsoft 365 API, this operation is `GET /v1.0/groups/:id/members` (base URL `https://graph.microsoft.com`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/list-entra-group-users.md) for the provider-specific parameters and requirements.

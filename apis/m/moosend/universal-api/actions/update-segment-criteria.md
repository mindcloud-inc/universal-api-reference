# Moosend: Update Segment Criteria

Updates segment criteria in Moosend.

```
PUT https://connect.mindcloud.co/v1/universal/moosend/latest/actions/update-segment-criteria
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/moosend/latest/actions/update-segment-criteria" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/moosend/latest/actions/update-segment-criteria', {
  method: 'PUT',
  headers: {
...
| `criteriaId` | string | yes | The ID of the criteria of a segment to be updated. |
...
Through the native Moosend API, this operation is `POST /lists/{{MailingListID}}/segments/{{SegmentID}}/criteria/{{CriteriaID}}/update.json` (base URL `https://api.moosend.com/v3`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-segment-criteria.md) for the provider-specific parameters and requirements.

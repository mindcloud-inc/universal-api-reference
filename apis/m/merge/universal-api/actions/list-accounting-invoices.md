# Merge: List Accounting Invoices



```
GET https://connect.mindcloud.co/v1/universal/merge/latest/actions/list-accounting-invoices
```
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/merge/latest/actions/list-accounting-invoices?${params}`, {
  headers: {
...
Through the native Merge API, this operation is `GET /api/accounting/v1/invoices` (base URL `https://api.merge.dev`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/list-accounting-invoices.md) for the provider-specific parameters and requirements.

# Microsoft Entra: List User Memberships



```
GET https://connect.mindcloud.co/v1/universal/microsoftEntra/latest/actions/list-user-memberships
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/microsoftEntra/latest/actions/list-user-memberships?connectionId=$CONNECTION_ID&limit=25&offset=0&userId=87d349ed-44d7-43e1-9a83-5f2406dee5bd" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/microsoftEntra/latest/actions/list-user-memberships?${params}`, {
  headers: {
...
Through the native Microsoft Entra API, this operation is `GET /users/:userId/memberOf` (base URL `https://graph.microsoft.com/v1.0`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/list-user-memberships.md) for the provider-specific parameters and requirements.

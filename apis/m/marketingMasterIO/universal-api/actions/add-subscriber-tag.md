# Marketing Master IO: Add Subscriber Tag

Adds a tag to a Messenger subscriber in Marketing Master IO.

```
PUT https://connect.mindcloud.co/v1/universal/marketingMasterIO/latest/actions/add-subscriber-tag
```
...
Through the native Marketing Master IO API, this operation is `POST /v1/messenger/subscriber/:subscriber_id/tags/:tag` (base URL `https://api.marketingmaster.io`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/add-subscriber-tag.md) for the provider-specific parameters and requirements.

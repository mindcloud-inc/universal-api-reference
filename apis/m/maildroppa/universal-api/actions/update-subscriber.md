# Maildroppa: Update Subscriber

Updates a subscriber in Maildroppa without changing the email.

```
PUT https://connect.mindcloud.co/v1/universal/maildroppa/latest/actions/update-subscriber
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/maildroppa/latest/actions/update-subscriber" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/maildroppa/latest/actions/update-subscriber', {
  method: 'PUT',
  headers: {
...
| `tags[]` | array | no | List of tag identifiers associated with the subscriber. |
| `values[]` | array | no | List of custom fields for the subscriber. |

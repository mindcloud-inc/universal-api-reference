# Myphoner: Archive Lead

Archives an existing lead in Myphoner.

```
PUT https://connect.mindcloud.co/v1/universal/myphoner/latest/actions/archive-lead
```
...
Through the native Myphoner API, this operation is `POST /leads/:leadId/archive` (base URL `https://{{credentials.subdomain}}.myphoner.com/api/v2`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/archive-lead.md) for the provider-specific parameters and requirements.

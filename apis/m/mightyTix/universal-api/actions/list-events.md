# Mighty Tix: List Events

Retrieves events from Mighty Tix.

```
GET https://connect.mindcloud.co/v1/universal/mightyTix/latest/actions/list-events
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/mightyTix/latest/actions/list-events?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/mightyTix/latest/actions/list-events?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `variables.filter` | object | no | Optional EventFilter object from the Mighty Tix Admin GraphQL docs. |
| `variables.paging` | object | no | Optional CursorPaging object from the Mighty Tix Admin GraphQL docs. |
| `variables.sorting[]` | array<object> | no | Optional array of EventSort objects from the Mighty Tix Admin GraphQL docs. |

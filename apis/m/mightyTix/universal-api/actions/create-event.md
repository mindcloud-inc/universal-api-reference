# Mighty Tix: Create Event

Creates a new event in Mighty Tix.

```
POST https://connect.mindcloud.co/v1/universal/mightyTix/latest/actions/create-event
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/mightyTix/latest/actions/create-event" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/mightyTix/latest/actions/create-event', {
  method: 'POST',
  headers: {
...
| --- | --- | --- | --- |
| `variables.input` | object | yes | CreateOneEventInput object from the Mighty Tix Admin GraphQL docs. |

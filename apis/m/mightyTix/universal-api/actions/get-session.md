# Mighty Tix: Get Session

Retrieves a session from Mighty Tix.

```
GET https://connect.mindcloud.co/v1/universal/mightyTix/latest/actions/get-session
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/mightyTix/latest/actions/get-session?connectionId=$CONNECTION_ID&variables.id=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/mightyTix/latest/actions/get-session?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `variables.id` | string | yes | Session ID from the Mighty Tix Admin GraphQL docs. |

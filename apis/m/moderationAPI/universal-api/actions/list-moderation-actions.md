# Moderation API: List Moderation Actions

Retrieves moderation actions from Moderation API.

```
GET https://connect.mindcloud.co/v1/universal/moderationAPI/latest/actions/list-moderation-actions
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/moderationAPI/latest/actions/list-moderation-actions?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/moderationAPI/latest/actions/list-moderation-actions?${params}`, {
  headers: {
...
Through the native Moderation API API, this operation is `GET /actions` (base URL `https://api.moderationapi.com/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-moderation-actions.md) for the provider-specific parameters and requirements.

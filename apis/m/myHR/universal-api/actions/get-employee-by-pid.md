# MyHR: Get Employee By PID



```
GET https://connect.mindcloud.co/v1/universal/myHR/latest/actions/get-employee-by-pid
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/myHR/latest/actions/get-employee-by-pid?connectionId=$CONNECTION_ID&employeePid=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/myHR/latest/actions/get-employee-by-pid?${params}`, {
  headers: {
...
Through the native MyHR API, this operation is `GET /employees/:employee_pid` (base URL `https://mindcloud.myhr.lu/api/v2`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-employee-by-pid.md) for the provider-specific parameters and requirements.

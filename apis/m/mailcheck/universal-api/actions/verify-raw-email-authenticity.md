# Mailcheck: Verify Raw Email Authenticity



```
POST https://connect.mindcloud.co/v1/universal/mailcheck/latest/actions/verify-raw-email-authenticity
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/mailcheck/latest/actions/verify-raw-email-authenticity" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/mailcheck/latest/actions/verify-raw-email-authenticity', {
  method: 'POST',
  headers: {
...
| --- | --- | --- | --- |
| `rawEmail` | string | yes | Full raw email source. MailCheck strips the body and analyzes only the headers. Example: `From: sender@example.com ...  Body is ignored`. |
...
| `privacy.body_stripped` | boolean | Whether the body was stripped before analysis. |
...
Through the native Mailcheck API, this operation is `POST /v1/verify/auth` (base URL `https://api.mailcheck.dev`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/verify-raw-email-authenticity.md) for the provider-specific parameters and requirements.

# Milvus Vector Store Universal API Authentication

Every Milvus Vector Store API request through MindCloud uses a MindCloud API key and the `connectionId` of a Milvus Vector Store connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Milvus Vector Store connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Milvus Vector Store action](README.md#actions-40) with your API key and `connectionId`.

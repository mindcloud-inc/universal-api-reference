# Mx Toolbox: Lookup CNAME Record



```
GET https://connect.mindcloud.co/v1/universal/mxToolbox/latest/actions/lookup-cname-record
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/mxToolbox/latest/actions/lookup-cname-record?connectionId=$CONNECTION_ID&hostname=Ava%20Chen" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/mxToolbox/latest/actions/lookup-cname-record?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `hostname` | string | yes | Hostname to resolve a CNAME record for. |
...
Through the native Mx Toolbox API, this operation is `GET /lookup/cname` (base URL `https://api.mxtoolbox.com/api/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/lookup-cname-record.md) for the provider-specific parameters and requirements.

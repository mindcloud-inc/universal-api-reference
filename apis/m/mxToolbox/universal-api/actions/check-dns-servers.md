# Mx Toolbox: Check DNS Servers



```
GET https://connect.mindcloud.co/v1/universal/mxToolbox/latest/actions/check-dns-servers
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/mxToolbox/latest/actions/check-dns-servers?connectionId=$CONNECTION_ID&domain=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/mxToolbox/latest/actions/check-dns-servers?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `domain` | string | yes | Domain to check DNS servers for. |
...
Through the native Mx Toolbox API, this operation is `GET /lookup/dns` (base URL `https://api.mxtoolbox.com/api/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/check-dns-servers.md) for the provider-specific parameters and requirements.

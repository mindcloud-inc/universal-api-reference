# Mx Toolbox: Lookup WHOIS



```
GET https://connect.mindcloud.co/v1/universal/mxToolbox/latest/actions/lookup-whois
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/mxToolbox/latest/actions/lookup-whois?connectionId=$CONNECTION_ID&domain=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/mxToolbox/latest/actions/lookup-whois?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `domain` | string | yes | Domain to run the WHOIS lookup against. |
...
Through the native Mx Toolbox API, this operation is `GET /lookup/whois` (base URL `https://api.mxtoolbox.com/api/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/lookup-whois.md) for the provider-specific parameters and requirements.

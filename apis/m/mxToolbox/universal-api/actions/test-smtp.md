# Mx Toolbox: Test SMTP



```
GET https://connect.mindcloud.co/v1/universal/mxToolbox/latest/actions/test-smtp
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/mxToolbox/latest/actions/test-smtp?connectionId=$CONNECTION_ID&hostname=Ava%20Chen" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/mxToolbox/latest/actions/test-smtp?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `hostname` | string | yes | Mail server hostname to test with SMTP. |
...
Through the native Mx Toolbox API, this operation is `GET /lookup/smtp` (base URL `https://api.mxtoolbox.com/api/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/test-smtp.md) for the provider-specific parameters and requirements.

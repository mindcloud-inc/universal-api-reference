# Mx Toolbox: Check Blocklist



```
GET https://connect.mindcloud.co/v1/universal/mxToolbox/latest/actions/check-blocklist
```
...
| --- | --- | --- | --- |
| `target` | string | yes | IP address or hostname to check on blocklists. |
...
Through the native Mx Toolbox API, this operation is `GET /lookup/blocklist` (base URL `https://api.mxtoolbox.com/api/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/check-blocklist.md) for the provider-specific parameters and requirements.

# Mx Toolbox: Lookup AAAA Record



```
GET https://connect.mindcloud.co/v1/universal/mxToolbox/latest/actions/lookup-aaaa-record
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/mxToolbox/latest/actions/lookup-aaaa-record?connectionId=$CONNECTION_ID&domain=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/mxToolbox/latest/actions/lookup-aaaa-record?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `domain` | string | yes | Domain or hostname to resolve an AAAA record for. |
...
Through the native Mx Toolbox API, this operation is `GET /lookup/aaaa` (base URL `https://api.mxtoolbox.com/api/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/lookup-aaaa-record.md) for the provider-specific parameters and requirements.

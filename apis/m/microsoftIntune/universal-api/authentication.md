# Microsoft Intune Universal API Authentication

Every Microsoft Intune API request through MindCloud uses a MindCloud API key and the `connectionId` of a Microsoft Intune connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Microsoft Intune connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Microsoft Intune action](README.md#actions-1) with your API key and `connectionId`.

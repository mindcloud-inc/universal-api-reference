# Mixpanel: Remove from Group List Property

Removes values from a group list in Mixpanel.

```
PUT https://connect.mindcloud.co/v1/universal/mixpanel/latest/actions/remove-from-group-list-property
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/mixpanel/latest/actions/remove-from-group-list-property" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/mixpanel/latest/actions/remove-from-group-list-property', {
  method: 'PUT',
  headers: {
...
| `remove` | object | yes | Object mapping group profile list properties to values to remove. Example: `[object Object]`. |
...
Through the native Mixpanel API, this operation is `POST https://api.mixpanel.com/groups` (base URL `https://mixpanel.com/api`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/remove-from-group-list-property.md) for the provider-specific parameters and requirements.

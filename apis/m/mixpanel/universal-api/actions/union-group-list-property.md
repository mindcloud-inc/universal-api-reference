# Mixpanel: Union Group List Property

Unions values into a group list in Mixpanel.

```
PUT https://connect.mindcloud.co/v1/universal/mixpanel/latest/actions/union-group-list-property
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/mixpanel/latest/actions/union-group-list-property" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/mixpanel/latest/actions/union-group-list-property', {
  method: 'PUT',
  headers: {
...
| `union` | object | yes | Object mapping group profile list properties to values that should be unioned. Example: `[object Object]`. |
...
Through the native Mixpanel API, this operation is `POST https://api.mixpanel.com/groups` (base URL `https://mixpanel.com/api`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/union-group-list-property.md) for the provider-specific parameters and requirements.

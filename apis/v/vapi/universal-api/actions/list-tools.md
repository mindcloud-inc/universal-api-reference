...
Retrieves a list of tools from Vapi.

```
GET https://connect.mindcloud.co/v1/universal/vapi/latest/actions/list-tools
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/vapi/latest/actions/list-tools?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/vapi/latest/actions/list-tools?${params}`, {
  headers: {
...
Through the native Vapi API, this operation is `GET /tool` (base URL `https://api.vapi.ai`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-tools.md) for the provider-specific parameters and requirements.

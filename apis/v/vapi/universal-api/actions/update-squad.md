...
```
PUT https://connect.mindcloud.co/v1/universal/vapi/latest/actions/update-squad
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/vapi/latest/actions/update-squad" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/vapi/latest/actions/update-squad', {
  method: 'PUT',
  headers: {
...
Through the native Vapi API, this operation is `PATCH /squad/:id` (base URL `https://api.vapi.ai`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-squad.md) for the provider-specific parameters and requirements.

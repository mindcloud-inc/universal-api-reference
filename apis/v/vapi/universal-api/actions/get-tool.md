...
Retrieves a tool from Vapi by ID.

```
GET https://connect.mindcloud.co/v1/universal/vapi/latest/actions/get-tool
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/vapi/latest/actions/get-tool?connectionId=$CONNECTION_ID&id=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/vapi/latest/actions/get-tool?${params}`, {
  headers: {
...
Through the native Vapi API, this operation is `GET /tool/:id` (base URL `https://api.vapi.ai`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-tool.md) for the provider-specific parameters and requirements.

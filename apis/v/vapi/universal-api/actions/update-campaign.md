# Vapi: Update Campaign

Updates an existing campaign in Vapi.

```
PUT https://connect.mindcloud.co/v1/universal/vapi/latest/actions/update-campaign
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/vapi/latest/actions/update-campaign" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/vapi/latest/actions/update-campaign', {
  method: 'PUT',
  headers: {
...
| --- | --- | --- | --- |
| `id` | string | yes |  |
| `name` | string | no | This is the name of the campaign. This is just for your own reference. |
| `assistantId` | string | no | This is the assistant ID that will be used for the campaign calls. Can only be updated if campaign is not in progress or has ended. |
| `workflowId` | string | no | This is the workflow ID that will be used for the campaign calls. Can only be updated if campaign is not in progress or has ended. |
| `squadId` | string | no | This is the squad ID that will be used for the campaign calls. Can only be updated if campaign is not in progress or has ended. |
| `phoneNumberId` | string | no | This is the phone number ID that will be used for the campaign calls. Can only be updated if campaign is not in progress or has ended. Note: `phoneNumberId` and `dialPlan` are mutually exclusive. |
| `dialPlan[]` | array<object> | no | This is a list of dial entries, each specifying a phone number and the customers to call using that number. Can only be updated if campaign is not in progress or has ended. Note: phoneNumberId and dialPlan are mutually exclusive. |
| `schedulePlan` | object | no |  |
| `status` | string | no | This is the status of the campaign. Can only be updated to 'ended' if you want to end the campaign. When set to 'ended', it will delete all scheduled calls. Calls in progress will be allowed to complete. |
...
Through the native Vapi API, this operation is `PATCH /campaign/:id` (base URL `https://api.vapi.ai`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-campaign.md) for the provider-specific parameters and requirements.

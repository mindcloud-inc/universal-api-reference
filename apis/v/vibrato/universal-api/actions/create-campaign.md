# Vibrato: Create campaign

Creates a new campaign in Vibrato.

```
POST https://connect.mindcloud.co/v1/universal/vibrato/latest/actions/create-campaign
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/vibrato/latest/actions/create-campaign" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/vibrato/latest/actions/create-campaign', {
  method: 'POST',
  headers: {
...
Through the native Vibrato API, this operation is `POST /campaigns/` (base URL `https://api.getvibrato.com/api/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-campaign.md) for the provider-specific parameters and requirements.

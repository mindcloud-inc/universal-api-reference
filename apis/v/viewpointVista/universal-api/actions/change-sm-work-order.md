# Viewpoint Vista: Change SM Work Order

Changes an existing Work Order record.

```
POST https://connect.mindcloud.co/v1/universal/viewpointVista/latest/actions/change-sm-work-order
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/viewpointVista/latest/actions/change-sm-work-order" \
  -H "Authorization: Bearer $MINDCLOUD_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
  "connectionId": "$CONNECTION_ID",
  "__key.sMWorkOrderID": 1
}'
```

```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/viewpointVista/latest/actions/change-sm-work-order', {
  method: 'POST',
  headers: {
...
| --- | --- | --- | --- |
| `__key` | object | no |  |
| `__key.sMWorkOrderID` | number | yes | Key sm/work_orders(SMWorkOrderID). |
| `serviceCenter` | string | no | Enter the Service center that this request is assigned to. |
| `customer` | number | no | The SM Customer for this order. **Required when `WorkOrderType` is `C-Customer`. |
...
Through the native Viewpoint Vista API, this operation is `POST v1/direct/subscribers/:subscriber_code/vista/:api/2/data/:object/actions/change` (base URL `https://api.xchange.trimble.com/connect/`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/change-sm-work-order.md) for the provider-specific parameters and requirements.

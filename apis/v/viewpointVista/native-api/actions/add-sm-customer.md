# Add SM Customer with Viewpoint Vista
...
- **API:** REST
- **Official documentation:** [Add SM Customer](https://direct-api.xchange.trimble.com/reference/post-directsubscriberssubscriber_codevistaar2datacustomersactionsadd)
...
| `Customer` | body | `number` | yes | Key to `ar/customer(CustGroup, Customer)`. `CustGroup` is defaulted based on the provided `SMCo`. Customer must already exist in the `ar/customers` endpoint. |

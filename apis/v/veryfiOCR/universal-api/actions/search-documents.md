# Veryfi OCR: Search Documents

Finds documents in Veryfi OCR.

```
GET https://connect.mindcloud.co/v1/universal/veryfiOCR/latest/actions/search-documents
```
...
Through the native Veryfi OCR API, this operation is `GET /api/v8/partner/documents` (base URL `https://api.veryfi.com/`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/search-documents.md) for the provider-specific parameters and requirements.

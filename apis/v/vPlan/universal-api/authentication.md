# vPlan Universal API Authentication

Every vPlan API request through MindCloud uses a MindCloud API key and the `connectionId` of a vPlan connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your vPlan connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [vPlan action](README.md#actions-56) with your API key and `connectionId`.

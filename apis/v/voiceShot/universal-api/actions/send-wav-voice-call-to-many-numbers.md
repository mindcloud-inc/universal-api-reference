# VoiceShot: Send WAV Voice Call To Many Numbers

Creates WAV voice calls in VoiceShot for many recipients.

```
POST https://connect.mindcloud.co/v1/universal/voiceShot/latest/actions/send-wav-voice-call-to-many-numbers
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/voiceShot/latest/actions/send-wav-voice-call-to-many-numbers" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/voiceShot/latest/actions/send-wav-voice-call-to-many-numbers', {
  method: 'POST',
  headers: {
...
| `filename` | string | yes | Previously uploaded WAV file to play for every recipient. |
...
Through the native VoiceShot API, this operation is `POST /ivrapi.asp` (base URL `https://api.voiceshot.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/send-wav-voice-call-to-many-numbers.md) for the provider-specific parameters and requirements.

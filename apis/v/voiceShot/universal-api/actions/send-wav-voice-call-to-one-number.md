# VoiceShot: Send WAV Voice Call To One Number

Creates a WAV voice call in VoiceShot.

```
POST https://connect.mindcloud.co/v1/universal/voiceShot/latest/actions/send-wav-voice-call-to-one-number
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/voiceShot/latest/actions/send-wav-voice-call-to-one-number" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/voiceShot/latest/actions/send-wav-voice-call-to-one-number', {
  method: 'POST',
  headers: {
...
| `filename` | string | yes | Previously uploaded WAV file to play. |
...
Through the native VoiceShot API, this operation is `POST /ivrapi.asp` (base URL `https://api.voiceshot.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/send-wav-voice-call-to-one-number.md) for the provider-specific parameters and requirements.

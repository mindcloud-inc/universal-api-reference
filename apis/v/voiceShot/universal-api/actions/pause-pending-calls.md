# VoiceShot: Pause Pending Calls

Pauses pending calls in a VoiceShot campaign.

```
PUT https://connect.mindcloud.co/v1/universal/voiceShot/latest/actions/pause-pending-calls
```
...
Through the native VoiceShot API, this operation is `POST /ivrapi.asp` (base URL `https://api.voiceshot.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/pause-pending-calls.md) for the provider-specific parameters and requirements.

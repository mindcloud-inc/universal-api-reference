# Viesus Universal API Authentication

Every Viesus API request through MindCloud uses a MindCloud API key and the `connectionId` of a Viesus connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Viesus connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Viesus action](README.md#actions-43) with your API key and `connectionId`.

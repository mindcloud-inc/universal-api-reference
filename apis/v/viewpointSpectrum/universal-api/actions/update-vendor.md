# Viewpoint Spectrum: Update Vendor



```
PUT https://connect.mindcloud.co/v1/universal/viewpointSpectrum/latest/actions/update-vendor
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/viewpointSpectrum/latest/actions/update-vendor" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/viewpointSpectrum/latest/actions/update-vendor', {
  method: 'PUT',
  headers: {
...
Through the native Viewpoint Spectrum API, this operation is `POST ws/UpdateVendor` (base URL `{{credentials.url}}:8482/`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-vendor.md) for the provider-specific parameters and requirements.

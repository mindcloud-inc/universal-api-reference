# Veryfi: Process a W-2

Creates a new W-2 in Veryfi.

```
POST https://connect.mindcloud.co/v1/universal/veryfi/latest/actions/post-api-v8-partner-w2s
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/veryfi/latest/actions/post-api-v8-partner-w2s" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/veryfi/latest/actions/post-api-v8-partner-w2s', {
  method: 'POST',
  headers: {
...
| `maxPagesToProcess` | number | no | Possible values: >= 1 and <= 2 Default value: 1 The number of pages to process for the document. The limit is 2 pages per document. |
...
Through the native Veryfi API, this operation is `POST /api/v8/partner/w2s` (base URL `https://api.veryfi.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/post-api-v8-partner-w2s.md) for the provider-specific parameters and requirements.

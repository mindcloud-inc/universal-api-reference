# Veryfi: Get Markdown Documents

Retrieves markdown documents from Veryfi.

```
GET https://connect.mindcloud.co/v1/universal/veryfi/latest/actions/get-api-v8-partner-document-to-markdown
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/veryfi/latest/actions/get-api-v8-partner-document-to-markdown?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/veryfi/latest/actions/get-api-v8-partner-document-to-markdown?${params}`, {
  headers: {
...
Through the native Veryfi API, this operation is `GET /api/v8/partner/document-to-markdown` (base URL `https://api.veryfi.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-api-v8-partner-document-to-markdown.md) for the provider-specific parameters and requirements.

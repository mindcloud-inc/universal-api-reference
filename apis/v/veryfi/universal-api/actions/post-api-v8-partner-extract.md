# Veryfi: Classify and possibly extract data from a document

Classifies a document and may extract data in Veryfi.

```
POST https://connect.mindcloud.co/v1/universal/veryfi/latest/actions/post-api-v8-partner-extract
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/veryfi/latest/actions/post-api-v8-partner-extract" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/veryfi/latest/actions/post-api-v8-partner-extract', {
  method: 'POST',
  headers: {
...
| `documentTypes[]` | array<object> | yes | Possible values: >= 1 The types to classify the document into. If not a preset type, it must be a valid blueprint name. string string |
...
Through the native Veryfi API, this operation is `POST /api/v8/partner/extract` (base URL `https://api.veryfi.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/post-api-v8-partner-extract.md) for the provider-specific parameters and requirements.

# Veryfi: Process a W-8BEN-E

Creates a new W-8BEN-E in Veryfi.

```
POST https://connect.mindcloud.co/v1/universal/veryfi/latest/actions/post-api-v8-partner-w-8ben-e
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/veryfi/latest/actions/post-api-v8-partner-w-8ben-e" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/veryfi/latest/actions/post-api-v8-partner-w-8ben-e', {
  method: 'POST',
  headers: {
...
Through the native Veryfi API, this operation is `POST /api/v8/partner/w-8ben-e` (base URL `https://api.veryfi.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/post-api-v8-partner-w-8ben-e.md) for the provider-specific parameters and requirements.

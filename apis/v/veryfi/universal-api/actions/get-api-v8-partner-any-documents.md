...
Retrieves AnyDocs from Veryfi.

```
GET https://connect.mindcloud.co/v1/universal/veryfi/latest/actions/get-api-v8-partner-any-documents
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/veryfi/latest/actions/get-api-v8-partner-any-documents?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/veryfi/latest/actions/get-api-v8-partner-any-documents?${params}`, {
  headers: {
...
| `blueprintName` | string | no | The name of the extraction blueprints. |
| `templateName` | string | no | Deprecated.The name of the extraction blueprints. |
...
Through the native Veryfi API, this operation is `GET /api/v8/partner/any-documents` (base URL `https://api.veryfi.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-api-v8-partner-any-documents.md) for the provider-specific parameters and requirements.

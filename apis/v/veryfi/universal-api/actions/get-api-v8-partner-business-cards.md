# Veryfi: Get Business Cards

Retrieves business cards from Veryfi.

```
GET https://connect.mindcloud.co/v1/universal/veryfi/latest/actions/get-api-v8-partner-business-cards
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/veryfi/latest/actions/get-api-v8-partner-business-cards?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/veryfi/latest/actions/get-api-v8-partner-business-cards?${params}`, {
  headers: {
...
Through the native Veryfi API, this operation is `GET /api/v8/partner/business-cards` (base URL `https://api.veryfi.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-api-v8-partner-business-cards.md) for the provider-specific parameters and requirements.

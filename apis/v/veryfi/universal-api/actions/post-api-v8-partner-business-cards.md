# Veryfi: Process a Business Card

Creates a new business card in Veryfi.

```
POST https://connect.mindcloud.co/v1/universal/veryfi/latest/actions/post-api-v8-partner-business-cards
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/veryfi/latest/actions/post-api-v8-partner-business-cards" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/veryfi/latest/actions/post-api-v8-partner-business-cards', {
  method: 'POST',
  headers: {
...
| `autoDelete` | boolean | no | Default value: false Delete this business_card from Veryfi after data has been extracted |
...
Through the native Veryfi API, this operation is `POST /api/v8/partner/business-cards` (base URL `https://api.veryfi.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/post-api-v8-partner-business-cards.md) for the provider-specific parameters and requirements.

# Veryfi: Split and process a PDF with multiple W-2s

Creates W-2 documents by splitting a PDF in Veryfi.

```
POST https://connect.mindcloud.co/v1/universal/veryfi/latest/actions/post-api-v8-partner-w2s-set
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/veryfi/latest/actions/post-api-v8-partner-w2s-set" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/veryfi/latest/actions/post-api-v8-partner-w2s-set', {
  method: 'POST',
  headers: {
...
| `maxPagesToProcess` | number | no | Possible values: >= 1 and <= 100 Default value: 100 Limit processing to number of pages. A page is a pdf page or an image |
...
Through the native Veryfi API, this operation is `POST /api/v8/partner/w2s-set` (base URL `https://api.veryfi.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/post-api-v8-partner-w2s-set.md) for the provider-specific parameters and requirements.

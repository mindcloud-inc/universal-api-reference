# Veryfi: Process a Check

Creates a new check in Veryfi.

```
POST https://connect.mindcloud.co/v1/universal/veryfi/latest/actions/post-api-v8-partner-checks
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/veryfi/latest/actions/post-api-v8-partner-checks" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/veryfi/latest/actions/post-api-v8-partner-checks', {
  method: 'POST',
  headers: {
...
Through the native Veryfi API, this operation is `POST /api/v8/partner/checks` (base URL `https://api.veryfi.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/post-api-v8-partner-checks.md) for the provider-specific parameters and requirements.

# Vouchery.io: Get Voucher Details



```
GET https://connect.mindcloud.co/v1/universal/voucheryio/latest/actions/get-voucher-details
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/voucheryio/latest/actions/get-voucher-details?connectionId=$CONNECTION_ID&code=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/voucheryio/latest/actions/get-voucher-details?${params}`, {
  headers: {
...
Through the native Vouchery.io API, this operation is `GET /vouchers/:code/extended` (base URL `https://mindcloud.sandbox.vouchery.app/api/v2.1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-voucher-details.md) for the provider-specific parameters and requirements.

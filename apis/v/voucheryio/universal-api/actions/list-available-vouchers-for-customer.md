# Vouchery.io: List Available Vouchers For Customer



```
GET https://connect.mindcloud.co/v1/universal/voucheryio/latest/actions/list-available-vouchers-for-customer
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/voucheryio/latest/actions/list-available-vouchers-for-customer?connectionId=$CONNECTION_ID&identifier=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/voucheryio/latest/actions/list-available-vouchers-for-customer?${params}`, {
  headers: {
...
| `value` | number |  |
| `valueLeft` | number |  |
| `valueReserved` | object |  |
| `voucherType` | string |  |

## Native endpoint

Through the native Vouchery.io API, this operation is `GET /customers/:identifier/available-vouchers` (base URL `https://mindcloud.sandbox.vouchery.app/api/v2.1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-available-vouchers-for-customer.md) for the provider-specific parameters and requirements.

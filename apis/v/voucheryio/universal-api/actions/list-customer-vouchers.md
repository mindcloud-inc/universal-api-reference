# Vouchery.io: List Customer Vouchers



```
GET https://connect.mindcloud.co/v1/universal/voucheryio/latest/actions/list-customer-vouchers
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/voucheryio/latest/actions/list-customer-vouchers?connectionId=$CONNECTION_ID&identifier=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/voucheryio/latest/actions/list-customer-vouchers?${params}`, {
  headers: {
...
Through the native Vouchery.io API, this operation is `GET /customers/:identifier/vouchers` (base URL `https://mindcloud.sandbox.vouchery.app/api/v2.1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-customer-vouchers.md) for the provider-specific parameters and requirements.

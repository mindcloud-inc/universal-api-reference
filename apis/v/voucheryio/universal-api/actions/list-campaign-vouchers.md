# Vouchery.io: List Campaign Vouchers



```
GET https://connect.mindcloud.co/v1/universal/voucheryio/latest/actions/list-campaign-vouchers
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/voucheryio/latest/actions/list-campaign-vouchers?connectionId=$CONNECTION_ID&campaignId=1" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/voucheryio/latest/actions/list-campaign-vouchers?${params}`, {
  headers: {
...
| `createdByTaskId` | string | no | Queue task UUID |
...
Through the native Vouchery.io API, this operation is `GET /campaigns/:campaign_id/vouchers` (base URL `https://mindcloud.sandbox.vouchery.app/api/v2.1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-campaign-vouchers.md) for the provider-specific parameters and requirements.

# Vimeo: List Videos with Tag

Retrieves videos with a specific tag from Vimeo.

```
GET https://connect.mindcloud.co/v1/universal/vimeo/latest/actions/list-videos-with-tag
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/vimeo/latest/actions/list-videos-with-tag?connectionId=$CONNECTION_ID&limit=25&offset=0&word=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/vimeo/latest/actions/list-videos-with-tag?${params}`, {
  headers: {
...
Through the native Vimeo API, this operation is `GET /tags/:word/videos` (base URL `https://api.vimeo.com`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/list-videos-with-tag.md) for the provider-specific parameters and requirements.

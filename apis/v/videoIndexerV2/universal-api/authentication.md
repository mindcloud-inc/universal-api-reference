# Video Indexer (V2) Universal API Authentication

Every Video Indexer (V2) API request through MindCloud uses a MindCloud API key and the `connectionId` of a Video Indexer (V2) connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Video Indexer (V2) connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Video Indexer (V2) action](README.md#actions-15) with your API key and `connectionId`.

# Vectara: Advanced Single Corpus Query

Retrieves advanced query results from a specific Vectara corpus.

```
GET https://connect.mindcloud.co/v1/universal/vectara/latest/actions/advanced-single-corpus-query
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/vectara/latest/actions/advanced-single-corpus-query?connectionId=$CONNECTION_ID&corpusKey=string&query=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/vectara/latest/actions/advanced-single-corpus-query?${params}`, {
  headers: {
...
| `search.metadataFilter` | string | no | Metadata filter expression for corpus search results. |
...
| Key | Type | Required | Description |
| --- | --- | --- | --- |
| `search.lexicalInterpolation` | number | no | Blend factor between semantic and lexical retrieval. |
...
Through the native Vectara API, this operation is `POST /v2/corpora/:corpus_key/query` (base URL `https://api.vectara.io`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/advanced-single-corpus-query.md) for the provider-specific parameters and requirements.

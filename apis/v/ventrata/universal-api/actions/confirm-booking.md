# Ventrata: Confirm Booking

Confirms an existing booking in Ventrata.

```
PUT https://connect.mindcloud.co/v1/universal/ventrata/latest/actions/confirm-booking
```
...
Through the native Ventrata API, this operation is `POST octo/bookings/:uuid/confirm` (base URL `https://api.ventrata.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/confirm-booking.md) for the provider-specific parameters and requirements.

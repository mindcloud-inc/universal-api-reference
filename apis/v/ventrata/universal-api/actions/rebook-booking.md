# Ventrata: Rebook Booking

Rebooks an existing booking in Ventrata.

```
PUT https://connect.mindcloud.co/v1/universal/ventrata/latest/actions/rebook-booking
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/ventrata/latest/actions/rebook-booking" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/ventrata/latest/actions/rebook-booking', {
  method: 'PUT',
  headers: {
...
Through the native Ventrata API, this operation is `POST octo/bookings/:uuid/rebook` (base URL `https://api.ventrata.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/rebook-booking.md) for the provider-specific parameters and requirements.

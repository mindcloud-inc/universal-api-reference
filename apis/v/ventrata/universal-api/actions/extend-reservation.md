# Ventrata: Extend Reservation

Extends an existing reservation in Ventrata.

```
PUT https://connect.mindcloud.co/v1/universal/ventrata/latest/actions/extend-reservation
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/ventrata/latest/actions/extend-reservation" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/ventrata/latest/actions/extend-reservation', {
  method: 'PUT',
  headers: {
...
Through the native Ventrata API, this operation is `POST octo/bookings/:uuid/extend` (base URL `https://api.ventrata.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/extend-reservation.md) for the provider-specific parameters and requirements.

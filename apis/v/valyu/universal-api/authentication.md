# Valyu Universal API Authentication

Every Valyu API request through MindCloud uses a MindCloud API key and the `connectionId` of a Valyu connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Valyu connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Valyu action](README.md#actions-11) with your API key and `connectionId`.

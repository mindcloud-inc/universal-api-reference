# Veterans Affairs Facilities Universal API Authentication

Every Veterans Affairs Facilities API request through MindCloud uses a MindCloud API key and the `connectionId` of a Veterans Affairs Facilities connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Veterans Affairs Facilities connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Veterans Affairs Facilities action](README.md#actions-8) with your API key and `connectionId`.

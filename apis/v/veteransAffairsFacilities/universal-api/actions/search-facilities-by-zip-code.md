# Veterans Affairs Facilities: Search Facilities by ZIP Code

Finds VA facilities near a ZIP code.

```
GET https://connect.mindcloud.co/v1/universal/veteransAffairsFacilities/latest/actions/search-facilities-by-zip-code
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/veteransAffairsFacilities/latest/actions/search-facilities-by-zip-code?connectionId=$CONNECTION_ID&limit=25&offset=0&zip=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/veteransAffairsFacilities/latest/actions/search-facilities-by-zip-code?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `zip` | string | yes | Five-digit or ZIP+4 code to search near. |
...
Through the native Veterans Affairs Facilities API, this operation is `GET /facilities` (base URL `https://sandbox-api.va.gov/services/va_facilities/v1`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/search-facilities-by-zip-code.md) for the provider-specific parameters and requirements.

# Verify550 Universal API Authentication

Every Verify550 API request through MindCloud uses a MindCloud API key and the `connectionId` of a Verify550 connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Verify550 connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Verify550 action](README.md#actions-7) with your API key and `connectionId`.

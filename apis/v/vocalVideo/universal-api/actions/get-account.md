# Vocal Video: Get Account

Retrieves account details from Vocal Video.

```
GET https://connect.mindcloud.co/v1/universal/vocalVideo/latest/actions/get-account
```
...
| --- | --- | --- |
| `account` | string | Workspace account name. |
| `user` | string | Authenticated user name. |

## Native endpoint

Through the native Vocal Video API, this operation is `GET /account` (base URL `https://vocalvideo.com/api/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-account.md) for the provider-specific parameters and requirements.

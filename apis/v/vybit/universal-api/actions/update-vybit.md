...
```
PUT https://connect.mindcloud.co/v1/universal/vybit/latest/actions/update-vybit
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/vybit/latest/actions/update-vybit" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/vybit/latest/actions/update-vybit', {
  method: 'PUT',
  headers: {
...
| `name` | string | no | Vybit display name |
...
Through the native Vybit API, this operation is `PATCH /vybit/{{key}}` (base URL `https://api.vybit.net/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-vybit.md) for the provider-specific parameters and requirements.

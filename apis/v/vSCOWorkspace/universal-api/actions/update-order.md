# VSCO Workspace: Update Order

Updates an existing order in VSCO Workspace.

```
PUT https://connect.mindcloud.co/v1/universal/vSCOWorkspace/latest/actions/update-order
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/vSCOWorkspace/latest/actions/update-order" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/vSCOWorkspace/latest/actions/update-order', {
  method: 'PUT',
  headers: {
...
| --- | --- | --- | --- |
| `id` | string | yes |  |
| `jobId` | string | no | A lowercase [ULID](https://github.com/ulid/spec) entity identifier |
...
Through the native VSCO Workspace API, this operation is `PUT /order/:id` (base URL `https://workspace.vsco.co/api/v2`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-order.md) for the provider-specific parameters and requirements.

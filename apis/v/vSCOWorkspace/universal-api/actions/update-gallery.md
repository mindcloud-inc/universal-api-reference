# VSCO Workspace: Update Gallery

Updates an existing gallery in VSCO Workspace.

```
PUT https://connect.mindcloud.co/v1/universal/vSCOWorkspace/latest/actions/update-gallery
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/vSCOWorkspace/latest/actions/update-gallery" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/vSCOWorkspace/latest/actions/update-gallery', {
  method: 'PUT',
  headers: {
...
Through the native VSCO Workspace API, this operation is `PUT /gallery/:id` (base URL `https://workspace.vsco.co/api/v2`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-gallery.md) for the provider-specific parameters and requirements.

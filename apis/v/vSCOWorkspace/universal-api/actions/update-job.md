# VSCO Workspace: Update Job

Updates an existing job in VSCO Workspace.

```
PUT https://connect.mindcloud.co/v1/universal/vSCOWorkspace/latest/actions/update-job
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/vSCOWorkspace/latest/actions/update-job" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/vSCOWorkspace/latest/actions/update-job', {
  method: 'PUT',
  headers: {
...
| --- | --- | --- | --- |
| `id` | string | yes |  |
| `bookingDate` | date | no | A date string consisting of year, month and day in the timezone of the event if specified or the studio. |
| `brandId` | string | no | A ULID entity identifier that is nullable. |
| `closed` | boolean | no | Whether or not the lead or job is closed. A closed reason might be provided as well. |
| `closedReasonId` | string | no | A ULID entity identifier that is nullable. |
| `closedDate` | date | no | A date string consisting of year, month and day in the timezone of the event if specified or the studio. |
| `completedDate` | date | no | A date string consisting of year, month and day in the timezone of the event if specified or the studio. |
| `contactFormId` | string | no | A ULID entity identifier that is nullable. |
| `customFields[]` | array<object> | no |  |
| `eventDate` | date | no | A date string consisting of year, month and day in the timezone of the event if specified or the studio. |
| `fulfillmentDate` | date | no | A date string consisting of year, month and day in the timezone of the event if specified or the studio. |
| `guestCount` | number | no |  |
| `inquiryDate` | date | no | A date string consisting of year, month and day in the timezone of the event if specified or the studio. |
| `jobTypeId` | string | no | A ULID entity identifier that is nullable. |
| `lastClientActivity` | object | no |  |
| `leadConfidence` | string | no |  |
| `leadDecisionExpectedByDate` | date | no | A date string consisting of year, month and day in the timezone of the event if specified or the studio. |
| `leadMaxBudget` | number | no |  |
| `leadNotes` | string | no |  |
| `leadRating` | number | no |  |
| `leadSourceId` | string | no | A ULID entity identifier that is nullable. |
| `leadStatusId` | string | no | A ULID entity identifier that is nullable. |
| `leadStatusChangedAt` | object | no |  |
| `name` | string | no | Name of the job that will override the title. |
| `nextInteractionDate` | object | no |  |
| `pinned` | boolean | no |  |
| `primarySessionId` | object | no |  |
| `previousInteractionDate` | object | no |  |
| `stage` | string | no | Specifies the stage that the job is in. |
| `staleDate` | object | no |  |
| `totalCost` | object | no |  |
| `totalProfit` | object | no |  |
| `totalRevenue` | object | no |  |
| `webLead` | boolean | no | Whether or not the lead or job came from a contact form. |
| `workflowId` | string | no | A ULID entity identifier that is nullable. |
...
Through the native VSCO Workspace API, this operation is `PUT /job/:id` (base URL `https://workspace.vsco.co/api/v2`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-job.md) for the provider-specific parameters and requirements.

# Vadivelu HTTP codes Universal API Authentication

Every Vadivelu HTTP codes API request through MindCloud uses a MindCloud API key and the `connectionId` of a Vadivelu HTTP codes connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Vadivelu HTTP codes connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Vadivelu HTTP codes action](README.md#actions-2) with your API key and `connectionId`.

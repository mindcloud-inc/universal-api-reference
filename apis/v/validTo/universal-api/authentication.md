# validTo Universal API Authentication

Every validTo API request through MindCloud uses a MindCloud API key and the `connectionId` of a validTo connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your validTo connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [validTo action](README.md#actions-7) with your API key and `connectionId`.

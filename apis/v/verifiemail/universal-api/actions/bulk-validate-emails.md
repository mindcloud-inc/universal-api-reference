# verifi.email: Bulk Validate Emails

Retrieves bulk email validation results from verifi.email using a JSON array.

```
GET https://connect.mindcloud.co/v1/universal/verifiemail/latest/actions/bulk-validate-emails
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/verifiemail/latest/actions/bulk-validate-emails?connectionId=$CONNECTION_ID&emails=ava%40example.com" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/verifiemail/latest/actions/bulk-validate-emails?${params}`, {
  headers: {
...
Through the native verifi.email API, this operation is `POST /v1/bulk/check` (base URL `https://api.verifi.email`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/bulk-validate-emails.md) for the provider-specific parameters and requirements.

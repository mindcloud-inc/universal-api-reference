# Verify (Tailr Made) Universal API Authentication

Every Verify (Tailr Made) API request through MindCloud uses a MindCloud API key and the `connectionId` of a Verify (Tailr Made) connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Verify (Tailr Made) connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Verify (Tailr Made) action](README.md#actions-1) with your API key and `connectionId`.

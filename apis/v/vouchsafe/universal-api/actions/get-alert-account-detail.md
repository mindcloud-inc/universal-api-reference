# Vouchsafe: Get Alert Account Detail

Retrieves alert account details from Vouchsafe.

```
GET https://connect.mindcloud.co/v1/universal/vouchsafe/latest/actions/get-alert-account-detail
```
...
Through the native Vouchsafe API, this operation is `GET /alerts/accounts/:id` (base URL `https://app.vouchsafe.id/api/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-alert-account-detail.md) for the provider-specific parameters and requirements.

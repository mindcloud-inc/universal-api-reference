# Vouchsafe: Verify Right To Rent

Verifies right to rent with a UK eVisa in Vouchsafe.

```
POST https://connect.mindcloud.co/v1/universal/vouchsafe/latest/actions/verify-right-to-rent
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/vouchsafe/latest/actions/verify-right-to-rent" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/vouchsafe/latest/actions/verify-right-to-rent', {
  method: 'POST',
  headers: {
...
| --- | --- | --- | --- |
| `payload` | object | no | The right to rent verification payload. |
| `payload.role` | list | yes | Role of the person requesting the verification. One of: `Agent`, `Landlord`. |
| `payload.companyName` | string | yes | Name of the company or agency requesting the verification. |

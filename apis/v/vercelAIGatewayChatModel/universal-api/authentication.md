# Vercel AI Gateway Chat Model Universal API Authentication

Every Vercel AI Gateway Chat Model API request through MindCloud uses a MindCloud API key and the `connectionId` of a Vercel AI Gateway Chat Model connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Vercel AI Gateway Chat Model connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Vercel AI Gateway Chat Model action](README.md#actions-4) with your API key and `connectionId`.

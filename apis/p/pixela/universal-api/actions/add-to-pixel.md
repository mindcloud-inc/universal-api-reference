...
```
PUT https://connect.mindcloud.co/v1/universal/pixela/latest/actions/add-to-pixel
```
...
| `quantity` | string | yes | Quantity to add to today's pixel. |
...
Through the native Pixela API, this operation is `PUT /v1/users/:username/graphs/:graphID/add` (base URL `https://pixe.la`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/add-to-pixel.md) for the provider-specific parameters and requirements.

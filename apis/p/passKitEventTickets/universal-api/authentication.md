# PassKit Event Tickets Universal API Authentication

Every PassKit Event Tickets API request through MindCloud uses a MindCloud API key and the `connectionId` of a PassKit Event Tickets connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your PassKit Event Tickets connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [PassKit Event Tickets action](README.md#actions-45) with your API key and `connectionId`.

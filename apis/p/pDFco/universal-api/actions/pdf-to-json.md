...
```
POST https://connect.mindcloud.co/v1/universal/pDFco/latest/actions/pdf-to-json
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/pDFco/latest/actions/pdf-to-json" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/pDFco/latest/actions/pdf-to-json', {
  method: 'POST',
  headers: {
...
| `inline` | boolean | no | Return output inline when true. |
...
| `url` | string | Temporary URL for converted JSON output. |

## Native endpoint

Through the native PDF.co API, this operation is `POST /pdf/convert/to/json2` (base URL `https://api.pdf.co/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/pdf-to-json.md) for the provider-specific parameters and requirements.

# PDFCrowd: Convert HTML String to Image

Creates an image from an HTML string in PDFCrowd.

```
POST https://connect.mindcloud.co/v1/universal/pDFCrowd/latest/actions/convert-html-string-to-image
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/pDFCrowd/latest/actions/convert-html-string-to-image" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/pDFCrowd/latest/actions/convert-html-string-to-image', {
  method: 'POST',
  headers: {
...
| --- | --- | --- | --- |
| `text` | string | yes | Raw HTML markup to convert into an image file. |

# PubNub Universal API Authentication

Every PubNub API request through MindCloud uses a MindCloud API key and the `connectionId` of a PubNub connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your PubNub connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [PubNub action](README.md#actions-34) with your API key and `connectionId`.

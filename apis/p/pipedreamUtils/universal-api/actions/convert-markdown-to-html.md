# Pipedream Utils: Formatting - [Text] Convert Markdown to HTML

Converts Markdown to HTML in Pipedream Utils.

```
GET https://connect.mindcloud.co/v1/universal/pipedreamUtils/latest/actions/convert-markdown-to-html
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/pipedreamUtils/latest/actions/convert-markdown-to-html?connectionId=$CONNECTION_ID&input=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/pipedreamUtils/latest/actions/convert-markdown-to-html?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `input` | string | yes | Markdown string to be converted to HTML |

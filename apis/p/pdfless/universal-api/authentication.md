# Pdfless Universal API Authentication

Every Pdfless API request through MindCloud uses a MindCloud API key and the `connectionId` of a Pdfless connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Pdfless connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Pdfless action](README.md#actions-1) with your API key and `connectionId`.

# Priority Matrix: Update Item

Updates an existing item in Priority Matrix.

```
PUT https://connect.mindcloud.co/v1/universal/priorityMatrix/latest/actions/update-item
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/priorityMatrix/latest/actions/update-item" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/priorityMatrix/latest/actions/update-item', {
  method: 'PUT',
  headers: {
...
| `descriptionText` | string | no | Plain text item notes or description. |
| `quadrant` | number | no | Priority Matrix quadrant number. |
| `completionPercentage` | number | no | Completion percentage from 0 to 100. |

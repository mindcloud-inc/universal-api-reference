# Planning Center: Update Person

Updates an existing person in Planning Center.

```
PUT https://connect.mindcloud.co/v1/universal/planningCenter/latest/actions/update-person
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/planningCenter/latest/actions/update-person" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/planningCenter/latest/actions/update-person', {
  method: 'PUT',
  headers: {
...
| --- | --- | --- | --- |
| `id` | string | yes |  |
| `data` | object | yes | JSON:API resource object containing the Person type, attributes, and optional relationships to update. |

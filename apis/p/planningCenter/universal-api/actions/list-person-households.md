# Planning Center: List Person Households

Retrieves households for a person in Planning Center.

```
GET https://connect.mindcloud.co/v1/universal/planningCenter/latest/actions/list-person-households
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/planningCenter/latest/actions/list-person-households?connectionId=$CONNECTION_ID&limit=25&offset=0&personId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/planningCenter/latest/actions/list-person-households?${params}`, {
  headers: {
...
Through the native Planning Center API, this operation is `GET /people/v2/people/:person_id/households` (base URL `https://api.planningcenteronline.com`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/list-person-households.md) for the provider-specific parameters and requirements.

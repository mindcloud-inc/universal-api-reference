# Pipio: Generate Dubbed Video Legacy

Creates a dubbed video in Pipio using the legacy workflow.

```
POST https://connect.mindcloud.co/v1/universal/pipio/latest/actions/generate-dubbed-video-legacy
```

Authenticate with `Authorization: Bearer $MINDCLOUD_API_KEY` and pass a Pipio `connectionId` ([setup](../authentication.md)).

## Example request

```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/pipio/latest/actions/generate-dubbed-video-legacy" \
  -H "Authorization: Bearer $MINDCLOUD_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
  "connectionId": "$CONNECTION_ID",
  "targetLanguage": "es",
  "sourceUrl": "https://cdn.pipio.ai/your-video.mp4"
}'
```

```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/pipio/latest/actions/generate-dubbed-video-legacy', {
  method: 'POST',
  headers: {
    Authorization: `Bearer ${process.env.MINDCLOUD_API_KEY}`,
    'Content-Type': 'application/json'
  },
  body: JSON.stringify({
    connectionId,
    "targetLanguage": "es",
    "sourceUrl": "https://cdn.pipio.ai/your-video.mp4"
  })
});

const { success, data } = await response.json();
```

## Inputs

Arguments are sent as JSON body fields ([conventions](../arguments.md)).

| Key | Type | Required | Description |
| --- | --- | --- | --- |
| `targetLanguage` | string | yes | Language code to translate and dub the video into. Default: `es`. Example: `es`. |
| `sourceUrl` | string | yes | The URL to your source video that will be dubbed. Example: `https://cdn.pipio.ai/your-video.mp4`. |

### Advanced

| Key | Type | Required | Description |
| --- | --- | --- | --- |
| `sourceLanguage` | string | no | Language code of the source video, or auto for automatic detection. Default: `auto`. Example: `auto`. |
| `voiceOnly` | boolean | no | Whether to generate translated audio only without lip sync. Default: `false`. |

## Response

The response envelope is `{ "success": true, "data": [...], "meta": {} }`. The `data` schema for this action is dynamic; it mirrors what the native Pipio API returns.

## Native endpoint

Through the native Pipio API, this operation is `POST https://project.pipio.ai/project/generate/dubbing` (base URL `https://avatar.pipio.ai`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/generate-dubbed-video-legacy.md) for the provider-specific parameters and requirements.


# PDF Tools by Tachytelic Universal API Authentication

Every PDF Tools by Tachytelic API request through MindCloud uses a MindCloud API key and the `connectionId` of a PDF Tools by Tachytelic connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your PDF Tools by Tachytelic connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [PDF Tools by Tachytelic action](README.md#actions-7) with your API key and `connectionId`.

# ProjectManager: Query Tasks

Finds tasks in ProjectManager.

```
GET https://connect.mindcloud.co/v1/universal/projectManager/latest/actions/query-tasks
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/projectManager/latest/actions/query-tasks?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/projectManager/latest/actions/query-tasks?${params}`, {
  headers: {
...
Through the native ProjectManager API, this operation is `GET /api/data/tasks` (base URL `https://api.projectmanager.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/query-tasks.md) for the provider-specific parameters and requirements.

# PDF Split and PDF Merge Universal API Authentication

Every PDF Split and PDF Merge API request through MindCloud uses a MindCloud API key and the `connectionId` of a PDF Split and PDF Merge connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your PDF Split and PDF Merge connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [PDF Split and PDF Merge action](README.md#actions-1) with your API key and `connectionId`.

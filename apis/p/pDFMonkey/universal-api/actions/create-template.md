# PDFMonkey: Create Template

Creates a template in PDFMonkey.

```
POST https://connect.mindcloud.co/v1/universal/pDFMonkey/latest/actions/create-template
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/pDFMonkey/latest/actions/create-template" \
  -H "Authorization: Bearer $MINDCLOUD_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
  "connectionId": "$CONNECTION_ID",
  "identifier": "Stage 3 Template",
  "pdfEngineDraftId": "c1931951-3a89-4066-9eb7-0adce6c9dcc8"
}'
```

```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/pDFMonkey/latest/actions/create-template', {
  method: 'POST',
  headers: {
...
| --- | --- | --- | --- |
| `identifier` | string | yes | Human name of the document template. Example: `Stage 3 Template`. |
...
Through the native PDFMonkey API, this operation is `POST /document_templates` (base URL `https://api.pdfmonkey.io/api/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-template.md) for the provider-specific parameters and requirements.

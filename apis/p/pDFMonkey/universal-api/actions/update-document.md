# PDFMonkey: Update Document

Updates an existing document in PDFMonkey.

```
PUT https://connect.mindcloud.co/v1/universal/pDFMonkey/latest/actions/update-document
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/pDFMonkey/latest/actions/update-document" \
  -H "Authorization: Bearer $MINDCLOUD_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
  "connectionId": "$CONNECTION_ID",
  "id": "2c55399c-f469-41d5-a584-1323270f2dd3"
}'
```

```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/pDFMonkey/latest/actions/update-document', {
  method: 'PUT',
  headers: {
...
| --- | --- | --- | --- |
| `id` | string | yes | ID of the Document to update. Example: `2c55399c-f469-41d5-a584-1323270f2dd3`. |
| `documentTemplateId` | string | no | ID of the source Template to use. Example: `12345678-90ab-cdef-1234-567890abcdef`. |
...
Through the native PDFMonkey API, this operation is `PUT /documents/:id` (base URL `https://api.pdfmonkey.io/api/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-document.md) for the provider-specific parameters and requirements.

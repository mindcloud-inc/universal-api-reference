# Papyrs: Update Heading Widget



```
PUT https://connect.mindcloud.co/v1/universal/papyrs/latest/actions/update-heading-widget
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/papyrs/latest/actions/update-heading-widget" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/papyrs/latest/actions/update-heading-widget', {
  method: 'PUT',
  headers: {
...
Through the native Papyrs API, this operation is `POST /page/:page_id/heading/update/:widget_id/` (base URL `https://{{credentials.subdomain}}.papyrs.com/api/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-heading-widget.md) for the provider-specific parameters and requirements.

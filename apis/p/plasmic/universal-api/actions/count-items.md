...
```
GET https://connect.mindcloud.co/v1/universal/plasmic/latest/actions/count-items
```
...
| --- | --- | --- | --- |
| `modelId` | string | yes | The Plasmic CMS model identifier to count. |
| `q` | string | no | A JSON-encoded Plasmic query object used to constrain the count. |
...
| --- | --- | --- |
| `count` | number | The number of rows matching the current query. |

## Native endpoint

Through the native Plasmic API, this operation is `GET /databases/{{credentials.cmsId}}/tables/:modelId/count` (base URL `https://data.plasmic.app/api/v1/cms`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/count-items.md) for the provider-specific parameters and requirements.

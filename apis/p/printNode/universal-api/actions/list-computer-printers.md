# PrintNode: List Computer Printers

Retrieves printers for specific computers from PrintNode.

```
GET https://connect.mindcloud.co/v1/universal/printNode/latest/actions/list-computer-printers
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/printNode/latest/actions/list-computer-printers?connectionId=$CONNECTION_ID&computerSet=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/printNode/latest/actions/list-computer-printers?${params}`, {
  headers: {
...
Through the native PrintNode API, this operation is `GET /computers/:computerSet/printers` (base URL `https://api.printnode.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-computer-printers.md) for the provider-specific parameters and requirements.

# PrintNode: List Computer Scales

Retrieves scales for a specific computer from PrintNode.

```
GET https://connect.mindcloud.co/v1/universal/printNode/latest/actions/list-computer-scales
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/printNode/latest/actions/list-computer-scales?connectionId=$CONNECTION_ID&computerId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/printNode/latest/actions/list-computer-scales?${params}`, {
  headers: {
...
Through the native PrintNode API, this operation is `GET /computer/:computerId/scales` (base URL `https://api.printnode.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-computer-scales.md) for the provider-specific parameters and requirements.

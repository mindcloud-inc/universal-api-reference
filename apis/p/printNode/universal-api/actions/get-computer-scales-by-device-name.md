# PrintNode: Get Computer Scales by Device Name

Retrieves scales by device name for a computer from PrintNode.

```
GET https://connect.mindcloud.co/v1/universal/printNode/latest/actions/get-computer-scales-by-device-name
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/printNode/latest/actions/get-computer-scales-by-device-name?connectionId=$CONNECTION_ID&computerId=string&deviceName=Ava%20Chen" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/printNode/latest/actions/get-computer-scales-by-device-name?${params}`, {
  headers: {
...
Through the native PrintNode API, this operation is `GET /computer/:computerId/scales/:deviceName` (base URL `https://api.printnode.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-computer-scales-by-device-name.md) for the provider-specific parameters and requirements.

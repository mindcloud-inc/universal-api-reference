# PrintNode: List Printer Print Jobs

Retrieves print jobs for specific printers from PrintNode.

```
GET https://connect.mindcloud.co/v1/universal/printNode/latest/actions/list-printer-print-jobs
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/printNode/latest/actions/list-printer-print-jobs?connectionId=$CONNECTION_ID&printerSet=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/printNode/latest/actions/list-printer-print-jobs?${params}`, {
  headers: {
...
Through the native PrintNode API, this operation is `GET /printers/:printerSet/printjobs` (base URL `https://api.printnode.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-printer-print-jobs.md) for the provider-specific parameters and requirements.

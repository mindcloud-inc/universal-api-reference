# PrintNode: List Print Jobs by Set

Retrieves specific print jobs from PrintNode by ID set.

```
GET https://connect.mindcloud.co/v1/universal/printNode/latest/actions/list-print-jobs-by-set
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/printNode/latest/actions/list-print-jobs-by-set?connectionId=$CONNECTION_ID&printJobSet=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/printNode/latest/actions/list-print-jobs-by-set?${params}`, {
  headers: {
...
Through the native PrintNode API, this operation is `GET /printjobs/:printJobSet` (base URL `https://api.printnode.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-print-jobs-by-set.md) for the provider-specific parameters and requirements.

# PrintNode: List Download Clients by Set

Retrieves specific client downloads from PrintNode by ID set.

```
GET https://connect.mindcloud.co/v1/universal/printNode/latest/actions/list-download-clients-by-set
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/printNode/latest/actions/list-download-clients-by-set?connectionId=$CONNECTION_ID&downloadSet=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/printNode/latest/actions/list-download-clients-by-set?${params}`, {
  headers: {

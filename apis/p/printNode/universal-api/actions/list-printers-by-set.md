# PrintNode: List Printers by Set

Retrieves specific printers from PrintNode by ID set.

```
GET https://connect.mindcloud.co/v1/universal/printNode/latest/actions/list-printers-by-set
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/printNode/latest/actions/list-printers-by-set?connectionId=$CONNECTION_ID&printerSet=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/printNode/latest/actions/list-printers-by-set?${params}`, {
  headers: {
...
Through the native PrintNode API, this operation is `GET /printers/:printerSet` (base URL `https://api.printnode.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-printers-by-set.md) for the provider-specific parameters and requirements.

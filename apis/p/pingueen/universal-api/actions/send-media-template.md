# Pingueen: Send Media Template



```
POST https://connect.mindcloud.co/v1/universal/pingueen/latest/actions/send-media-template
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/pingueen/latest/actions/send-media-template" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/pingueen/latest/actions/send-media-template', {
  method: 'POST',
  headers: {
...
| `success` | boolean | Whether the media template message was accepted successfully. |

## Native endpoint

Through the native Pingueen API, this operation is `POST /template` (base URL `https://api.pingueen.it/ext/v2/{{credentials.businessname}}`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/send-media-template.md) for the provider-specific parameters and requirements.

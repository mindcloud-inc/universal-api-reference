# Process Plan: List Process Instance Tasks for Tag



```
GET https://connect.mindcloud.co/v1/universal/processPlan/latest/actions/list-process-instance-tasks-for-tag
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/processPlan/latest/actions/list-process-instance-tasks-for-tag?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/processPlan/latest/actions/list-process-instance-tasks-for-tag?${params}`, {
  headers: {
...
Through the native Process Plan API, this operation is `GET /tag/:tagId/process_instance_task/list` (base URL `https://apius0.processplan.com/api/v4`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-process-instance-tasks-for-tag.md) for the provider-specific parameters and requirements.

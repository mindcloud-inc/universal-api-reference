# Process Plan: List Audit Entries



```
GET https://connect.mindcloud.co/v1/universal/processPlan/latest/actions/list-audit-entries
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/processPlan/latest/actions/list-audit-entries?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/processPlan/latest/actions/list-audit-entries?${params}`, {
  headers: {
...
Through the native Process Plan API, this operation is `GET /audit_entry/list` (base URL `https://apius0.processplan.com/api/v4`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-audit-entries.md) for the provider-specific parameters and requirements.

# Process Plan: Refresh Private URL for File



```
GET https://connect.mindcloud.co/v1/universal/processPlan/latest/actions/refresh-private-url-for-file
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/processPlan/latest/actions/refresh-private-url-for-file?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/processPlan/latest/actions/refresh-private-url-for-file?${params}`, {
  headers: {
...
Through the native Process Plan API, this operation is `GET /file/:fileId/refresh_private_url` (base URL `https://apius0.processplan.com/api/v4`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/refresh-private-url-for-file.md) for the provider-specific parameters and requirements.

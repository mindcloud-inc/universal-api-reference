# Process Plan: Convert Process Template Tasks IDs to Objects



```
GET https://connect.mindcloud.co/v1/universal/processPlan/latest/actions/convert-process-template-tasks-i-ds-to-objects
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/processPlan/latest/actions/convert-process-template-tasks-i-ds-to-objects?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/processPlan/latest/actions/convert-process-template-tasks-i-ds-to-objects?${params}`, {
  headers: {
...
Through the native Process Plan API, this operation is `POST /process_template_task/id_list/to_object/list` (base URL `https://apius0.processplan.com/api/v4`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/convert-process-template-tasks-i-ds-to-objects.md) for the provider-specific parameters and requirements.

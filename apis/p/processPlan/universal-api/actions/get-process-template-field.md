# Process Plan: Get Process Template Field



```
GET https://connect.mindcloud.co/v1/universal/processPlan/latest/actions/get-process-template-field
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/processPlan/latest/actions/get-process-template-field?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/processPlan/latest/actions/get-process-template-field?${params}`, {
  headers: {
...
Through the native Process Plan API, this operation is `GET /process_template_field/:processTemplateFieldId` (base URL `https://apius0.processplan.com/api/v4`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-process-template-field.md) for the provider-specific parameters and requirements.

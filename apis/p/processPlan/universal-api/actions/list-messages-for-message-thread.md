# Process Plan: List Messages for Message Thread



```
GET https://connect.mindcloud.co/v1/universal/processPlan/latest/actions/list-messages-for-message-thread
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/processPlan/latest/actions/list-messages-for-message-thread?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/processPlan/latest/actions/list-messages-for-message-thread?${params}`, {
  headers: {
...
Through the native Process Plan API, this operation is `GET /message_thread/:messageThreadId/message/list` (base URL `https://apius0.processplan.com/api/v4`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-messages-for-message-thread.md) for the provider-specific parameters and requirements.

# Process Plan: List Canceled Process Instance Tasks



```
GET https://connect.mindcloud.co/v1/universal/processPlan/latest/actions/list-canceled-process-instance-tasks
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/processPlan/latest/actions/list-canceled-process-instance-tasks?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/processPlan/latest/actions/list-canceled-process-instance-tasks?${params}`, {
  headers: {
...
Through the native Process Plan API, this operation is `GET /process_instance_task/list/canceled` (base URL `https://apius0.processplan.com/api/v4`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-canceled-process-instance-tasks.md) for the provider-specific parameters and requirements.

# Process Plan: Count Process Template Headers With Process Instance Header Pending



```
GET https://connect.mindcloud.co/v1/universal/processPlan/latest/actions/count-process-template-headers-with-process-instance-header-pending
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/processPlan/latest/actions/count-process-template-headers-with-process-instance-header-pending?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/processPlan/latest/actions/count-process-template-headers-with-process-instance-header-pending?${params}`, {
  headers: {
...
| `process_template_header_list[].th_ih_pending_count` | number |  |
...
| `process_template_header_list[].th_launch_id_list[]` | string |  |
...
Through the native Process Plan API, this operation is `GET /process_template_header/list/with/process_instance_header/pending/count` (base URL `https://apius0.processplan.com/api/v4`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/count-process-template-headers-with-process-instance-header-pending.md) for the provider-specific parameters and requirements.

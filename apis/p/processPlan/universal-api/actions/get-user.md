# Process Plan: Get User



```
GET https://connect.mindcloud.co/v1/universal/processPlan/latest/actions/get-user
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/processPlan/latest/actions/get-user?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/processPlan/latest/actions/get-user?${params}`, {
  headers: {
...
Through the native Process Plan API, this operation is `GET /user/:userId` (base URL `https://apius0.processplan.com/api/v4`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-user.md) for the provider-specific parameters and requirements.

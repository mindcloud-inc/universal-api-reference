# Process Plan: List Pending Process Instance Headers



```
GET https://connect.mindcloud.co/v1/universal/processPlan/latest/actions/list-pending-process-instance-headers
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/processPlan/latest/actions/list-pending-process-instance-headers?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/processPlan/latest/actions/list-pending-process-instance-headers?${params}`, {
  headers: {
...
Through the native Process Plan API, this operation is `GET /process_instance_header/list/pending` (base URL `https://apius0.processplan.com/api/v4`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-pending-process-instance-headers.md) for the provider-specific parameters and requirements.

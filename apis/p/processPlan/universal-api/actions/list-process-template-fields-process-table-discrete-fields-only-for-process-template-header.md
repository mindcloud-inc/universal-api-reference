# Process Plan: List Process Template Fields Process Table Discrete Fields Only for Process Template Header



```
GET https://connect.mindcloud.co/v1/universal/processPlan/latest/actions/list-process-template-fields-process-table-discrete-fields-only-for-process-template-header
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/processPlan/latest/actions/list-process-template-fields-process-table-discrete-fields-only-for-process-template-header?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/processPlan/latest/actions/list-process-template-fields-process-table-discrete-fields-only-for-process-template-header?${params}`, {
  headers: {
...
Through the native Process Plan API, this operation is `GET /process_template_header/:processTemplateHeaderId/process_template_field/list/process_table_discrete_fields_only` (base URL `https://apius0.processplan.com/api/v4`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-process-template-fields-process-table-discrete-fields-only-for-process-template-header.md) for the provider-specific parameters and requirements.

# Process Plan: List Completed Process Instance Tasks for Me



```
GET https://connect.mindcloud.co/v1/universal/processPlan/latest/actions/list-completed-process-instance-tasks-for-me
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/processPlan/latest/actions/list-completed-process-instance-tasks-for-me?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/processPlan/latest/actions/list-completed-process-instance-tasks-for-me?${params}`, {
  headers: {
...
Through the native Process Plan API, this operation is `GET /user/me/process_instance_task/list/completed` (base URL `https://apius0.processplan.com/api/v4`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-completed-process-instance-tasks-for-me.md) for the provider-specific parameters and requirements.

# PickFu Universal API Authentication

Every PickFu API request through MindCloud uses a MindCloud API key and the `connectionId` of a PickFu connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your PickFu connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [PickFu action](README.md#actions-16) with your API key and `connectionId`.

# Pabbly Email Verification Universal API Authentication

Every Pabbly Email Verification API request through MindCloud uses a MindCloud API key and the `connectionId` of a Pabbly Email Verification connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Pabbly Email Verification connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Pabbly Email Verification action](README.md#actions-1) with your API key and `connectionId`.

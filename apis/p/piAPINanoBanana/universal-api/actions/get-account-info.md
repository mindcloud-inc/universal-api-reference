# PiAPI/NanoBanana: Get Account Info



```
GET https://connect.mindcloud.co/v1/universal/piAPINanoBanana/latest/actions/get-account-info
```
...
| `data.type` | string |  |
| `data.wallet.gpts_remain` | number |  |
| `data.wallet.llm_remain` | number |  |
...
Through the native PiAPI/NanoBanana API, this operation is `GET /account/info` (base URL `https://api.piapi.ai`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-account-info.md) for the provider-specific parameters and requirements.

# PiAPI/NanoBanana Universal API Authentication

Every PiAPI/NanoBanana API request through MindCloud uses a MindCloud API key and the `connectionId` of a PiAPI/NanoBanana connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your PiAPI/NanoBanana connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [PiAPI/NanoBanana action](README.md#actions-7) with your API key and `connectionId`.

# Pabbly Subscription Billing Universal API Authentication

Every Pabbly Subscription Billing API request through MindCloud uses a MindCloud API key and the `connectionId` of a Pabbly Subscription Billing connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Pabbly Subscription Billing connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Pabbly Subscription Billing action](README.md#actions-88) with your API key and `connectionId`.

# Pipefile Universal API Authentication

Every Pipefile API request through MindCloud uses a MindCloud API key and the `connectionId` of a Pipefile connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Pipefile connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Pipefile action](README.md#actions-7) with your API key and `connectionId`.

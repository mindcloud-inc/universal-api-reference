# PassKit Membership Universal API Authentication

Every PassKit Membership API request through MindCloud uses a MindCloud API key and the `connectionId` of a PassKit Membership connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your PassKit Membership connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [PassKit Membership action](README.md#actions-14) with your API key and `connectionId`.

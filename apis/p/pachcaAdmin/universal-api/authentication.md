# Pachca (Admin) Universal API Authentication

Every Pachca (Admin) API request through MindCloud uses a MindCloud API key and the `connectionId` of a Pachca (Admin) connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Pachca (Admin) connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Pachca (Admin) action](README.md#actions-46) with your API key and `connectionId`.

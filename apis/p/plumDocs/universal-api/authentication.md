# PlumDocs Universal API Authentication

Every PlumDocs API request through MindCloud uses a MindCloud API key and the `connectionId` of a PlumDocs connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your PlumDocs connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [PlumDocs action](README.md#actions-4) with your API key and `connectionId`.

# PokéAPI Universal API Authentication

Every PokéAPI API request through MindCloud uses a MindCloud API key and the `connectionId` of a PokéAPI connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your PokéAPI connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [PokéAPI action](README.md#actions-99) with your API key and `connectionId`.

# PromptLayer Run Agent Universal API Authentication

Every PromptLayer Run Agent API request through MindCloud uses a MindCloud API key and the `connectionId` of a PromptLayer Run Agent connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your PromptLayer Run Agent connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [PromptLayer Run Agent action](README.md#actions-40) with your API key and `connectionId`.

# PDF API Hub Universal API Authentication

Every PDF API Hub API request through MindCloud uses a MindCloud API key and the `connectionId` of a PDF API Hub connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your PDF API Hub connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [PDF API Hub action](README.md#actions-5) with your API key and `connectionId`.

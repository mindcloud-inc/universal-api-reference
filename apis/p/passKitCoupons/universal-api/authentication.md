# PassKit Coupons Universal API Authentication

Every PassKit Coupons API request through MindCloud uses a MindCloud API key and the `connectionId` of a PassKit Coupons connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your PassKit Coupons connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [PassKit Coupons action](README.md#actions-25) with your API key and `connectionId`.

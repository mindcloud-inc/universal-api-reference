# Pinghome Universal API Authentication

Every Pinghome API request through MindCloud uses a MindCloud API key and the `connectionId` of a Pinghome connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Pinghome connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Pinghome action](README.md#actions-72) with your API key and `connectionId`.

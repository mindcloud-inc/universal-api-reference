# Peach: Create One-Time Payment

Creates a one-time payment in Peach.

```
POST https://connect.mindcloud.co/v1/universal/peach/latest/actions/create-one-time-payment
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/peach/latest/actions/create-one-time-payment" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/peach/latest/actions/create-one-time-payment', {
  method: 'POST',
  headers: {
...
Through the native Peach API, this operation is `POST /payments` (base URL `https://api.peach-in.com/v4`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-one-time-payment.md) for the provider-specific parameters and requirements.

# PiAPI/Flux.1 Universal API Authentication

Every PiAPI/Flux.1 API request through MindCloud uses a MindCloud API key and the `connectionId` of a PiAPI/Flux.1 connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your PiAPI/Flux.1 connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [PiAPI/Flux.1 action](README.md#actions-13) with your API key and `connectionId`.

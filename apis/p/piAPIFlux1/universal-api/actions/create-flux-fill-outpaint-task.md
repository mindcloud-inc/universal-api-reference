# PiAPI/Flux.1: Create Flux Fill Outpaint Task

Creates a Flux fill outpaint task in PiAPI/Flux.1.

```
POST https://connect.mindcloud.co/v1/universal/piAPIFlux1/latest/actions/create-flux-fill-outpaint-task
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/piAPIFlux1/latest/actions/create-flux-fill-outpaint-task" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/piAPIFlux1/latest/actions/create-flux-fill-outpaint-task', {
  method: 'POST',
  headers: {
...
| `input.customSettings[]` | array | yes |  |
...
Through the native PiAPI/Flux.1 API, this operation is `POST /api/v1/task` (base URL `https://api.piapi.ai`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-flux-fill-outpaint-task.md) for the provider-specific parameters and requirements.

# PiAPI/Flux.1: Create Flux Text to Image Task

Creates a Flux text-to-image task in PiAPI/Flux.1.

```
POST https://connect.mindcloud.co/v1/universal/piAPIFlux1/latest/actions/create-flux-text-to-image-task
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/piAPIFlux1/latest/actions/create-flux-text-to-image-task" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/piAPIFlux1/latest/actions/create-flux-text-to-image-task', {
  method: 'POST',
  headers: {
...
| `input.width` | number | no |  |
| `input.height` | number | no |  |
...
Through the native PiAPI/Flux.1 API, this operation is `POST /api/v1/task` (base URL `https://api.piapi.ai`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-flux-text-to-image-task.md) for the provider-specific parameters and requirements.

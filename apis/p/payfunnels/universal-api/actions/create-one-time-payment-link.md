# Payfunnels: Create One-Time Payment Link

Creates a one-time payment link in Payfunnels.

```
POST https://connect.mindcloud.co/v1/universal/payfunnels/latest/actions/create-one-time-payment-link
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/payfunnels/latest/actions/create-one-time-payment-link" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/payfunnels/latest/actions/create-one-time-payment-link', {
  method: 'POST',
  headers: {
...
| `amount` | number | yes | One-time payment amount. |
...
Through the native Payfunnels API, this operation is `POST /v1/paymentlinks/onetime` (base URL `https://api.payfunnels.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-one-time-payment-link.md) for the provider-specific parameters and requirements.

# PiAPI/Sora: Create Sora2 Pro Text to Video Task



```
POST https://connect.mindcloud.co/v1/universal/piAPISora/latest/actions/create-sora2-pro-text-to-video-task
```
...
| --- | --- | --- | --- |
| `input.prompt` | string | yes | Text prompt for the Sora2 Pro video. Example: `A dramatic tracking shot of a red race car in heavy rain`. |
| `input.aspectRatio` | string | no | Aspect ratio sent to PiAPI. Example: `16:9`. |
| `input.resolution` | list | no | Sora2 Pro output resolution. One of: `1080p`, `720p`. |
...
Through the native PiAPI/Sora API, this operation is `POST /api/v1/task` (base URL `https://api.piapi.ai`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-sora2-pro-text-to-video-task.md) for the provider-specific parameters and requirements.

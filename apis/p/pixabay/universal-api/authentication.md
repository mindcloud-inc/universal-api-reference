# Pixabay Universal API Authentication

Every Pixabay API request through MindCloud uses a MindCloud API key and the `connectionId` of a Pixabay connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Pixabay connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Pixabay action](README.md#actions-57) with your API key and `connectionId`.

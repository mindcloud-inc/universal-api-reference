# Pixabay: Search Animal Images

Finds animal images in Pixabay.

```
GET https://connect.mindcloud.co/v1/universal/pixabay/latest/actions/search-animal-images
```
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/pixabay/latest/actions/search-animal-images?${params}`, {
  headers: {
...
Through the native Pixabay API, this operation is `GET /api/` (base URL `https://pixabay.com`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/search-animal-images.md) for the provider-specific parameters and requirements.

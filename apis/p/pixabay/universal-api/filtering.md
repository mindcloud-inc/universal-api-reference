# Pixabay Universal API Filtering

Filterable list actions accept a `where` query parameter written as an RSQL expression. MindCloud translates it into the filtering format Pixabay expects, and each action page lists the fields available to filter.
...
## Pixabay actions that support filtering

- [Get Image by ID](actions/get-image-by-id.md)
- [Get Video by ID](actions/get-video-by-id.md)

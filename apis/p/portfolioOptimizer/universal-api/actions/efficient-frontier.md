...
```
GET https://connect.mindcloud.co/v1/universal/portfolioOptimizer/latest/actions/efficient-frontier
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/portfolioOptimizer/latest/actions/efficient-frontier?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/portfolioOptimizer/latest/actions/efficient-frontier?${params}`, {
  headers: {
...
Through the native Portfolio Optimizer API, this operation is `POST /v1/portfolios/analysis/mean-variance/efficient-frontier` (base URL `https://api.portfoliooptimizer.io`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/efficient-frontier.md) for the provider-specific parameters and requirements.

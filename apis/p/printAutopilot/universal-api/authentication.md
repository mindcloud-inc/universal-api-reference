# Print Autopilot Universal API Authentication

Every Print Autopilot API request through MindCloud uses a MindCloud API key and the `connectionId` of a Print Autopilot connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Print Autopilot connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Print Autopilot action](README.md#actions-4) with your API key and `connectionId`.

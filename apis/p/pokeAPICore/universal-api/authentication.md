# PokeAPI Core Universal API Authentication

Every PokeAPI Core API request through MindCloud uses a MindCloud API key and the `connectionId` of a PokeAPI Core connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your PokeAPI Core connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [PokeAPI Core action](README.md#actions-24) with your API key and `connectionId`.

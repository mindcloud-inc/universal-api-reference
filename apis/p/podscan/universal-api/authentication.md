# Podscan Universal API Authentication

Every Podscan API request through MindCloud uses a MindCloud API key and the `connectionId` of a Podscan connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Podscan connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Podscan action](README.md#actions-48) with your API key and `connectionId`.

# Pipeline CRM Universal API Authentication

Every Pipeline CRM API request through MindCloud uses a MindCloud API key and the `connectionId` of a Pipeline CRM connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Pipeline CRM connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Pipeline CRM action](README.md#actions-24) with your API key and `connectionId`.

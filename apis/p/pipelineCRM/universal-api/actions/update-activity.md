# Pipeline CRM: Update Activity

Updates an existing activity note in Pipeline CRM.

```
PUT https://connect.mindcloud.co/v1/universal/pipelineCRM/latest/actions/update-activity
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/pipelineCRM/latest/actions/update-activity" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/pipelineCRM/latest/actions/update-activity', {
  method: 'PUT',
  headers: {
...
Through the native Pipeline CRM API, this operation is `PUT /notes/:id` (base URL `https://api.pipelinecrm.com/api/v3`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-activity.md) for the provider-specific parameters and requirements.

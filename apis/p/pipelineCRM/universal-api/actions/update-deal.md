# Pipeline CRM: Update Deal

Updates an existing deal in Pipeline CRM.

```
PUT https://connect.mindcloud.co/v1/universal/pipelineCRM/latest/actions/update-deal
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/pipelineCRM/latest/actions/update-deal" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/pipelineCRM/latest/actions/update-deal', {
  method: 'PUT',
  headers: {
...
| --- | --- | --- | --- |
| `deliverReassignmentEmail` | boolean | no | Send an assignment email when the deal owner changes. Default: `true`. |
...
Through the native Pipeline CRM API, this operation is `PUT /deals/:id` (base URL `https://api.pipelinecrm.com/api/v3`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-deal.md) for the provider-specific parameters and requirements.

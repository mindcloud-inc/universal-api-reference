# Pipeline CRM: Update Calendar Entry

Updates an existing calendar entry in Pipeline CRM.

```
PUT https://connect.mindcloud.co/v1/universal/pipelineCRM/latest/actions/update-calendar-entry
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/pipelineCRM/latest/actions/update-calendar-entry" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/pipelineCRM/latest/actions/update-calendar-entry', {
  method: 'PUT',
  headers: {
...
| --- | --- | --- | --- |
| `deliverReassignmentEmail` | boolean | no | Send an assignment email when the calendar entry owner changes. Default: `false`. |
| `id` | number | yes | Calendar entry ID |
| `calendarEntry.type` | string | no | Calendar entry type: CalendarEvent or CalendarTask. |
| `calendarEntry.categoryId` | number | no | The event category ID. |
...
Through the native Pipeline CRM API, this operation is `PUT /calendar_entries/:id` (base URL `https://api.pipelinecrm.com/api/v3`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-calendar-entry.md) for the provider-specific parameters and requirements.

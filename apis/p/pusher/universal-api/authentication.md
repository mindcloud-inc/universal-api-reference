# Pusher Universal API Authentication

Every Pusher API request through MindCloud uses a MindCloud API key and the `connectionId` of a Pusher connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Pusher connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Pusher action](README.md#actions-6) with your API key and `connectionId`.

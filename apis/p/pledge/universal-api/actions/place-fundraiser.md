# Pledge: Place Fundraiser

Updates fundraiser placement in Pledge.

```
PUT https://connect.mindcloud.co/v1/universal/pledge/latest/actions/place-fundraiser
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/pledge/latest/actions/place-fundraiser" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/pledge/latest/actions/place-fundraiser', {
  method: 'PUT',
  headers: {
...
Through the native Pledge API, this operation is `PUT /fundraisers/[:id]` (base URL `https://api.pledge.to/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/place-fundraiser.md) for the provider-specific parameters and requirements.

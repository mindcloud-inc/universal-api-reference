# Pledge: Update Fundraiser

Updates an existing fundraiser in Pledge.

```
PUT https://connect.mindcloud.co/v1/universal/pledge/latest/actions/update-fundraiser
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/pledge/latest/actions/update-fundraiser" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/pledge/latest/actions/update-fundraiser', {
  method: 'PUT',
  headers: {
...
Through the native Pledge API, this operation is `PATCH /fundraisers/[:id]` (base URL `https://api.pledge.to/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-fundraiser.md) for the provider-specific parameters and requirements.

# PushCallMe Universal API Authentication

Every PushCallMe API request through MindCloud uses a MindCloud API key and the `connectionId` of a PushCallMe connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your PushCallMe connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [PushCallMe action](README.md#actions-2) with your API key and `connectionId`.

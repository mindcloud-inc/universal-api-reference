# PostGrid Print & Mail Universal API Authentication

Every PostGrid Print & Mail API request through MindCloud uses a MindCloud API key and the `connectionId` of a PostGrid Print & Mail connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your PostGrid Print & Mail connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [PostGrid Print & Mail action](README.md#actions-24) with your API key and `connectionId`.

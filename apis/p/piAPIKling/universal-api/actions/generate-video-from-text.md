# PiAPI/Kling: Generate Video from Text



```
POST https://connect.mindcloud.co/v1/universal/piAPIKling/latest/actions/generate-video-from-text
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/piAPIKling/latest/actions/generate-video-from-text" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/piAPIKling/latest/actions/generate-video-from-text', {
  method: 'POST',
  headers: {
...
| --- | --- | --- | --- |
| `input.prompt` | string | yes | Describe the video you want Kling to generate. |
...
| `input.enableAudio` | boolean | no | Enable native audio generation when the selected Kling version supports it. |
| `input.cfgScale` | number | no | Guidance scale for Kling video generation. |
...
Through the native PiAPI/Kling API, this operation is `POST /api/v1/task` (base URL `https://api.piapi.ai`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/generate-video-from-text.md) for the provider-specific parameters and requirements.

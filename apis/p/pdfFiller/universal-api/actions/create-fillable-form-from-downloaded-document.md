# PdfFiller: Create Fillable Form From Downloaded Document

Creates a fillable form in PdfFiller from a downloaded document.

```
POST https://connect.mindcloud.co/v1/universal/pdfFiller/latest/actions/create-fillable-form-from-downloaded-document
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/pdfFiller/latest/actions/create-fillable-form-from-downloaded-document" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/pdfFiller/latest/actions/create-fillable-form-from-downloaded-document', {
  method: 'POST',
  headers: {
...
Through the native PdfFiller API, this operation is `POST /v2/fillable_forms` (base URL `https://api.pdffiller.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-fillable-form-from-downloaded-document.md) for the provider-specific parameters and requirements.

# pretix Universal API Authentication

Every pretix API request through MindCloud uses a MindCloud API key and the `connectionId` of a pretix connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your pretix connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [pretix action](README.md#actions-30) with your API key and `connectionId`.

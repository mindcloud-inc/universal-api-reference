# PiAPI/Wanx: Create Text to Video with LoRA

Creates a LoRA text-to-video task in PiAPI/Wanx.

```
POST https://connect.mindcloud.co/v1/universal/piAPIWanx/latest/actions/create-text-to-video14b-lora
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/piAPIWanx/latest/actions/create-text-to-video14b-lora" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/piAPIWanx/latest/actions/create-text-to-video14b-lora', {
  method: 'POST',
  headers: {
...
Through the native PiAPI/Wanx API, this operation is `POST /api/v1/task` (base URL `https://api.piapi.ai`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-text-to-video14b-lora.md) for the provider-specific parameters and requirements.

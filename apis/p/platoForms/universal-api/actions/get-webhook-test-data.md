# PlatoForms: Get Webhook Test Data

Retrieves webhook test data for a form from PlatoForms.

```
GET https://connect.mindcloud.co/v1/universal/platoForms/latest/actions/get-webhook-test-data
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/platoForms/latest/actions/get-webhook-test-data?connectionId=$CONNECTION_ID&form_identifier=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/platoForms/latest/actions/get-webhook-test-data?${params}`, {
  headers: {
...
Through the native PlatoForms API, this operation is `GET /form/{{form_identifier}}/webhook/` (base URL `https://api.platoforms.com/v4`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-webhook-test-data.md) for the provider-specific parameters and requirements.

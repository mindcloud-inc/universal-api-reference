# Print.one Postcards Universal API Authentication

Every Print.one Postcards API request through MindCloud uses a MindCloud API key and the `connectionId` of a Print.one Postcards connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Print.one Postcards connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Print.one Postcards action](README.md#actions-34) with your API key and `connectionId`.

# Pipedream: Automatically subscribe a listener to events from new workflows / sources

Creates an auto-subscription for new workflow or source events in Pipedream.

```
POST https://connect.mindcloud.co/v1/universal/pipedream/latest/actions/auto-subscribe-listener-to-events
```
...
Through the native Pipedream API, this operation is `POST /auto_subscriptions` (base URL `https://api.pipedream.com/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/auto-subscribe-listener-to-events.md) for the provider-specific parameters and requirements.

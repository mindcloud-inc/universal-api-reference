# PagePixels: Get Screenshots From Configuration



```
GET https://connect.mindcloud.co/v1/universal/pagePixels/latest/actions/get-screenshots-from-configuration
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/pagePixels/latest/actions/get-screenshots-from-configuration?connectionId=$CONNECTION_ID&screenshotConfigurationId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/pagePixels/latest/actions/get-screenshots-from-configuration?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `screenshotConfigurationId` | string | yes | The screenshot configuration ID whose screenshots should be listed. |
...
Through the native PagePixels API, this operation is `GET /screenshot_configs/:screenshot_config_id/screenshots` (base URL `https://api.pagepixels.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-screenshots-from-configuration.md) for the provider-specific parameters and requirements.

# DataScope Forms: Update Location

Updates an existing location in DataScope Forms.

```
PUT https://connect.mindcloud.co/v1/universal/dataScopeForms/latest/actions/update-location
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/dataScopeForms/latest/actions/update-location" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/dataScopeForms/latest/actions/update-location', {
  method: 'PUT',
  headers: {
...
| --- | --- | --- | --- |
| `id` | number | yes | Internal identifier of the location to update. |
...
Through the native DataScope Forms API, this operation is `POST /external/locations/[:id]` (base URL `https://www.mydatascope.com/api`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-location.md) for the provider-specific parameters and requirements.

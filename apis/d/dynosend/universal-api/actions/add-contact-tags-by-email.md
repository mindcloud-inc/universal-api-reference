# Dynosend: Add Contact Tags by Email

Adds tags to a Dynosend contact by email address.

```
PUT https://connect.mindcloud.co/v1/universal/dynosend/latest/actions/add-contact-tags-by-email
```
...
| `email` | string | yes | The email address of the contact to tag. |
...
Through the native Dynosend API, this operation is `PATCH /contacts/addtag` (base URL `https://api.dynosend.com/api/v2`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/add-contact-tags-by-email.md) for the provider-specific parameters and requirements.

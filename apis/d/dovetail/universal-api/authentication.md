# Dovetail Universal API Authentication

Every Dovetail API request through MindCloud uses a MindCloud API key and the `connectionId` of a Dovetail connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Dovetail connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Dovetail action](README.md#actions-25) with your API key and `connectionId`.

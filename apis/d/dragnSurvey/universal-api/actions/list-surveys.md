# Drag'n Survey: List Surveys

Retrieves available surveys from Drag'n Survey.

```
GET https://connect.mindcloud.co/v1/universal/dragnSurvey/latest/actions/list-surveys
```
...
Through the native Drag'n Survey API, this operation is `GET /surveys` (base URL `https://developer.dragnsurvey.com/api/v2.0.0`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-surveys.md) for the provider-specific parameters and requirements.

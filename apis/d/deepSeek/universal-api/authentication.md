# DeepSeek Universal API Authentication

Every DeepSeek API request through MindCloud uses a MindCloud API key and the `connectionId` of a DeepSeek connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your DeepSeek connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [DeepSeek action](README.md#actions-5) with your API key and `connectionId`.

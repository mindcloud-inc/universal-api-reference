# Datalyse: Get Opportunities

Retrieves a list of opportunities from Datalyse.

```
GET https://connect.mindcloud.co/v1/universal/datalyse/latest/actions/get-opportunities
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/datalyse/latest/actions/get-opportunities?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/datalyse/latest/actions/get-opportunities?${params}`, {
  headers: {
...
Through the native Datalyse API, this operation is `POST /api/1.0/opportunities/get.json` (base URL `https://api.datalyse.io`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-opportunities.md) for the provider-specific parameters and requirements.

# Deskpro: Get Ticket Log

Retrieves a ticket log from Deskpro.

```
GET https://connect.mindcloud.co/v1/universal/deskpro/latest/actions/get-ticket-log
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/deskpro/latest/actions/get-ticket-log?connectionId=$CONNECTION_ID&ticketId=3&logId=17" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/deskpro/latest/actions/get-ticket-log?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `ticketId` | number | yes | The Deskpro ticket id containing the log entry. Example: `3`. |
| `logId` | number | yes | The Deskpro ticket log id to retrieve. Example: `17`. |
...
Through the native Deskpro API, this operation is `GET /tickets/:ticketId/logs/:logId` (base URL `{{credentials.helpdeskUrl}}/api/v2`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-ticket-log.md) for the provider-specific parameters and requirements.

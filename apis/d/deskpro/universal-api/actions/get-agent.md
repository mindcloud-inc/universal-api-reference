...
```
GET https://connect.mindcloud.co/v1/universal/deskpro/latest/actions/get-agent
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/deskpro/latest/actions/get-agent?connectionId=$CONNECTION_ID&agentId=1" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/deskpro/latest/actions/get-agent?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `agentId` | number | yes | The Deskpro agent id to retrieve. Example: `1`. |
...
Through the native Deskpro API, this operation is `GET /agents/:agentId` (base URL `{{credentials.helpdeskUrl}}/api/v2`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-agent.md) for the provider-specific parameters and requirements.

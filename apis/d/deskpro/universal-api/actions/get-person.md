...
Retrieves a person record from Deskpro.

```
GET https://connect.mindcloud.co/v1/universal/deskpro/latest/actions/get-person
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/deskpro/latest/actions/get-person?connectionId=$CONNECTION_ID&id=1" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/deskpro/latest/actions/get-person?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `id` | number | yes | The id of the person resource to retrieve, for example 1. Example: `1`. |
...
Through the native Deskpro API, this operation is `GET /people/:id` (base URL `{{credentials.helpdeskUrl}}/api/v2`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-person.md) for the provider-specific parameters and requirements.

# Doctly Universal API Authentication

Every Doctly API request through MindCloud uses a MindCloud API key and the `connectionId` of a Doctly connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Doctly connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Doctly action](README.md#actions-9) with your API key and `connectionId`.

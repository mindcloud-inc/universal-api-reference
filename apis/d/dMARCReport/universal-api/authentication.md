# DMARC Report Universal API Authentication

Every DMARC Report API request through MindCloud uses a MindCloud API key and the `connectionId` of a DMARC Report connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your DMARC Report connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [DMARC Report action](README.md#actions-24) with your API key and `connectionId`.

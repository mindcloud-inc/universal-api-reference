# DotCMS: Get Default Site

Retrieves the default site from DotCMS.

```
GET https://connect.mindcloud.co/v1/universal/dotCMS/latest/actions/get-default-site
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/dotCMS/latest/actions/get-default-site?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/dotCMS/latest/actions/get-default-site?${params}`, {
  headers: {
...
Through the native DotCMS API, this operation is `GET /api/v1/site/defaultSite` (base URL `https://demo.dotcms.com/`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-default-site.md) for the provider-specific parameters and requirements.

...
```
GET https://connect.mindcloud.co/v1/universal/databricks/latest/actions/get-job
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/databricks/latest/actions/get-job?connectionId=$CONNECTION_ID&jobId=1" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/databricks/latest/actions/get-job?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `jobId` | number | yes | The canonical identifier of the job to retrieve information about. This field is required. |
| `pageToken` | string | no | Use `next_page_token` returned from the previous GetJob response to request the next page of the job's array properties. |

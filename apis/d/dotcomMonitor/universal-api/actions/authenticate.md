# Dotcom Monitor: Authenticate

Authenticates with Dotcom Monitor and starts a session.

```
GET https://connect.mindcloud.co/v1/universal/dotcomMonitor/latest/actions/authenticate
```
...
Through the native Dotcom Monitor API, this operation is `POST /login` (base URL `https://api.dotcom-monitor.com/config_api_v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/authenticate.md) for the provider-specific parameters and requirements.

# Dropcontact Universal API Authentication

Every Dropcontact API request through MindCloud uses a MindCloud API key and the `connectionId` of a Dropcontact connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Dropcontact connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Dropcontact action](README.md#actions-3) with your API key and `connectionId`.

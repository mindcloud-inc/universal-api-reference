# DIDWW SMS OUT Universal API Authentication

Every DIDWW SMS OUT API request through MindCloud uses a MindCloud API key and the `connectionId` of a DIDWW SMS OUT connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your DIDWW SMS OUT connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [DIDWW SMS OUT action](README.md#actions-4) with your API key and `connectionId`.

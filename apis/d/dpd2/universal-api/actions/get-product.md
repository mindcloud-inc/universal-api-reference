...
Retrieves a product from DPD by ID.

```
GET https://connect.mindcloud.co/v1/universal/dpd2/latest/actions/get-product
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/dpd2/latest/actions/get-product?connectionId=$CONNECTION_ID&id=1" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/dpd2/latest/actions/get-product?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `id` | number | yes | Unique product ID. |
...
Through the native Dpd2 API, this operation is `GET /products/:id` (base URL `https://api.getdpd.com/v2`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-product.md) for the provider-specific parameters and requirements.

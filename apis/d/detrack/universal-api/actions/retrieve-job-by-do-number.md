# Detrack: Retrieve Job By D.O. Number

Retrieves a job from Detrack by D.O. number.

```
GET https://connect.mindcloud.co/v1/universal/detrack/latest/actions/retrieve-job-by-do-number
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/detrack/latest/actions/retrieve-job-by-do-number?connectionId=$CONNECTION_ID&doNumber=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/detrack/latest/actions/retrieve-job-by-do-number?${params}`, {
  headers: {
...
Through the native Detrack API, this operation is `GET /dn/jobs/:do_number` (base URL `https://app.detrack.com/api/v2`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/retrieve-job-by-do-number.md) for the provider-specific parameters and requirements.

# Detrack: Retrieve Job By D.O. Number And Date

Retrieves a job from Detrack by D.O. number and date.

```
GET https://connect.mindcloud.co/v1/universal/detrack/latest/actions/retrieve-job-by-do-number-and-date
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/detrack/latest/actions/retrieve-job-by-do-number-and-date?connectionId=$CONNECTION_ID&doNumber=string&date=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/detrack/latest/actions/retrieve-job-by-do-number-and-date?${params}`, {
  headers: {
...
Through the native Detrack API, this operation is `GET /dn/jobs/:do_number/:date` (base URL `https://app.detrack.com/api/v2`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/retrieve-job-by-do-number-and-date.md) for the provider-specific parameters and requirements.

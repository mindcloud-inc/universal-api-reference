# Detrack: Update Job By D.O. Number

Updates an existing job in Detrack by D.O. number.

```
PUT https://connect.mindcloud.co/v1/universal/detrack/latest/actions/update-job-by-do-number
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/detrack/latest/actions/update-job-by-do-number" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/detrack/latest/actions/update-job-by-do-number', {
  method: 'PUT',
  headers: {
...
Through the native Detrack API, this operation is `PUT /dn/jobs/:do_number` (base URL `https://app.detrack.com/api/v2`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-job-by-do-number.md) for the provider-specific parameters and requirements.

# D&D 5e Universal API Authentication

Every D&D 5e API request through MindCloud uses a MindCloud API key and the `connectionId` of a D&D 5e connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your D&D 5e connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [D&D 5e action](README.md#actions-13) with your API key and `connectionId`.

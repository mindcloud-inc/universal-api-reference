# Dreamstudio: Remove Background

Removes the background from an image in Dreamstudio.

```
PUT https://connect.mindcloud.co/v1/universal/dreamstudio/latest/actions/remove-background
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/dreamstudio/latest/actions/remove-background" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/dreamstudio/latest/actions/remove-background', {
  method: 'PUT',
  headers: {
...
| --- | --- | --- | --- |
| `image` | file | yes | Image file to remove the background from. |

# Dreamstudio: Inpaint Image

Fills masked regions in an image in Dreamstudio.

```
PUT https://connect.mindcloud.co/v1/universal/dreamstudio/latest/actions/inpaint-image
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/dreamstudio/latest/actions/inpaint-image" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/dreamstudio/latest/actions/inpaint-image', {
  method: 'PUT',
  headers: {
...
| --- | --- | --- | --- |
| `image` | file | yes | Image file to inpaint. |
| `prompt` | string | yes | Prompt describing the desired inpainted result. |

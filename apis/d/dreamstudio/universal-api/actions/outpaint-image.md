# Dreamstudio: Outpaint Image

Expands an image beyond its borders in Dreamstudio.

```
PUT https://connect.mindcloud.co/v1/universal/dreamstudio/latest/actions/outpaint-image
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/dreamstudio/latest/actions/outpaint-image" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/dreamstudio/latest/actions/outpaint-image', {
  method: 'PUT',
  headers: {
...
| --- | --- | --- | --- |
| `image` | file | yes | Image file to outpaint. |
| `prompt` | string | no | Optional prompt that describes what to add around the expanded canvas. |

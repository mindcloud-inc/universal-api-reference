# Digit.ink Universal API Authentication

Every Digit.ink API request through MindCloud uses a MindCloud API key and the `connectionId` of a Digit.ink connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Digit.ink connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Digit.ink action](README.md#actions-14) with your API key and `connectionId`.

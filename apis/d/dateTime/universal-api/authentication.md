# Date & Time Universal API Authentication

Every Date & Time API request through MindCloud uses a MindCloud API key and the `connectionId` of a Date & Time connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Date & Time connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Date & Time action](README.md#actions-30) with your API key and `connectionId`.

# Docparser Universal API Authentication

Every Docparser API request through MindCloud uses a MindCloud API key and the `connectionId` of a Docparser connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Docparser connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Docparser action](README.md#actions-19) with your API key and `connectionId`.

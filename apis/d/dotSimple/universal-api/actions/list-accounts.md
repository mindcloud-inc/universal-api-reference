# DotSimple: List Accounts

Retrieves connected social accounts from DotSimple.

```
GET https://connect.mindcloud.co/v1/universal/dotSimple/latest/actions/list-accounts
```

Authenticate with `Authorization: Bearer $MINDCLOUD_API_KEY` and pass a DotSimple `connectionId` ([setup](../authentication.md)).
...
The response envelope is `{ "success": true, "data": [...], "meta": {} }`. The `data` schema for this action is dynamic; it mirrors what the native DotSimple API returns.

## Native endpoint

Through the native DotSimple API, this operation is `GET /:workspaceUuid/accounts` (base URL `https://app.dotsimple.io/app/api`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-accounts.md) for the provider-specific parameters and requirements.

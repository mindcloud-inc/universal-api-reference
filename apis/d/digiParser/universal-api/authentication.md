# DigiParser Universal API Authentication

Every DigiParser API request through MindCloud uses a MindCloud API key and the `connectionId` of a DigiParser connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your DigiParser connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [DigiParser action](README.md#actions-5) with your API key and `connectionId`.

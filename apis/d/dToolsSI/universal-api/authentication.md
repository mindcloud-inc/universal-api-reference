# D-Tools SI Universal API Authentication

Every D-Tools SI API request through MindCloud uses a MindCloud API key and the `connectionId` of a D-Tools SI connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your D-Tools SI connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [D-Tools SI action](README.md#actions-7) with your API key and `connectionId`.

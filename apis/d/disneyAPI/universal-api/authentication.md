# Disney API Universal API Authentication

Every Disney API API request through MindCloud uses a MindCloud API key and the `connectionId` of a Disney API connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Disney API connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Disney API action](README.md#actions-2) with your API key and `connectionId`.

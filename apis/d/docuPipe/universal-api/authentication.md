# DocuPipe Universal API Authentication

Every DocuPipe API request through MindCloud uses a MindCloud API key and the `connectionId` of a DocuPipe connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your DocuPipe connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [DocuPipe action](README.md#actions-71) with your API key and `connectionId`.

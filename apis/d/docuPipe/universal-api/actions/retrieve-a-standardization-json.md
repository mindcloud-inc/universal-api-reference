# DocuPipe: Retrieve a Standardization JSON

Retrieves standardization JSON from DocuPipe.

```
GET https://connect.mindcloud.co/v1/universal/docuPipe/latest/actions/retrieve-a-standardization-json
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/docuPipe/latest/actions/retrieve-a-standardization-json?connectionId=$CONNECTION_ID&standardizationId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/docuPipe/latest/actions/retrieve-a-standardization-json?${params}`, {
  headers: {
...
Through the native DocuPipe API, this operation is `GET /standardization/:standardizationId` (base URL `https://app.docupipe.ai`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/retrieve-a-standardization-json.md) for the provider-specific parameters and requirements.

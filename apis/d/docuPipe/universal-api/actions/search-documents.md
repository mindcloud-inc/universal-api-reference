# DocuPipe: Search Documents

Finds documents in DocuPipe.

```
GET https://connect.mindcloud.co/v1/universal/docuPipe/latest/actions/search-documents
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/docuPipe/latest/actions/search-documents?connectionId=$CONNECTION_ID&limit=25&offset=0&query=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/docuPipe/latest/actions/search-documents?${params}`, {
  headers: {
...
Through the native DocuPipe API, this operation is `GET /documents/search` (base URL `https://app.docupipe.ai`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/search-documents.md) for the provider-specific parameters and requirements.

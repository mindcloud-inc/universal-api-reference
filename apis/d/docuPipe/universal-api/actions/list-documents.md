# DocuPipe: List Documents

Retrieves documents from DocuPipe.

```
GET https://connect.mindcloud.co/v1/universal/docuPipe/latest/actions/list-documents
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/docuPipe/latest/actions/list-documents?connectionId=$CONNECTION_ID&limit=25&offset=0" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/docuPipe/latest/actions/list-documents?${params}`, {
  headers: {
...
| `excludePayload` | boolean | no | Whether to exclude the result payload from the response Default: `true`. |
...
Through the native DocuPipe API, this operation is `GET /documents` (base URL `https://app.docupipe.ai`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/list-documents.md) for the provider-specific parameters and requirements.

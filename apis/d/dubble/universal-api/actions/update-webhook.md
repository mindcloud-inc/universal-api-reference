# Dubble: Update Webhook

Updates an existing webhook in Dubble.

```
PUT https://connect.mindcloud.co/v1/universal/dubble/latest/actions/update-webhook
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/dubble/latest/actions/update-webhook" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/dubble/latest/actions/update-webhook', {
  method: 'PUT',
  headers: {
...
| `targetUrl` | string | no | The URL where the webhook will send data |
| `triggers[]` | array<string> | no | Trigger events for the webhook |

# Dubble: Add Guide to Collection

Adds a guide to a collection in Dubble.

```
PUT https://connect.mindcloud.co/v1/universal/dubble/latest/actions/add-guide-to-collection
```
...
Through the native Dubble API, this operation is `PUT /guides/:guideId/collections/:collectionId` (base URL `https://api.dubble.so/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/add-guide-to-collection.md) for the provider-specific parameters and requirements.

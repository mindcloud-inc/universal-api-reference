# Dubble Universal API Authentication

Every Dubble API request through MindCloud uses a MindCloud API key and the `connectionId` of a Dubble connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Dubble connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Dubble action](README.md#actions-16) with your API key and `connectionId`.

# Dynamic Content Snippet Universal API Authentication

Every Dynamic Content Snippet API request through MindCloud uses a MindCloud API key and the `connectionId` of a Dynamic Content Snippet connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Dynamic Content Snippet connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Dynamic Content Snippet action](README.md#actions-2) with your API key and `connectionId`.

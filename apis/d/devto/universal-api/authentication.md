# Dev.to Universal API Authentication

Every Dev.to API request through MindCloud uses a MindCloud API key and the `connectionId` of a Dev.to connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Dev.to connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Dev.to action](README.md#actions-24) with your API key and `connectionId`.

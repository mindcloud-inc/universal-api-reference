# Dukaan: Search Orders

Finds orders in Dukaan by search query.

```
GET https://connect.mindcloud.co/v1/universal/dukaan/latest/actions/search-orders
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/dukaan/latest/actions/search-orders?connectionId=$CONNECTION_ID&limit=25&offset=0&search=12631944" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/dukaan/latest/actions/search-orders?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `search` | string | yes | Order search text or order number. Example: `12631944`. |
| `createdAtBefore` | date | no | Upper bound order creation date. Example: `2024-02-20`. |
...
Through the native Dukaan API, this operation is `GET api/seller-front/order-list/` (base URL `https://api.mydukaan.io`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/search-orders.md) for the provider-specific parameters and requirements.

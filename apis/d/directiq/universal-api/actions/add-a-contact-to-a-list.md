# DirectIQ: Add a contact to a list

Adds a contact to a list in DirectIQ.

```
PUT https://connect.mindcloud.co/v1/universal/directiq/latest/actions/add-a-contact-to-a-list
```
...
Through the native DirectIQ API, this operation is `PUT /contacts/lists/addcontact/{id}` (base URL `https://rest.directiq.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/add-a-contact-to-a-list.md) for the provider-specific parameters and requirements.

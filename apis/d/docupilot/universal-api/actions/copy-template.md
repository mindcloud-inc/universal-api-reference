# Docupilot: Copy Template

Copies a template in Docupilot.

```
POST https://connect.mindcloud.co/v1/universal/docupilot/latest/actions/copy-template
```
...
Through the native Docupilot API, this operation is `POST /dashboard/api/v2/templates/{id}/copy/` (base URL `https://api.docupilot.app`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/copy-template.md) for the provider-specific parameters and requirements.

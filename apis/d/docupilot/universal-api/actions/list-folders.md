# Docupilot: List Folders

Retrieves folders from Docupilot.

```
GET https://connect.mindcloud.co/v1/universal/docupilot/latest/actions/list-folders
```
...
| `permission` | list | no | One of: `manage`, `read`, `write`. |
...
Through the native Docupilot API, this operation is `GET /dashboard/api/v2/folders/` (base URL `https://api.docupilot.app`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-folders.md) for the provider-specific parameters and requirements.

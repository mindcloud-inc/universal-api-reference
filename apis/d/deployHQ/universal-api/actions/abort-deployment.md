# DeployHQ: Abort Deployment

Aborts a running deployment in DeployHQ.

```
PUT https://connect.mindcloud.co/v1/universal/deployHQ/latest/actions/abort-deployment
```
...
Through the native DeployHQ API, this operation is `POST /projects/:project_id/deployments/:id/abort` (base URL `https://{{credentials.account}}.deployhq.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/abort-deployment.md) for the provider-specific parameters and requirements.

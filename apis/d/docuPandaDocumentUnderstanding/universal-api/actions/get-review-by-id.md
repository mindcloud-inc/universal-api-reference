# DocuPanda - Document Understanding: Retrieve a Review by ID

Retrieves a review by ID from DocuPanda.

```
GET https://connect.mindcloud.co/v1/universal/docuPandaDocumentUnderstanding/latest/actions/get-review-by-id
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/docuPandaDocumentUnderstanding/latest/actions/get-review-by-id?connectionId=$CONNECTION_ID&review_id=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/docuPandaDocumentUnderstanding/latest/actions/get-review-by-id?${params}`, {
  headers: {
...
Through the native DocuPanda - Document Understanding API, this operation is `GET /review/:review_id` (base URL `https://app.docupipe.ai`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-review-by-id.md) for the provider-specific parameters and requirements.

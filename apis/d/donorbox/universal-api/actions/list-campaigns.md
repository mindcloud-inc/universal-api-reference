# Donorbox: List Campaigns

Retrieves campaigns from Donorbox.

```
GET https://connect.mindcloud.co/v1/universal/donorbox/latest/actions/list-campaigns
```
...
| --- | --- | --- | --- |
| `id` | string | no | Filter campaigns by exact campaign ID. |
...
Through the native Donorbox API, this operation is `GET /campaigns` (base URL `https://donorbox.org/api/v1`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/list-campaigns.md) for the provider-specific parameters and requirements.

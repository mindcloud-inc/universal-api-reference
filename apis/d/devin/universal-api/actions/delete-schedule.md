# Devin: Delete Schedule

Deletes an existing schedule from Devin.

```
DELETE https://connect.mindcloud.co/v1/universal/devin/latest/actions/delete-schedule
```
...
```bash
curl -X DELETE "https://connect.mindcloud.co/v1/universal/devin/latest/actions/delete-schedule?connectionId=$CONNECTION_ID&orgId=string&scheduleId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/devin/latest/actions/delete-schedule?${params}`, {
  headers: {
...
| `scheduled_session_id` | string | Deleted scheduled session ID. |
| `updated_at` | date | Update time. |

## Native endpoint

Through the native Devin API, this operation is `DELETE /v3/organizations/:org_id/schedules/:schedule_id` (base URL `https://api.devin.ai`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/delete-schedule.md) for the provider-specific parameters and requirements.

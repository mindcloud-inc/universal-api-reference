# Devin: Update Schedule

Updates an existing schedule in Devin.

```
PUT https://connect.mindcloud.co/v1/universal/devin/latest/actions/update-schedule
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/devin/latest/actions/update-schedule" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/devin/latest/actions/update-schedule', {
  method: 'PUT',
  headers: {
...
| `prompt` | string | no | Prompt to run for scheduled sessions. |
| `scheduledAt` | string | no | ISO 8601 datetime for one-time schedules. |
...
Through the native Devin API, this operation is `PATCH /v3/organizations/:org_id/schedules/:schedule_id` (base URL `https://api.devin.ai`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-schedule.md) for the provider-specific parameters and requirements.

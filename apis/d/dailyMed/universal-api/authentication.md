# DailyMed Universal API Authentication

Every DailyMed API request through MindCloud uses a MindCloud API key and the `connectionId` of a DailyMed connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your DailyMed connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [DailyMed action](README.md#actions-12) with your API key and `connectionId`.

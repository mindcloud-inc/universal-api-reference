# DateX (Legacy) Universal API Filtering

Filterable list actions accept a `where` query parameter written as an RSQL expression. MindCloud translates it into the filtering format DateX (Legacy) expects, and each action page lists the fields available to filter.
...
## DateX (Legacy) actions that support filtering

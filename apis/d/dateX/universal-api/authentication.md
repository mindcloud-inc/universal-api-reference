# DateX (Legacy) Universal API Authentication

Every DateX (Legacy) API request through MindCloud uses a MindCloud API key and the `connectionId` of a DateX (Legacy) connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your DateX (Legacy) connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [DateX (Legacy) action](README.md#actions-16) with your API key and `connectionId`.

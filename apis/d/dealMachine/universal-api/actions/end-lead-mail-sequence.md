# DealMachine: End Lead Mail Sequence



```
PUT https://connect.mindcloud.co/v1/universal/dealMachine/latest/actions/end-lead-mail-sequence
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/dealMachine/latest/actions/end-lead-mail-sequence" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/dealMachine/latest/actions/end-lead-mail-sequence', {
  method: 'PUT',
  headers: {
...
| `mailerCampaignId` | string | yes | The DealMachine mail sequence ID to end for the lead. |
...
Through the native DealMachine API, this operation is `POST /public/v1/leads/:lead_id/end-mail-sequence` (base URL `https://api.dealmachine.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/end-lead-mail-sequence.md) for the provider-specific parameters and requirements.

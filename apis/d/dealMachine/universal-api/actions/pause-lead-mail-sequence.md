# DealMachine: Pause Lead Mail Sequence



```
PUT https://connect.mindcloud.co/v1/universal/dealMachine/latest/actions/pause-lead-mail-sequence
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/dealMachine/latest/actions/pause-lead-mail-sequence" \
  -H "Authorization: Bearer $MINDCLOUD_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
  "connectionId": "$CONNECTION_ID",
  "leadId": 1,
  "mailerCampaignId": "string"
}'
```

```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/dealMachine/latest/actions/pause-lead-mail-sequence', {
  method: 'PUT',
  headers: {
...
| `mailerCampaignId` | string | yes | The DealMachine mail sequence ID to pause for the lead. |
...
Through the native DealMachine API, this operation is `POST /public/v1/leads/:lead_id/pause-mail-sequence` (base URL `https://api.dealmachine.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/pause-lead-mail-sequence.md) for the provider-specific parameters and requirements.

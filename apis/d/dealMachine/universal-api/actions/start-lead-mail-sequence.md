# DealMachine: Start Lead Mail Sequence



```
PUT https://connect.mindcloud.co/v1/universal/dealMachine/latest/actions/start-lead-mail-sequence
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/dealMachine/latest/actions/start-lead-mail-sequence" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/dealMachine/latest/actions/start-lead-mail-sequence', {
  method: 'PUT',
  headers: {
...
| `mailerCampaignId` | string | no | Optional DealMachine mail sequence ID. If omitted, the default mail sequence is used. |
...
Through the native DealMachine API, this operation is `POST /public/v1/leads/:lead_id/start-mail-sequence` (base URL `https://api.dealmachine.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/start-lead-mail-sequence.md) for the provider-specific parameters and requirements.

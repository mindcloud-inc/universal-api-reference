# DealMachine: Assign Team Member To Lead



```
PUT https://connect.mindcloud.co/v1/universal/dealMachine/latest/actions/assign-team-member-to-lead
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/dealMachine/latest/actions/assign-team-member-to-lead" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/dealMachine/latest/actions/assign-team-member-to-lead', {
  method: 'PUT',
  headers: {
...
| `assignToId` | number | yes | The DealMachine team member ID to assign to the lead. |
...
Through the native DealMachine API, this operation is `POST /public/v1/leads/:lead_id/assign-lead` (base URL `https://api.dealmachine.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/assign-team-member-to-lead.md) for the provider-specific parameters and requirements.

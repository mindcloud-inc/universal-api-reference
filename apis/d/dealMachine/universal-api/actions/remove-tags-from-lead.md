# DealMachine: Remove Tags From Lead



```
PUT https://connect.mindcloud.co/v1/universal/dealMachine/latest/actions/remove-tags-from-lead
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/dealMachine/latest/actions/remove-tags-from-lead" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/dealMachine/latest/actions/remove-tags-from-lead', {
  method: 'PUT',
  headers: {
...
| `tagIds` | string | yes | One or more DealMachine tag IDs. Use a comma-separated string when sending multiple IDs. |
...
Through the native DealMachine API, this operation is `POST /public/v1/leads/:lead_id/remove-tags` (base URL `https://api.dealmachine.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/remove-tags-from-lead.md) for the provider-specific parameters and requirements.

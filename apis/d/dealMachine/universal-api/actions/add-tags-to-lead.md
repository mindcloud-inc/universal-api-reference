# DealMachine: Add Tags To Lead



```
PUT https://connect.mindcloud.co/v1/universal/dealMachine/latest/actions/add-tags-to-lead
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/dealMachine/latest/actions/add-tags-to-lead" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/dealMachine/latest/actions/add-tags-to-lead', {
  method: 'PUT',
  headers: {
...
Through the native DealMachine API, this operation is `POST /public/v1/leads/:lead_id/add-tags` (base URL `https://api.dealmachine.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/add-tags-to-lead.md) for the provider-specific parameters and requirements.

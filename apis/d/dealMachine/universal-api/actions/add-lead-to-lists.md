# DealMachine: Add Lead To Lists



```
PUT https://connect.mindcloud.co/v1/universal/dealMachine/latest/actions/add-lead-to-lists
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/dealMachine/latest/actions/add-lead-to-lists" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/dealMachine/latest/actions/add-lead-to-lists', {
  method: 'PUT',
  headers: {
...
Through the native DealMachine API, this operation is `POST /public/v1/leads/:lead_id/add-to-list` (base URL `https://api.dealmachine.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/add-lead-to-lists.md) for the provider-specific parameters and requirements.

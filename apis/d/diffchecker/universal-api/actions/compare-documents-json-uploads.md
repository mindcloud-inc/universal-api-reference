# Diffchecker: Compare Documents (JSON, Uploads)

Compares documents in Diffchecker and returns a JSON diff from uploads.

```
GET https://connect.mindcloud.co/v1/universal/diffchecker/latest/actions/compare-documents-json-uploads
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/diffchecker/latest/actions/compare-documents-json-uploads?connectionId=$CONNECTION_ID&left_pdf=string&right_pdf=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/diffchecker/latest/actions/compare-documents-json-uploads?${params}`, {
  headers: {

...
Retrieves detailed office information from Dialpad.

```
GET https://connect.mindcloud.co/v1/universal/dialpad/latest/actions/get-office
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/dialpad/latest/actions/get-office?connectionId=$CONNECTION_ID&id=6115773416611840" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/dialpad/latest/actions/get-office?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `id` | string | yes | The Dialpad office ID to retrieve. Example: `6115773416611840`. |
...
Through the native Dialpad API, this operation is `GET /offices/:id` (base URL `https://dialpad.com/api/v2`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-office.md) for the provider-specific parameters and requirements.

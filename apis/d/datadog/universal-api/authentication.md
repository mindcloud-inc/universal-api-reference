# Datadog Universal API Authentication

Every Datadog API request through MindCloud uses a MindCloud API key and the `connectionId` of a Datadog connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Datadog connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Datadog action](README.md#actions-24) with your API key and `connectionId`.

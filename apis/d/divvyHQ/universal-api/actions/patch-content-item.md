# DivvyHQ: Patch Content Item



```
PUT https://connect.mindcloud.co/v1/universal/divvyHQ/latest/actions/patch-content-item
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/divvyHQ/latest/actions/patch-content-item" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/divvyHQ/latest/actions/patch-content-item', {
  method: 'PUT',
  headers: {
...
Through the native DivvyHQ API, this operation is `PATCH /contentitems/:id/` (base URL `https://app.divvyhq.com/api/2.0`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/patch-content-item.md) for the provider-specific parameters and requirements.

# DivvyHQ: Create Content Item



```
POST https://connect.mindcloud.co/v1/universal/divvyHQ/latest/actions/create-content-item
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/divvyHQ/latest/actions/create-content-item" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/divvyHQ/latest/actions/create-content-item', {
  method: 'POST',
  headers: {
...
Through the native DivvyHQ API, this operation is `POST /contentitems/` (base URL `https://app.divvyhq.com/api/2.0`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-content-item.md) for the provider-specific parameters and requirements.

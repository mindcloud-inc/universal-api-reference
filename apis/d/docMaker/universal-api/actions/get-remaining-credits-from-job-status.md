# DocMaker: Get Remaining Credits From Job Status

Retrieves remaining credits from a DocMaker job.

```
GET https://connect.mindcloud.co/v1/universal/docMaker/latest/actions/get-remaining-credits-from-job-status
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/docMaker/latest/actions/get-remaining-credits-from-job-status?connectionId=$CONNECTION_ID&jobId=7f901fa7-4cb5-46d3-9a83-c9f5a0c860a2" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/docMaker/latest/actions/get-remaining-credits-from-job-status?${params}`, {
  headers: {
...
Through the native DocMaker API, this operation is `GET /jobs/:jobId` (base URL `https://api.v2.docmaker.co`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-remaining-credits-from-job-status.md) for the provider-specific parameters and requirements.

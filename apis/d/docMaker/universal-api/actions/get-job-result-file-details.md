# DocMaker: Get Job Result File Details

Retrieves result file details from a DocMaker job.

```
GET https://connect.mindcloud.co/v1/universal/docMaker/latest/actions/get-job-result-file-details
```
...
Through the native DocMaker API, this operation is `GET /jobs/:jobId` (base URL `https://api.v2.docmaker.co`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-job-result-file-details.md) for the provider-specific parameters and requirements.

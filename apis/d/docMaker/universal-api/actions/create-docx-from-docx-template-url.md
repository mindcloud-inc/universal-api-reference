# DocMaker: Create DOCX from DOCX Template URL

Creates a DOCX from a DOCX template URL in DocMaker.

```
POST https://connect.mindcloud.co/v1/universal/docMaker/latest/actions/create-docx-from-docx-template-url
```
...
Through the native DocMaker API, this operation is `POST /docx_fill_convert` (base URL `https://api.v2.docmaker.co`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-docx-from-docx-template-url.md) for the provider-specific parameters and requirements.

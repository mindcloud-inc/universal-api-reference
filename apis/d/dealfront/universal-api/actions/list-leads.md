# Dealfront: List Leads

Retrieves leads from Dealfront.

```
GET https://connect.mindcloud.co/v1/universal/dealfront/latest/actions/list-leads
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/dealfront/latest/actions/list-leads?connectionId=$CONNECTION_ID&limit=25&offset=0&accountId=1&startDate=string&endDate=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/dealfront/latest/actions/list-leads?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `accountId` | number | yes | ID of the account whose leads you want to list. |

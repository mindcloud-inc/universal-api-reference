# Ragic: Add Record Comment

Adds a comment to a record in Ragic.

```
PUT https://connect.mindcloud.co/v1/universal/ragic/latest/actions/add-record-comment
```
...
| `comment` | string | yes | The comment text to add to the record. |

### Advanced

| Key | Type | Required | Description |
| --- | --- | --- | --- |
| `attachment` | string | no | Optional attachment file to upload with the comment. |
...
Through the native Ragic API, this operation is `POST /:tabFolderPath/:sheetIndex/:recordId` (base URL `{{credentials.serverUrl}}/mindcloud`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/add-record-comment.md) for the provider-specific parameters and requirements.

...
```
GET https://connect.mindcloud.co/v1/universal/rocketReach/latest/actions/lookup-person
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/rocketReach/latest/actions/lookup-person?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/rocketReach/latest/actions/lookup-person?${params}`, {
  headers: {
...
| `return_cached_emails` | boolean | no | When false, email fields stay null until the lookup completes and verified emails are available from check status or webhooks. |
| `title` | string | no |  |
...
Through the native RocketReach API, this operation is `GET /person/lookup` (base URL `https://api.rocketreach.co/api/v2`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/lookup-person.md) for the provider-specific parameters and requirements.

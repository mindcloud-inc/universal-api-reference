# RAYNET CRM: List Deals



```
GET https://connect.mindcloud.co/v1/universal/rAYNETCRM/latest/actions/list-deals
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/rAYNETCRM/latest/actions/list-deals?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/rAYNETCRM/latest/actions/list-deals?${params}`, {
  headers: {
...
Through the native RAYNET CRM API, this operation is `GET businessCase/` (base URL `https://app.raynetcrm.com/api/v2/`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-deals.md) for the provider-specific parameters and requirements.

# RAYNET CRM Universal API Authentication

Every RAYNET CRM API request through MindCloud uses a MindCloud API key and the `connectionId` of a RAYNET CRM connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your RAYNET CRM connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [RAYNET CRM action](README.md#actions-26) with your API key and `connectionId`.

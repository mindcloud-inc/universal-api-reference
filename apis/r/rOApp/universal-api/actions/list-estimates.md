# RO App: List Estimates



```
GET https://connect.mindcloud.co/v1/universal/rOApp/latest/actions/list-estimates
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/rOApp/latest/actions/list-estimates?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/rOApp/latest/actions/list-estimates?${params}`, {
  headers: {
...
| `types[]` | array<number> | no | List of estimate type IDs |
| `branches[]` | array<number> | no | List of location IDs |
| `ids[]` | array<number> | no | List of estimate IDs |
| `numbers[]` | array<string> | no | List of estimate document numbers |
| `statuses[]` | array<number> | no | List of estimate status IDs |
| `managers[]` | array<number> | no | List of Employee IDs |
| `clientsIds[]` | array<number> | no | List of client IDs |
| `clientNames[]` | array<string> | no | List of client names |
| `clientPhones[]` | array<string> | no | List of Client phone numbers |
...
| `dueDate[]` | array<date> | no | Filter by Due date" date and time. Accepts an array with one or two ISO 8601 date-time values. If one value is provided, it represents the start (left) boundary. If two values are provided, they define a date range (start and end). Examples: ["2025-05-01T00:00:00Z"] — filter from May 1, 2025 onward. ["2025-05-01T00:00:00Z", "2025-05-02T00:00:00Z"] — filter between May 1 and May 2, 2025. |
...
Through the native RO App API, this operation is `GET /estimates` (base URL `https://api.roapp.io/v2`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-estimates.md) for the provider-specific parameters and requirements.

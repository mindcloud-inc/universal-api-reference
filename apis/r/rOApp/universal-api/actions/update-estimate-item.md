# RO App: Update Estimate Item



```
PUT https://connect.mindcloud.co/v1/universal/rOApp/latest/actions/update-estimate-item
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/rOApp/latest/actions/update-estimate-item" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/rOApp/latest/actions/update-estimate-item', {
  method: 'PUT',
  headers: {
...
| `itemId` | string | yes | Estimate Item ID |
...
Through the native RO App API, this operation is `PATCH /estimates/:estimate_id/items/:item_id` (base URL `https://api.roapp.io/v2`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-estimate-item.md) for the provider-specific parameters and requirements.

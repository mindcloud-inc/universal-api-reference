# RO App: Update Order Item



```
PUT https://connect.mindcloud.co/v1/universal/rOApp/latest/actions/update-order-item
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/rOApp/latest/actions/update-order-item" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/rOApp/latest/actions/update-order-item', {
  method: 'PUT',
  headers: {
...
| `itemId` | number | yes | Order Item ID |
...
| `discount` | object | no | Item discount object |
...
| `warranty` | object | no | Item warranty object |
...
Through the native RO App API, this operation is `PATCH /orders/:order_id/items/:item_id` (base URL `https://api.roapp.io/v2`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-order-item.md) for the provider-specific parameters and requirements.

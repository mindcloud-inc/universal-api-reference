# Recut URL Shortener Universal API Authentication

Every Recut URL Shortener API request through MindCloud uses a MindCloud API key and the `connectionId` of a Recut URL Shortener connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Recut URL Shortener connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Recut URL Shortener action](README.md#actions-35) with your API key and `connectionId`.

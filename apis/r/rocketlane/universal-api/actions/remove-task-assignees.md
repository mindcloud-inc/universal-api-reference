...
Removes assignees from a task in Rocketlane.

```
PUT https://connect.mindcloud.co/v1/universal/rocketlane/latest/actions/remove-task-assignees
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/rocketlane/latest/actions/remove-task-assignees" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/rocketlane/latest/actions/remove-task-assignees', {
  method: 'PUT',
  headers: {
...
Through the native Rocketlane API, this operation is `POST /1.0/tasks/:taskId/remove-assignees` (base URL `https://api.rocketlane.com/api`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/remove-task-assignees.md) for the provider-specific parameters and requirements.

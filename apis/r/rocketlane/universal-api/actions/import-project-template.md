# Rocketlane: Import Project Template

Imports a project template into Rocketlane.

```
PUT https://connect.mindcloud.co/v1/universal/rocketlane/latest/actions/import-project-template
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/rocketlane/latest/actions/import-project-template" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/rocketlane/latest/actions/import-project-template', {
  method: 'PUT',
  headers: {
...
Through the native Rocketlane API, this operation is `POST /1.0/projects/:projectId/import-template` (base URL `https://api.rocketlane.com/api`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/import-project-template.md) for the provider-specific parameters and requirements.

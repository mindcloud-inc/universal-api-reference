...
```
GET https://connect.mindcloud.co/v1/universal/rocketlane/latest/actions/get-time-entry
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/rocketlane/latest/actions/get-time-entry?connectionId=$CONNECTION_ID&timeEntryId=1" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/rocketlane/latest/actions/get-time-entry?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `timeEntryId` | number | yes | The unique, system-generated identifier, which can be used to identify the time entry globally. |
...
Through the native Rocketlane API, this operation is `GET /1.0/time-entries/:timeEntryId` (base URL `https://api.rocketlane.com/api`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-time-entry.md) for the provider-specific parameters and requirements.

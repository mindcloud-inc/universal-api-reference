...
```
PUT https://connect.mindcloud.co/v1/universal/rocketlane/latest/actions/add-task-dependencies
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/rocketlane/latest/actions/add-task-dependencies" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/rocketlane/latest/actions/add-task-dependencies', {
  method: 'PUT',
  headers: {
...
Through the native Rocketlane API, this operation is `POST /1.0/tasks/:taskId/add-dependencies` (base URL `https://api.rocketlane.com/api`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/add-task-dependencies.md) for the provider-specific parameters and requirements.

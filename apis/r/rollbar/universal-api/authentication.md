# Rollbar Universal API Authentication

Every Rollbar API request through MindCloud uses a MindCloud API key and the `connectionId` of a Rollbar connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Rollbar connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Rollbar action](README.md#actions-95) with your API key and `connectionId`.

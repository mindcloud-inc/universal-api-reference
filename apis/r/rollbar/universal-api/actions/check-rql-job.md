# Rollbar: Check RQL Job

Retrieves an RQL job from Rollbar.

```
GET https://connect.mindcloud.co/v1/universal/rollbar/latest/actions/check-rql-job
```
...
Through the native Rollbar API, this operation is `GET /rql/job/:jobId` (base URL `https://api.rollbar.com/api/1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/check-rql-job.md) for the provider-specific parameters and requirements.

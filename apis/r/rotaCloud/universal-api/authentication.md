# RotaCloud Universal API Authentication

Every RotaCloud API request through MindCloud uses a MindCloud API key and the `connectionId` of a RotaCloud connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your RotaCloud connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [RotaCloud action](README.md#actions-126) with your API key and `connectionId`.

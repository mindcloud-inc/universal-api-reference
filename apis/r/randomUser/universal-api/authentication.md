# Random User Universal API Authentication

Every Random User API request through MindCloud uses a MindCloud API key and the `connectionId` of a Random User connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Random User connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Random User action](README.md#actions-1) with your API key and `connectionId`.

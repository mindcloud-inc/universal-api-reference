# Rijksoverheid Universal API Authentication

Every Rijksoverheid API request through MindCloud uses a MindCloud API key and the `connectionId` of a Rijksoverheid connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Rijksoverheid connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Rijksoverheid action](README.md#actions-2) with your API key and `connectionId`.

# random.dog Universal API Authentication

Every random.dog API request through MindCloud uses a MindCloud API key and the `connectionId` of a random.dog connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your random.dog connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [random.dog action](README.md#actions-1) with your API key and `connectionId`.

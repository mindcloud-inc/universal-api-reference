...
Retrieves chats from Retell AI.

```
GET https://connect.mindcloud.co/v1/universal/retellAI/latest/actions/list-chat
```
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/retellAI/latest/actions/list-chat?${params}`, {
  headers: {
...
Through the native Retell AI API, this operation is `GET /list-chat` (base URL `https://api.retellai.com`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/list-chat.md) for the provider-specific parameters and requirements.

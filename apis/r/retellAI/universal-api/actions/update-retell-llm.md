# Retell AI: Update Retell LLM

Updates a Retell LLM in Retell AI.

```
PUT https://connect.mindcloud.co/v1/universal/retellAI/latest/actions/update-retell-llm
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/retellAI/latest/actions/update-retell-llm" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/retellAI/latest/actions/update-retell-llm', {
  method: 'PUT',
  headers: {
...
Through the native Retell AI API, this operation is `PATCH /update-retell-llm/{llm_id}` (base URL `https://api.retellai.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-retell-llm.md) for the provider-specific parameters and requirements.

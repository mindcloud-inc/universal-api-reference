# Retell AI: Get Voice Agent

Retrieves a voice agent from Retell AI.

```
GET https://connect.mindcloud.co/v1/universal/retellAI/latest/actions/get-voice-agent
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/retellAI/latest/actions/get-voice-agent?connectionId=$CONNECTION_ID&agentId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/retellAI/latest/actions/get-voice-agent?${params}`, {
  headers: {
...
Through the native Retell AI API, this operation is `GET /get-agent/{agent_id}` (base URL `https://api.retellai.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-voice-agent.md) for the provider-specific parameters and requirements.

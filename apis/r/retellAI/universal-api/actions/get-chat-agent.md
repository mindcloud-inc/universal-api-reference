# Retell AI: Get Chat Agent

Retrieves a chat agent from Retell AI.

```
GET https://connect.mindcloud.co/v1/universal/retellAI/latest/actions/get-chat-agent
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/retellAI/latest/actions/get-chat-agent?connectionId=$CONNECTION_ID&agentId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/retellAI/latest/actions/get-chat-agent?${params}`, {
  headers: {
...
Through the native Retell AI API, this operation is `GET /get-chat-agent/{agent_id}` (base URL `https://api.retellai.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-chat-agent.md) for the provider-specific parameters and requirements.

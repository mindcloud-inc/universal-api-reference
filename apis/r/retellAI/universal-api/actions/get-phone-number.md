# Retell AI: Get Phone Number

Retrieves a phone number from Retell AI.

```
GET https://connect.mindcloud.co/v1/universal/retellAI/latest/actions/get-phone-number
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/retellAI/latest/actions/get-phone-number?connectionId=$CONNECTION_ID&phoneNumber=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/retellAI/latest/actions/get-phone-number?${params}`, {
  headers: {
...
Through the native Retell AI API, this operation is `GET /get-phone-number/{phone_number}` (base URL `https://api.retellai.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-phone-number.md) for the provider-specific parameters and requirements.

# ReviewStream Universal API Authentication

Every ReviewStream API request through MindCloud uses a MindCloud API key and the `connectionId` of a ReviewStream connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your ReviewStream connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [ReviewStream action](README.md#actions-2) with your API key and `connectionId`.

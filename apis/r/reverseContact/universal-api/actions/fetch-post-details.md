# Reverse Contact: Fetch Post Details



```
GET https://connect.mindcloud.co/v1/universal/reverseContact/latest/actions/fetch-post-details
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/reverseContact/latest/actions/fetch-post-details?connectionId=$CONNECTION_ID&activityId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/reverseContact/latest/actions/fetch-post-details?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `activityId` | string | yes | Social activity or post ID to fetch live. |
...
Through the native Reverse Contact API, this operation is `POST /v2/fetch/post/live` (base URL `https://api.reversecontact.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/fetch-post-details.md) for the provider-specific parameters and requirements.

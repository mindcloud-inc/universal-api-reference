# Reverse Contact: Fetch Person Reactions



```
GET https://connect.mindcloud.co/v1/universal/reverseContact/latest/actions/fetch-person-reactions
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/reverseContact/latest/actions/fetch-person-reactions?connectionId=$CONNECTION_ID&url=https%3A%2F%2Fexample.com" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/reverseContact/latest/actions/fetch-person-reactions?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `url` | string | yes | Public Social profile URL whose reactions should be fetched. |
...
Through the native Reverse Contact API, this operation is `POST /v2/fetch/persons/reactions/live` (base URL `https://api.reversecontact.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/fetch-person-reactions.md) for the provider-specific parameters and requirements.

# Ritekit Universal API Authentication

Every Ritekit API request through MindCloud uses a MindCloud API key and the `connectionId` of a Ritekit connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Ritekit connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Ritekit action](README.md#actions-20) with your API key and `connectionId`.

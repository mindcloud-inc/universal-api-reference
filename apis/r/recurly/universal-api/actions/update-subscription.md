# Recurly: Update Subscription



```
PUT https://connect.mindcloud.co/v1/universal/recurly/latest/actions/update-subscription
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/recurly/latest/actions/update-subscription" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/recurly/latest/actions/update-subscription', {
  method: 'PUT',
  headers: {
...
| `collectionMethod` | string | no | One of: `0`, `1`. |
| `customerNotes` | string | no |  |
| `netTerms` | number | no |  |
| `nextBillDate` | date | no |  |
| `poNumber` | string | no |  |
| `remainingBillingCycles` | number | no |  |
| `renewalBillingCycles` | number | no |  |

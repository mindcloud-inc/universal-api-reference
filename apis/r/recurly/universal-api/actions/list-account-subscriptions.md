# Recurly: List Account Subscriptions



```
GET https://connect.mindcloud.co/v1/universal/recurly/latest/actions/list-account-subscriptions
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/recurly/latest/actions/list-account-subscriptions?connectionId=$CONNECTION_ID&accountId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/recurly/latest/actions/list-account-subscriptions?${params}`, {
  headers: {
...
| `endTime` | string | no |  |
| `ids` | string | no |  |
| `state` | string | no | One of: `0`, `1`, `2`, `3`, `4`, `5`. |
...
Through the native Recurly API, this operation is `GET /accounts/:account_id/subscriptions` (base URL `https://v3.recurly.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-account-subscriptions.md) for the provider-specific parameters and requirements.

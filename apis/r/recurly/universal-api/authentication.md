# Recurly Universal API Authentication

Every Recurly API request through MindCloud uses a MindCloud API key and the `connectionId` of a Recurly connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Recurly connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Recurly action](README.md#actions-23) with your API key and `connectionId`.

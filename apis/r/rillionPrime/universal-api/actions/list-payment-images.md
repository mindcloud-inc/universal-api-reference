# Rillion Prime: List Payment Images
...
| --- | --- | --- | --- |
| `paymentId` | string | yes | Optional query value for PaymentId. |
| `imageType` | string | yes | Optional query value for ImageType. |
...
Through the native Rillion Prime API, this operation is `GET /payment/images` (base URL `{{credentials.baseUrl}}`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-payment-images.md) for the provider-specific parameters and requirements.

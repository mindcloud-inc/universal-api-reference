# Receipt Bot Universal API Authentication

Every Receipt Bot API request through MindCloud uses a MindCloud API key and the `connectionId` of a Receipt Bot connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Receipt Bot connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Receipt Bot action](README.md#actions-2) with your API key and `connectionId`.

# Remove.bg Universal API Authentication

Every Remove.bg API request through MindCloud uses a MindCloud API key and the `connectionId` of a Remove.bg connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Remove.bg connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Remove.bg action](README.md#actions-3) with your API key and `connectionId`.

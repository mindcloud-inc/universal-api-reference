# Reoon Email Verifier Universal API Authentication

Every Reoon Email Verifier API request through MindCloud uses a MindCloud API key and the `connectionId` of a Reoon Email Verifier connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Reoon Email Verifier connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Reoon Email Verifier action](README.md#actions-5) with your API key and `connectionId`.

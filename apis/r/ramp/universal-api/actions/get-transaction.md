...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/ramp/latest/actions/get-transaction?connectionId=$CONNECTION_ID" \
...
Through the native Ramp API, this operation is `GET transactions/:transactionId` (base URL `https://api.ramp.com/developer/v1/`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-transaction.md) for the provider-specific parameters and requirements.

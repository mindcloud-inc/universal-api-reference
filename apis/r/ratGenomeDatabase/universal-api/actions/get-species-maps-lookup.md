# Rat Genome Database: Get Species Maps Lookup



```
GET https://connect.mindcloud.co/v1/universal/ratGenomeDatabase/latest/actions/get-species-maps-lookup
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/ratGenomeDatabase/latest/actions/get-species-maps-lookup?connectionId=$CONNECTION_ID&speciesTypeKey=1" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/ratGenomeDatabase/latest/actions/get-species-maps-lookup?${params}`, {
  headers: {
...
Through the native Rat Genome Database API, this operation is `GET /lookup/maps/:speciesTypeKey` (base URL `https://rest.rgd.mcw.edu/rgdws`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-species-maps-lookup.md) for the provider-specific parameters and requirements.

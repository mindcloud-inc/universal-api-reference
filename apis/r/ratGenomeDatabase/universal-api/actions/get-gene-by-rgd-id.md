# Rat Genome Database: Get Gene By RGD ID



```
GET https://connect.mindcloud.co/v1/universal/ratGenomeDatabase/latest/actions/get-gene-by-rgd-id
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/ratGenomeDatabase/latest/actions/get-gene-by-rgd-id?connectionId=$CONNECTION_ID&rgdId=1" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/ratGenomeDatabase/latest/actions/get-gene-by-rgd-id?${params}`, {
  headers: {
...
Through the native Rat Genome Database API, this operation is `GET /genes/:rgdId` (base URL `https://rest.rgd.mcw.edu/rgdws`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-gene-by-rgd-id.md) for the provider-specific parameters and requirements.

# Rat Genome Database: Get Gene By Symbol



```
GET https://connect.mindcloud.co/v1/universal/ratGenomeDatabase/latest/actions/get-gene-by-symbol
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/ratGenomeDatabase/latest/actions/get-gene-by-symbol?connectionId=$CONNECTION_ID&symbol=string&speciesTypeKey=1" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/ratGenomeDatabase/latest/actions/get-gene-by-symbol?${params}`, {
  headers: {
...
Through the native Rat Genome Database API, this operation is `GET /genes/:symbol/:speciesTypeKey` (base URL `https://rest.rgd.mcw.edu/rgdws`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-gene-by-symbol.md) for the provider-specific parameters and requirements.

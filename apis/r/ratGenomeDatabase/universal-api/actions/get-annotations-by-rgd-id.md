# Rat Genome Database: Get Annotations By RGD ID



```
GET https://connect.mindcloud.co/v1/universal/ratGenomeDatabase/latest/actions/get-annotations-by-rgd-id
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/ratGenomeDatabase/latest/actions/get-annotations-by-rgd-id?connectionId=$CONNECTION_ID&rgdId=1" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/ratGenomeDatabase/latest/actions/get-annotations-by-rgd-id?${params}`, {
  headers: {
...
Through the native Rat Genome Database API, this operation is `GET /annotations/rgdId/:rgdId` (base URL `https://rest.rgd.mcw.edu/rgdws`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-annotations-by-rgd-id.md) for the provider-specific parameters and requirements.

# Rat Genome Database: Get Genes By Keyword



```
GET https://connect.mindcloud.co/v1/universal/ratGenomeDatabase/latest/actions/get-genes-by-keyword
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/ratGenomeDatabase/latest/actions/get-genes-by-keyword?connectionId=$CONNECTION_ID&keyword=string&speciesTypeKey=1" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/ratGenomeDatabase/latest/actions/get-genes-by-keyword?${params}`, {
  headers: {
...
Through the native Rat Genome Database API, this operation is `GET /genes/keyword/:keyword/:speciesTypeKey` (base URL `https://rest.rgd.mcw.edu/rgdws`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-genes-by-keyword.md) for the provider-specific parameters and requirements.

# Rat Genome Database: Get Maps By Species



```
GET https://connect.mindcloud.co/v1/universal/ratGenomeDatabase/latest/actions/get-maps-by-species
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/ratGenomeDatabase/latest/actions/get-maps-by-species?connectionId=$CONNECTION_ID&speciesTypeKey=1" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/ratGenomeDatabase/latest/actions/get-maps-by-species?${params}`, {
  headers: {
...
Through the native Rat Genome Database API, this operation is `GET /maps/:speciesTypeKey` (base URL `https://rest.rgd.mcw.edu/rgdws`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-maps-by-species.md) for the provider-specific parameters and requirements.

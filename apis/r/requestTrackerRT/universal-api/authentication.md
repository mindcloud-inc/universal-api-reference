# Request Tracker (RT) Universal API Authentication

Every Request Tracker (RT) API request through MindCloud uses a MindCloud API key and the `connectionId` of a Request Tracker (RT) connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Request Tracker (RT) connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Request Tracker (RT) action](README.md#actions-24) with your API key and `connectionId`.

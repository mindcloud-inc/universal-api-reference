# ReputationLync: Validate API Key

Validates an API key in ReputationLync.

```
GET https://connect.mindcloud.co/v1/universal/reputationLync/latest/actions/validate-api-key
```
...
Through the native ReputationLync API, this operation is `POST /validateApiKey` (base URL `https://reputationlync.com/app/api/customer`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/validate-api-key.md) for the provider-specific parameters and requirements.

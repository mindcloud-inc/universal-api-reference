# Recruitee ATS: Update Candidate CV



```
PUT https://connect.mindcloud.co/v1/universal/recruiteeATS/latest/actions/update-candidate-cv
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/recruiteeATS/latest/actions/update-candidate-cv" \
  -H "Authorization: Bearer $MINDCLOUD_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
  "connectionId": "$CONNECTION_ID",
  "id": 1,
  "candidate.remote_cv_url": "https://example.com"
}'
```

```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/recruiteeATS/latest/actions/update-candidate-cv', {
  method: 'PUT',
  headers: {
...
| `candidate.remote_cv_url` | string | yes | Public URL to the replacement candidate CV file. |
...
| `candidate.cvParseStatus` | string |  |
...
Through the native Recruitee ATS API, this operation is `PATCH /c/:company_id/candidates/:id/update_cv` (base URL `https://api.recruitee.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-candidate-cv.md) for the provider-specific parameters and requirements.

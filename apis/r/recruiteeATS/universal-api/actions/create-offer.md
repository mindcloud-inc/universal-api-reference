# Recruitee ATS: Create Offer



```
POST https://connect.mindcloud.co/v1/universal/recruiteeATS/latest/actions/create-offer
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/recruiteeATS/latest/actions/create-offer" \
  -H "Authorization: Bearer $MINDCLOUD_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
  "connectionId": "$CONNECTION_ID",
  "offer.title": "string",
  "offer.location_ids": 1,
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/recruiteeATS/latest/actions/create-offer', {
  method: 'POST',
  headers: {
...
| `offer.location_ids` | list<number> | yes | Offer location IDs. |
| `offer.description` | string | yes | Offer description. |
| `offer.requirements` | string | yes | Offer requirements. |

### Advanced

| Key | Type | Required | Description |
| --- | --- | --- | --- |
| `offer.kind` | string | no | Offer kind. Default: `job`. |
| `offer.department_id` | number | no | Department ID. |
| `offer.on_site` | boolean | no | Whether the offer is on-site. Default: `true`. |
| `offer.hybrid` | boolean | no | Whether the offer is hybrid. |
| `offer.remote` | boolean | no | Whether the offer is remote. Default: `false`. |
| `offer.visibility_options` | list<string> | no | Visibility options for the offer. |
| `offer.locations_question` | string | no | Location picker question text. |
| `offer.locations_question_type` | string | no | Location question type. Default: `single_choice`. |
| `offer.locations_question_required` | boolean | no | Whether the location question is required. |
...
Through the native Recruitee ATS API, this operation is `POST /c/:company_id/offers` (base URL `https://api.recruitee.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-offer.md) for the provider-specific parameters and requirements.

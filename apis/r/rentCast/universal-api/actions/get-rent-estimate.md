...
```
GET https://connect.mindcloud.co/v1/universal/rentCast/latest/actions/get-rent-estimate
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/rentCast/latest/actions/get-rent-estimate?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/rentCast/latest/actions/get-rent-estimate?${params}`, {
  headers: {
...
| `propertyType` | list<string> | no | The subject property type. One of: `Apartment`, `Condo`, `Manufactured`, `Multi-Family`, `Single Family`, `Townhouse`. |
...
Through the native RentCast API, this operation is `GET /avm/rent/long-term` (base URL `https://api.rentcast.io/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-rent-estimate.md) for the provider-specific parameters and requirements.

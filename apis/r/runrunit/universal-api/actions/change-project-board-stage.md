# Runrun.it: Change Project Board Stage

Updates a project's board stage in Runrun.it.

```
PUT https://connect.mindcloud.co/v1/universal/runrunit/latest/actions/change-project-board-stage
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/runrunit/latest/actions/change-project-board-stage" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/runrunit/latest/actions/change-project-board-stage', {
  method: 'PUT',
  headers: {
...
Through the native Runrun.it API, this operation is `POST /projects/:id/change_board_stage` (base URL `https://runrun.it/api/v1.0`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/change-project-board-stage.md) for the provider-specific parameters and requirements.

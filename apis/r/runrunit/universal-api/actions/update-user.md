# Runrun.it: Update User

Updates an existing user in Runrun.it.

```
PUT https://connect.mindcloud.co/v1/universal/runrunit/latest/actions/update-user
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/runrunit/latest/actions/update-user" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/runrunit/latest/actions/update-user', {
  method: 'PUT',
  headers: {
...
Through the native Runrun.it API, this operation is `PUT /users/:id` (base URL `https://runrun.it/api/v1.0`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-user.md) for the provider-specific parameters and requirements.

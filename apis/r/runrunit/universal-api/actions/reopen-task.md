# Runrun.it: Reopen Task

Reopens a task in Runrun.it.

```
PUT https://connect.mindcloud.co/v1/universal/runrunit/latest/actions/reopen-task
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/runrunit/latest/actions/reopen-task" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/runrunit/latest/actions/reopen-task', {
  method: 'PUT',
  headers: {
...
| Key | Type | Required | Description |
| --- | --- | --- | --- |
| `reopenOrPauseDescendantTasks` | boolean | no | Force pause/reopen all descendant tasks |
...
Through the native Runrun.it API, this operation is `POST /tasks/:id/reopen` (base URL `https://runrun.it/api/v1.0`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/reopen-task.md) for the provider-specific parameters and requirements.

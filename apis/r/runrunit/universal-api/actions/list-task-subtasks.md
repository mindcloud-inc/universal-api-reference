# Runrun.it: List Task Subtasks

Retrieves subtasks for a task in Runrun.it.

```
GET https://connect.mindcloud.co/v1/universal/runrunit/latest/actions/list-task-subtasks
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/runrunit/latest/actions/list-task-subtasks?connectionId=$CONNECTION_ID&limit=25&offset=0&id=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/runrunit/latest/actions/list-task-subtasks?${params}`, {
  headers: {
...
Through the native Runrun.it API, this operation is `GET /tasks/:id/subtasks` (base URL `https://runrun.it/api/v1.0`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/list-task-subtasks.md) for the provider-specific parameters and requirements.

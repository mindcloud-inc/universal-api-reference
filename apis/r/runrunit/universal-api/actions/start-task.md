# Runrun.it: Start Task

Starts a task in Runrun.it.

```
PUT https://connect.mindcloud.co/v1/universal/runrunit/latest/actions/start-task
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/runrunit/latest/actions/start-task" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/runrunit/latest/actions/start-task', {
  method: 'PUT',
  headers: {
...
Through the native Runrun.it API, this operation is `POST /tasks/:id/play` (base URL `https://runrun.it/api/v1.0`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/start-task.md) for the provider-specific parameters and requirements.

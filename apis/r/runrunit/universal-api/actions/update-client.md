# Runrun.it: Update Client

Updates an existing client in Runrun.it.

```
PUT https://connect.mindcloud.co/v1/universal/runrunit/latest/actions/update-client
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/runrunit/latest/actions/update-client" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/runrunit/latest/actions/update-client', {
  method: 'PUT',
  headers: {
...
| Key | Type | Required | Description |
| --- | --- | --- | --- |
| `client.name` | string | no | Client's name |
...
Through the native Runrun.it API, this operation is `PUT /clients/:id` (base URL `https://runrun.it/api/v1.0`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-client.md) for the provider-specific parameters and requirements.

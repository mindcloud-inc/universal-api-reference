# Runrun.it: Create Project

Creates a new project in Runrun.it.

```
POST https://connect.mindcloud.co/v1/universal/runrunit/latest/actions/create-project
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/runrunit/latest/actions/create-project" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/runrunit/latest/actions/create-project', {
  method: 'POST',
  headers: {
...
| `project.clientId` | number | yes | Client ID for the project. |
| `project.startDate` | date | no | Project start date. |
| `project.desiredDate` | date | no | Desired delivery date for the project. |
...
Through the native Runrun.it API, this operation is `POST /projects` (base URL `https://runrun.it/api/v1.0`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-project.md) for the provider-specific parameters and requirements.

# RemasterMedia: Process Mediafile

Processes a mediafile in RemasterMedia with action-specific options.

```
POST https://connect.mindcloud.co/v1/universal/remasterMedia/latest/actions/process-mediafile
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/remasterMedia/latest/actions/process-mediafile" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/remasterMedia/latest/actions/process-mediafile', {
  method: 'POST',
  headers: {
...
| --- | --- | --- | --- |
| `source_id` | string | yes | ID of the mediafile to process. |
| `action` | string | yes | Process action to perform, such as remaster, waveform, poster, or denoise. |

### Advanced

| Key | Type | Required | Description |
| --- | --- | --- | --- |
| `options` | object | no | Action-specific options object. |
...
| --- | --- | --- |
| `mediafile` | object | Processed mediafile object. |
...
Through the native RemasterMedia API, this operation is `POST /mediafiles/process` (base URL `https://api-sandbox.remastermedia.com/v2`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/process-mediafile.md) for the provider-specific parameters and requirements.

# RemasterMedia: Denoise Mediafile

Creates a denoised mediafile in RemasterMedia using a preset.

```
POST https://connect.mindcloud.co/v1/universal/remasterMedia/latest/actions/denoise-mediafile
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/remasterMedia/latest/actions/denoise-mediafile" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/remasterMedia/latest/actions/denoise-mediafile', {
  method: 'POST',
  headers: {
...
| `options.preset` | string | no | Optional noise-reduction preset name. Direct noise-reduction parameters can be supplied instead. |
...
Through the native RemasterMedia API, this operation is `POST /mediafiles/process` (base URL `https://api-sandbox.remastermedia.com/v2`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/denoise-mediafile.md) for the provider-specific parameters and requirements.

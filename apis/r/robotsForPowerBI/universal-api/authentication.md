# Robots for Power BI Universal API Authentication

Every Robots for Power BI API request through MindCloud uses a MindCloud API key and the `connectionId` of a Robots for Power BI connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Robots for Power BI connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Robots for Power BI action](README.md#actions-3) with your API key and `connectionId`.

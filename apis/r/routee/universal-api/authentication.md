# Routee Universal API Authentication

Every Routee API request through MindCloud uses a MindCloud API key and the `connectionId` of a Routee connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Routee connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Routee action](README.md#actions-188) with your API key and `connectionId`.

...
```
GET https://connect.mindcloud.co/v1/universal/raisely/latest/actions/list-campaign-subscriptions
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/raisely/latest/actions/list-campaign-subscriptions?connectionId=$CONNECTION_ID&limit=25&offset=0&campaign=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/raisely/latest/actions/list-campaign-subscriptions?${params}`, {
  headers: {
...
Through the native Raisely API, this operation is `GET /campaigns/:campaign/subscriptions` (base URL `https://api.raisely.com/v3`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/list-campaign-subscriptions.md) for the provider-specific parameters and requirements.

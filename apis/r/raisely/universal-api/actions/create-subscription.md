...
```
POST https://connect.mindcloud.co/v1/universal/raisely/latest/actions/create-subscription
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/raisely/latest/actions/create-subscription" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/raisely/latest/actions/create-subscription', {
  method: 'POST',
  headers: {
...
| `data.count` | number | yes | The frequency of the subscription, used in conjunction with the `interval` property. For instance, a monthly subscription would have a `count` of `1` and an `interval` of `MONTH`. A fortnightly subscription would have a `count` of `2` and an `interval` of `WEEK`. Examples: `1`, `4` |
| `data.currency` | string | yes | ISO alpha-3 letter currency code Examples: `AUD`, `USD` |
| `data.customer` | string | no | The gateway customer ID (only used when importing new subscriptions) Example: `cus_xxxxxxxxxxxxxx` |
| `data.email` | string | yes | The user's email address. Raisely uses this as a unique identifier and will associate the subscription with an existing record if the email matches. Example: `harveymilk@example.com` |
| `data.firstName` | string | yes | The first name of the person Example: `Leila` |
| `data.fullName` | string | no | The full name of the person Example: `Leila Norma Eulalia Josefa Magistrado de Lima` |
| `data.interval` | string | yes | The interval of the subscriptions. See `count`. Examples: `WEEK`, `MONTH`, `YEAR` |
...
| `data.lastName` | string | yes | The last name of the person Example: `de Lima` |
| `data.message` | string | no | A public message left by the donor Example: `Glad to help out with the fundraising!` |
| `data.method` | string | yes | The payment gateway used for the subscription. Use STRIPE_INTENT for new subscriptions. Examples: `STRIPE_INTENT`, `CREDIT_CARD`, `CUSTOM`, `PAYPAL` |
...
| `data.source` | string | no | The source of this subscription. Subscriptions created through the API should have a source of `OFFLINE`. Examples: `OFFLINE`, `ONLINE` Default: `OFFLINE`. |
| `data.token` | string | yes | Payment gateway token. Set to `null` when adding offline subscriptions with `card` and `customer`. Examples: `seti_xxxxxxxxxxxxxxxxxx`, `EXISTING_CARD` |
...
Through the native Raisely API, this operation is `POST /subscriptions` (base URL `https://api.raisely.com/v3`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-subscription.md) for the provider-specific parameters and requirements.

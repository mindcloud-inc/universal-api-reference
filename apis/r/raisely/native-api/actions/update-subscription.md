# Update Subscription with Raisely
...
- **Official documentation:** [Update Subscription](https://developers.raisely.com/reference/patchsubscription)
...
| `count` | body | `number` | no | The frequency of the subscription, used in conjunction with the `interval` property. For instance, a monthly subscription would have a `count` of `1` and an `interval` of `MONTH`. A fortnightly subscription would have a `count` of `2` and an `interval` of `WEEK`. Examples: `1`, `4` |
| `currency` | body | `string` | no | ISO alpha-3 letter currency code Examples: `AUD`, `USD` |
| `customer` | body | `string` | no | The gateway customer ID (only used when importing new subscriptions) Example: `cus_xxxxxxxxxxxxxx` |
| `email` | body | `string` | no | The user's email address. Raisely uses this as a unique identifier and will associate the subscription with an existing record if the email matches. Example: `harveymilk@example.com` |
...
| `interval` | body | `string` | no | The interval of the subscriptions. See `count`. Examples: `WEEK`, `MONTH`, `YEAR` |
...
| `uuid` | path | `string` | no | `public` Unique identifier for the item |
...
| `method` | body | `string` | no | The payment gateway used for the subscription. Use STRIPE_INTENT for new subscriptions. Examples: `STRIPE_INTENT`, `CREDIT_CARD`, `CUSTOM`, `PAYPAL` |
...
| `token` | body | `string` | no | Payment gateway token. Set to `null` when adding offline subscriptions with `card` and `customer`. Examples: `seti_xxxxxxxxxxxxxxxxxx`, `EXISTING_CARD` |

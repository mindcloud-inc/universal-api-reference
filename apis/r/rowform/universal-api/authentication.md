# Rowform Universal API Authentication

Every Rowform API request through MindCloud uses a MindCloud API key and the `connectionId` of a Rowform connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Rowform connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Rowform action](README.md#actions-6) with your API key and `connectionId`.

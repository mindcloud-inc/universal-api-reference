# Restoplace: Get Address Info

Retrieves address information from Restoplace.

```
GET https://connect.mindcloud.co/v1/universal/restoplace/latest/actions/get-address-info
```
...
Through the native Restoplace API, this operation is `GET /info/` (base URL `https://api.restoplace.cc`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-address-info.md) for the provider-specific parameters and requirements.

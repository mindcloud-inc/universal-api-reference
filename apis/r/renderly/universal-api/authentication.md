# Renderly Universal API Authentication

Every Renderly API request through MindCloud uses a MindCloud API key and the `connectionId` of a Renderly connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Renderly connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Renderly action](README.md#actions-9) with your API key and `connectionId`.

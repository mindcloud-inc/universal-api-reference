# RealFaviconGenerator Universal API Authentication

Every RealFaviconGenerator API request through MindCloud uses a MindCloud API key and the `connectionId` of a RealFaviconGenerator connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your RealFaviconGenerator connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [RealFaviconGenerator action](README.md#actions-3) with your API key and `connectionId`.

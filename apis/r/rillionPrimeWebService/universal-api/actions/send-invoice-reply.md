# Rillion Prime Web Service: Send Invoice Reply

Confirm back to Prime how an exported invoice was processed in the ERP.

```
PUT https://connect.mindcloud.co/v1/universal/rillionPrimeWebService/latest/actions/send-invoice-reply
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/rillionPrimeWebService/latest/actions/send-invoice-reply" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/rillionPrimeWebService/latest/actions/send-invoice-reply', {
  method: 'PUT',
  headers: {
...
Through the native Rillion Prime Web Service API, this operation is `POST` (base URL `{{credentials.baseUrl}}`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/send-invoice-reply.md) for the provider-specific parameters and requirements.

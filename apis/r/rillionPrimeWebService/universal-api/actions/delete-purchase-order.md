# Rillion Prime Web Service: Delete Purchase Order

Delete a purchase order from Prime. Undocumented in the vendor guide — confirm semantics with Rillion before production use.

```
DELETE https://connect.mindcloud.co/v1/universal/rillionPrimeWebService/latest/actions/delete-purchase-order
```
...
```bash
curl -X DELETE "https://connect.mindcloud.co/v1/universal/rillionPrimeWebService/latest/actions/delete-purchase-order?connectionId=$CONNECTION_ID&purchaseOrder=%5Bobject%20Object%5D&purchaseOrder.company=string&purchaseOrder.purchaseOrderNo=string&purchaseOrder.purchaseOrderMatchType=1&purchaseOrder.supplier=string&purchaseOrder.currency=string&purchaseOrder.purchaseOrderLine%5B%5D.company=string&purchaseOrder.purchaseOrderLine%5B%5D.purchaseOrderNo=string&purchaseOrder.purchaseOrderLine%5B%5D.lineNo=string&purchaseOrder.purchaseOrderLine%5B%5D.item=string&purchaseOrder.purchaseOrderLine%5B%5D.number=1&purchaseOrder.purchaseOrderLine%5B%5D.price=1&purchaseOrder.purchaseOrderLine%5B%5D.amount=1&purchaseOrder.purchaseOrderLine%5B%5D.discount=1&transferFromQueue=true" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/rillionPrimeWebService/latest/actions/delete-purchase-order?${params}`, {
  headers: {
...
| `transferFromQueue` | boolean | yes | Process the deletion from the queue immediately. |
...
Through the native Rillion Prime Web Service API, this operation is `POST` (base URL `{{credentials.baseUrl}}`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/delete-purchase-order.md) for the provider-specific parameters and requirements.

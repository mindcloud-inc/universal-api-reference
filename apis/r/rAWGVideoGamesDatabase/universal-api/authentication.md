# RAWG Video Games Database Universal API Authentication

Every RAWG Video Games Database API request through MindCloud uses a MindCloud API key and the `connectionId` of a RAWG Video Games Database connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your RAWG Video Games Database connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [RAWG Video Games Database action](README.md#actions-1) with your API key and `connectionId`.

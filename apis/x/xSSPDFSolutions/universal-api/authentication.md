# XSS PDF Solutions Universal API Authentication

Every XSS PDF Solutions API request through MindCloud uses a MindCloud API key and the `connectionId` of a XSS PDF Solutions connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your XSS PDF Solutions connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [XSS PDF Solutions action](README.md#actions-8) with your API key and `connectionId`.

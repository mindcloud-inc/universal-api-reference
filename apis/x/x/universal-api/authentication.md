# X Universal API Authentication

Every X API request through MindCloud uses a MindCloud API key and the `connectionId` of a X connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your X connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [X action](README.md#actions-2) with your API key and `connectionId`.

# Xodo Sign Universal API Authentication

Every Xodo Sign API request through MindCloud uses a MindCloud API key and the `connectionId` of a Xodo Sign connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Xodo Sign connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Xodo Sign action](README.md#actions-23) with your API key and `connectionId`.

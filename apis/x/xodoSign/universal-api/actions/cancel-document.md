# Xodo Sign: Cancel Document

Cancels an existing document in Xodo Sign.

```
DELETE https://connect.mindcloud.co/v1/universal/xodoSign/latest/actions/cancel-document
```
...
```bash
curl -X DELETE "https://connect.mindcloud.co/v1/universal/xodoSign/latest/actions/cancel-document?connectionId=$CONNECTION_ID&business_id=string&document_hash=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/xodoSign/latest/actions/cancel-document?${params}`, {
  headers: {
...
| `document_hash` | string | yes | The unique document hash to cancel. |

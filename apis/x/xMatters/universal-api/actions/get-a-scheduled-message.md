# xMatters: Get a scheduled message

Retrieves a scheduled message from your xMatters instance.

```
GET https://connect.mindcloud.co/v1/universal/xMatters/latest/actions/get-a-scheduled-message
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/xMatters/latest/actions/get-a-scheduled-message?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/xMatters/latest/actions/get-a-scheduled-message?${params}`, {
  headers: {
...
Through the native xMatters API, this operation is `GET scheduled-messages/{scheduledMessageId}` (base URL `https://mindcloud.xmatters.com/api/xm/1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-a-scheduled-message.md) for the provider-specific parameters and requirements.

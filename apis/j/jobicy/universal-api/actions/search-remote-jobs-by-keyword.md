# Jobicy: Search Remote Jobs by Keyword



```
GET https://connect.mindcloud.co/v1/universal/jobicy/latest/actions/search-remote-jobs-by-keyword
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/jobicy/latest/actions/search-remote-jobs-by-keyword?connectionId=$CONNECTION_ID&tag=python" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/jobicy/latest/actions/search-remote-jobs-by-keyword?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `tag` | string | yes | Keyword to search across the job title and description. Example: `python`. |
...
Through the native Jobicy API, this operation is `GET /remote-jobs` (base URL `https://jobicy.com/api/v2`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/search-remote-jobs-by-keyword.md) for the provider-specific parameters and requirements.

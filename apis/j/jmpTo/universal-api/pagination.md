# JmpTo Universal API Pagination

Paginated list actions accept `limit` and `offset` as query parameters. MindCloud translates them into whatever pagination model JmpTo expects, so the request shape stays the same even when the native API uses pages or cursors.
...
## JmpTo actions that support pagination
...
- [List Custom Splash Pages](actions/list-custom-splash-pages.md)

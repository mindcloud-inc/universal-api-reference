# Joonto Universal API Authentication

Every Joonto API request through MindCloud uses a MindCloud API key and the `connectionId` of a Joonto connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Joonto connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Joonto action](README.md#actions-13) with your API key and `connectionId`.

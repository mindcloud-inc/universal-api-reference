# Google Cloud Document AI: Update Schema

Updates an existing schema in Google Cloud Document AI.

```
PUT https://connect.mindcloud.co/v1/universal/googleCloudDocumentAI/latest/actions/update-schema
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/googleCloudDocumentAI/latest/actions/update-schema" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/googleCloudDocumentAI/latest/actions/update-schema', {
  method: 'PUT',
  headers: {
...
| `displayName` | string | no | Schema display name. |

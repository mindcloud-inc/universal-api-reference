# Google AI Studio: Count Tokens

Counts prompt tokens for a Gemini model in Google AI Studio.

```
GET https://connect.mindcloud.co/v1/universal/googleAIStudio/latest/actions/count-tokens
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/googleAIStudio/latest/actions/count-tokens?connectionId=$CONNECTION_ID&model=gemini-2.5-flash%3AcountTokens" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/googleAIStudio/latest/actions/count-tokens?${params}`, {
  headers: {
...
| Key | Type | Required | Description |
| --- | --- | --- | --- |
| `generateContentRequest` | object | no | Optional full GenerateContentRequest payload. Example: `[object Object]`. |

# Google AI Studio: Generate Content

Generates content with a Gemini model in Google AI Studio.

```
POST https://connect.mindcloud.co/v1/universal/googleAIStudio/latest/actions/generate-content
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/googleAIStudio/latest/actions/generate-content" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/googleAIStudio/latest/actions/generate-content', {
  method: 'POST',
  headers: {
...
| --- | --- | --- | --- |
| `model` | string | yes | Required. Model endpoint token including suffix, for example `gemini-2.5-flash:generateContent`. Example: `gemini-2.5-flash:generateContent`. |

# Grafana Universal API Authentication

Every Grafana API request through MindCloud uses a MindCloud API key and the `connectionId` of a Grafana connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Grafana connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Grafana action](README.md#actions-46) with your API key and `connectionId`.

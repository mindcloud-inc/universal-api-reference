# Groq Universal API Authentication

Every Groq API request through MindCloud uses a MindCloud API key and the `connectionId` of a Groq connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Groq connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Groq action](README.md#actions-23) with your API key and `connectionId`.

...
```
PUT https://connect.mindcloud.co/v1/universal/ghost/latest/actions/update-page
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/ghost/latest/actions/update-page" \
  -H "Authorization: Bearer $MINDCLOUD_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
  "connectionId": "$CONNECTION_ID",
  "id": "string",
  "pages[].updated_at": "string"
}'
```

```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/ghost/latest/actions/update-page', {
  method: 'PUT',
  headers: {
...
| --- | --- | --- | --- |
| `id` | string | yes | Ghost page ID to update. |
| `pages[].updated_at` | string | yes | Current updated_at value required by Ghost optimistic locking. |
| `pages[].title` | string | no | Updated page title. |
| `pages[].lexical` | string | no | Updated lexical editor JSON content. |
| `pages[].html` | string | no | Updated HTML content. |
...
Through the native Ghost API, this operation is `PUT /pages/:id/` (base URL `{{credentials.adminDomain}}/ghost/api/admin`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-page.md) for the provider-specific parameters and requirements.

# Ghost: Get Post by ID

Retrieves a post from Ghost by ID.

```
GET https://connect.mindcloud.co/v1/universal/ghost/latest/actions/get-post-by-id
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/ghost/latest/actions/get-post-by-id?connectionId=$CONNECTION_ID&id=69b0579966c805000821a7fc" \
...
const params = new URLSearchParams({
  connectionId,
  "id": "69b0579966c805000821a7fc"
});

const response = await fetch(`https://connect.mindcloud.co/v1/universal/ghost/latest/actions/get-post-by-id?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `id` | string | yes | Ghost post ID to retrieve. Example: `69b0579966c805000821a7fc`. |
| `include` | string | no | Comma-separated related resources to include. Example: `authors,tags`. |
| `formats` | string | no | Comma-separated content formats to return. Example: `lexical`. |
...
Through the native Ghost API, this operation is `GET /posts/:id/` (base URL `{{credentials.adminDomain}}/ghost/api/admin`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-post-by-id.md) for the provider-specific parameters and requirements.

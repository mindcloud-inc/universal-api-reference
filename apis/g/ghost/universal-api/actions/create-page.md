...
```
POST https://connect.mindcloud.co/v1/universal/ghost/latest/actions/create-page
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/ghost/latest/actions/create-page" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/ghost/latest/actions/create-page', {
  method: 'POST',
  headers: {
...
| --- | --- | --- | --- |
| `pages[].title` | string | yes | Title for the new Ghost page. |
| `pages[].status` | string | no | Initial status for the page. |
| `pages[].lexical` | string | no | Lexical editor JSON content for the page. |
| `pages[].html` | string | no | HTML content for the page. |
| `source` | string | no | Optional source format for the create payload. |
...
Through the native Ghost API, this operation is `POST /pages/` (base URL `{{credentials.adminDomain}}/ghost/api/admin`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-page.md) for the provider-specific parameters and requirements.

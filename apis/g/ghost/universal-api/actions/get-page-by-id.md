# Ghost: Get Page by ID

Retrieves a page from Ghost by ID.

```
GET https://connect.mindcloud.co/v1/universal/ghost/latest/actions/get-page-by-id
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/ghost/latest/actions/get-page-by-id?connectionId=$CONNECTION_ID&id=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/ghost/latest/actions/get-page-by-id?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `id` | string | yes | Ghost page ID to fetch. |
...
Through the native Ghost API, this operation is `GET /pages/:id/` (base URL `{{credentials.adminDomain}}/ghost/api/admin`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-page-by-id.md) for the provider-specific parameters and requirements.

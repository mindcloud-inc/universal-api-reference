...
```
PUT https://connect.mindcloud.co/v1/universal/ghost/latest/actions/publish-post
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/ghost/latest/actions/publish-post" \
  -H "Authorization: Bearer $MINDCLOUD_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
  "connectionId": "$CONNECTION_ID",
  "id": "69b2ea278186310001a3956b",
  "posts[].updatedAt": "2026-03-12T16:32:21.000Z"
}'
```

```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/ghost/latest/actions/publish-post', {
  method: 'PUT',
  headers: {
...
| --- | --- | --- | --- |
| `id` | string | yes | Ghost post ID to publish. Example: `69b2ea278186310001a3956b`. |
| `posts[].updatedAt` | string | yes | Current updated timestamp for optimistic locking. Example: `2026-03-12T16:32:21.000Z`. |
...
Through the native Ghost API, this operation is `PUT /posts/:id/` (base URL `{{credentials.adminDomain}}/ghost/api/admin`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/publish-post.md) for the provider-specific parameters and requirements.

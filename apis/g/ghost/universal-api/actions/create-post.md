...
```
POST https://connect.mindcloud.co/v1/universal/ghost/latest/actions/create-post
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/ghost/latest/actions/create-post" \
  -H "Authorization: Bearer $MINDCLOUD_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
  "connectionId": "$CONNECTION_ID",
  "posts[].title": "Stage 3 Draft Post"
}'
```

```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/ghost/latest/actions/create-post', {
  method: 'POST',
  headers: {
...
| --- | --- | --- | --- |
| `posts[].title` | string | yes | Title for the new post. Example: `Stage 3 Draft Post`. |
| `posts[].status` | string | no | Initial status for the new post. Example: `draft`. |
| `posts[].lexical` | string | no | Ghost Lexical editor payload. Example: `[object Object]`. |
| `posts[].html` | string | no | HTML content for the new post when source=html. Example: `<p>Hello from MindCloud</p>`. |
...
Through the native Ghost API, this operation is `POST /posts/` (base URL `{{credentials.adminDomain}}/ghost/api/admin`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-post.md) for the provider-specific parameters and requirements.

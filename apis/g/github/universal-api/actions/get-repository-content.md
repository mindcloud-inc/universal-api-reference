...
```
GET https://connect.mindcloud.co/v1/universal/github/latest/actions/get-repository-content
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/github/latest/actions/get-repository-content?connectionId=$CONNECTION_ID&owner=string&repo=string&path=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/github/latest/actions/get-repository-content?${params}`, {
  headers: {
...
| `path` | string | yes | Specify the file path or directory path to retrieve. |
| `ref` | string | no | The name of the commit, branch, or tag. |

# Guestmeter Universal API Authentication

Every Guestmeter API request through MindCloud uses a MindCloud API key and the `connectionId` of a Guestmeter connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Guestmeter connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Guestmeter action](README.md#actions-3) with your API key and `connectionId`.

# Google Cloud Storage Universal API Authentication

Every Google Cloud Storage API request through MindCloud uses a MindCloud API key and the `connectionId` of a Google Cloud Storage connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Google Cloud Storage connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Google Cloud Storage action](README.md#actions-14) with your API key and `connectionId`.

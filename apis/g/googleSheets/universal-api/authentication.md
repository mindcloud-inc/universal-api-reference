# Google Sheets Universal API Authentication

Every Google Sheets API request through MindCloud uses a MindCloud API key and the `connectionId` of a Google Sheets connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Google Sheets connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Google Sheets action](README.md#actions-15) with your API key and `connectionId`.

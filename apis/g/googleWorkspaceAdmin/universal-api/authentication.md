# Google Workspace Admin Universal API Authentication

Every Google Workspace Admin API request through MindCloud uses a MindCloud API key and the `connectionId` of a Google Workspace Admin connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Google Workspace Admin connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Google Workspace Admin action](README.md#actions-26) with your API key and `connectionId`.

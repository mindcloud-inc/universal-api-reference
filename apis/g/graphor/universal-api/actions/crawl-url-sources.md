# Graphor: Crawl URL Sources

Creates a new source in Graphor by crawling a URL.

```
POST https://connect.mindcloud.co/v1/universal/graphor/latest/actions/crawl-url-sources
```
...
| `url` | string | yes | The public web page URL whose linked pages should be crawled and ingested. |
...
Through the native Graphor API, this operation is `POST /ingest-url` (base URL `https://sources.graphorlm.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/crawl-url-sources.md) for the provider-specific parameters and requirements.

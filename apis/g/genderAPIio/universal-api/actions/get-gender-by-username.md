# GenderAPI.io: Get Gender by Username

Retrieves gender details from GenderAPI.io by username.

```
GET https://connect.mindcloud.co/v1/universal/genderAPIio/latest/actions/get-gender-by-username
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/genderAPIio/latest/actions/get-gender-by-username?connectionId=$CONNECTION_ID&username=Ava%20Chen" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/genderAPIio/latest/actions/get-gender-by-username?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `username` | string | yes | The username or nickname to analyze. |
| `country` | string | no | Optional ISO 3166-1 alpha-2 country code to improve prediction accuracy. |
| `askToAI` | boolean | no | Ask GenderAPI's AI model when the username cannot be matched from the database. |
...
| `name` | string | The extracted or normalized first name. |
...
Through the native GenderAPI.io API, this operation is `POST /api/username` (base URL `https://api.genderapi.io`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-gender-by-username.md) for the provider-specific parameters and requirements.

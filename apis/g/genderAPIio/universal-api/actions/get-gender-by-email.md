# GenderAPI.io: Get Gender by Email

Retrieves gender details from GenderAPI.io by email address.

```
GET https://connect.mindcloud.co/v1/universal/genderAPIio/latest/actions/get-gender-by-email
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/genderAPIio/latest/actions/get-gender-by-email?connectionId=$CONNECTION_ID&email=ava%40example.com" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/genderAPIio/latest/actions/get-gender-by-email?${params}`, {
  headers: {
...
| `askToAI` | boolean | no | Ask GenderAPI's AI model when the name extracted from the email is not found in the database. |
...
Through the native GenderAPI.io API, this operation is `POST /api/email` (base URL `https://api.genderapi.io`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-gender-by-email.md) for the provider-specific parameters and requirements.

# Giphy: Get Random Sticker

Retrieves a random sticker from Giphy.

```
GET https://connect.mindcloud.co/v1/universal/giphy/latest/actions/get-random-sticker
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/giphy/latest/actions/get-random-sticker?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/giphy/latest/actions/get-random-sticker?${params}`, {
  headers: {
...
Through the native Giphy API, this operation is `GET /v1/stickers/random` (base URL `https://api.giphy.com/`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-random-sticker.md) for the provider-specific parameters and requirements.

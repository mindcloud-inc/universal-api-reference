# Galileo: Query Traces

Finds traces in a Galileo project by filters.

```
GET https://connect.mindcloud.co/v1/universal/galileo/latest/actions/query-traces
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/galileo/latest/actions/query-traces?connectionId=$CONNECTION_ID&projectId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/galileo/latest/actions/query-traces?${params}`, {
  headers: {
...
Through the native Galileo API, this operation is `POST /v2/projects/:project_id/traces/search` (base URL `https://api.galileo.ai`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/query-traces.md) for the provider-specific parameters and requirements.

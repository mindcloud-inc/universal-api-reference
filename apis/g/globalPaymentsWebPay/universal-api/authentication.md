# Global Payments WebPay Universal API Authentication

Every Global Payments WebPay API request through MindCloud uses a MindCloud API key and the `connectionId` of a Global Payments WebPay connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Global Payments WebPay connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Global Payments WebPay action](README.md#actions-40) with your API key and `connectionId`.

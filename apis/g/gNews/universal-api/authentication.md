# GNews Universal API Authentication

Every GNews API request through MindCloud uses a MindCloud API key and the `connectionId` of a GNews connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your GNews connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [GNews action](README.md#actions-2) with your API key and `connectionId`.

# GREEN-API for WhatsApp Universal API Authentication

Every GREEN-API for WhatsApp API request through MindCloud uses a MindCloud API key and the `connectionId` of a GREEN-API for WhatsApp connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your GREEN-API for WhatsApp connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [GREEN-API for WhatsApp action](README.md#actions-1) with your API key and `connectionId`.

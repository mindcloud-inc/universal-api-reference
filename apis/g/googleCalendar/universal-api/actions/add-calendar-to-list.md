# Google Calendar: Add Calendar to List

Adds an existing calendar to the Google Calendar list.

```
POST https://connect.mindcloud.co/v1/universal/googleCalendar/latest/actions/add-calendar-to-list
```
...
Through the native Google Calendar API, this operation is `POST users/me/calendarList` (base URL `https://www.googleapis.com/calendar/v3`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/add-calendar-to-list.md) for the provider-specific parameters and requirements.

...
```
PUT https://connect.mindcloud.co/v1/universal/googleCloudPubSub/latest/actions/acknowledge-messages
```
...
| --- | --- | --- | --- |
| `subscription` | string | yes | Required. The subscription whose message is being acknowledged. Format is `projects/{project}/subscriptions/{sub}`. |
...
Through the native Google Cloud Pub/Sub API, this operation is `POST /v1/:subscription:acknowledge` (base URL `https://pubsub.googleapis.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/acknowledge-messages.md) for the provider-specific parameters and requirements.

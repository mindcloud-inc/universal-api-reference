# Grain Universal API Authentication

Every Grain API request through MindCloud uses a MindCloud API key and the `connectionId` of a Grain connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Grain connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Grain action](README.md#actions-20) with your API key and `connectionId`.

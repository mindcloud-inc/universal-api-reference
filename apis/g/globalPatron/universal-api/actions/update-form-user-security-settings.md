# Global Patron: Update Form User Security Settings

Updates form user security settings in Global Patron.

```
PUT https://connect.mindcloud.co/v1/universal/globalPatron/latest/actions/update-form-user-security-settings
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/globalPatron/latest/actions/update-form-user-security-settings" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/globalPatron/latest/actions/update-form-user-security-settings', {
  method: 'PUT',
  headers: {
...
| `userSecurityId` | string | yes | ID of the user security record. |
...
| `id` | string | Updated user security record identifier. |
| `message` | string | Provider status message. |

## Native endpoint

Through the native Global Patron API, this operation is `POST /api/restricted/form/{formId}/usersecurity/{userSecurityId}` (base URL `https://api.globalpatron.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-form-user-security-settings.md) for the provider-specific parameters and requirements.

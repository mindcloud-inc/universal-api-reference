# GrowthBook Universal API Authentication

Every GrowthBook API request through MindCloud uses a MindCloud API key and the `connectionId` of a GrowthBook connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your GrowthBook connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [GrowthBook action](README.md#actions-179) with your API key and `connectionId`.

# Get a Newsletter Universal API Authentication

Every Get a Newsletter API request through MindCloud uses a MindCloud API key and the `connectionId` of a Get a Newsletter connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Get a Newsletter connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Get a Newsletter action](README.md#actions-1) with your API key and `connectionId`.

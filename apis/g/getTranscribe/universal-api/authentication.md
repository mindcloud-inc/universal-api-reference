# GetTranscribe Universal API Authentication

Every GetTranscribe API request through MindCloud uses a MindCloud API key and the `connectionId` of a GetTranscribe connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your GetTranscribe connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [GetTranscribe action](README.md#actions-7) with your API key and `connectionId`.

# Google Ads: Update Responsive Search Ad

Updates an existing responsive search ad in Google Ads.

```
PUT https://connect.mindcloud.co/v1/universal/googleAds/latest/actions/update-responsive-search-ad
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/googleAds/latest/actions/update-responsive-search-ad" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/googleAds/latest/actions/update-responsive-search-ad', {
  method: 'PUT',
  headers: {
...
| `operations[].updateMask` | string | no | Default: `status`. |
...
Through the native Google Ads API, this operation is `POST v22/customers/:customerId/adGroupAds:mutate` (base URL `https://googleads.googleapis.com/`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-responsive-search-ad.md) for the provider-specific parameters and requirements.

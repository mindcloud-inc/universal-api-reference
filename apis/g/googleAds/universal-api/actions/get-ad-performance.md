# Google Ads: Get Ad Performance



```
GET https://connect.mindcloud.co/v1/universal/googleAds/latest/actions/get-ad-performance
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/googleAds/latest/actions/get-ad-performance?connectionId=$CONNECTION_ID&customerId=1234567890&startDate=2026-02-01&endDate=2026-02-28" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/googleAds/latest/actions/get-ad-performance?${params}`, {
  headers: {
...
| `adGroupAd.ad.resourceName` | string |  |
| `adGroupAd.ad.type` | string |  |
...
Through the native Google Ads API, this operation is `POST v22/customers/:customerId/googleAds:search` (base URL `https://googleads.googleapis.com/`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-ad-performance.md) for the provider-specific parameters and requirements.

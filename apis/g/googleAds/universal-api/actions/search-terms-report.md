...
```
GET https://connect.mindcloud.co/v1/universal/googleAds/latest/actions/search-terms-report
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/googleAds/latest/actions/search-terms-report?connectionId=$CONNECTION_ID&customerId=1234567890" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/googleAds/latest/actions/search-terms-report?${params}`, {
  headers: {
...
| `filterClause` | string | no | Optional GAQL filter clause fragment without WHERE (advanced). Example: `metrics.impressions > 100`. |
...
| `searchTermView.resourceName` | string |  |
| `searchTermView.searchTerm` | string |  |
| `searchTermView.status` | string |  |

## Native endpoint

Through the native Google Ads API, this operation is `POST v22/customers/:customerId/googleAds:search` (base URL `https://googleads.googleapis.com/`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/search-terms-report.md) for the provider-specific parameters and requirements.

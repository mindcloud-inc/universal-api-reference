...
```
DELETE https://connect.mindcloud.co/v1/universal/googleAds/latest/actions/remove-campaign
```
...
```bash
curl -X DELETE "https://connect.mindcloud.co/v1/universal/googleAds/latest/actions/remove-campaign?connectionId=$CONNECTION_ID&customerId=1234567890&operations%5B%5D=%5Bobject%20Object%5D&operations%5B%5D.remove=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/googleAds/latest/actions/remove-campaign?${params}`, {
  headers: {
...
Through the native Google Ads API, this operation is `POST v22/customers/:customerId/campaigns:mutate` (base URL `https://googleads.googleapis.com/`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/remove-campaign.md) for the provider-specific parameters and requirements.

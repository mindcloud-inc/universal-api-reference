# Google Ads: Pause Ad Group

Updates an ad group to paused status in Google Ads.

```
PUT https://connect.mindcloud.co/v1/universal/googleAds/latest/actions/pause-ad-group
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/googleAds/latest/actions/pause-ad-group" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/googleAds/latest/actions/pause-ad-group', {
  method: 'PUT',
  headers: {
...
Through the native Google Ads API, this operation is `POST v22/customers/:customerId/adGroups:mutate` (base URL `https://googleads.googleapis.com/`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/pause-ad-group.md) for the provider-specific parameters and requirements.

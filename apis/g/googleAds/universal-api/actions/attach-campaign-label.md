# Google Ads: Attach Campaign Label

Attaches a label to a campaign in Google Ads.

```
POST https://connect.mindcloud.co/v1/universal/googleAds/latest/actions/attach-campaign-label
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/googleAds/latest/actions/attach-campaign-label" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/googleAds/latest/actions/attach-campaign-label', {
  method: 'POST',
  headers: {
...
Through the native Google Ads API, this operation is `POST v22/customers/:customerId/campaignLabels:mutate` (base URL `https://googleads.googleapis.com/`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/attach-campaign-label.md) for the provider-specific parameters and requirements.

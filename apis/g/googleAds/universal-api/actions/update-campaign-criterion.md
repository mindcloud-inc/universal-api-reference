# Google Ads: Update Campaign Criterion

Updates an existing campaign criterion in Google Ads.

```
PUT https://connect.mindcloud.co/v1/universal/googleAds/latest/actions/update-campaign-criterion
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/googleAds/latest/actions/update-campaign-criterion" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/googleAds/latest/actions/update-campaign-criterion', {
  method: 'PUT',
  headers: {
...
| `operations[].update.bidModifier` | number | no |  |
...
| `operations[].updateMask` | string | no | Default: `status,bid_modifier`. |
...
Through the native Google Ads API, this operation is `POST v22/customers/:customerId/campaignCriteria:mutate` (base URL `https://googleads.googleapis.com/`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-campaign-criterion.md) for the provider-specific parameters and requirements.

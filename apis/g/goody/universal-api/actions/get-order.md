...
Retrieves an order from Goody.

```
GET https://connect.mindcloud.co/v1/universal/goody/latest/actions/get-order
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/goody/latest/actions/get-order?connectionId=$CONNECTION_ID&id=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/goody/latest/actions/get-order?${params}`, {
  headers: {
...
Through the native Goody API, this operation is `GET /v1/orders/:id` (base URL `https://api.ongoody.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-order.md) for the provider-specific parameters and requirements.

# Google Contacts: Search Directory People

Finds directory people in Google Contacts by prefix query.

```
GET https://connect.mindcloud.co/v1/universal/googleContacts/latest/actions/search-directory-people
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/googleContacts/latest/actions/search-directory-people?connectionId=$CONNECTION_ID&query=string&readMask=names%2CemailAddresses%2Corganizations%2CphoneNumbers" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/googleContacts/latest/actions/search-directory-people?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `query` | string | yes | Prefix query string to search directory people. |
...
Through the native Google Contacts API, this operation is `GET /v1/people\:searchDirectoryPeople` (base URL `https://people.googleapis.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/search-directory-people.md) for the provider-specific parameters and requirements.

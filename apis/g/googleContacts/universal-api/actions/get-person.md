# Google Contacts: Get Person

Retrieves a person from Google Contacts.

```
GET https://connect.mindcloud.co/v1/universal/googleContacts/latest/actions/get-person
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/googleContacts/latest/actions/get-person?connectionId=$CONNECTION_ID&resourceName=Ava%20Chen&personFields=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/googleContacts/latest/actions/get-person?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `resourceName` | string | yes | Person ID segment (use `me` or a contact ID like `c123`, not `people/...`). |
| `personFields` | string | yes | Comma-separated person fields to include. |
| `sources` | string | no |  |

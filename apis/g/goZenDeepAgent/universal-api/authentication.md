# GoZen DeepAgent Universal API Authentication

Every GoZen DeepAgent API request through MindCloud uses a MindCloud API key and the `connectionId` of a GoZen DeepAgent connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your GoZen DeepAgent connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [GoZen DeepAgent action](README.md#actions-3) with your API key and `connectionId`.

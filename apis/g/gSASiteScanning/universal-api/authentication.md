# GSA Site Scanning Universal API Authentication

Every GSA Site Scanning API request through MindCloud uses a MindCloud API key and the `connectionId` of a GSA Site Scanning connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your GSA Site Scanning connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [GSA Site Scanning action](README.md#actions-3) with your API key and `connectionId`.

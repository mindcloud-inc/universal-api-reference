# Gumroad: Mark Sale as Shipped

Marks a sale as shipped in Gumroad.

```
PUT https://connect.mindcloud.co/v1/universal/gumroad/latest/actions/mark-sale-as-shipped
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/gumroad/latest/actions/mark-sale-as-shipped" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/gumroad/latest/actions/mark-sale-as-shipped', {
  method: 'PUT',
  headers: {
...
| `trackingUrl` | string | no | The shipment tracking URL. |
...
| `sale.shipped` | boolean |  |
| `sale.skuExternalId` | string |  |
| `sale.skuId` | string |  |
| `sale.state` | string |  |
| `sale.streetAddress` | string |  |
| `sale.timestamp` | string |  |
| `sale.trackingUrl` | string |  |
...
Through the native Gumroad API, this operation is `PUT /sales/:id/mark_as_shipped` (base URL `https://api.gumroad.com/v2`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/mark-sale-as-shipped.md) for the provider-specific parameters and requirements.

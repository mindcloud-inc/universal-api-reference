...
```
GET https://connect.mindcloud.co/v1/universal/giftUp/latest/actions/list-items
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/giftUp/latest/actions/list-items?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/giftUp/latest/actions/list-items?${params}`, {
  headers: {
...
Through the native Gift Up API, this operation is `GET /items` (base URL `https://api.giftup.app`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-items.md) for the provider-specific parameters and requirements.

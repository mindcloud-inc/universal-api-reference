# Grand Avenue Software: List Equipment Activity Records

Retrieves equipment activity records from Grand Avenue Software.

```
GET https://connect.mindcloud.co/v1/universal/grandAvenueSoftware/latest/actions/list-equipment-activity-records
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/grandAvenueSoftware/latest/actions/list-equipment-activity-records?connectionId=$CONNECTION_ID&limit=25&offset=0" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/grandAvenueSoftware/latest/actions/list-equipment-activity-records?${params}`, {
  headers: {
...
Through the native Grand Avenue Software API, this operation is `GET /EquipmentActivityRecords` (base URL `{{credentials.baseUrl}}`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/list-equipment-activity-records.md) for the provider-specific parameters and requirements.

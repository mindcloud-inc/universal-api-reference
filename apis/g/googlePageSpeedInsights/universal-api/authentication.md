# Google PageSpeed Insights Universal API Authentication

Every Google PageSpeed Insights API request through MindCloud uses a MindCloud API key and the `connectionId` of a Google PageSpeed Insights connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Google PageSpeed Insights connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Google PageSpeed Insights action](README.md#actions-1) with your API key and `connectionId`.

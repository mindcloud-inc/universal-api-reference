# GatewayAPI SMS Universal API Authentication

Every GatewayAPI SMS API request through MindCloud uses a MindCloud API key and the `connectionId` of a GatewayAPI SMS connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your GatewayAPI SMS connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [GatewayAPI SMS action](README.md#actions-5) with your API key and `connectionId`.

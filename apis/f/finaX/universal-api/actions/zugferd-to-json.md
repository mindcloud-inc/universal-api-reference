# finaX: ZUGFeRD To JSON

Retrieves invoice JSON from a ZUGFeRD file in finaX.

```
GET https://connect.mindcloud.co/v1/universal/finaX/latest/actions/zugferd-to-json
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/finaX/latest/actions/zugferd-to-json?connectionId=$CONNECTION_ID&file=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/finaX/latest/actions/zugferd-to-json?${params}`, {
  headers: {
...
Through the native finaX API, this operation is `POST /v1/json/zugferd/` (base URL `https://api.finax.dev`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/zugferd-to-json.md) for the provider-specific parameters and requirements.

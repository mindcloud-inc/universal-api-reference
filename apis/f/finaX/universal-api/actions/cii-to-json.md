...
```
GET https://connect.mindcloud.co/v1/universal/finaX/latest/actions/cii-to-json
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/finaX/latest/actions/cii-to-json?connectionId=$CONNECTION_ID&xml=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/finaX/latest/actions/cii-to-json?${params}`, {
  headers: {
...
Through the native finaX API, this operation is `POST /v1/json/cii/` (base URL `https://api.finax.dev`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/cii-to-json.md) for the provider-specific parameters and requirements.

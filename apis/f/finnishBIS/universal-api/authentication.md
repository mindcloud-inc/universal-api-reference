# Finnish BIS Universal API Authentication

Every Finnish BIS API request through MindCloud uses a MindCloud API key and the `connectionId` of a Finnish BIS connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Finnish BIS connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Finnish BIS action](README.md#actions-3) with your API key and `connectionId`.

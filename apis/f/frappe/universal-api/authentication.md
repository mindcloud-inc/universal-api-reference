# Frappe Universal API Authentication

Every Frappe API request through MindCloud uses a MindCloud API key and the `connectionId` of a Frappe connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Frappe connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Frappe action](README.md#actions-46) with your API key and `connectionId`.

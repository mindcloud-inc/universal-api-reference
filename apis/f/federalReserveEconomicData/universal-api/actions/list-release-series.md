# Federal Reserve Economic Data: List Release Series

Retrieves series for a release from Federal Reserve Economic Data.

```
GET https://connect.mindcloud.co/v1/universal/federalReserveEconomicData/latest/actions/list-release-series
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/federalReserveEconomicData/latest/actions/list-release-series?connectionId=$CONNECTION_ID&limit=25&offset=0&release_id=1" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/federalReserveEconomicData/latest/actions/list-release-series?${params}`, {
  headers: {
...
Through the native Federal Reserve Economic Data API, this operation is `GET /fred/release/series` (base URL `https://api.stlouisfed.org`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/list-release-series.md) for the provider-specific parameters and requirements.

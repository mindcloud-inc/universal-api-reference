# Fatture in Cloud: Create Issued Document

Creates a new issued document in Fatture in Cloud.

```
POST https://connect.mindcloud.co/v1/universal/fattureInCloud/latest/actions/create-issued-document
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/fattureInCloud/latest/actions/create-issued-document" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/fattureInCloud/latest/actions/create-issued-document', {
  method: 'POST',
  headers: {
...
| `data` | object | yes | The issued document payload inside the provider data envelope. |
...
Through the native Fatture in Cloud API, this operation is `POST /c/:company_id/issued_documents` (base URL `https://api-v2.fattureincloud.it`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-issued-document.md) for the provider-specific parameters and requirements.

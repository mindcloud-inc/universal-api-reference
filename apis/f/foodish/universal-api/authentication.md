# Foodish Universal API Authentication

Every Foodish API request through MindCloud uses a MindCloud API key and the `connectionId` of a Foodish connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Foodish connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Foodish action](README.md#actions-2) with your API key and `connectionId`.

# Fiserv: Create Refund

Creates a refund for a payment intent in Fiserv.

```
POST https://connect.mindcloud.co/v1/universal/fiserv/latest/actions/create-refund
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/fiserv/latest/actions/create-refund" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/fiserv/latest/actions/create-refund', {
  method: 'POST',
  headers: {
...
| `requestBody` | object | yes | JSON request body from the official create refund schema. |

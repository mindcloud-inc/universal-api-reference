# Fiserv: Create Checkout Session

Creates a checkout session in Fiserv.

```
POST https://connect.mindcloud.co/v1/universal/fiserv/latest/actions/create-checkout-session
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/fiserv/latest/actions/create-checkout-session" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/fiserv/latest/actions/create-checkout-session', {
  method: 'POST',
  headers: {
...
| `requestBody` | object | yes | JSON request body from the official create checkout session schema. |

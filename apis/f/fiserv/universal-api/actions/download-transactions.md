# Fiserv: Download Transactions

Downloads a list of transactions from Fiserv.

```
GET https://connect.mindcloud.co/v1/universal/fiserv/latest/actions/download-transactions
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/fiserv/latest/actions/download-transactions?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/fiserv/latest/actions/download-transactions?${params}`, {
  headers: {
...
Through the native Fiserv API, this operation is `POST /transactions/download` (base URL `https://bankinghub-cert.fiservapis.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/download-transactions.md) for the provider-specific parameters and requirements.

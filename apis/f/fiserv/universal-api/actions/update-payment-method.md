# Fiserv: Update Payment Method

Updates an existing payment method in Fiserv.

```
PUT https://connect.mindcloud.co/v1/universal/fiserv/latest/actions/update-payment-method
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/fiserv/latest/actions/update-payment-method" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/fiserv/latest/actions/update-payment-method', {
  method: 'PUT',
  headers: {
...
| `requestBody` | object | yes | JSON request body from the official update payment method schema. |

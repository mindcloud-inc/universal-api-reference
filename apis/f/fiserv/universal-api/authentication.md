# Fiserv Universal API Authentication

Every Fiserv API request through MindCloud uses a MindCloud API key and the `connectionId` of a Fiserv connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Fiserv connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Fiserv action](README.md#actions-41) with your API key and `connectionId`.

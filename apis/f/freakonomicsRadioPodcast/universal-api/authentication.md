# Freakonomics Radio Podcast Universal API Authentication

Every Freakonomics Radio Podcast API request through MindCloud uses a MindCloud API key and the `connectionId` of a Freakonomics Radio Podcast connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Freakonomics Radio Podcast connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Freakonomics Radio Podcast action](README.md#actions-1) with your API key and `connectionId`.

# FleetWire Universal API Authentication

Every FleetWire API request through MindCloud uses a MindCloud API key and the `connectionId` of a FleetWire connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your FleetWire connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [FleetWire action](README.md#actions-11) with your API key and `connectionId`.

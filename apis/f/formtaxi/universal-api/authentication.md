# Form.taxi Universal API Authentication

Every Form.taxi API request through MindCloud uses a MindCloud API key and the `connectionId` of a Form.taxi connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Form.taxi connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Form.taxi action](README.md#actions-2) with your API key and `connectionId`.

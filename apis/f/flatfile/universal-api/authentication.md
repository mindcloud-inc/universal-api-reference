# Flatfile Universal API Authentication

Every Flatfile API request through MindCloud uses a MindCloud API key and the `connectionId` of a Flatfile connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Flatfile connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Flatfile action](README.md#actions-51) with your API key and `connectionId`.

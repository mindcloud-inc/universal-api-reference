# Fleetio Universal API Authentication

Every Fleetio API request through MindCloud uses a MindCloud API key and the `connectionId` of a Fleetio connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Fleetio connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Fleetio action](README.md#actions-38) with your API key and `connectionId`.

# Fleetio: Update Fuel Entry

Updates an existing fuel entry in Fleetio.

```
PUT https://connect.mindcloud.co/v1/universal/fleetio/latest/actions/update-fuel-entry
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/fleetio/latest/actions/update-fuel-entry" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/fleetio/latest/actions/update-fuel-entry', {
  method: 'PUT',
  headers: {
...
| `date` | date | no | We recommend using [ISO-8601](/docs/overview/date-formatting) formatted dates to avoid ambiguity. |
| `usGallons` | number | no | The fuel volume amount in US gallons. This field will _only_ be used if the [Vehicle](/docs/api/vehicles) is [configured to use US gallons](https://help.fleetio.com/s/article/Fuel-Settings#vehicle-settings), otherwise it will be ignored. |
...
| `meterEntryAttributes` | object | no | Each Fuel Entry requires an associated [Meter Entry](/docs/api/meter-entries) |
...
Through the native Fleetio API, this operation is `PATCH fuel_entries/:id` (base URL `https://secure.fleetio.com/api/`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-fuel-entry.md) for the provider-specific parameters and requirements.

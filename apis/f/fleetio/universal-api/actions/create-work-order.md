# Fleetio: Create Work Order

Creates a new work order in Fleetio.

```
POST https://connect.mindcloud.co/v1/universal/fleetio/latest/actions/create-work-order
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/fleetio/latest/actions/create-work-order" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/fleetio/latest/actions/create-work-order', {
  method: 'POST',
  headers: {
...
| --- | --- | --- | --- |
| `issuedAt` | date | yes | The date and time at which this Work Order was issued. We recommend using [ISO-8601](/docs/overview/date-formatting) formatted dates to avoid ambiguity. |
| `workOrderStatusId` | number | yes |  |
...
| `commentsAttributes[]` | array<object> | no | A list of `Comments` to be added to this Work Order. Accepts multiple values as an array. |
| `workOrderLineItemsAttributes[]` | array<object> | no | A list of `Work Order Line Items` to be added to this Work Order. Accepts multiple values as an array. |
| `workOrderSubLineItemsAttributes[]` | array<object> | no | A list of `Work Order Sub Line Items` to be added to this Work Order. Accepts multiple values as an array. |
| `issueIds[]` | array<number> | no | A list of `Issues` to be added to this Work Order. Accepts multiple values as an array. |
| `labelIds[]` | array<number> | no | A list of `Labels` to be added to this Work Order. Accepts multiple values as an array. |
...
Through the native Fleetio API, this operation is `POST work_orders` (base URL `https://secure.fleetio.com/api/`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-work-order.md) for the provider-specific parameters and requirements.

# Create Work Order with Fleetio
...
- **Official documentation:** [Create Work Order](https://developer.fleetio.com/docs/api/work-orders-create)

## Parameters

| Parameter | Location | Type | Required | Description |
| --- | --- | --- | --- | --- |
| `issued_at` | body | `date` | yes | The date and time at which this Work Order was issued. We recommend using [ISO-8601](/docs/overview/date-formatting) formatted dates to avoid ambiguity. |
| `work_order_status_id` | body | `number` | yes | — |
...
| `comments_attributes[]` | body | `array<object>` | no | A list of `Comments` to be added to this Work Order. Send multiple values as a array. |
| `work_order_line_items_attributes[]` | body | `array<object>` | no | A list of `Work Order Line Items` to be added to this Work Order. Send multiple values as a array. |
| `work_order_sub_line_items_attributes[]` | body | `array<object>` | no | A list of `Work Order Sub Line Items` to be added to this Work Order. Send multiple values as a array. |
| `issue_ids[]` | body | `array<number>` | no | A list of `Issues` to be added to this Work Order. Send multiple values as a array. |
| `label_ids[]` | body | `array<number>` | no | A list of `Labels` to be added to this Work Order. Send multiple values as a array. |

# Follow Up Boss - Legacy Universal API Authentication

Every Follow Up Boss - Legacy API request through MindCloud uses a MindCloud API key and the `connectionId` of a Follow Up Boss - Legacy connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Follow Up Boss - Legacy connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Follow Up Boss - Legacy action](README.md#actions-2) with your API key and `connectionId`.

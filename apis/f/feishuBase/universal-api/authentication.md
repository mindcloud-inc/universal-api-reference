# Feishu Base Universal API Authentication

Every Feishu Base API request through MindCloud uses a MindCloud API key and the `connectionId` of a Feishu Base connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Feishu Base connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Feishu Base action](README.md#actions-5) with your API key and `connectionId`.

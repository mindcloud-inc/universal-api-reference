# Fillout Forms: Delete Field

Deletes a field from Fillout.

```
DELETE https://connect.mindcloud.co/v1/universal/filloutForms/latest/actions/delete-field
```
...
```bash
curl -X DELETE "https://connect.mindcloud.co/v1/universal/filloutForms/latest/actions/delete-field?connectionId=$CONNECTION_ID&databaseId=67ef4d500c50cce9&tableId=t7nUTgYUjzF&fieldId=vkuDPV6ZqBb" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/filloutForms/latest/actions/delete-field?${params}`, {
  headers: {
...
| `fieldId` | string | yes | The unique identifier of the field. You can also use the field name instead of the ID. Note: You cannot delete the primary field (the first field in a table). Example: `vkuDPV6ZqBb`. |

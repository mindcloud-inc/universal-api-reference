# 123FormBuild Universal API Authentication

Every 123FormBuild API request through MindCloud uses a MindCloud API key and the `connectionId` of a 123FormBuild connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your 123FormBuild connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [123FormBuild action](README.md#actions-23) with your API key and `connectionId`.

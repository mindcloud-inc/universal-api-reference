# Flexopus: Update Event

Updates an existing event in Flexopus.

```
PUT https://connect.mindcloud.co/v1/universal/flexopus/latest/actions/update-event
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/flexopus/latest/actions/update-event" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/flexopus/latest/actions/update-event', {
  method: 'PUT',
  headers: {
...
| `toTime` | date | no | When the event will end. |
...
Through the native Flexopus API, this operation is `PUT /events/:id` (base URL `{{credentials.tenantBaseUrl}}/api/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-event.md) for the provider-specific parameters and requirements.

# Federal Communications Commission: Search Facilities

Retrieves FCC facilities matching a keyword.

```
GET https://connect.mindcloud.co/v1/universal/federalCommunicationsCommission/latest/actions/search-facilities
```
...
| --- | --- | --- | --- |
| `keyword` | string | no | Facility search keyword. |
...
Through the native Federal Communications Commission API, this operation is `GET /api/service/facility/search/{keyword}` (base URL `https://publicfiles.fcc.gov`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/search-facilities.md) for the provider-specific parameters and requirements.

# Financial Modeling Prep Universal API Authentication

Every Financial Modeling Prep API request through MindCloud uses a MindCloud API key and the `connectionId` of a Financial Modeling Prep connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Financial Modeling Prep connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Financial Modeling Prep action](README.md#actions-40) with your API key and `connectionId`.

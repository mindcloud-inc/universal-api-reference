# Flotiq: Archive Content Object

Archives a content object in Flotiq.

```
PUT https://connect.mindcloud.co/v1/universal/flotiq/latest/actions/archive-content-object
```
...
Through the native Flotiq API, this operation is `POST /content/{{name}}/{{id}}/archive` (base URL `https://api.flotiq.com/api/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/archive-content-object.md) for the provider-specific parameters and requirements.

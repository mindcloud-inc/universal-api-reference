# Finage Universal API Authentication

Every Finage API request through MindCloud uses a MindCloud API key and the `connectionId` of a Finage connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Finage connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Finage action](README.md#actions-1) with your API key and `connectionId`.

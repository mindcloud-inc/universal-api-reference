# Fraser Direct Universal API Authentication

Every Fraser Direct API request through MindCloud uses a MindCloud API key and the `connectionId` of a Fraser Direct connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Fraser Direct connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Fraser Direct action](README.md#actions-7) with your API key and `connectionId`.

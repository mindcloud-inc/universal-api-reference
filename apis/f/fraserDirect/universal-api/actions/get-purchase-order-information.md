...
Through the native Fraser Direct API, this operation is `GET /GetPOInformation` (base URL `{{credentials.baseURL}}`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-purchase-order-information.md) for the provider-specific parameters and requirements.

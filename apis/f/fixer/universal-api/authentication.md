# Fixer Universal API Authentication

Every Fixer API request through MindCloud uses a MindCloud API key and the `connectionId` of a Fixer connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Fixer connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Fixer action](README.md#actions-6) with your API key and `connectionId`.

# Freshworks CRM Universal API Authentication

Every Freshworks CRM API request through MindCloud uses a MindCloud API key and the `connectionId` of a Freshworks CRM connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Freshworks CRM connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Freshworks CRM action](README.md#actions-107) with your API key and `connectionId`.

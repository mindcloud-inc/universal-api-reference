# Finnish Railway Traffic Universal API Authentication

Every Finnish Railway Traffic API request through MindCloud uses a MindCloud API key and the `connectionId` of a Finnish Railway Traffic connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Finnish Railway Traffic connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Finnish Railway Traffic action](README.md#actions-31) with your API key and `connectionId`.

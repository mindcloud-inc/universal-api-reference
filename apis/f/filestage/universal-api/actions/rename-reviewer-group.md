# Filestage: Rename Reviewer Group

Updates a reviewer group name in Filestage.

```
PUT https://connect.mindcloud.co/v1/universal/filestage/latest/actions/rename-reviewer-group
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/filestage/latest/actions/rename-reviewer-group" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/filestage/latest/actions/rename-reviewer-group', {
  method: 'PUT',
  headers: {
...
| Key | Type | Required | Description |
| --- | --- | --- | --- |
| `name` | string | no | The new name for the reviewer group. |
...
Through the native Filestage API, this operation is `PUT /steps/{stepId}/name` (base URL `https://api.filestage.io/ext/v2`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/rename-reviewer-group.md) for the provider-specific parameters and requirements.

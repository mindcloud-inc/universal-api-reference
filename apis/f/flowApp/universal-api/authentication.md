# Flow App Universal API Authentication

Every Flow App API request through MindCloud uses a MindCloud API key and the `connectionId` of a Flow App connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Flow App connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Flow App action](README.md#actions-12) with your API key and `connectionId`.

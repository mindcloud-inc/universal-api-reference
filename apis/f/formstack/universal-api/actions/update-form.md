# Formstack: Update Form

Updates an existing form in Formstack.

```
PUT https://connect.mindcloud.co/v1/universal/formstack/latest/actions/update-form
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/formstack/latest/actions/update-form" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/formstack/latest/actions/update-form', {
  method: 'PUT',
  headers: {
...
| `name` | string | no | Name of the form. Example: `Renamed Intake Form`. |
...
| `folder` | number | no | ID of the folder where the form will be moved. Example: `123456`. |

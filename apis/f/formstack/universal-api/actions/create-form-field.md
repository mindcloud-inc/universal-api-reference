# Formstack: Create Form Field

Creates a new field in a Formstack form.

```
POST https://connect.mindcloud.co/v1/universal/formstack/latest/actions/create-form-field
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/formstack/latest/actions/create-form-field" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/formstack/latest/actions/create-form-field', {
  method: 'POST',
  headers: {
...
| `label` | string | no | Label of the field. Example: `Batch C temporary field`. |
| `type` | list<string> | no | Type of the field. One of: `address`, `checkbox`, `creditcard`, `datetime`, `email`, `embed`, `file`, `matrix`, `name`, `number`, `phone`, `product`, `radio`, `rating`, `richtext`, `section`, `select`, `signature`, `table`, `text`, `textarea`. Default: `text`. |
| `supportingText` | string | no | Description of the field. Example: `Temporary field for Batch C validation`. |
| `required` | boolean | no | Mark the field as required. |
| `defaultValue` | string | no | Default value on the live form. Example: `Example value`. |

### Advanced

| Key | Type | Required | Description |
| --- | --- | --- | --- |
| `internalLabel` | string | no | Internal label of the field. Example: `batch_c_temp_field`. |
...
Through the native Formstack API, this operation is `POST /forms/:formId/fields` (base URL `https://www.formstack.com/api/v2025`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-form-field.md) for the provider-specific parameters and requirements.

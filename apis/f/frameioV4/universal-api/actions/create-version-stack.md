# Frame.io v4: Create Version Stack

Creates a new version stack in Frame.io v4.

```
POST https://connect.mindcloud.co/v1/universal/frameioV4/latest/actions/create-version-stack
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/frameioV4/latest/actions/create-version-stack" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/frameioV4/latest/actions/create-version-stack', {
  method: 'POST',
  headers: {
...
Through the native Frame.io v4 API, this operation is `POST /accounts/:accountId/folders/:folderId/version_stacks` (base URL `https://api.frame.io/v4`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-version-stack.md) for the provider-specific parameters and requirements.

# Frame.io v4: List Version Stacks

Retrieves version stacks from a folder in Frame.io v4.

```
GET https://connect.mindcloud.co/v1/universal/frameioV4/latest/actions/list-version-stacks
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/frameioV4/latest/actions/list-version-stacks?connectionId=$CONNECTION_ID&limit=25&offset=0&accountId=string&folderId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/frameioV4/latest/actions/list-version-stacks?${params}`, {
  headers: {
...
Through the native Frame.io v4 API, this operation is `GET /accounts/:accountId/folders/:folderId/version_stacks` (base URL `https://api.frame.io/v4`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/list-version-stacks.md) for the provider-specific parameters and requirements.

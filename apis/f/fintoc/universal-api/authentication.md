# Fintoc Universal API Authentication

Every Fintoc API request through MindCloud uses a MindCloud API key and the `connectionId` of a Fintoc connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Fintoc connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Fintoc action](README.md#actions-17) with your API key and `connectionId`.

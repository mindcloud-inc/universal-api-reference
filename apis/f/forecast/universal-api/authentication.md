# Forecast Universal API Authentication

Every Forecast API request through MindCloud uses a MindCloud API key and the `connectionId` of a Forecast connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Forecast connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Forecast action](README.md#actions-2) with your API key and `connectionId`.

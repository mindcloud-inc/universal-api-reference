# Zenclass Universal API Authentication

Every Zenclass API request through MindCloud uses a MindCloud API key and the `connectionId` of a Zenclass connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Zenclass connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Zenclass action](README.md#actions-16) with your API key and `connectionId`.

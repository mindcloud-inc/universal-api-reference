...
```
POST https://connect.mindcloud.co/v1/universal/zammad/latest/actions/create-group
```
...
| --- | --- | --- | --- |
| `name` | string | yes | Group name. Example: `MC TEST GROUP`. |
...
Through the native Zammad API, this operation is `POST /groups` (base URL `{{credentials.baseUrl}}/api/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-group.md) for the provider-specific parameters and requirements.

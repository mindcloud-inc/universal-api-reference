# Ziflow: Get Workflow Template

Retrieves a workflow template from Ziflow by ID.

```
GET https://connect.mindcloud.co/v1/universal/ziflow/latest/actions/get-workflow-template
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/ziflow/latest/actions/get-workflow-template?connectionId=$CONNECTION_ID&templateId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/ziflow/latest/actions/get-workflow-template?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `templateId` | string | yes | Workflow template ID. |
...
Through the native Ziflow API, this operation is `GET /workflowtemplates/:templateId` (base URL `https://api.ziflow.io/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-workflow-template.md) for the provider-specific parameters and requirements.

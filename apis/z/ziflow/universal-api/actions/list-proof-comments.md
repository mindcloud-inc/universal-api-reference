# Ziflow: List Proof Comments

Retrieves proof comments from Ziflow by proof ID.

```
GET https://connect.mindcloud.co/v1/universal/ziflow/latest/actions/list-proof-comments
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/ziflow/latest/actions/list-proof-comments?connectionId=$CONNECTION_ID&id=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/ziflow/latest/actions/list-proof-comments?${params}`, {
  headers: {
...
Through the native Ziflow API, this operation is `GET /proofs/:id/comments` (base URL `https://api.ziflow.io/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-proof-comments.md) for the provider-specific parameters and requirements.

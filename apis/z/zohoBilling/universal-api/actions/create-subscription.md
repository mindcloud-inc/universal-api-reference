...
```
POST https://connect.mindcloud.co/v1/universal/zohoBilling/latest/actions/create-subscription
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/zohoBilling/latest/actions/create-subscription" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/zohoBilling/latest/actions/create-subscription', {
  method: 'POST',
  headers: {
...
Through the native Zoho Billing API, this operation is `POST /subscriptions` (base URL `{{credentials.accessTokenRequest.api_domain}}/billing/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-subscription.md) for the provider-specific parameters and requirements.

# Zenvoices Universal API Authentication

Every Zenvoices API request through MindCloud uses a MindCloud API key and the `connectionId` of a Zenvoices connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Zenvoices connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Zenvoices action](README.md#actions-26) with your API key and `connectionId`.

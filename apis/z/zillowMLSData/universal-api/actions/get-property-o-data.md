# Zillow MLS Data: Get property (OData)

Retrieves a property record from Zillow MLS Data using OData.

```
GET https://connect.mindcloud.co/v1/universal/zillowMLSData/latest/actions/get-property-o-data
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/zillowMLSData/latest/actions/get-property-o-data?connectionId=$CONNECTION_ID&dataset=string&listingKey=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/zillowMLSData/latest/actions/get-property-o-data?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `dataset` | string | yes | Bridge dataset code that contains the OData property. |

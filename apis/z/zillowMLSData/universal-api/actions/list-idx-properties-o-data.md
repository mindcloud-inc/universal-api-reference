# Zillow MLS Data: List IDX properties (OData)

Retrieves IDX property records from Zillow MLS Data using OData.

```
GET https://connect.mindcloud.co/v1/universal/zillowMLSData/latest/actions/list-idx-properties-o-data
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/zillowMLSData/latest/actions/list-idx-properties-o-data?connectionId=$CONNECTION_ID&dataset=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/zillowMLSData/latest/actions/list-idx-properties-o-data?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `dataset` | string | yes | Bridge dataset code that scopes the OData IDX properties query. |
...
Through the native Zillow MLS Data API, this operation is `GET /OData/:dataset/idx/Properties` (base URL `https://api.bridgedataoutput.com/api/v2`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-idx-properties-o-data.md) for the provider-specific parameters and requirements.

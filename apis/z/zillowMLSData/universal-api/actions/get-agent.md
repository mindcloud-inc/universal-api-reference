# Zillow MLS Data: Get agent

Retrieves a specific agent from Zillow MLS Data.

```
GET https://connect.mindcloud.co/v1/universal/zillowMLSData/latest/actions/get-agent
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/zillowMLSData/latest/actions/get-agent?connectionId=$CONNECTION_ID&dataset=string&agentId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/zillowMLSData/latest/actions/get-agent?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `dataset` | string | yes | Bridge dataset code that contains the agent. |
| `agentId` | string | yes | Agent identifier from the Bridge dataset. |

# Zillow MLS Data: List listings

Retrieves listings from a Zillow MLS Data dataset.

```
GET https://connect.mindcloud.co/v1/universal/zillowMLSData/latest/actions/list-listings
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/zillowMLSData/latest/actions/list-listings?connectionId=$CONNECTION_ID&dataset=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/zillowMLSData/latest/actions/list-listings?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `dataset` | string | yes | Bridge dataset code to read listings from. |

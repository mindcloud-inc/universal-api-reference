# Zillow MLS Data Universal API Authentication

Every Zillow MLS Data API request through MindCloud uses a MindCloud API key and the `connectionId` of a Zillow MLS Data connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Zillow MLS Data connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Zillow MLS Data action](README.md#actions-14) with your API key and `connectionId`.

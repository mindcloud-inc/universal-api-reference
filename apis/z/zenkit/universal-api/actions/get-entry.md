...
Retrieves an item from Zenkit.

```
GET https://connect.mindcloud.co/v1/universal/zenkit/latest/actions/get-entry
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/zenkit/latest/actions/get-entry?connectionId=$CONNECTION_ID&listAllId=string&listEntryAllId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/zenkit/latest/actions/get-entry?${params}`, {
  headers: {
...
Through the native Zenkit API, this operation is `GET /lists/:listAllId/entries/:listEntryAllId` (base URL `https://zenkit.com/api/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-entry.md) for the provider-specific parameters and requirements.

...
```
PUT https://connect.mindcloud.co/v1/universal/zenkit/latest/actions/update-entry
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/zenkit/latest/actions/update-entry" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/zenkit/latest/actions/update-entry', {
  method: 'PUT',
  headers: {
...
Through the native Zenkit API, this operation is `PUT /lists/:listId/entries/:listEntryId` (base URL `https://zenkit.com/api/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-entry.md) for the provider-specific parameters and requirements.

# Zoho CRM Universal API Authentication

Every Zoho CRM API request through MindCloud uses a MindCloud API key and the `connectionId` of a Zoho CRM connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Zoho CRM connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Zoho CRM action](README.md#actions-38) with your API key and `connectionId`.

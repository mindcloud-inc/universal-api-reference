# Zillow Agent Reviews Universal API Authentication

Every Zillow Agent Reviews API request through MindCloud uses a MindCloud API key and the `connectionId` of a Zillow Agent Reviews connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Zillow Agent Reviews connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Zillow Agent Reviews action](README.md#actions-2) with your API key and `connectionId`.

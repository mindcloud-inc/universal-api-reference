# Zendesk Universal API Authentication

Every Zendesk API request through MindCloud uses a MindCloud API key and the `connectionId` of a Zendesk connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Zendesk connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Zendesk action](README.md#actions-30) with your API key and `connectionId`.

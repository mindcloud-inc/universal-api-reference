...
Retrieves projects from a Zoho Projects portal.

```
GET https://connect.mindcloud.co/v1/universal/zohoProjects/latest/actions/list-projects
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/zohoProjects/latest/actions/list-projects?connectionId=$CONNECTION_ID&portalId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/zohoProjects/latest/actions/list-projects?${params}`, {
  headers: {
...
Through the native Zoho Projects API, this operation is `GET /portal/[:PORTALID]/projects` (base URL `https://projectsapi.zoho.com/api/v3`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-projects.md) for the provider-specific parameters and requirements.

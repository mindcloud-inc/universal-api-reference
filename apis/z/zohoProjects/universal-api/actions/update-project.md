# Zoho Projects: Update Project

Updates an existing project in Zoho Projects.

```
PUT https://connect.mindcloud.co/v1/universal/zohoProjects/latest/actions/update-project
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/zohoProjects/latest/actions/update-project" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/zohoProjects/latest/actions/update-project', {
  method: 'PUT',
  headers: {
...
| `projectType` | string | no | Project type. Accepted values: active, template, archived. |
...
| `completedTime` | string | no | Project completed time. |
...
Through the native Zoho Projects API, this operation is `PATCH /portal/[:PORTALID]/projects/[:PROJECTID]` (base URL `https://projectsapi.zoho.com/api/v3`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-project.md) for the provider-specific parameters and requirements.

# Zillow Public Records Universal API Authentication

Every Zillow Public Records API request through MindCloud uses a MindCloud API key and the `connectionId` of a Zillow Public Records connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Zillow Public Records connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Zillow Public Records action](README.md#actions-2) with your API key and `connectionId`.

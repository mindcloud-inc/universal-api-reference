# Zoom Scheduler Universal API Authentication

Every Zoom Scheduler API request through MindCloud uses a MindCloud API key and the `connectionId` of a Zoom Scheduler connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Zoom Scheduler connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Zoom Scheduler action](README.md#actions-1) with your API key and `connectionId`.

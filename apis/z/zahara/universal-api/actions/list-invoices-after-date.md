# Zahara: List Invoices After Date

Retrieves invoices from Zahara after a specific date.

```
GET https://connect.mindcloud.co/v1/universal/zahara/latest/actions/list-invoices-after-date
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/zahara/latest/actions/list-invoices-after-date?connectionId=$CONNECTION_ID&date=2023-01-01&skip=1&take=5" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/zahara/latest/actions/list-invoices-after-date?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `date` | string | yes | Return invoices created after this date. Example: `2023-01-01`. |
| `skip` | number | yes | Number of records to skip. Example: `1`. |
| `take` | number | yes | Number of records to return. Example: `5`. |
...
Through the native Zahara API, this operation is `GET /api/{{credentials.businessUnitApiKey}}/Invoice/After/{{date}}/{{skip}}/{{take}}` (base URL `https://api.myzahara.net`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-invoices-after-date.md) for the provider-specific parameters and requirements.

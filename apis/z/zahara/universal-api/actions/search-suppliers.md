# Zahara: Search Suppliers

Finds suppliers in Zahara by search term.

```
GET https://connect.mindcloud.co/v1/universal/zahara/latest/actions/search-suppliers
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/zahara/latest/actions/search-suppliers?connectionId=$CONNECTION_ID&searchTerm=Adobe" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/zahara/latest/actions/search-suppliers?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `searchTerm` | string | yes | Supplier search text. Example: `Adobe`. |
...
Through the native Zahara API, this operation is `GET /api/{{credentials.businessUnitApiKey}}/Supplier/Search/{{searchTerm}}` (base URL `https://api.myzahara.net`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/search-suppliers.md) for the provider-specific parameters and requirements.

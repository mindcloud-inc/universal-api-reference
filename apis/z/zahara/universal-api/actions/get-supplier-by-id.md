# Zahara: Get Supplier By ID

Retrieves a supplier by ID from Zahara.

```
GET https://connect.mindcloud.co/v1/universal/zahara/latest/actions/get-supplier-by-id
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/zahara/latest/actions/get-supplier-by-id?connectionId=$CONNECTION_ID&supplierId=729006" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/zahara/latest/actions/get-supplier-by-id?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `supplierId` | number | yes | The Zahara supplier ID. Example: `729006`. |
...
Through the native Zahara API, this operation is `GET /api/{{credentials.businessUnitApiKey}}/Supplier/Get/{{supplierId}}` (base URL `https://api.myzahara.net`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-supplier-by-id.md) for the provider-specific parameters and requirements.

# Zahara: List Purchase Orders After Date

Retrieves purchase orders from Zahara after a specific date.

```
GET https://connect.mindcloud.co/v1/universal/zahara/latest/actions/list-purchase-orders-after-date
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/zahara/latest/actions/list-purchase-orders-after-date?connectionId=$CONNECTION_ID&date=2025-06-01" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/zahara/latest/actions/list-purchase-orders-after-date?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `date` | string | yes | Return purchase orders updated after this date. Example: `2025-06-01`. |
...
Through the native Zahara API, this operation is `GET /api/{{credentials.businessUnitApiKey}}/PurchaseOrder/After/{{date}}` (base URL `https://api.myzahara.net`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-purchase-orders-after-date.md) for the provider-specific parameters and requirements.

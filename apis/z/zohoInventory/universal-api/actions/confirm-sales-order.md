# Zoho Inventory: Confirm Sales Order

Marks a sales order as confirmed in Zoho Inventory.

```
PUT https://connect.mindcloud.co/v1/universal/zohoInventory/latest/actions/confirm-sales-order
```
...
Through the native Zoho Inventory API, this operation is `POST /salesorders/:salesorder_id/status/confirmed` (base URL `{{credentials.accessTokenRequest.api_domain}}/inventory/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/confirm-sales-order.md) for the provider-specific parameters and requirements.

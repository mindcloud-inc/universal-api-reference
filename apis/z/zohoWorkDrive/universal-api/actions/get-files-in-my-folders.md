# Zoho WorkDrive: Get Files in My Folders

Retrieves files and folders from My Folders in Zoho WorkDrive.

```
GET https://connect.mindcloud.co/v1/universal/zohoWorkDrive/latest/actions/get-files-in-my-folders
```
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/zohoWorkDrive/latest/actions/get-files-in-my-folders?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `myfolderId` | string | yes | The private-space folder ID. |
...
Through the native Zoho WorkDrive API, this operation is `GET /api/v1/privatespace/:myfolderId/files` (base URL `{{credentials.accessTokenRequest.api_domain}}/workdrive`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/get-files-in-my-folders.md) for the provider-specific parameters and requirements.

...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/zenoti/latest/actions/list-services?connectionId=$CONNECTION_ID&limit=25&offset=0&centerId=string" \
...
Through the native Zenoti API, this operation is `GET Centers/:centerId/services` (base URL `https://api.zenoti.com/v1/`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/list-services.md) for the provider-specific parameters and requirements.

# Zenoti: Get Memberships Report



```
GET https://connect.mindcloud.co/v1/universal/zenoti/latest/actions/list-memberships
```
...
| `membershipStatus[].status` | list | no |  |
| `membershipType[].type` | list | no |  |
| `dateType` | list | no |  |
| `startDate` | date | no |  |
| `endDate` | date | no |  |
| `liabilityType` | list | no |  |
| `gracePeriod` | boolean | no | Default: `false`. |
| `membershipStatus[]` | array | no |  |
| `membershipType[]` | array | no |  |

...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/zenoti/latest/actions/list-purchases-by-guest?connectionId=$CONNECTION_ID&guestId=string" \
...
Through the native Zenoti API, this operation is `GET guests/:guestId/products` (base URL `https://api.zenoti.com/v1/`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-purchases-by-guest.md) for the provider-specific parameters and requirements.

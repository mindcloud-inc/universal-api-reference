...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/zenoti/latest/actions/list-sales?connectionId=$CONNECTION_ID&limit=25&offset=0" \
...
Through the native Zenoti API, this operation is `POST reports/sales/accrual_basis/flat_file` (base URL `https://api.zenoti.com/v1/`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/list-sales.md) for the provider-specific parameters and requirements.

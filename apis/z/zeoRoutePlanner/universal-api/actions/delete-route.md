# Zeo Route Planner: Delete Route

Deletes an existing route from Zeo Route Planner.

```
DELETE https://connect.mindcloud.co/v1/universal/zeoRoutePlanner/latest/actions/delete-route
```
...
```bash
curl -X DELETE "https://connect.mindcloud.co/v1/universal/zeoRoutePlanner/latest/actions/delete-route?connectionId=$CONNECTION_ID&driverId=1&routeId=1" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/zeoRoutePlanner/latest/actions/delete-route?${params}`, {
  headers: {
...
| `routeId` | number | yes | Route id we get from route list. |

# Zyte: Get Domain Usage Breakdown

Retrieves usage metrics grouped by domain from Zyte.

```
GET https://connect.mindcloud.co/v1/universal/zyte/latest/actions/get-domain-usage-breakdown
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/zyte/latest/actions/get-domain-usage-breakdown?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/zyte/latest/actions/get-domain-usage-breakdown?${params}`, {
  headers: {
...
Through the native Zyte API, this operation is `GET /api/stats` (base URL `https://zyte-api-stats.zyte.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-domain-usage-breakdown.md) for the provider-specific parameters and requirements.

# Zyte: Get Yearly Usage Trend

Retrieves yearly usage trends from Zyte.

```
GET https://connect.mindcloud.co/v1/universal/zyte/latest/actions/get-yearly-usage-trend
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/zyte/latest/actions/get-yearly-usage-trend?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/zyte/latest/actions/get-yearly-usage-trend?${params}`, {
  headers: {
...
Through the native Zyte API, this operation is `GET /api/stats` (base URL `https://zyte-api-stats.zyte.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-yearly-usage-trend.md) for the provider-specific parameters and requirements.

# Zyte: Get Job Posting Extraction Usage

Retrieves job posting extraction usage metrics from Zyte.

```
GET https://connect.mindcloud.co/v1/universal/zyte/latest/actions/get-job-posting-extraction-usage
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/zyte/latest/actions/get-job-posting-extraction-usage?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/zyte/latest/actions/get-job-posting-extraction-usage?${params}`, {
  headers: {
...
Through the native Zyte API, this operation is `GET /api/stats` (base URL `https://zyte-api-stats.zyte.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-job-posting-extraction-usage.md) for the provider-specific parameters and requirements.

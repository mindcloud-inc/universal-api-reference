# ZAP POST: Create Batch Submission Valid Records Only

Creates a batch submission using only valid records.

```
POST https://connect.mindcloud.co/v1/universal/zAPPOST/latest/actions/create-batch-submission-valid-records-only
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/zAPPOST/latest/actions/create-batch-submission-valid-records-only" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/zAPPOST/latest/actions/create-batch-submission-valid-records-only', {
  method: 'POST',
  headers: {

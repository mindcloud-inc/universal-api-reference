# ZAP POST Universal API Authentication

Every ZAP POST API request through MindCloud uses a MindCloud API key and the `connectionId` of a ZAP POST connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your ZAP POST connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [ZAP POST action](README.md#actions-12) with your API key and `connectionId`.

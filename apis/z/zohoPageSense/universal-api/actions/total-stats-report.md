# Zoho PageSense: Total Stats Report

Retrieves a total stats report from Zoho PageSense.

```
GET https://connect.mindcloud.co/v1/universal/zohoPageSense/latest/actions/total-stats-report
```
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/zohoPageSense/latest/actions/total-stats-report?${params}`, {
  headers: {
      ...
      "fullTrackingReports": [
        {
          ...
          "entrances": 1,
          "exitRate": 1,
          "fullTrackingFirstDataTrackedTime": "string",
          "fullTrackingFirstDataTrackedTimeInLong": 1,
          "pageViews": 1,
          "success": true,
          "uniquePageViews": 1
        }
...
| `fullTrackingReports[].fullTrackingFirstDataTrackedTime` | string |  |
| `fullTrackingReports[].fullTrackingFirstDataTrackedTimeInLong` | number |  |
| `fullTrackingReports[].pageViews` | number |  |
| `fullTrackingReports[].success` | boolean |  |
...
Through the native Zoho PageSense API, this operation is `POST /portal/:portalName/fulltrackingreports` (base URL `https://pagesense.zoho.com/pagesense/rest/v1`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/total-stats-report.md) for the provider-specific parameters and requirements.

...
```
PUT https://connect.mindcloud.co/v1/universal/zohoInvoice/latest/actions/update-contact
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/zohoInvoice/latest/actions/update-contact" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/zohoInvoice/latest/actions/update-contact', {
  method: 'PUT',
  headers: {
...
| Key | Type | Required | Description |
| --- | --- | --- | --- |
| `customFields[]` | array<object> | no | Custom fields or Additional of the contact which we can create to add more information. |
...
| `customFields[].label` | string | no | Label of the custom field. Example: `Tier`. |
...
Through the native Zoho Invoice API, this operation is `PUT /contacts/:contact_id` (base URL `https://www.zohoapis.com/invoice/v3`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-contact.md) for the provider-specific parameters and requirements.

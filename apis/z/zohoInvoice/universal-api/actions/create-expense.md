...
```
POST https://connect.mindcloud.co/v1/universal/zohoInvoice/latest/actions/create-expense
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/zohoInvoice/latest/actions/create-expense" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/zohoInvoice/latest/actions/create-expense', {
  method: 'POST',
  headers: {
...
| `amount` | number | yes | Total expense value. Example: `112.5`. |
...
| `description` | string | no | Description of the expense. Example: `Taxi to client meeting`. |
...
Through the native Zoho Invoice API, this operation is `POST /expenses` (base URL `https://www.zohoapis.com/invoice/v3`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-expense.md) for the provider-specific parameters and requirements.

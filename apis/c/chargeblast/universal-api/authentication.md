# Chargeblast Universal API Authentication

Every Chargeblast API request through MindCloud uses a MindCloud API key and the `connectionId` of a Chargeblast connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Chargeblast connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Chargeblast action](README.md#actions-9) with your API key and `connectionId`.

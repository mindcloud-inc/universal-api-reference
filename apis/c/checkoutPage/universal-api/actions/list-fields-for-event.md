# Checkout Page: List Fields For Event

Retrieves fields for an event in Checkout Page.

```
GET https://connect.mindcloud.co/v1/universal/checkoutPage/latest/actions/list-fields-for-event
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/checkoutPage/latest/actions/list-fields-for-event?connectionId=$CONNECTION_ID&pageId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/checkoutPage/latest/actions/list-fields-for-event?${params}`, {
  headers: {
...
Through the native Checkout Page API, this operation is `GET /v1/events/:pageId/fields` (base URL `https://api.checkoutpage.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-fields-for-event.md) for the provider-specific parameters and requirements.

# Checkout Page: Update Field For Event

Updates a field for an event in Checkout Page.

```
PUT https://connect.mindcloud.co/v1/universal/checkoutPage/latest/actions/update-field-for-event
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/checkoutPage/latest/actions/update-field-for-event" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/checkoutPage/latest/actions/update-field-for-event', {
  method: 'PUT',
  headers: {
...
Through the native Checkout Page API, this operation is `PATCH /v1/events/:pageId/fields/:fieldId` (base URL `https://api.checkoutpage.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-field-for-event.md) for the provider-specific parameters and requirements.

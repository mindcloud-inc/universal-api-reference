# Checkout Page: Archive Checkout Page

Archives a checkout page in Checkout Page.

```
DELETE https://connect.mindcloud.co/v1/universal/checkoutPage/latest/actions/archive-checkout-page
```
...
```bash
curl -X DELETE "https://connect.mindcloud.co/v1/universal/checkoutPage/latest/actions/archive-checkout-page?connectionId=$CONNECTION_ID&pageId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/checkoutPage/latest/actions/archive-checkout-page?${params}`, {
  headers: {
...
Through the native Checkout Page API, this operation is `DELETE /v1/checkout-pages/:pageId` (base URL `https://api.checkoutpage.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/archive-checkout-page.md) for the provider-specific parameters and requirements.

# Checkout Page: Update Checkout Page

Updates a checkout page in Checkout Page.

```
PUT https://connect.mindcloud.co/v1/universal/checkoutPage/latest/actions/update-checkout-page
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/checkoutPage/latest/actions/update-checkout-page" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/checkoutPage/latest/actions/update-checkout-page', {
  method: 'PUT',
  headers: {
...
| --- | --- | --- | --- |
| `pageId` | string | yes | The unique identifier of the checkout page. |
...
| `confirmationCheckoutTitle` | string | no | Confirmation title. |
| `confirmationCheckoutMessage` | string | no | Confirmation message. Accepts HTML or plaintext. |
...
Through the native Checkout Page API, this operation is `PATCH /v1/checkout-pages/:pageId` (base URL `https://api.checkoutpage.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-checkout-page.md) for the provider-specific parameters and requirements.

# Checkout Page: Create Field For Event

Creates a field for an event in Checkout Page.

```
POST https://connect.mindcloud.co/v1/universal/checkoutPage/latest/actions/create-field-for-event
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/checkoutPage/latest/actions/create-field-for-event" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/checkoutPage/latest/actions/create-field-for-event', {
  method: 'POST',
  headers: {
...
Through the native Checkout Page API, this operation is `POST /v1/events/:pageId/fields` (base URL `https://api.checkoutpage.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-field-for-event.md) for the provider-specific parameters and requirements.

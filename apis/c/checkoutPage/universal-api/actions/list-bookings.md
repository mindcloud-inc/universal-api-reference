# Checkout Page: List Bookings

Retrieves bookings from Checkout Page.

```
GET https://connect.mindcloud.co/v1/universal/checkoutPage/latest/actions/list-bookings
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/checkoutPage/latest/actions/list-bookings?connectionId=$CONNECTION_ID&limit=25&offset=0" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/checkoutPage/latest/actions/list-bookings?${params}`, {
  headers: {
...
| `status` | string | no | List all bookings |

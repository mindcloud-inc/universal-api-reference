# Convert Universal API Authentication

Every Convert API request through MindCloud uses a MindCloud API key and the `connectionId` of a Convert connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Convert connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Convert action](README.md#actions-30) with your API key and `connectionId`.

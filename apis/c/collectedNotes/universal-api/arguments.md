# Collected Notes Universal API Arguments

Arguments are the inputs a Collected Notes action needs. Each [action page](README.md#actions-21) lists its exact keys, types, and required fields. Keys are case-sensitive, and requests with missing or invalid required arguments fail instead of guessing.
...
Every Collected Notes response uses the same envelope. Single-record actions still return one item inside `data`:

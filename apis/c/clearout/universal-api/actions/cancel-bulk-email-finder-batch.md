# Clearout: Cancel Bulk Email Finder Batch

Cancels a running bulk email finder batch in Clearout.

```
PUT https://connect.mindcloud.co/v1/universal/clearout/latest/actions/cancel-bulk-email-finder-batch
```
...
Through the native Clearout API, this operation is `POST /email_finder/list/cancel` (base URL `https://api.clearout.io/v2`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/cancel-bulk-email-finder-batch.md) for the provider-specific parameters and requirements.

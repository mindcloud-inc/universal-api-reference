# Centerpoint: Get Opportunity



```
GET https://connect.mindcloud.co/v1/universal/centerpoint/latest/actions/get-opportunity
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/centerpoint/latest/actions/get-opportunity?connectionId=$CONNECTION_ID&OPPORTUNITY_ID=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/centerpoint/latest/actions/get-opportunity?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `OPPORTUNITY_ID` | string | yes | The opportunity id to retrieve. |
...
| `fields[documents]` | string | no | Optional fields documents query parameter. |
...
Through the native Centerpoint API, this operation is `GET opportunities/:OPPORTUNITY_ID` (base URL `https://api.centerpointconnect.io/centerpoint/`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-opportunity.md) for the provider-specific parameters and requirements.

# Centerpoint: Get Location



```
GET https://connect.mindcloud.co/v1/universal/centerpoint/latest/actions/get-location
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/centerpoint/latest/actions/get-location?connectionId=$CONNECTION_ID&LOCATION_ID=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/centerpoint/latest/actions/get-location?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `LOCATION_ID` | string | yes | The location id to retrieve. |
...
Through the native Centerpoint API, this operation is `GET locations/:LOCATION_ID` (base URL `https://api.centerpointconnect.io/centerpoint/`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-location.md) for the provider-specific parameters and requirements.

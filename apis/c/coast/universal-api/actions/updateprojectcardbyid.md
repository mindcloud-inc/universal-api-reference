# Coast: Update Project Card By ID



```
PUT https://connect.mindcloud.co/v1/universal/coast/latest/actions/updateprojectcardbyid
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/coast/latest/actions/updateprojectcardbyid" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/coast/latest/actions/updateprojectcardbyid', {
  method: 'PUT',
  headers: {
...
Through the native Coast API, this operation is `PATCH /v2/cards/:cardId` (base URL `https://public.coastpay.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/updateprojectcardbyid.md) for the provider-specific parameters and requirements.

# Coast: Get All Departments



```
GET https://connect.mindcloud.co/v1/universal/coast/latest/actions/getdepartments
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/coast/latest/actions/getdepartments?connectionId=$CONNECTION_ID&limit=25&offset=0" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/coast/latest/actions/getdepartments?${params}`, {
  headers: {
...
Through the native Coast API, this operation is `GET /v2/departments` (base URL `https://public.coastpay.com`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/getdepartments.md) for the provider-specific parameters and requirements.

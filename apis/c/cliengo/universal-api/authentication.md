# Cliengo Universal API Authentication

Every Cliengo API request through MindCloud uses a MindCloud API key and the `connectionId` of a Cliengo connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Cliengo connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Cliengo action](README.md#actions-24) with your API key and `connectionId`.

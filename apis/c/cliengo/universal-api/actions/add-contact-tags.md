# Cliengo: Add Contact Tags



```
PUT https://connect.mindcloud.co/v1/universal/cliengo/latest/actions/add-contact-tags
```
...
| `tag` | string | yes | Tag to add to the contact. |
...
Through the native Cliengo API, this operation is `POST /contacts/:contactId/tags` (base URL `https://api.cliengo.com/1.0`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/add-contact-tags.md) for the provider-specific parameters and requirements.

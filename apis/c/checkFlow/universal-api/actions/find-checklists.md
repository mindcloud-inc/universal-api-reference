# CheckFlow: Find Checklists



```
GET https://connect.mindcloud.co/v1/universal/checkFlow/latest/actions/find-checklists
```
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/checkFlow/latest/actions/find-checklists?${params}`, {
  headers: {
...
| `createIfNotExists` | boolean | no | Whether to create a checklist if no existing checklist matches the search. Example: `false`. |
...
Through the native CheckFlow API, this operation is `GET /api/checklist/find` (base URL `https://app.checkflow.io`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/find-checklists.md) for the provider-specific parameters and requirements.

# CheckFlow Universal API Authentication

Every CheckFlow API request through MindCloud uses a MindCloud API key and the `connectionId` of a CheckFlow connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your CheckFlow connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [CheckFlow action](README.md#actions-34) with your API key and `connectionId`.

...
```
GET https://connect.mindcloud.co/v1/universal/cATS/latest/actions/filter-jobs
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/cATS/latest/actions/filter-jobs?connectionId=$CONNECTION_ID&field=title&filter=contains&value=MindCloud%20Stage%203%20Job" \
...
const params = new URLSearchParams({
  ...
  "field": "title",
  "filter": "contains",
  "value": "MindCloud Stage 3 Job"
});

const response = await fetch(`https://connect.mindcloud.co/v1/universal/cATS/latest/actions/filter-jobs?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `query` | string | no | The optional string to search within jobs for. |
| `field` | string | yes | The field to filter on. Example: `title`. |
| `filter` | string | yes | The filter operator to use. Example: `contains`. |
| `value` | string | yes | The value to filter by. Example: `MindCloud Stage 3 Job`. |
...
Through the native CATS API, this operation is `POST /jobs/search` (base URL `https://api.catsone.com/v3`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/filter-jobs.md) for the provider-specific parameters and requirements.

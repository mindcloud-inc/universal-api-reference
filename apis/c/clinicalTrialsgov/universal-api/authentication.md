# ClinicalTrials.gov Universal API Authentication

Every ClinicalTrials.gov API request through MindCloud uses a MindCloud API key and the `connectionId` of a ClinicalTrials.gov connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your ClinicalTrials.gov connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [ClinicalTrials.gov action](README.md#actions-14) with your API key and `connectionId`.

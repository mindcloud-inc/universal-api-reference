# CPSC Recalls Retrieval Universal API Authentication

Every CPSC Recalls Retrieval API request through MindCloud uses a MindCloud API key and the `connectionId` of a CPSC Recalls Retrieval connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your CPSC Recalls Retrieval connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [CPSC Recalls Retrieval action](README.md#actions-1) with your API key and `connectionId`.

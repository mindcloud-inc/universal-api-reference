# ChargeOver Universal API Pagination

Paginated list actions accept `limit` and `offset` as query parameters. MindCloud translates them into whatever pagination model ChargeOver expects, so the request shape stays the same even when the native API uses pages or cursors.
...
## ChargeOver actions that support pagination

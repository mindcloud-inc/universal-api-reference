# Cratejoy: Update Subscription

Updates an existing subscription in Cratejoy.

```
PUT https://connect.mindcloud.co/v1/universal/cratejoy/latest/actions/update-subscription
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/cratejoy/latest/actions/update-subscription" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/cratejoy/latest/actions/update-subscription', {
  method: 'PUT',
  headers: {
...
| `note` | string | no | A note for the subscription. |

# Cratejoy: Reactivate Subscription

Reactivates a subscription in Cratejoy.

```
PUT https://connect.mindcloud.co/v1/universal/cratejoy/latest/actions/reactivate-subscription
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/cratejoy/latest/actions/reactivate-subscription" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/cratejoy/latest/actions/reactivate-subscription', {
  method: 'PUT',
  headers: {
...
| `logNote` | string | no | A note recorded when reactivating the subscription. |

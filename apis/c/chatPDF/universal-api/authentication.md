# ChatPDF Universal API Authentication

Every ChatPDF API request through MindCloud uses a MindCloud API key and the `connectionId` of a ChatPDF connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your ChatPDF connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [ChatPDF action](README.md#actions-6) with your API key and `connectionId`.

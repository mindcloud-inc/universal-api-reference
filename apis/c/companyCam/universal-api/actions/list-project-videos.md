# CompanyCam: List Project Videos

Retrieve videos captured at a specified project.

```
GET https://connect.mindcloud.co/v1/universal/companyCam/latest/actions/list-project-videos
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/companyCam/latest/actions/list-project-videos?connectionId=$CONNECTION_ID&limit=25&offset=0&projectId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/companyCam/latest/actions/list-project-videos?${params}`, {
  headers: {
...
Through the native CompanyCam API, this operation is `GET projects/:projectId/videos` (base URL `https://api.companycam.com/v2/`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/list-project-videos.md) for the provider-specific parameters and requirements.

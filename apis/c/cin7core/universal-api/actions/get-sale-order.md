# Cin7 Core: Get Sale Order



```
GET https://connect.mindcloud.co/v1/universal/cin7core/latest/actions/get-sale-order
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/cin7core/latest/actions/get-sale-order?connectionId=$CONNECTION_ID&SaleID=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/cin7core/latest/actions/get-sale-order?${params}`, {
  headers: {
...
Through the native Cin7 Core API, this operation is `GET sale/order` (base URL `https://inventory.dearsystems.com/externalapi/v2/`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-sale-order.md) for the provider-specific parameters and requirements.

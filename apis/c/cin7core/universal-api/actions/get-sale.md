...
```
GET https://connect.mindcloud.co/v1/universal/cin7core/latest/actions/get-sale
```
...
| --- | --- | --- | --- |
| `ID` | string | yes |  |
| `CombineAdditionalCharges` | boolean | no |  |
| `HideInventoryMovements` | boolean | no |  |
| `IncludeTransactions` | boolean | no |  |
| `CountryFormat` | string | no |  |
...
Through the native Cin7 Core API, this operation is `GET sale` (base URL `https://inventory.dearsystems.com/externalapi/v2/`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-sale.md) for the provider-specific parameters and requirements.

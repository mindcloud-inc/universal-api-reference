# CNPJá: Get Office SUFRAMA

Retrieves office SUFRAMA details from CNPJá.

```
GET https://connect.mindcloud.co/v1/universal/cNPJ/latest/actions/get-office-suframa
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/cNPJ/latest/actions/get-office-suframa?connectionId=$CONNECTION_ID&taxId=04337168000148" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/cNPJ/latest/actions/get-office-suframa?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `taxId` | string | yes | CNPJ number without punctuation. Example: `04337168000148`. |
...
| `status.text` | string |  |
| `statusDate` | string |  |
| `suframa[].approvalDate` | string |  |
| `suframa[].approved` | boolean |  |
| `suframa[].incentives[].basis` | string |  |
| `suframa[].incentives[].benefit` | string |  |
| `suframa[].incentives[].purpose` | string |  |
| `suframa[].incentives[].tribute` | string |  |
| `suframa[].number` | string |  |
| `suframa[].since` | string |  |
| `suframa[].status.id` | number |  |
| `suframa[].status.text` | string |  |
| `taxId` | string |  |
| `updated` | string |  |

## Native endpoint

Through the native CNPJá API, this operation is `GET /office/:taxId` (base URL `https://api.cnpja.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-office-suframa.md) for the provider-specific parameters and requirements.

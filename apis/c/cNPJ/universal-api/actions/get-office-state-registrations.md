# CNPJá: Get Office State Registrations

Retrieves office state registrations from CNPJá.

```
GET https://connect.mindcloud.co/v1/universal/cNPJ/latest/actions/get-office-state-registrations
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/cNPJ/latest/actions/get-office-state-registrations?connectionId=$CONNECTION_ID&taxId=37335118000180&registrations=ORIGIN" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/cNPJ/latest/actions/get-office-state-registrations?${params}`, {
  headers: {
...
| `registrations` | string | yes | Use ORIGIN, ALL, or a comma-separated state list such as PR,RS,SC. Example: `ORIGIN`. |
...
Through the native CNPJá API, this operation is `GET /office/:taxId` (base URL `https://api.cnpja.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-office-state-registrations.md) for the provider-specific parameters and requirements.

# Cirra Universal API Authentication

Every Cirra API request through MindCloud uses a MindCloud API key and the `connectionId` of a Cirra connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Cirra connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Cirra action](README.md#actions-24) with your API key and `connectionId`.

# Convertio Universal API Authentication

Every Convertio API request through MindCloud uses a MindCloud API key and the `connectionId` of a Convertio connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Convertio connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Convertio action](README.md#actions-6) with your API key and `connectionId`.

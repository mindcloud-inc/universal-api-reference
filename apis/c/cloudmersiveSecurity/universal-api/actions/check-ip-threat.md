# Cloudmersive Security: Check IP Threat

Checks an IP address for threats in Cloudmersive Security.

```
GET https://connect.mindcloud.co/v1/universal/cloudmersiveSecurity/latest/actions/check-ip-threat
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/cloudmersiveSecurity/latest/actions/check-ip-threat?connectionId=$CONNECTION_ID&ipAddress=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/cloudmersiveSecurity/latest/actions/check-ip-threat?${params}`, {
  headers: {

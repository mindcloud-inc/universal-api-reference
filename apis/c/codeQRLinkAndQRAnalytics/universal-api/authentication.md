# CodeQR - Link and QR Analytics Universal API Authentication

Every CodeQR - Link and QR Analytics API request through MindCloud uses a MindCloud API key and the `connectionId` of a CodeQR - Link and QR Analytics connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your CodeQR - Link and QR Analytics connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [CodeQR - Link and QR Analytics action](README.md#actions-24) with your API key and `connectionId`.

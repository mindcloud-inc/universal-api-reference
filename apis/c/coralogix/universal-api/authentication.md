# Coralogix Universal API Authentication

Every Coralogix API request through MindCloud uses a MindCloud API key and the `connectionId` of a Coralogix connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Coralogix connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Coralogix action](README.md#actions-39) with your API key and `connectionId`.

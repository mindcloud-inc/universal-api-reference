# ClickSend SMS Universal API Authentication

Every ClickSend SMS API request through MindCloud uses a MindCloud API key and the `connectionId` of a ClickSend SMS connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your ClickSend SMS connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [ClickSend SMS action](README.md#actions-29) with your API key and `connectionId`.

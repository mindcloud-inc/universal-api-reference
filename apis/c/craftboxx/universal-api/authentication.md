# Craftboxx Universal API Authentication

Every Craftboxx API request through MindCloud uses a MindCloud API key and the `connectionId` of a Craftboxx connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Craftboxx connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Craftboxx action](README.md#actions-26) with your API key and `connectionId`.

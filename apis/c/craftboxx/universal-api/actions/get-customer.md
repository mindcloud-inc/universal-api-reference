# Craftboxx: Get Customer

Returns a specific customer from Craftboxx.

```
GET https://connect.mindcloud.co/v1/universal/craftboxx/latest/actions/get-customer
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/craftboxx/latest/actions/get-customer?connectionId=$CONNECTION_ID&customerId=1" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/craftboxx/latest/actions/get-customer?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `customerId` | number | yes | The Craftboxx customer ID. |
...
Through the native Craftboxx API, this operation is `GET customers/:customerId` (base URL `https://api.craftboxx.de`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-customer.md) for the provider-specific parameters and requirements.

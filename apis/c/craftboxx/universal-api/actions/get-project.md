# Craftboxx: Get Project

Returns a specific project from Craftboxx.

```
GET https://connect.mindcloud.co/v1/universal/craftboxx/latest/actions/get-project
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/craftboxx/latest/actions/get-project?connectionId=$CONNECTION_ID&projectId=1" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/craftboxx/latest/actions/get-project?${params}`, {
  headers: {
...
Through the native Craftboxx API, this operation is `GET projects/:projectId` (base URL `https://api.craftboxx.de`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-project.md) for the provider-specific parameters and requirements.

# Craftboxx: List Articles

Returns articles from Craftboxx.

```
GET https://connect.mindcloud.co/v1/universal/craftboxx/latest/actions/list-articles
```
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/craftboxx/latest/actions/list-articles?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `q` | string | no | Search articles by a broad Craftboxx query string. |
...
Through the native Craftboxx API, this operation is `GET articles` (base URL `https://api.craftboxx.de`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/list-articles.md) for the provider-specific parameters and requirements.

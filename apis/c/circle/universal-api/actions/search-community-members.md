# Circle: Search Community Members

Finds community members in Circle by search query.

```
GET https://connect.mindcloud.co/v1/universal/circle/latest/actions/search-community-members
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/circle/latest/actions/search-community-members?connectionId=$CONNECTION_ID&limit=25&offset=0&email=ava%40example.com" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/circle/latest/actions/search-community-members?${params}`, {
  headers: {
...
Through the native Circle API, this operation is `GET /api/admin/v2/community_members/search` (base URL `https://{{credentials.subdomain}}.circle.so`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/search-community-members.md) for the provider-specific parameters and requirements.

...
```
PUT https://connect.mindcloud.co/v1/universal/circle/latest/actions/update-space
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/circle/latest/actions/update-space" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/circle/latest/actions/update-space', {
  method: 'PUT',
  headers: {
...
| `name` | string | no | Space name |
| `spaceGroupId` | number | no | Space group ID |
...
Through the native Circle API, this operation is `PUT /api/admin/v2/spaces/[:id]` (base URL `https://{{credentials.subdomain}}.circle.so`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-space.md) for the provider-specific parameters and requirements.

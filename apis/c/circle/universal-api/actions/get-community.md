# Circle: Get Community

Retrieves current community details from Circle.

```
GET https://connect.mindcloud.co/v1/universal/circle/latest/actions/get-community
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/circle/latest/actions/get-community?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/circle/latest/actions/get-community?${params}`, {
  headers: {
...
Through the native Circle API, this operation is `GET /api/admin/v2/community` (base URL `https://{{credentials.subdomain}}.circle.so`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-community.md) for the provider-specific parameters and requirements.

# CoinMarketCap: Get Historical Cryptocurrency OHLCV

Retrieves historical cryptocurrency OHLCV data from CoinMarketCap.

```
GET https://connect.mindcloud.co/v1/universal/coinMarketCap/latest/actions/get-historical-cryptocurrency-ohlcv
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/coinMarketCap/latest/actions/get-historical-cryptocurrency-ohlcv?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/coinMarketCap/latest/actions/get-historical-cryptocurrency-ohlcv?${params}`, {
  headers: {
...
Through the native CoinMarketCap API, this operation is `GET /v2/cryptocurrency/ohlcv/historical` (base URL `https://pro-api.coinmarketcap.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-historical-cryptocurrency-ohlcv.md) for the provider-specific parameters and requirements.

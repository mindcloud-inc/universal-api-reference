# Chatforma: Get Current Account

Retrieves current account authorization status from Chatforma.

```
GET https://connect.mindcloud.co/v1/universal/chatforma/latest/actions/get-current-account
```
...
Through the native Chatforma API, this operation is `GET /auth-test` (base URL `https://api.pro.chatforma.com/public/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-current-account.md) for the provider-specific parameters and requirements.

# ConnectPay: Activate Card

Activates a ChipAndPin card in ConnectPay.

```
PUT https://connect.mindcloud.co/v1/universal/connectPay/latest/actions/activate-card
```
...
Through the native ConnectPay API, this operation is `POST /baas/ob/cards/:cardId/activate` (base URL `https://api-stage.connectpay.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/activate-card.md) for the provider-specific parameters and requirements.

# CallKeeper: Add Party To Call

Updates a call in CallKeeper by adding a party.

```
PUT https://connect.mindcloud.co/v1/universal/callKeeper/latest/actions/add-party-to-call
```
...
Through the native CallKeeper API, this operation is `POST /calls/:call_id/add-party` (base URL `https://api.callkeeper.ai`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/add-party-to-call.md) for the provider-specific parameters and requirements.

# 2Captcha Universal API Authentication

Every 2Captcha API request through MindCloud uses a MindCloud API key and the `connectionId` of a 2Captcha connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your 2Captcha connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [2Captcha action](README.md#actions-40) with your API key and `connectionId`.

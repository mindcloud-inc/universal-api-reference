# 2Captcha: Create Lemin Task With Proxy

Creates a proxied Lemin captcha task in 2Captcha.

```
POST https://connect.mindcloud.co/v1/universal/captcha/latest/actions/create-lemin-task-with-proxy
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/captcha/latest/actions/create-lemin-task-with-proxy" \
  -H "Authorization: Bearer $MINDCLOUD_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
  "connectionId": "$CONNECTION_ID",
  "task.websiteURL": "https://example.com",
  "task.captchaId": "string",
  "task.divId": "string",
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/captcha/latest/actions/create-lemin-task-with-proxy', {
  method: 'POST',
  headers: {
...
| `task.captchaId` | string | yes |  |
| `task.divId` | string | yes |  |
| `task.leminApiServerSubdomain` | string | no |  |
...
Through the native 2Captcha API, this operation is `POST /createTask` (base URL `https://api.2captcha.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-lemin-task-with-proxy.md) for the provider-specific parameters and requirements.

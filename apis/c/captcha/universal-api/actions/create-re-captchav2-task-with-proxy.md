# 2Captcha: Create reCAPTCHA V2 Task With Proxy

Creates a proxied reCAPTCHA v2 task in 2Captcha.

```
POST https://connect.mindcloud.co/v1/universal/captcha/latest/actions/create-re-captchav2-task-with-proxy
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/captcha/latest/actions/create-re-captchav2-task-with-proxy" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/captcha/latest/actions/create-re-captchav2-task-with-proxy', {
  method: 'POST',
  headers: {
...
| `task.recaptchaDataSValue` | string | no |  |
...
Through the native 2Captcha API, this operation is `POST /createTask` (base URL `https://api.2captcha.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-re-captchav2-task-with-proxy.md) for the provider-specific parameters and requirements.

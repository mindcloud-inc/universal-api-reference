# 2Captcha: Create DataDome Slider Task

Creates a DataDome slider captcha task in 2Captcha.

```
POST https://connect.mindcloud.co/v1/universal/captcha/latest/actions/create-data-dome-slider-task
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/captcha/latest/actions/create-data-dome-slider-task" \
  -H "Authorization: Bearer $MINDCLOUD_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
  "connectionId": "$CONNECTION_ID",
  "task.websiteURL": "https://example.com",
  "task.captchaUrl": "https://example.com",
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/captcha/latest/actions/create-data-dome-slider-task', {
  method: 'POST',
  headers: {
...
| `task.captchaUrl` | string | yes |  |
| `task.userAgent` | string | yes |  |
...
Through the native 2Captcha API, this operation is `POST /createTask` (base URL `https://api.2captcha.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-data-dome-slider-task.md) for the provider-specific parameters and requirements.

# Currencylayer: Get Historical Exchange Rates By Source Currency

Retrieves historical exchange rates by source currency from Currencylayer on a date.

```
GET https://connect.mindcloud.co/v1/universal/currencylayer/latest/actions/get-historical-exchange-rates-by-source-currency
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/currencylayer/latest/actions/get-historical-exchange-rates-by-source-currency?connectionId=$CONNECTION_ID&date=string&source=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/currencylayer/latest/actions/get-historical-exchange-rates-by-source-currency?${params}`, {
  headers: {

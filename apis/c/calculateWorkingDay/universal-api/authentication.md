# Calculate Working Day Universal API Authentication

Every Calculate Working Day API request through MindCloud uses a MindCloud API key and the `connectionId` of a Calculate Working Day connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Calculate Working Day connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Calculate Working Day action](README.md#actions-7) with your API key and `connectionId`.

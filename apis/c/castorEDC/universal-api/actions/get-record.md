# Castor EDC: Get Record

Retrieves a record from Castor EDC by ID.

```
GET https://connect.mindcloud.co/v1/universal/castorEDC/latest/actions/get-record
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/castorEDC/latest/actions/get-record?connectionId=$CONNECTION_ID&study_id=string&record_id=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/castorEDC/latest/actions/get-record?${params}`, {
  headers: {
...
| `record_id` | string | yes | Record ID of the Record to fetch |
...
Through the native Castor EDC API, this operation is `GET /study/:study_id/record/:record_id` (base URL `https://us.castoredc.com/api`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-record.md) for the provider-specific parameters and requirements.

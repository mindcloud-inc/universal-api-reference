# Chroma Vector Store Universal API Authentication

Every Chroma Vector Store API request through MindCloud uses a MindCloud API key and the `connectionId` of a Chroma Vector Store connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Chroma Vector Store connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Chroma Vector Store action](README.md#actions-9) with your API key and `connectionId`.

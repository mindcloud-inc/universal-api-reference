# CSVBox Universal API Authentication

Every CSVBox API request through MindCloud uses a MindCloud API key and the `connectionId` of a CSVBox connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your CSVBox connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [CSVBox action](README.md#actions-3) with your API key and `connectionId`.

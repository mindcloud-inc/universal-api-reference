# ConvertHub Universal API Authentication

Every ConvertHub API request through MindCloud uses a MindCloud API key and the `connectionId` of a ConvertHub connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your ConvertHub connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [ConvertHub action](README.md#actions-16) with your API key and `connectionId`.

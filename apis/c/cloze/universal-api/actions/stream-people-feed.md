# Cloze: Stream People Feed

Retrieves the people feed from Cloze.

```
GET https://connect.mindcloud.co/v1/universal/cloze/latest/actions/stream-people-feed
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/cloze/latest/actions/stream-people-feed?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/cloze/latest/actions/stream-people-feed?${params}`, {
  headers: {
...
| `includeauditedchanges` | boolean | no | Return change audit details in results for each delivered person. |
...
| `people[]` | array<object> | Array of streamed person records |
...
Through the native Cloze API, this operation is `GET /v1/people/feed` (base URL `https://api.cloze.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/stream-people-feed.md) for the provider-specific parameters and requirements.

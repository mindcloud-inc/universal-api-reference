# CalendarLink Universal API Authentication

Every CalendarLink API request through MindCloud uses a MindCloud API key and the `connectionId` of a CalendarLink connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your CalendarLink connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [CalendarLink action](README.md#actions-9) with your API key and `connectionId`.

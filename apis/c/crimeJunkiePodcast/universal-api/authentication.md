# Crime Junkie Podcast Universal API Authentication

Every Crime Junkie Podcast API request through MindCloud uses a MindCloud API key and the `connectionId` of a Crime Junkie Podcast connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Crime Junkie Podcast connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Crime Junkie Podcast action](README.md#actions-58) with your API key and `connectionId`.

# List Play Stats with College Football Data

Retrieves player-play associations from College Football Data.

## Endpoint

- **Method:** `GET`
- **Path:** `/plays/stats`
- **Base URL:** `https://api.collegefootballdata.com`
- **Official documentation:** [List Play Stats](https://api.collegefootballdata.com/)

## Parameters

| Parameter | Location | Type | Required | Description |
| --- | --- | --- | --- | --- |
| `year` | query | `number` | no | Optional year filter |
| `week` | query | `number` | no | Optional week filter |
| `team` | query | `string` | no | Optional team filter |
| `gameId` | query | `number` | no | Optional gameId filter |
| `athleteId` | query | `number` | no | Optional athleteId filter |
| `statTypeId` | query | `number` | no | Optional statTypeId filter |
| `seasonType` | query | `string` | no | Optional season type filter |
| `conference` | query | `string` | no | Optional conference filter |

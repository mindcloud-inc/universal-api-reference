# ChangeCrab: Create Post

Creates a new post in ChangeCrab.

```
POST https://connect.mindcloud.co/v1/universal/changeCrab/latest/actions/create-post
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/changeCrab/latest/actions/create-post" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/changeCrab/latest/actions/create-post', {
  method: 'POST',
  headers: {
...
Through the native ChangeCrab API, this operation is `POST /changelogs/:id/posts` (base URL `https://changecrab.com/api`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-post.md) for the provider-specific parameters and requirements.

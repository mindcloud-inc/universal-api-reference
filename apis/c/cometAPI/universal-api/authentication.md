# CometAPI Universal API Authentication

Every CometAPI API request through MindCloud uses a MindCloud API key and the `connectionId` of a CometAPI connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your CometAPI connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [CometAPI action](README.md#actions-86) with your API key and `connectionId`.

# ClassMarker: List Recent Results for Specific Group Exam



```
GET https://connect.mindcloud.co/v1/universal/classMarker/latest/actions/list-recent-results-for-specific-group-exam
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/classMarker/latest/actions/list-recent-results-for-specific-group-exam?connectionId=$CONNECTION_ID&groupId=1&testId=1" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/classMarker/latest/actions/list-recent-results-for-specific-group-exam?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `groupId` | number | yes | Numeric ClassMarker group ID. |
...
Through the native ClassMarker API, this operation is `GET /v1/groups/{group_id}/tests/{test_id}/recent_results.json` (base URL `https://api.classmarker.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-recent-results-for-specific-group-exam.md) for the provider-specific parameters and requirements.

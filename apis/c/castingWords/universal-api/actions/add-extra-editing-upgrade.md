# CastingWords: Add Extra Editing Upgrade

Updates a CastingWords order to add extra editing.

```
PUT https://connect.mindcloud.co/v1/universal/castingWords/latest/actions/add-extra-editing-upgrade
```
...
Through the native CastingWords API, this operation is `POST audiofile/:audiofile_id/upgrade` (base URL `https://castingwords.com/store/API4`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/add-extra-editing-upgrade.md) for the provider-specific parameters and requirements.

# Certs 365 Universal API Authentication

Every Certs 365 API request through MindCloud uses a MindCloud API key and the `connectionId` of a Certs 365 connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Certs 365 connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Certs 365 action](README.md#actions-25) with your API key and `connectionId`.

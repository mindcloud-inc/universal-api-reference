# ClickUp Universal API Authentication

Every ClickUp API request through MindCloud uses a MindCloud API key and the `connectionId` of a ClickUp connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your ClickUp connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [ClickUp action](README.md#actions-25) with your API key and `connectionId`.

# Cloudflare Browser Run Universal API Authentication

Every Cloudflare Browser Run API request through MindCloud uses a MindCloud API key and the `connectionId` of a Cloudflare Browser Run connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Cloudflare Browser Run connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Cloudflare Browser Run action](README.md#actions-25) with your API key and `connectionId`.

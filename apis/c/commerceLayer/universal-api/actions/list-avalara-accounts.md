# Commerce Layer: List Avalara Accounts



```
GET https://connect.mindcloud.co/v1/universal/commerceLayer/latest/actions/list-avalara-accounts
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/commerceLayer/latest/actions/list-avalara-accounts?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/commerceLayer/latest/actions/list-avalara-accounts?${params}`, {
  headers: {
...
Through the native Commerce Layer API, this operation is `GET /api/avalara_accounts` (base URL `{{credentials.coreApiEndpoint}}`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-avalara-accounts.md) for the provider-specific parameters and requirements.

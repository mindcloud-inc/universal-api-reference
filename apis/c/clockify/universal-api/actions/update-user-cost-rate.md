# Clockify: Update User Cost Rate

Updates a user cost rate in Clockify.

```
PUT https://connect.mindcloud.co/v1/universal/clockify/latest/actions/update-user-cost-rate
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/clockify/latest/actions/update-user-cost-rate" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/clockify/latest/actions/update-user-cost-rate', {
  method: 'PUT',
  headers: {
...
Through the native Clockify API, this operation is `PUT workspaces/:workspaceId/users/:userId/cost-rate` (base URL `https://api.clockify.me/api/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-user-cost-rate.md) for the provider-specific parameters and requirements.

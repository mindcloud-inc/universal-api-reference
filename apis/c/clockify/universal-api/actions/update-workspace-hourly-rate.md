# Clockify: Update Workspace Hourly Rate

Updates a workspace hourly rate in Clockify.

```
PUT https://connect.mindcloud.co/v1/universal/clockify/latest/actions/update-workspace-hourly-rate
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/clockify/latest/actions/update-workspace-hourly-rate" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/clockify/latest/actions/update-workspace-hourly-rate', {
  method: 'PUT',
  headers: {
...
Through the native Clockify API, this operation is `PUT workspaces/:workspaceId/hourly-rate` (base URL `https://api.clockify.me/api/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-workspace-hourly-rate.md) for the provider-specific parameters and requirements.

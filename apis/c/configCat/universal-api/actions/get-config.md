# ConfigCat: Get Config

Retrieves an existing config from ConfigCat.

```
GET https://connect.mindcloud.co/v1/universal/configCat/latest/actions/get-config
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/configCat/latest/actions/get-config?connectionId=$CONNECTION_ID&configId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/configCat/latest/actions/get-config?${params}`, {
  headers: {

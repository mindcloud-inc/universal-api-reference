# ConfigCat: Update Environment

Updates an existing environment in ConfigCat.

```
PUT https://connect.mindcloud.co/v1/universal/configCat/latest/actions/update-environment
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/configCat/latest/actions/update-environment" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/configCat/latest/actions/update-environment', {
  method: 'PUT',
  headers: {
...
| `environment` | object | yes | Raw ConfigCat environment body. |

# Cloudmersive: Validate Street Address

Validates a street address in Cloudmersive.

```
GET https://connect.mindcloud.co/v1/universal/cloudmersive/latest/actions/validate-street-address
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/cloudmersive/latest/actions/validate-street-address?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/cloudmersive/latest/actions/validate-street-address?${params}`, {
  headers: {
...
| `StreetAddress` | string | no | Street address to validate. |

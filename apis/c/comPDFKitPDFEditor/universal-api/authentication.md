# ComPDFKit PDF Editor Universal API Authentication

Every ComPDFKit PDF Editor API request through MindCloud uses a MindCloud API key and the `connectionId` of a ComPDFKit PDF Editor connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your ComPDFKit PDF Editor connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [ComPDFKit PDF Editor action](README.md#actions-56) with your API key and `connectionId`.

# Cerbo: List Encounter Estimates

Retrieves encounter estimates from Cerbo.

```
GET https://connect.mindcloud.co/v1/universal/cerbo/latest/actions/list-encounter-estimates
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/cerbo/latest/actions/list-encounter-estimates?connectionId=$CONNECTION_ID&limit=25&offset=0" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/cerbo/latest/actions/list-encounter-estimates?${params}`, {
  headers: {
...
Through the native Cerbo API, this operation is `GET /patients/:pt_id/encounters/:encounter_id/estimates` (base URL `https://{{credentials.tenant}}.md-hq.com/api/v1`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/list-encounter-estimates.md) for the provider-specific parameters and requirements.

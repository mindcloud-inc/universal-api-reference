# Cerbo Universal API Authentication

Every Cerbo API request through MindCloud uses a MindCloud API key and the `connectionId` of a Cerbo connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Cerbo connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Cerbo action](README.md#actions-127) with your API key and `connectionId`.

# Cirra MCP Tests - Do Not Delete Universal API Authentication

Every Cirra MCP Tests - Do Not Delete API request through MindCloud uses a MindCloud API key and the `connectionId` of a Cirra MCP Tests - Do Not Delete connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Cirra MCP Tests - Do Not Delete connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Cirra MCP Tests - Do Not Delete action](README.md#actions-4) with your API key and `connectionId`.

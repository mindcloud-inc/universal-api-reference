# Clarifai Universal API Authentication

Every Clarifai API request through MindCloud uses a MindCloud API key and the `connectionId` of a Clarifai connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Clarifai connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Clarifai action](README.md#actions-41) with your API key and `connectionId`.

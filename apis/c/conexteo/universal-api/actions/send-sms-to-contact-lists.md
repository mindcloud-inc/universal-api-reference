# Conexteo: Send SMS To Contact Lists

Creates an SMS message for Conexteo contact lists.

```
POST https://connect.mindcloud.co/v1/universal/conexteo/latest/actions/send-sms-to-contact-lists
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/conexteo/latest/actions/send-sms-to-contact-lists" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/conexteo/latest/actions/send-sms-to-contact-lists', {
  method: 'POST',
  headers: {
...
| `contactlist[]` | array<string> | yes | Array of contact-list identifiers. |
...
Through the native Conexteo API, this operation is `POST /messages/sms/contactlist` (base URL `https://api.conexteo.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/send-sms-to-contact-lists.md) for the provider-specific parameters and requirements.

# Conexteo: Send Dynamic SMS

Creates a dynamic SMS message in Conexteo.

```
POST https://connect.mindcloud.co/v1/universal/conexteo/latest/actions/send-dynamic-sms
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/conexteo/latest/actions/send-dynamic-sms" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/conexteo/latest/actions/send-dynamic-sms', {
  method: 'POST',
  headers: {
...
| `recipients[]` | array<object> | yes | Array of recipient objects. Each item must include recipient and may include additional dynamic string properties. |
...
Through the native Conexteo API, this operation is `POST /messages/sms/dynamic` (base URL `https://api.conexteo.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/send-dynamic-sms.md) for the provider-specific parameters and requirements.

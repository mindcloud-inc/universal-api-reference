...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/clioGrow/latest/actions/list-contact-notes?connectionId=$CONNECTION_ID&contactId=string" \
...
Through the native Clio Grow API, this operation is `GET /contacts/{contact_id}/notes` (base URL `https://api.clio.com/grow`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-contact-notes.md) for the provider-specific parameters and requirements.

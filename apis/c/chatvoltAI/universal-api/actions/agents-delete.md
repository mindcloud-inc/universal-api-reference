# Chatvolt AI: Delete Agent

Deletes an agent from Chatvolt AI.

```
DELETE https://connect.mindcloud.co/v1/universal/chatvoltAI/latest/actions/agents-delete
```
...
```bash
curl -X DELETE "https://connect.mindcloud.co/v1/universal/chatvoltAI/latest/actions/agents-delete?connectionId=$CONNECTION_ID&id=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/chatvoltAI/latest/actions/agents-delete?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `id` | string | yes | ID of the agent to be deleted. |
...
Through the native Chatvolt AI API, this operation is `DELETE /agents/{id}` (base URL `https://api.chatvolt.ai`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/agents-delete.md) for the provider-specific parameters and requirements.

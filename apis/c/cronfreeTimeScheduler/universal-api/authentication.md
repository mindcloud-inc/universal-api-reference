# Cronfree Time Scheduler Universal API Authentication

Every Cronfree Time Scheduler API request through MindCloud uses a MindCloud API key and the `connectionId` of a Cronfree Time Scheduler connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Cronfree Time Scheduler connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Cronfree Time Scheduler action](README.md#actions-2) with your API key and `connectionId`.

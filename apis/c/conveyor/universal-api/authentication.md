# Conveyor Universal API Authentication

Every Conveyor API request through MindCloud uses a MindCloud API key and the `connectionId` of a Conveyor connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Conveyor connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Conveyor action](README.md#actions-27) with your API key and `connectionId`.

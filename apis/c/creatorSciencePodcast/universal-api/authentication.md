# Creator Science Podcast Universal API Authentication

Every Creator Science Podcast API request through MindCloud uses a MindCloud API key and the `connectionId` of a Creator Science Podcast connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Creator Science Podcast connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Creator Science Podcast action](README.md#actions-1) with your API key and `connectionId`.

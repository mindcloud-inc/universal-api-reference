# CardClan Universal API Authentication

Every CardClan API request through MindCloud uses a MindCloud API key and the `connectionId` of a CardClan connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your CardClan connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [CardClan action](README.md#actions-11) with your API key and `connectionId`.

# CalendarHero: Update Address

Updates an address in CalendarHero.

```
PUT https://connect.mindcloud.co/v1/universal/calendarHero/latest/actions/update-address
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/calendarHero/latest/actions/update-address" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/calendarHero/latest/actions/update-address', {
  method: 'PUT',
  headers: {
...
Through the native CalendarHero API, this operation is `PUT /user/settings/address` (base URL `https://api.calendarhero.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-address.md) for the provider-specific parameters and requirements.

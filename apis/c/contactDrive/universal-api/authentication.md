# ContactDrive Universal API Authentication

Every ContactDrive API request through MindCloud uses a MindCloud API key and the `connectionId` of a ContactDrive connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your ContactDrive connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [ContactDrive action](README.md#actions-4) with your API key and `connectionId`.

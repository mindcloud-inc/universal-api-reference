# Cheddar: Run Outstanding Invoice

Executes an outstanding invoice in Cheddar.

```
PUT https://connect.mindcloud.co/v1/universal/cheddar/latest/actions/run-outstanding-invoice
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/cheddar/latest/actions/run-outstanding-invoice" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/cheddar/latest/actions/run-outstanding-invoice', {
  method: 'PUT',
  headers: {
...
| `ccCardCode` | string | no | 3-4 digit card verification value. |
...
Through the native Cheddar API, this operation is `POST /customers/run-outstanding/productCode/{{credentials.productCode}}/code/:customerCode` (base URL `https://getcheddar.com/xml`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/run-outstanding-invoice.md) for the provider-specific parameters and requirements.

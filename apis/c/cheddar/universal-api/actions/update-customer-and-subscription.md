# Cheddar: Update Customer and Subscription

Updates customer and subscription details in Cheddar.

```
PUT https://connect.mindcloud.co/v1/universal/cheddar/latest/actions/update-customer-and-subscription
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/cheddar/latest/actions/update-customer-and-subscription" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/cheddar/latest/actions/update-customer-and-subscription', {
  method: 'PUT',
  headers: {
...
| `subscription` | object | no | Subscription fields to update for the customer. |
| `subscription.planCode` | string | no | Pricing plan code to set on the subscription. |
| `subscription.method` | string | no | Payment method: cc or paypal. |
...
Through the native Cheddar API, this operation is `POST /customers/edit/productCode/{{credentials.productCode}}/code/:customerCode` (base URL `https://getcheddar.com/xml`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-customer-and-subscription.md) for the provider-specific parameters and requirements.

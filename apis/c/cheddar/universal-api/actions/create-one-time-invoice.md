# Cheddar: Create One-Time Invoice

Creates a one-time invoice in Cheddar.

```
POST https://connect.mindcloud.co/v1/universal/cheddar/latest/actions/create-one-time-invoice
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/cheddar/latest/actions/create-one-time-invoice" \
  ...
  "customerCode": "string",
  "charges": {},
  "charges[].chargeCode": "string",
  "charges[].quantity": 1,
  "charges[].eachAmount": 1
}'
```

```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/cheddar/latest/actions/create-one-time-invoice', {
  method: 'POST',
  headers: {
...
| `charges` | list<object> | yes | Charge rows to include on the one-time invoice. |
| `charges[].chargeCode` | string | yes | Charge code for one invoice line. |
| `charges[].quantity` | number | yes | Positive integer quantity for one invoice line. |
| `charges[].eachAmount` | number | yes | Positive or negative amount with two-digit decimal precision for one invoice line. |
| `charges[].description` | string | no | Description for one invoice line. |
...
Through the native Cheddar API, this operation is `POST /invoices/new/productCode/{{credentials.productCode}}/code/:customerCode` (base URL `https://getcheddar.com/xml`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-one-time-invoice.md) for the provider-specific parameters and requirements.

# Cheddar: Get Customer

Retrieves customer billing details from Cheddar.

```
GET https://connect.mindcloud.co/v1/universal/cheddar/latest/actions/get-customer
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/cheddar/latest/actions/get-customer?connectionId=$CONNECTION_ID&customerCode=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/cheddar/latest/actions/get-customer?${params}`, {
  headers: {
...
Through the native Cheddar API, this operation is `GET /customers/get/productCode/{{credentials.productCode}}/code/:customerCode` (base URL `https://getcheddar.com/xml`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-customer.md) for the provider-specific parameters and requirements.

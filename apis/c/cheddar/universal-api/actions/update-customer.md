# Cheddar: Update Customer

Updates existing customer details in Cheddar.

```
PUT https://connect.mindcloud.co/v1/universal/cheddar/latest/actions/update-customer
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/cheddar/latest/actions/update-customer" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/cheddar/latest/actions/update-customer', {
  method: 'PUT',
  headers: {
...
| `notes` | string | no | Internal notes for the customer. |
...
Through the native Cheddar API, this operation is `POST /customers/edit-customer/productCode/{{credentials.productCode}}/code/:customerCode` (base URL `https://getcheddar.com/xml`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-customer.md) for the provider-specific parameters and requirements.

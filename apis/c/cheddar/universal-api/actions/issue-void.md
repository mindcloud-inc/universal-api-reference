...
```
PUT https://connect.mindcloud.co/v1/universal/cheddar/latest/actions/issue-void
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/cheddar/latest/actions/issue-void" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/cheddar/latest/actions/issue-void', {
  method: 'PUT',
  headers: {
...
Through the native Cheddar API, this operation is `POST /invoices/void/productCode/{{credentials.productCode}}` (base URL `https://getcheddar.com/xml`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/issue-void.md) for the provider-specific parameters and requirements.

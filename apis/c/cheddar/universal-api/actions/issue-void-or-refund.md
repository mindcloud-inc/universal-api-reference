# Cheddar: Issue Void or Refund

Voids or refunds a billed invoice in Cheddar.

```
PUT https://connect.mindcloud.co/v1/universal/cheddar/latest/actions/issue-void-or-refund
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/cheddar/latest/actions/issue-void-or-refund" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/cheddar/latest/actions/issue-void-or-refund', {
  method: 'PUT',
  headers: {
...
Through the native Cheddar API, this operation is `POST /invoices/void-or-refund/productCode/{{credentials.productCode}}` (base URL `https://getcheddar.com/xml`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/issue-void-or-refund.md) for the provider-specific parameters and requirements.

# Cheddar: Get Pricing Plan

Retrieves pricing plan details from Cheddar.

```
GET https://connect.mindcloud.co/v1/universal/cheddar/latest/actions/get-pricing-plan
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/cheddar/latest/actions/get-pricing-plan?connectionId=$CONNECTION_ID&planCode=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/cheddar/latest/actions/get-pricing-plan?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `planCode` | string | yes | Pricing plan code from Cheddar. |
...
Through the native Cheddar API, this operation is `GET /plans/get/productCode/{{credentials.productCode}}/code/:planCode` (base URL `https://getcheddar.com/xml`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-pricing-plan.md) for the provider-specific parameters and requirements.

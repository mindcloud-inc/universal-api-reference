# Cheddar: Issue Refund

Creates a refund for a billed invoice in Cheddar.

```
POST https://connect.mindcloud.co/v1/universal/cheddar/latest/actions/issue-refund
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/cheddar/latest/actions/issue-refund" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/cheddar/latest/actions/issue-refund', {
  method: 'POST',
  headers: {
...
| `amount` | number | yes | Refund amount, less than or equal to the refundable amount. |
...
Through the native Cheddar API, this operation is `POST /invoices/refund/productCode/{{credentials.productCode}}` (base URL `https://getcheddar.com/xml`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/issue-refund.md) for the provider-specific parameters and requirements.

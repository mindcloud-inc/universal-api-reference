# Cheddar: Update Subscription

Updates an existing subscription in Cheddar.

```
PUT https://connect.mindcloud.co/v1/universal/cheddar/latest/actions/update-subscription
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/cheddar/latest/actions/update-subscription" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/cheddar/latest/actions/update-subscription', {
  method: 'PUT',
  headers: {
...
| `planCode` | string | no | Pricing plan code to set on the subscription. |
| `method` | string | no | Payment method: cc or paypal. |
| `couponCode` | string | no | Promotion coupon code to apply. |
| `changeBillDate` | date | no | Date or datetime for the next billable invoice. |
...
Through the native Cheddar API, this operation is `POST /customers/edit-subscription/productCode/{{credentials.productCode}}/code/:customerCode` (base URL `https://getcheddar.com/xml`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-subscription.md) for the provider-specific parameters and requirements.

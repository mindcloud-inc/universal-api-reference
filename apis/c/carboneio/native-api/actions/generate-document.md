# Generate Document with Carbone.io
...
- **Method:** `POST`
- **Path:** `/render/[:templateId-or-versionId]`
- **Base URL:** `https://api.carbone.io`
- **Official documentation:** [Generate Document](https://carbone.io/documentation/developer/http-api/generate-reports.html#generate-a-report)
...
| `data` | body | `object` | yes | JSON dataset merged into the stored template. |
...
| `template` | body | `string` | no | Optional base64-encoded template contents to override the stored template at render time. |
| `templateId-or-versionId` | path | `string` | yes | Template ID (64-bit) or Version ID (SHA-256) to render. |

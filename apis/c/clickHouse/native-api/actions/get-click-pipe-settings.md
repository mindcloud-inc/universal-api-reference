# Get ClickPipe Settings with ClickHouse

## Endpoint

- **Method:** `GET`
- **Path:** `/v1/organizations/[:organizationId]/services/[:serviceId]/clickpipes/[:clickPipeId]/settings`
- **Base URL:** `https://api.clickhouse.cloud`
- **Official documentation:** [Get ClickPipe Settings](https://api.clickhouse.cloud/v1)

## Parameters

| Parameter | Location | Type | Required | Description |
| --- | --- | --- | --- | --- |
| `organizationId` | path | `string` | yes | ID of the organization that owns the service. |
| `serviceId` | path | `string` | yes | ID of the requested service. |
| `clickPipeId` | path | `string` | yes | ID of the requested ClickPipe. |

# ClickHouse Universal API Authentication

Every ClickHouse API request through MindCloud uses a MindCloud API key and the `connectionId` of a ClickHouse connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your ClickHouse connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [ClickHouse action](README.md#actions-24) with your API key and `connectionId`.

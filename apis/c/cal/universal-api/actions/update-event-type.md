...
```
PUT https://connect.mindcloud.co/v1/universal/cal/latest/actions/update-event-type
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/cal/latest/actions/update-event-type" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/cal/latest/actions/update-event-type', {
  method: 'PUT',
  headers: {
...
| --- | --- | --- | --- |
| `afterEventBuffer` | number | no | Updated buffer after each booking, in minutes. |
...
| `beforeEventBuffer` | number | no | Updated buffer before each booking, in minutes. |
...
| `customName` | string | no | Updated custom label for this event type. |
| `description` | string | no | Updated event type description. |
...
| `lengthInMinutes` | number | no | Updated event duration in minutes. |
| `lengthInMinutesOptions[]` | array<number> | no | Updated allowed duration options in minutes. |
...
| `minimumBookingNotice` | number | no | Updated minimum notice before booking, in minutes. |
| `offsetStart` | number | no | Updated start offset in minutes. |
...
| `scheduleId` | list | no | Updated schedule ID assigned to the event type. |
...
| `slotInterval` | number | no | Updated slot interval in minutes. |
| `slug` | string | no | Updated event type slug. |
...
| `title` | string | no | Updated event type title. |
...
Through the native Cal.com API, this operation is `PATCH /event-types/:eventTypeId` (base URL `https://api.cal.com/v2`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-event-type.md) for the provider-specific parameters and requirements.

# CDC Content Services Universal API Authentication

Every CDC Content Services API request through MindCloud uses a MindCloud API key and the `connectionId` of a CDC Content Services connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your CDC Content Services connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [CDC Content Services action](README.md#actions-17) with your API key and `connectionId`.

# CDC Content Services Universal API Arguments

Arguments are the inputs a CDC Content Services action needs. Each [action page](README.md#actions-17) lists its exact keys, types, and required fields. Keys are case-sensitive, and requests with missing or invalid required arguments fail instead of guessing.
...
Every CDC Content Services response uses the same envelope. Single-record actions still return one item inside `data`:

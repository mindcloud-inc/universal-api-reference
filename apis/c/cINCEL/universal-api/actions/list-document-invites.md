# CINCEL: List Document Invites



```
GET https://connect.mindcloud.co/v1/universal/cINCEL/latest/actions/list-document-invites
```
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/cINCEL/latest/actions/list-document-invites?${params}`, {
  headers: {
...
| `includeDeleted` | boolean | no | Include deleted invites. |
| `nameLike` | string | no | Filter invites by name substring. |
...
Through the native CINCEL API, this operation is `GET /teams/:team/folders/:folder/documents/:document/invites` (base URL `https://api.cincel.digital/v3`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/list-document-invites.md) for the provider-specific parameters and requirements.

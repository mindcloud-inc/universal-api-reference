# COVID-19 JHU CSSE: List Latest US Daily Report

Retrieves the latest archived United States COVID-19 daily report rows.

```
GET https://connect.mindcloud.co/v1/universal/cOVID19JHUCSSE/latest/actions/list-latest-us-daily-report
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/cOVID19JHUCSSE/latest/actions/list-latest-us-daily-report?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/cOVID19JHUCSSE/latest/actions/list-latest-us-daily-report?${params}`, {
  headers: {
...
Through the native COVID-19 JHU CSSE API, this operation is `GET /csse_covid_19_daily_reports_us/03-09-2023.csv` (base URL `https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-latest-us-daily-report.md) for the provider-specific parameters and requirements.

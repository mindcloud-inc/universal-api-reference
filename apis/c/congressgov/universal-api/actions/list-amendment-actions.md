# Congress.gov: List Amendment Actions

Retrieves actions for an amendment from Congress.gov.

```
GET https://connect.mindcloud.co/v1/universal/congressgov/latest/actions/list-amendment-actions
```
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/congressgov/latest/actions/list-amendment-actions?${params}`, {
  headers: {
...
Through the native Congress.gov API, this operation is `GET /amendment/:congress/:amendmentType/:amendmentNumber/actions` (base URL `https://api.congress.gov/v3`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/list-amendment-actions.md) for the provider-specific parameters and requirements.

# Click2Mail: Update Job

Updates an existing job in Click2Mail.

```
PUT https://connect.mindcloud.co/v1/universal/click2Mail/latest/actions/update-job
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/click2Mail/latest/actions/update-job" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/click2Mail/latest/actions/update-job', {
  method: 'PUT',
  headers: {
...
| Key | Type | Required | Description |
| --- | --- | --- | --- |
| `layout` | string | no | The specific type of the product |
| `productionTime` | string | no | The desired production time |
| `envelope` | string | no | If this is an enveloped product this determines the envelope in which the product is to be mailed; otherwise provide no value |
| `color` | string | no | Print in color or black and white |
| `paperType` | string | no | Sets the paper the mailing is to be printed on |
| `printOption` | string | no | Sets simplex or duplex printing |
...
| `crid` | string | no | Required for EDDM jobs |

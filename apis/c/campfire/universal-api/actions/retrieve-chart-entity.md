# Campfire: Retrieve Chart Entity

Retrieves a chart entity from Campfire.

```
GET https://connect.mindcloud.co/v1/universal/campfire/latest/actions/retrieve-chart-entity
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/campfire/latest/actions/retrieve-chart-entity?connectionId=$CONNECTION_ID&id=1" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/campfire/latest/actions/retrieve-chart-entity?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `id` | number | yes | The chart entity ID to retrieve. |
...
Through the native Campfire API, this operation is `GET /coa/api/entity/:id` (base URL `https://api.meetcampfire.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/retrieve-chart-entity.md) for the provider-specific parameters and requirements.

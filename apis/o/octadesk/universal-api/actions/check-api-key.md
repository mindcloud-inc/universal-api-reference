# Octadesk: Check API Key

Checks whether an Octadesk API key is valid.

```
GET https://connect.mindcloud.co/v1/universal/octadesk/latest/actions/check-api-key
```
...
Through the native Octadesk API, this operation is `GET /auth/check` (base URL `{{credentials.baseUrl}}`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/check-api-key.md) for the provider-specific parameters and requirements.

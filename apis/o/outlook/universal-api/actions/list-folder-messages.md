# Outlook: List Folder Messages

Retrieves emails from a specific Outlook mail folder.

```
GET https://connect.mindcloud.co/v1/universal/outlook/latest/actions/list-folder-messages
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/outlook/latest/actions/list-folder-messages?connectionId=$CONNECTION_ID&folderId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/outlook/latest/actions/list-folder-messages?${params}`, {
  headers: {
...
Through the native Outlook API, this operation is `GET /me/mailFolders/:folderId/messages` (base URL `https://graph.microsoft.com/v1.0`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-folder-messages.md) for the provider-specific parameters and requirements.

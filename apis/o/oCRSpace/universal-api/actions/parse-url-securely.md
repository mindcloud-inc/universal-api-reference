# OCRSpace: Parse URL Securely



```
GET https://connect.mindcloud.co/v1/universal/oCRSpace/latest/actions/parse-url-securely
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/oCRSpace/latest/actions/parse-url-securely?connectionId=$CONNECTION_ID&url=https%3A%2F%2Fdl.a9t9.com%2Focr%2Fsolarcell.jpg" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/oCRSpace/latest/actions/parse-url-securely?${params}`, {
  headers: {
...
Through the native OCRSpace API, this operation is `POST /parse/image` (base URL `https://api.ocr.space`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/parse-url-securely.md) for the provider-specific parameters and requirements.

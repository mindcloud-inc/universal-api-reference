# OCRSpace Universal API Authentication

Every OCRSpace API request through MindCloud uses a MindCloud API key and the `connectionId` of a OCRSpace connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your OCRSpace connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [OCRSpace action](README.md#actions-17) with your API key and `connectionId`.

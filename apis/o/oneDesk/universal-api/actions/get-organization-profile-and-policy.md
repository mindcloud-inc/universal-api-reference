# OneDesk: Get Organization Profile And Policy

Retrieves organization profile and policy from OneDesk.

```
GET https://connect.mindcloud.co/v1/universal/oneDesk/latest/actions/get-organization-profile-and-policy
```

Authenticate with `Authorization: Bearer $MINDCLOUD_API_KEY` and pass a OneDesk `connectionId` ([setup](../authentication.md)).

## Example request

```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/oneDesk/latest/actions/get-organization-profile-and-policy?connectionId=$CONNECTION_ID" \
  -H "Authorization: Bearer $MINDCLOUD_API_KEY"
```

```js
const params = new URLSearchParams({
  connectionId
});

const response = await fetch(`https://connect.mindcloud.co/v1/universal/oneDesk/latest/actions/get-organization-profile-and-policy?${params}`, {
  headers: {
    Authorization: `Bearer ${process.env.MINDCLOUD_API_KEY}`
  }
});

const { success, data } = await response.json();
```



## Response

```json
{
  "success": true,
  "data": [
    {
      "dateFormat": "string",
      "defaultThreadVisibilityType": "string",
      "organizationName": "Ava Chen",
      "organizationUri": "string"
    }
  ],
  "meta": {}
}
```

### Response fields

| Key | Type | Description |
| --- | --- | --- |
| `dateFormat` | string |  |
| `defaultThreadVisibilityType` | string |  |
| `organizationName` | string |  |
| `organizationUri` | string |  |

## Native endpoint

Through the native OneDesk API, this operation is `GET /rest/public/organization/profileAndPolicy` (base URL `https://app.onedesk.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-organization-profile-and-policy.md) for the provider-specific parameters and requirements.


# Outseta: Add New Invoice

Creates a new invoice in Outseta.

```
POST https://connect.mindcloud.co/v1/universal/outseta/latest/actions/add-new-invoice
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/outseta/latest/actions/add-new-invoice" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/outseta/latest/actions/add-new-invoice', {
  method: 'POST',
  headers: {
...
| `invoiceLineItems[].description` | string | no |  |
| `invoiceLineItems[].amount` | number | no |  |
...
Through the native Outseta API, this operation is `POST /billing/invoices` (base URL `https://{{credentials.subdomain}}.outseta.com/api/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/add-new-invoice.md) for the provider-specific parameters and requirements.

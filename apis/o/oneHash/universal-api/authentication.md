# OneHash Universal API Authentication

Every OneHash API request through MindCloud uses a MindCloud API key and the `connectionId` of a OneHash connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your OneHash connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [OneHash action](README.md#actions-37) with your API key and `connectionId`.

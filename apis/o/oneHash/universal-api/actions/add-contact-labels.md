# OneHash: Add Contact Labels

Updates a contact's labels in OneHash.

```
PUT https://connect.mindcloud.co/v1/universal/oneHash/latest/actions/add-contact-labels
```
...
Through the native OneHash API, this operation is `POST /api/v1/accounts/:accountId/contacts/:id/labels` (base URL `https://chat.onehash.ai`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/add-contact-labels.md) for the provider-specific parameters and requirements.

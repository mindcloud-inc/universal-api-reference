# Oxylabs: Search Google Web

Searches Google web results with Oxylabs.

```
GET https://connect.mindcloud.co/v1/universal/oxylabs/latest/actions/search-google-web
```
...
| --- | --- | --- | --- |
| `query` | string | yes | Search term to submit to Google Web Search. |
| `pages` | number | no | Number of result pages to retrieve. |
| `startPage` | number | no | First Google results page number to request. |
...
Through the native Oxylabs API, this operation is `POST /v1/queries` (base URL `https://realtime.oxylabs.io`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/search-google-web.md) for the provider-specific parameters and requirements.

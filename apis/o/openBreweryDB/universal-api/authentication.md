# Open Brewery DB Universal API Authentication

Every Open Brewery DB API request through MindCloud uses a MindCloud API key and the `connectionId` of a Open Brewery DB connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Open Brewery DB connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Open Brewery DB action](README.md#actions-5) with your API key and `connectionId`.

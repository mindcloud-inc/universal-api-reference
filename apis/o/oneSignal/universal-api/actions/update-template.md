# OneSignal: Update Template

Updates an existing template in OneSignal.

```
PUT https://connect.mindcloud.co/v1/universal/oneSignal/latest/actions/update-template
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/oneSignal/latest/actions/update-template" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/oneSignal/latest/actions/update-template', {
  method: 'PUT',
  headers: {
...
| --- | --- | --- | --- |
| `contents` | object | no | Localized template contents keyed by language code. |
| `name` | string | no | An updated name for the template. |

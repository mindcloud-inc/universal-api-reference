# OnceOnly: Cancel Lease

Cancels an AI lease in OnceOnly.

```
PUT https://connect.mindcloud.co/v1/universal/onceOnly/latest/actions/cancel-lease
```
...
| --- | --- | --- | --- |
| `key` | string | yes | Lease key to cancel. |
...
Through the native OnceOnly API, this operation is `POST /v1/ai/cancel` (base URL `https://api.onceonly.tech`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/cancel-lease.md) for the provider-specific parameters and requirements.

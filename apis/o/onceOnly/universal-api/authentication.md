# OnceOnly Universal API Authentication

Every OnceOnly API request through MindCloud uses a MindCloud API key and the `connectionId` of a OnceOnly connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your OnceOnly connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [OnceOnly action](README.md#actions-28) with your API key and `connectionId`.

# openpm: List Connected Packages



```
GET https://connect.mindcloud.co/v1/universal/openpm/latest/actions/list-connected-packages
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/openpm/latest/actions/list-connected-packages?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/openpm/latest/actions/list-connected-packages?${params}`, {
  headers: {
...
Through the native openpm API, this operation is `GET /packages/connected` (base URL `https://openpm.ai/api`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-connected-packages.md) for the provider-specific parameters and requirements.

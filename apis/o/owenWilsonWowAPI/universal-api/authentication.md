# Owen Wilson Wow API Universal API Authentication

Every Owen Wilson Wow API API request through MindCloud uses a MindCloud API key and the `connectionId` of a Owen Wilson Wow API connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Owen Wilson Wow API connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Owen Wilson Wow API action](README.md#actions-5) with your API key and `connectionId`.

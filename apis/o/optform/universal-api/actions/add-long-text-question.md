# Optform: Add Long Text Question

Creates a new long-text question in an Optform form.

```
POST https://connect.mindcloud.co/v1/universal/optform/latest/actions/add-long-text-question
```
...
Through the native Optform API, this operation is `POST /api/Form/questions` (base URL `https://optform.azure-api.net`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/add-long-text-question.md) for the provider-specific parameters and requirements.

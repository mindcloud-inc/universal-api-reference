...
```
PUT https://connect.mindcloud.co/v1/universal/oneAll/latest/actions/cast-vote
```
...
Through the native OneAll API, this operation is `PUT /loudvoice/votes/comments/<comment_token>/authors/<author_token>.json` (base URL `https://mindcloudco.api.oneall.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/cast-vote.md) for the provider-specific parameters and requirements.

# Once.to Universal API Authentication

Every Once.to API request through MindCloud uses a MindCloud API key and the `connectionId` of a Once.to connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Once.to connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Once.to action](README.md#actions-7) with your API key and `connectionId`.

# Ontraport: Retrieve Contact Object Meta

Retrieves metadata for contact objects in Ontraport.

```
GET https://connect.mindcloud.co/v1/universal/ontraport/latest/actions/retrieve-contact-object-meta
```
...
Through the native Ontraport API, this operation is `GET /Contacts/meta` (base URL `https://api.ontraport.com/1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/retrieve-contact-object-meta.md) for the provider-specific parameters and requirements.

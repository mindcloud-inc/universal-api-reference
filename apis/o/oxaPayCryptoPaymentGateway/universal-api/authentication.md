# OxaPay Crypto Payment Gateway Universal API Authentication

Every OxaPay Crypto Payment Gateway API request through MindCloud uses a MindCloud API key and the `connectionId` of a OxaPay Crypto Payment Gateway connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your OxaPay Crypto Payment Gateway connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [OxaPay Crypto Payment Gateway action](README.md#actions-13) with your API key and `connectionId`.

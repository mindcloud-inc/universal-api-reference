# Ollama Universal API Authentication

Every Ollama API request through MindCloud uses a MindCloud API key and the `connectionId` of a Ollama connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Ollama connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Ollama action](README.md#actions-10) with your API key and `connectionId`.

# Opportify: Get Email Batch Export Status

Retrieves the status of an email batch export job in Opportify.

```
GET https://connect.mindcloud.co/v1/universal/opportify/latest/actions/get-email-batch-export-status
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/opportify/latest/actions/get-email-batch-export-status?connectionId=$CONNECTION_ID&jobId=string&exportId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/opportify/latest/actions/get-email-batch-export-status?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `jobId` | string | yes | The unique identifier of the batch job. Format: uuid. Example: "84d22c8b-2cb6-4606-bfb1-361244a097e4". |
| `exportId` | string | yes | The unique identifier of the export job. Format: uuid. Example: "6f8d88ef-0896-4f69-90cd-7cc6ce5e6ddf". |
...
Through the native Opportify API, this operation is `GET /email/batch/:jobId/exports/:exportId` (base URL `https://api.opportify.ai/insights/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-email-batch-export-status.md) for the provider-specific parameters and requirements.

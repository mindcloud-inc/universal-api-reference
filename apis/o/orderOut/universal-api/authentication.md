# OrderOut Universal API Authentication

Every OrderOut API request through MindCloud uses a MindCloud API key and the `connectionId` of a OrderOut connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your OrderOut connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [OrderOut action](README.md#actions-10) with your API key and `connectionId`.

# Outlign Universal API Authentication

Every Outlign API request through MindCloud uses a MindCloud API key and the `connectionId` of a Outlign connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Outlign connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Outlign action](README.md#actions-38) with your API key and `connectionId`.

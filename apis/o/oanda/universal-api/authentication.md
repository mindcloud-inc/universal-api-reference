# Oanda Universal API Authentication

Every Oanda API request through MindCloud uses a MindCloud API key and the `connectionId` of a Oanda connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Oanda connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Oanda action](README.md#actions-13) with your API key and `connectionId`.

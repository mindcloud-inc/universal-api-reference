# OpenFDA: Count Animal Veterinary Event Records

Counts animal veterinary event records in OpenFDA.

```
GET https://connect.mindcloud.co/v1/universal/openFDA/latest/actions/count-animal-veterinary-event-records
```
...
Through the native OpenFDA API, this operation is `GET /animalandveterinary/event.json` (base URL `https://api.fda.gov`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/count-animal-veterinary-event-records.md) for the provider-specific parameters and requirements.

# One-Time Secret Universal API Authentication

Every One-Time Secret API request through MindCloud uses a MindCloud API key and the `connectionId` of a One-Time Secret connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your One-Time Secret connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [One-Time Secret action](README.md#actions-23) with your API key and `connectionId`.

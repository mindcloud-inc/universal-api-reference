# OneHash ERP Universal API Authentication

Every OneHash ERP API request through MindCloud uses a MindCloud API key and the `connectionId` of a OneHash ERP connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your OneHash ERP connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [OneHash ERP action](README.md#actions-1) with your API key and `connectionId`.

# Odoo Universal API Filtering

Filterable list actions accept a `where` query parameter written as an RSQL expression. MindCloud translates it into the filtering format Odoo expects, and each action page lists the fields available to filter.
...
## Odoo actions that support filtering

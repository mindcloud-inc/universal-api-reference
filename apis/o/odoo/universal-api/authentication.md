# Odoo Universal API Authentication

Every Odoo API request through MindCloud uses a MindCloud API key and the `connectionId` of a Odoo connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Odoo connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Odoo action](README.md#actions-76) with your API key and `connectionId`.

# OpenThesaurus Universal API Authentication

Every OpenThesaurus API request through MindCloud uses a MindCloud API key and the `connectionId` of a OpenThesaurus connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your OpenThesaurus connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [OpenThesaurus action](README.md#actions-1) with your API key and `connectionId`.

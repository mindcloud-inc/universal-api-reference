# Omniconvert Explore Universal API Authentication

Every Omniconvert Explore API request through MindCloud uses a MindCloud API key and the `connectionId` of a Omniconvert Explore connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Omniconvert Explore connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Omniconvert Explore action](README.md#actions-12) with your API key and `connectionId`.

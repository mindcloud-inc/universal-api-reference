# IP2Proxy Universal API Authentication

Every IP2Proxy API request through MindCloud uses a MindCloud API key and the `connectionId` of a IP2Proxy connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your IP2Proxy connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [IP2Proxy action](README.md#actions-2) with your API key and `connectionId`.

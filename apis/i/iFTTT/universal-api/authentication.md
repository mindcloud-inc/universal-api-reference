# IFTTT Universal API Authentication

Every IFTTT API request through MindCloud uses a MindCloud API key and the `connectionId` of a IFTTT connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your IFTTT connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [IFTTT action](README.md#actions-1) with your API key and `connectionId`.

# IP2Location IO Universal API Authentication

Every IP2Location IO API request through MindCloud uses a MindCloud API key and the `connectionId` of a IP2Location IO connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your IP2Location IO connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [IP2Location IO action](README.md#actions-1) with your API key and `connectionId`.

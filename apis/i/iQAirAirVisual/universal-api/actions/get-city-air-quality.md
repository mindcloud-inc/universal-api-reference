# IQAir AirVisual: Get City Air Quality



```
GET https://connect.mindcloud.co/v1/universal/iQAirAirVisual/latest/actions/get-city-air-quality
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/iQAirAirVisual/latest/actions/get-city-air-quality?connectionId=$CONNECTION_ID&country=USA&state=California&city=Los%20Angeles" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/iQAirAirVisual/latest/actions/get-city-air-quality?${params}`, {
  headers: {
...
| `city` | string | yes | City name exactly as returned by the List Cities action. Example: `Los Angeles`. |
...
| --- | --- | --- |
| `city` | string | City name for the requested air quality record. |
| `country` | string | Country name for the requested air quality record. |
...
| `location` | object | Geospatial location payload for the city. |
...
| `state` | string | State name for the requested air quality record. |

## Native endpoint

Through the native IQAir AirVisual API, this operation is `GET /v2/city` (base URL `https://api.airvisual.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-city-air-quality.md) for the provider-specific parameters and requirements.

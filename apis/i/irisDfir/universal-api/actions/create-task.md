# Iris Dfir: Create Task



```
POST https://connect.mindcloud.co/v1/universal/irisDfir/latest/actions/create-task
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/irisDfir/latest/actions/create-task" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/irisDfir/latest/actions/create-task', {
  method: 'POST',
  headers: {
...
| `taskTags` | string | no | Optional comma-separated task tags. |
...
Through the native Iris Dfir API, this operation is `POST /api/v2/cases/:case_identifier/tasks` (base URL `https://v200.beta.dfir-iris.org`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-task.md) for the provider-specific parameters and requirements.

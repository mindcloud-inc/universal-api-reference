...
```
GET https://connect.mindcloud.co/v1/universal/irisDfir/latest/actions/get-asset
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/irisDfir/latest/actions/get-asset?connectionId=$CONNECTION_ID&caseIdentifier=1&identifier=1" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/irisDfir/latest/actions/get-asset?${params}`, {
  headers: {
...
Through the native Iris Dfir API, this operation is `GET /api/v2/cases/:case_identifier/assets/:identifier` (base URL `https://v200.beta.dfir-iris.org`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-asset.md) for the provider-specific parameters and requirements.

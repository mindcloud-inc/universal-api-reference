# Iris Dfir: Update Asset



```
PUT https://connect.mindcloud.co/v1/universal/irisDfir/latest/actions/update-asset
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/irisDfir/latest/actions/update-asset" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/irisDfir/latest/actions/update-asset', {
  method: 'PUT',
  headers: {
...
Through the native Iris Dfir API, this operation is `PUT /api/v2/cases/:case_identifier/assets/:identifier` (base URL `https://v200.beta.dfir-iris.org`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-asset.md) for the provider-specific parameters and requirements.

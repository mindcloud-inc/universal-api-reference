# icanhazdadjoke Universal API Authentication

Every icanhazdadjoke API request through MindCloud uses a MindCloud API key and the `connectionId` of a icanhazdadjoke connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your icanhazdadjoke connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [icanhazdadjoke action](README.md#actions-4) with your API key and `connectionId`.

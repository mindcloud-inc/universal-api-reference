# Intervals.icu Universal API Authentication

Every Intervals.icu API request through MindCloud uses a MindCloud API key and the `connectionId` of a Intervals.icu connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Intervals.icu connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Intervals.icu action](README.md#actions-144) with your API key and `connectionId`.

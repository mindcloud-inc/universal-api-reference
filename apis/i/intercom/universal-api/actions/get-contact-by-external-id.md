# Intercom: Get Contact By External ID



```
GET https://connect.mindcloud.co/v1/universal/intercom/latest/actions/get-contact-by-external-id
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/intercom/latest/actions/get-contact-by-external-id?connectionId=$CONNECTION_ID&externalId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/intercom/latest/actions/get-contact-by-external-id?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `externalId` | string | yes | External contact identifier |
...
Through the native Intercom API, this operation is `GET /contacts/find_by_external_id/:external_id` (base URL `https://api.intercom.io`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-contact-by-external-id.md) for the provider-specific parameters and requirements.

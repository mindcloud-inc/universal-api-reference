# InviteReferrals Universal API Authentication

Every InviteReferrals API request through MindCloud uses a MindCloud API key and the `connectionId` of a InviteReferrals connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your InviteReferrals connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [InviteReferrals action](README.md#actions-2) with your API key and `connectionId`.

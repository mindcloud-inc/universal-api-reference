# Infobip: Get Email Message Logs



```
GET https://connect.mindcloud.co/v1/universal/infobip/latest/actions/get-email-message-logs
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/infobip/latest/actions/get-email-message-logs?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/infobip/latest/actions/get-email-message-logs?${params}`, {
  headers: {
...
Through the native Infobip API, this operation is `GET /email/4/logs` (base URL `https://rkpzwe.api.infobip.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-email-message-logs.md) for the provider-specific parameters and requirements.

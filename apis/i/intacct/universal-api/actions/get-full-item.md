# Sage Intacct: Get Full Item



```
GET https://connect.mindcloud.co/v1/universal/intacct/latest/actions/get-full-item
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/intacct/latest/actions/get-full-item?connectionId=$CONNECTION_ID&itemType=PROJECT&itemId=16296" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/intacct/latest/actions/get-full-item?${params}`, {
  headers: {
...
| `itemId` | string | yes | Default: `16296`. |
...
Through the native Sage Intacct API, this operation is `POST` (base URL `https://api.intacct.com/ia/xml/xmlgw.phtml`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-full-item.md) for the provider-specific parameters and requirements.

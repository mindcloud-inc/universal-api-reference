# Insightful: List Shared Settings

Retrieves shared settings from your Insightful account.

```
GET https://connect.mindcloud.co/v1/universal/insightful/latest/actions/list-shared-settings
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/insightful/latest/actions/list-shared-settings?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/insightful/latest/actions/list-shared-settings?${params}`, {
  headers: {
...
Through the native Insightful API, this operation is `GET /shared-settings` (base URL `https://app.insightful.io/api/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-shared-settings.md) for the provider-specific parameters and requirements.

# ImageRouter Universal API Authentication

Every ImageRouter API request through MindCloud uses a MindCloud API key and the `connectionId` of a ImageRouter connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your ImageRouter connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [ImageRouter action](README.md#actions-4) with your API key and `connectionId`.

# IN-D KYC India: Match Faces For UID

Retrieves face match results in IN-D KYC India by UID.

```
GET https://connect.mindcloud.co/v1/universal/iNDKYCIndia/latest/actions/match-faces-for-uid
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/iNDKYCIndia/latest/actions/match-faces-for-uid?connectionId=$CONNECTION_ID&uid=test-uid&image%5B%5D=iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8%2Fx8AAwMCAO%2B%2Fp9sAAAAASUVORK5CYII%3D%2CiVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8%2Fx8AAwMCAO%2B%2Fp9sAAAAASUVORK5CYII%3D" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/iNDKYCIndia/latest/actions/match-faces-for-uid?${params}`, {
  headers: {
...
Through the native IN-D KYC India API, this operation is `POST /api/facematch/{uid}` (base URL `https://api.kyc.in-d.ai`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/match-faces-for-uid.md) for the provider-specific parameters and requirements.

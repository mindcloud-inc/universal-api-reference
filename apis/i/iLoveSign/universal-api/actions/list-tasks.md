# iLoveSign: List Tasks



```
GET https://connect.mindcloud.co/v1/universal/iLoveSign/latest/actions/list-tasks
```
...
| --- | --- | --- | --- |
| `page` | number | no | One-based task results page. Default: `1`. |
| `tool` | string | no | Optional task tool filter, such as split or compress. |
| `status` | string | no | Optional task status filter, such as TaskSuccess. |
| `customInt` | number | no | Optional custom_int filter value. |

# imgix: Add Asset From Origin

Adds an asset from origin to imgix.

```
POST https://connect.mindcloud.co/v1/universal/imgix/latest/actions/add-asset-from-origin
```
...
Through the native imgix API, this operation is `POST sources/:sourceId/assets/add/:originPath` (base URL `https://api.imgix.com/api/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/add-asset-from-origin.md) for the provider-specific parameters and requirements.

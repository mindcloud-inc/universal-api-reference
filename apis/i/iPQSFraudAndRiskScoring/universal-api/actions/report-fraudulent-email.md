# IPQS Fraud and Risk Scoring: Report Fraudulent Email

Reports a fraudulent email address to IPQS.

```
POST https://connect.mindcloud.co/v1/universal/iPQSFraudAndRiskScoring/latest/actions/report-fraudulent-email
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/iPQSFraudAndRiskScoring/latest/actions/report-fraudulent-email" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/iPQSFraudAndRiskScoring/latest/actions/report-fraudulent-email', {
  method: 'POST',
  headers: {
...
| --- | --- | --- | --- |
| `email` | string | yes | Email address to report. |

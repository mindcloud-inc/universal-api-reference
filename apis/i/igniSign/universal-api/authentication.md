# IgniSign Universal API Authentication

Every IgniSign API request through MindCloud uses a MindCloud API key and the `connectionId` of a IgniSign connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your IgniSign connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [IgniSign action](README.md#actions-41) with your API key and `connectionId`.

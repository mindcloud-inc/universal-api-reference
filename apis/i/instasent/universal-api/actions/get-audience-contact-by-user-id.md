# Instasent: Get Audience Contact by User ID



```
GET https://connect.mindcloud.co/v1/universal/instasent/latest/actions/get-audience-contact-by-user-id
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/instasent/latest/actions/get-audience-contact-by-user-id?connectionId=$CONNECTION_ID&project=string&userId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/instasent/latest/actions/get-audience-contact-by-user-id?${params}`, {
  headers: {
...
Through the native Instasent API, this operation is `GET /project/:project/audience/user/:userId` (base URL `https://api.instasent.com/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-audience-contact-by-user-id.md) for the provider-specific parameters and requirements.

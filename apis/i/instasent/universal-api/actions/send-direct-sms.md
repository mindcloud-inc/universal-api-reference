# Instasent: Send Direct SMS



```
POST https://connect.mindcloud.co/v1/universal/instasent/latest/actions/send-direct-sms
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/instasent/latest/actions/send-direct-sms" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/instasent/latest/actions/send-direct-sms', {
  method: 'POST',
  headers: {
...
| `senderId` | string | yes | SMS sender identifier. |
| `audienceId` | string | yes | Audience identifier. |
| `text` | string | yes | SMS message text. |
| `allowUnicode` | boolean | no | Whether to allow Unicode characters in the SMS message. |
...
| `metadata.autoCreated` | boolean | Whether the audience contact was auto-created during the send request |

## Native endpoint

Through the native Instasent API, this operation is `POST /project/:project/channel/sms/sms/direct/:senderId/:audienceId` (base URL `https://api.instasent.com/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/send-direct-sms.md) for the provider-specific parameters and requirements.

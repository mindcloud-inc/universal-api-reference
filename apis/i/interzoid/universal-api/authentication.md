# Interzoid Universal API Authentication

Every Interzoid API request through MindCloud uses a MindCloud API key and the `connectionId` of a Interzoid connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Interzoid connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Interzoid action](README.md#actions-11) with your API key and `connectionId`.

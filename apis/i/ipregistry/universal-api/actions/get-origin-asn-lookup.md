# Ipregistry: Get Origin ASN Lookup



```
GET https://connect.mindcloud.co/v1/universal/ipregistry/latest/actions/get-origin-asn-lookup
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/ipregistry/latest/actions/get-origin-asn-lookup?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/ipregistry/latest/actions/get-origin-asn-lookup?${params}`, {
  headers: {
...
| Key | Type | Required | Description |
| --- | --- | --- | --- |
| `fields` | string | no | Optional response filter for origin ASN lookup results. Example: `name,location.country.code`. |
...
Through the native Ipregistry API, this operation is `GET /AS` (base URL `https://api.ipregistry.co`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-origin-asn-lookup.md) for the provider-specific parameters and requirements.

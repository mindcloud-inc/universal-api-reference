# Leadboxer: Update Custom Tracking Domain

Updates a custom tracking domain for a dataset in Leadboxer.

```
PUT https://connect.mindcloud.co/v1/universal/leadboxer/latest/actions/update-custom-tracking-domain
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/leadboxer/latest/actions/update-custom-tracking-domain" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/leadboxer/latest/actions/update-custom-tracking-domain', {
  method: 'PUT',
  headers: {
...
| --- | --- | --- | --- |
| `ctd` | string | yes | The custom tracking domain value. |

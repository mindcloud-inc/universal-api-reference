# Loqate Universal API Authentication

Every Loqate API request through MindCloud uses a MindCloud API key and the `connectionId` of a Loqate connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Loqate connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Loqate action](README.md#actions-20) with your API key and `connectionId`.

# LightwaveRF Lighting: Delete Device

Deletes an existing device from LightwaveRF Lighting.

```
DELETE https://connect.mindcloud.co/v1/universal/lightwaveRFLighting/latest/actions/delete-device
```
...
```bash
curl -X DELETE "https://connect.mindcloud.co/v1/universal/lightwaveRFLighting/latest/actions/delete-device?connectionId=$CONNECTION_ID&deviceId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/lightwaveRFLighting/latest/actions/delete-device?${params}`, {
  headers: {

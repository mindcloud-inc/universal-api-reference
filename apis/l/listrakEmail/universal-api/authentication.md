# Listrak Email Universal API Authentication

Every Listrak Email API request through MindCloud uses a MindCloud API key and the `connectionId` of a Listrak Email connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Listrak Email connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Listrak Email action](README.md#actions-7) with your API key and `connectionId`.

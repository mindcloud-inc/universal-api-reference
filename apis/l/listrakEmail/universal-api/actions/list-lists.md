...
```
GET https://connect.mindcloud.co/v1/universal/listrakEmail/latest/actions/list-lists
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/listrakEmail/latest/actions/list-lists?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/listrakEmail/latest/actions/list-lists?${params}`, {
  headers: {
...
Through the native Listrak Email API, this operation is `GET /v1/List` (base URL `https://api.listrak.com/email`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-lists.md) for the provider-specific parameters and requirements.

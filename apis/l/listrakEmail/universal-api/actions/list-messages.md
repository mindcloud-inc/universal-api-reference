# Listrak Email: List Messages



```
GET https://connect.mindcloud.co/v1/universal/listrakEmail/latest/actions/list-messages
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/listrakEmail/latest/actions/list-messages?connectionId=$CONNECTION_ID&limit=25&offset=0&listID=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/listrakEmail/latest/actions/list-messages?${params}`, {
  headers: {
...
Through the native Listrak Email API, this operation is `GET /v1/List/:listID/Message` (base URL `https://api.listrak.com/email`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/list-messages.md) for the provider-specific parameters and requirements.

# Logiwa Legacy WMS Universal API Authentication

Every Logiwa Legacy WMS API request through MindCloud uses a MindCloud API key and the `connectionId` of a Logiwa Legacy WMS connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Logiwa Legacy WMS connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Logiwa Legacy WMS action](README.md#actions-16) with your API key and `connectionId`.

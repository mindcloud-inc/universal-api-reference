# Lettermint Universal API Authentication

Every Lettermint API request through MindCloud uses a MindCloud API key and the `connectionId` of a Lettermint connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Lettermint connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Lettermint action](README.md#actions-3) with your API key and `connectionId`.

# LogMeIn Universal API Authentication

Every LogMeIn API request through MindCloud uses a MindCloud API key and the `connectionId` of a LogMeIn connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your LogMeIn connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [LogMeIn action](README.md#actions-40) with your API key and `connectionId`.

# Leadfeeder: Get Leads

Retrieves leads for an account in Leadfeeder by date range.

```
GET https://connect.mindcloud.co/v1/universal/leadfeeder/latest/actions/get-leads
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/leadfeeder/latest/actions/get-leads?connectionId=$CONNECTION_ID&limit=25&offset=0&startDate=2026-04-01&endDate=2026-04-13" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/leadfeeder/latest/actions/get-leads?${params}`, {
  headers: {
...
Through the native Leadfeeder API, this operation is `GET /accounts/:accountId/leads` (base URL `https://api.leadfeeder.com`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/get-leads.md) for the provider-specific parameters and requirements.

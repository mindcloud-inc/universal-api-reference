# Leadfeeder: Get Lead Visits

Retrieves visits for a lead in Leadfeeder by date range.

```
GET https://connect.mindcloud.co/v1/universal/leadfeeder/latest/actions/get-lead-visits
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/leadfeeder/latest/actions/get-lead-visits?connectionId=$CONNECTION_ID&limit=25&offset=0&leadId=string&startDate=2026-04-01&endDate=2026-04-13" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/leadfeeder/latest/actions/get-lead-visits?${params}`, {
  headers: {
...
Through the native Leadfeeder API, this operation is `GET /accounts/:accountId/leads/:leadId/visits` (base URL `https://api.leadfeeder.com`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/get-lead-visits.md) for the provider-specific parameters and requirements.

# Grouped Advanced Search with LeadIQ
...
- **Official documentation:** [Grouped Advanced Search](https://developer.leadiq.com/#query-groupedAdvancedSearch)
...
| `variables.input.limit` | body | `number` | no | Maximum number of company groups to return. |
| `variables.input.limitPerCompany` | body | `number` | no | Maximum number of people to return within each company group. |
...
| `variables.input.sortCompaniesBy[]` | body | `array<string>` | no | Optional array of company sort values such as NameAsc, SizeDesc, or IndustryAsc. |
...
| `variables.input.skip` | body | `number` | no | Number of matching company groups to skip before returning results. |

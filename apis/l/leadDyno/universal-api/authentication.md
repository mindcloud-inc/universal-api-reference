# LeadDyno Universal API Authentication

Every LeadDyno API request through MindCloud uses a MindCloud API key and the `connectionId` of a LeadDyno connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your LeadDyno connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [LeadDyno action](README.md#actions-40) with your API key and `connectionId`.

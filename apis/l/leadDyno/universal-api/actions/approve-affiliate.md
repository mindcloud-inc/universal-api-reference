# LeadDyno: Approve Affiliate

Approves an affiliate account in LeadDyno.

```
PUT https://connect.mindcloud.co/v1/universal/leadDyno/latest/actions/approve-affiliate
```
...
Through the native LeadDyno API, this operation is `POST /affiliates/:id/approve` (base URL `https://api.leaddyno.com/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/approve-affiliate.md) for the provider-specific parameters and requirements.

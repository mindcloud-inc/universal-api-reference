# LLMLayer Universal API Authentication

Every LLMLayer API request through MindCloud uses a MindCloud API key and the `connectionId` of a LLMLayer connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your LLMLayer connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [LLMLayer action](README.md#actions-31) with your API key and `connectionId`.

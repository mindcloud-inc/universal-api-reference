...
```
POST https://connect.mindcloud.co/v1/universal/linkly/latest/actions/create-link
```
...
| --- | --- | --- | --- |
| `url` | string | yes | URL. Required to create a new link. Example: `https://example.com`. |
| `name` | string | no | Optional display name for the link. Example: `Campaign landing page`. |

### Advanced

| Key | Type | Required | Description |
| --- | --- | --- | --- |
| `slug` | string | no | Optional custom short-code slug. Linkly's current runtime rejects custom slug values in this workspace with `slug must be empty`. Example: `spring-launch`. |
...
| `body_tags` | string | no | Optional custom body tags. |
| `deleted` | boolean | no | Whether the link should be marked deleted. |
| `fb_pixel_id` | string | no | Optional Facebook Pixel ID. |
| `ga4_tag_id` | string | no | Optional Google Analytics 4 tag ID. |
| `gtm_id` | string | no | Optional Google Tag Manager ID. |
| `head_tags` | string | no | Optional custom head tags. |
| `linkify_words` | string | no | Optional linkify words configuration. |
| `og_description` | string | no | Optional Open Graph description. |
| `og_image` | string | no | Optional Open Graph image URL. |
| `og_title` | string | no | Optional Open Graph title. |
| `replacements` | string | no | Optional replacement rules string. |
| `spam` | boolean | no | Whether the link should be marked as spam. |
...
Through the native Linkly API, this operation is `POST /link` (base URL `https://app.linklyhq.com/api/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-link.md) for the provider-specific parameters and requirements.

# LLMWhisperer Universal API Examples

These examples use the MindCloud API key and LLMWhisperer connection described in [authentication.md](authentication.md). Replace `$CONNECTION_ID` with the connection ID you copied from the Connections page.

## Get Usage Metrics

Retrieves account usage metrics from LLMWhisperer.

```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/lLMWhisperer/latest/actions/get-usage-metrics?connectionId=$CONNECTION_ID" \
  -H "Authorization: Bearer $MINDCLOUD_API_KEY"
```

```js
const params = new URLSearchParams({
  connectionId
});

const response = await fetch(`https://connect.mindcloud.co/v1/universal/lLMWhisperer/latest/actions/get-usage-metrics?${params}`, {
  headers: {
    Authorization: `Bearer ${process.env.MINDCLOUD_API_KEY}`
  }
});

const { success, data } = await response.json();
```

Example response:

```json
{
  "success": true,
  "data": [
    {
      "current_page_count": 1,
      "current_page_count_excel": 1,
      "current_page_count_form": 1,
      "current_page_count_high_quality": 1,
      "current_page_count_low_cost": 1,
      "current_page_count_native_text": 1,
      "current_page_count_signature": 1,
      "current_page_count_table": 1,
      "daily_quota": 1,
      "monthly_quota": 1,
      "overage_page_count": 1,
      "subscription_plan": "string",
      "today_page_count": 1
    }
  ],
  "meta": {}
}
```

See the full [Get Usage Metrics action reference](actions/get-usage-metrics.md), or [try it interactively](https://mindcloud.co/docs/universal/rest/lLMWhisperer/latest/actions/get-usage-metrics).

## Register Webhook Endpoint

Registers a webhook endpoint in LLMWhisperer.

```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/lLMWhisperer/latest/actions/register-webhook-endpoint" \
  -H "Authorization: Bearer $MINDCLOUD_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
  "connectionId": "$CONNECTION_ID",
  "url": "https://example.com",
  "authToken": "string",
  "webhookName": "Ava Chen"
}'
```

```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/lLMWhisperer/latest/actions/register-webhook-endpoint', {
  method: 'POST',
  headers: {
    Authorization: `Bearer ${process.env.MINDCLOUD_API_KEY}`,
    'Content-Type': 'application/json'
  },
  body: JSON.stringify({
    connectionId,
    "url": "https://example.com",
    "authToken": "string",
    "webhookName": "Ava Chen"
  })
});

const { success, data } = await response.json();
```

Example response:

```json
{
  "success": true,
  "data": [
    {
      "message": "string"
    }
  ],
  "meta": {}
}
```

See the full [Register Webhook Endpoint action reference](actions/register-webhook-endpoint.md), or [try it interactively](https://mindcloud.co/docs/universal/rest/lLMWhisperer/latest/actions/register-webhook-endpoint).

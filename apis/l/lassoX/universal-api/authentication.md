# Lasso X Universal API Authentication

Every Lasso X API request through MindCloud uses a MindCloud API key and the `connectionId` of a Lasso X connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Lasso X connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Lasso X action](README.md#actions-24) with your API key and `connectionId`.

# Loggly (Send Data) Universal API Authentication

Every Loggly (Send Data) API request through MindCloud uses a MindCloud API key and the `connectionId` of a Loggly (Send Data) connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Loggly (Send Data) connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Loggly (Send Data) action](README.md#actions-6) with your API key and `connectionId`.

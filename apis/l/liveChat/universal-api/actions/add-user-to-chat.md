# LiveChat: Add User To Chat

Updates a chat by adding a user in LiveChat.

```
PUT https://connect.mindcloud.co/v1/universal/liveChat/latest/actions/add-user-to-chat
```
...
| `userType` | string | yes | Possible values: agent or customer. |
| `visibility` | string | yes | Possible values: all or agents. |
...
Through the native LiveChat API, this operation is `POST /add_user_to_chat` (base URL `https://api.livechatinc.com/v3.6/agent/action`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/add-user-to-chat.md) for the provider-specific parameters and requirements.

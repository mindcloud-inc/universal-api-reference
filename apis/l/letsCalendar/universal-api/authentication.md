# Let's Calendar Universal API Authentication

Every Let's Calendar API request through MindCloud uses a MindCloud API key and the `connectionId` of a Let's Calendar connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Let's Calendar connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Let's Calendar action](README.md#actions-17) with your API key and `connectionId`.

# Lnk.Bio Universal API Authentication

Every Lnk.Bio API request through MindCloud uses a MindCloud API key and the `connectionId` of a Lnk.Bio connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Lnk.Bio connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Lnk.Bio action](README.md#actions-5) with your API key and `connectionId`.

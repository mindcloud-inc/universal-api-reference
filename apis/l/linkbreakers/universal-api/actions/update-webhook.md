# Linkbreakers: Update a Webhook

Updates an existing webhook in Linkbreakers.

```
PUT https://connect.mindcloud.co/v1/universal/linkbreakers/latest/actions/update-webhook
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/linkbreakers/latest/actions/update-webhook" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/linkbreakers/latest/actions/update-webhook', {
  method: 'PUT',
  headers: {
...
| `endpointUrl` | string | no | The webhook endpoint URL. |
...
| --- | --- | --- |
| `webhook` | object | Updated webhook configuration. |

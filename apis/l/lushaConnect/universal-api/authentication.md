# Lusha Connect Universal API Authentication

Every Lusha Connect API request through MindCloud uses a MindCloud API key and the `connectionId` of a Lusha Connect connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Lusha Connect connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Lusha Connect action](README.md#actions-5) with your API key and `connectionId`.

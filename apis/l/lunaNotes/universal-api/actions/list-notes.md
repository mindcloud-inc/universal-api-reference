# LunaNotes: List Notes

Retrieves notes from LunaNotes.

```
GET https://connect.mindcloud.co/v1/universal/lunaNotes/latest/actions/list-notes
```
...
| `isPublic` | boolean | no | Filter by public visibility status. |
| `status` | string | no | Filter by note lifecycle status. |
...
Through the native LunaNotes API, this operation is `GET /v1/notes` (base URL `https://api.lunanotes.io`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/list-notes.md) for the provider-specific parameters and requirements.

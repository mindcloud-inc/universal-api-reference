# lc.cx Universal API Authentication

Every lc.cx API request through MindCloud uses a MindCloud API key and the `connectionId` of a lc.cx connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your lc.cx connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [lc.cx action](README.md#actions-18) with your API key and `connectionId`.

# Lettr Universal API Authentication

Every Lettr API request through MindCloud uses a MindCloud API key and the `connectionId` of a Lettr connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Lettr connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Lettr action](README.md#actions-32) with your API key and `connectionId`.

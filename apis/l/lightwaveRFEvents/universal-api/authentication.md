# LightwaveRF Events Universal API Authentication

Every LightwaveRF Events API request through MindCloud uses a MindCloud API key and the `connectionId` of a LightwaveRF Events connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your LightwaveRF Events connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [LightwaveRF Events action](README.md#actions-1) with your API key and `connectionId`.

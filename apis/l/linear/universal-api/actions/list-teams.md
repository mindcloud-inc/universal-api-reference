...
```
GET https://connect.mindcloud.co/v1/universal/linear/latest/actions/list-teams
```
...
Through the native Linear API, this operation is `POST` (base URL `https://api.linear.app/graphql/`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/list-teams.md) for the provider-specific parameters and requirements.

# UiPath Orchestrator Universal API Authentication

Every UiPath Orchestrator API request through MindCloud uses a MindCloud API key and the `connectionId` of a UiPath Orchestrator connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your UiPath Orchestrator connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [UiPath Orchestrator action](README.md#actions-30) with your API key and `connectionId`.

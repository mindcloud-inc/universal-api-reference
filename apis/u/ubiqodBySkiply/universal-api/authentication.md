# Ubiqod by Skiply Universal API Authentication

Every Ubiqod by Skiply API request through MindCloud uses a MindCloud API key and the `connectionId` of a Ubiqod by Skiply connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Ubiqod by Skiply connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Ubiqod by Skiply action](README.md#actions-27) with your API key and `connectionId`.

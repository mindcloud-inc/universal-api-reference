# Underdog Protocol: Create Webhook

Creates a new webhook in Underdog Protocol.

```
POST https://connect.mindcloud.co/v1/universal/underdogProtocol/latest/actions/create-webhook
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/underdogProtocol/latest/actions/create-webhook" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/underdogProtocol/latest/actions/create-webhook', {
  method: 'POST',
  headers: {
...
| --- | --- | --- | --- |
| `url` | string | yes | Encoded URL for the callback endpoint. |

# Unbounce Universal API Authentication

Every Unbounce API request through MindCloud uses a MindCloud API key and the `connectionId` of a Unbounce connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Unbounce connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Unbounce action](README.md#actions-18) with your API key and `connectionId`.

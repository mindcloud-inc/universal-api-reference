# Unkey: Get API key by hash

Retrieves an API key from Unkey by hash.

```
GET https://connect.mindcloud.co/v1/universal/unkey/latest/actions/get-api-key-by-hash
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/unkey/latest/actions/get-api-key-by-hash?connectionId=$CONNECTION_ID&key=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/unkey/latest/actions/get-api-key-by-hash?${params}`, {
  headers: {
...
Through the native Unkey API, this operation is `POST /v2/keys.whoami` (base URL `https://api.unkey.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-api-key-by-hash.md) for the provider-specific parameters and requirements.

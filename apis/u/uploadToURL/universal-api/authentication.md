# Upload to URL Universal API Authentication

Every Upload to URL API request through MindCloud uses a MindCloud API key and the `connectionId` of a Upload to URL connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Upload to URL connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Upload to URL action](README.md#actions-4) with your API key and `connectionId`.

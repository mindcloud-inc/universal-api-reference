# Userback Universal API Authentication

Every Userback API request through MindCloud uses a MindCloud API key and the `connectionId` of a Userback connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Userback connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Userback action](README.md#actions-23) with your API key and `connectionId`.

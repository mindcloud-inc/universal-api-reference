# UK Bank Holidays Universal API Authentication

Every UK Bank Holidays API request through MindCloud uses a MindCloud API key and the `connectionId` of a UK Bank Holidays connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your UK Bank Holidays connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [UK Bank Holidays action](README.md#actions-1) with your API key and `connectionId`.

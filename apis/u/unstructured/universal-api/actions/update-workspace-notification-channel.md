# Unstructured: Update Workspace Notification Channel

Updates a workspace notification channel in Unstructured.

```
PUT https://connect.mindcloud.co/v1/universal/unstructured/latest/actions/update-workspace-notification-channel
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/unstructured/latest/actions/update-workspace-notification-channel" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/unstructured/latest/actions/update-workspace-notification-channel', {
  method: 'PUT',
  headers: {
...
Through the native Unstructured API, this operation is `PATCH /notifications/channels/:channel_id` (base URL `https://platform.unstructuredapp.io/api/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-workspace-notification-channel.md) for the provider-specific parameters and requirements.

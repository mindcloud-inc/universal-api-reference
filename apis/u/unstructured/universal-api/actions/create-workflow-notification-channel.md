# Unstructured: Create Workflow Notification Channel

Creates a workflow notification channel in Unstructured.

```
POST https://connect.mindcloud.co/v1/universal/unstructured/latest/actions/create-workflow-notification-channel
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/unstructured/latest/actions/create-workflow-notification-channel" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/unstructured/latest/actions/create-workflow-notification-channel', {
  method: 'POST',
  headers: {
...
Through the native Unstructured API, this operation is `POST /workflows/:workflow_id/notifications/channels` (base URL `https://platform.unstructuredapp.io/api/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-workflow-notification-channel.md) for the provider-specific parameters and requirements.

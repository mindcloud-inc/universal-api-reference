# UProc: Check Credit Card Checksum



```
GET https://connect.mindcloud.co/v1/universal/uProc/latest/actions/check-credit-card-checksum
```
...
| --- | --- | --- | --- |
| `credit_card` | string | yes | Credit card number to validate. Default: `4024007151839544`. |
...
Through the native UProc API, this operation is `POST /process` (base URL `https://api.uproc.io/api/v2`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/check-credit-card-checksum.md) for the provider-specific parameters and requirements.

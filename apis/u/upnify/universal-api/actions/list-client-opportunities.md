# Upnify: List Client Opportunities

Retrieves opportunities for a client in Upnify.

```
GET https://connect.mindcloud.co/v1/universal/upnify/latest/actions/list-client-opportunities
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/upnify/latest/actions/list-client-opportunities?connectionId=$CONNECTION_ID&tkCliente=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/upnify/latest/actions/list-client-opportunities?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `tkCliente` | string | yes | El código token que identifica a un cliente de manera única dentro del CRM, del cual requerimos consultar las oportunidades asociadas. ¿Dónde obtengo el token? |
...
| `concepto` | string | Indica el concepto de la oportunidad asociada al cliente. |
...
| `empresa` | string | Contiene el nombre de la empresa a la que pertenece el cliente. |
...
| `montoPagado` | number | Contiene un monto que determina una cantidad monetaria a cubrir por el cliente. |
...
| `tieneProductos` | number | Indica si el cliente tiene asociado productos. |
...
| `tkProspecto` | string | Código token que corresponde e identifica de forma única a un cliente en el sistema. |
...
Through the native Upnify API, this operation is `GET v4/clientes/:tkCliente/oportunidades` (base URL `https://api.upnify.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-client-opportunities.md) for the provider-specific parameters and requirements.

...
```
POST https://connect.mindcloud.co/v1/universal/upnify/latest/actions/create-user
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/upnify/latest/actions/create-user" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/upnify/latest/actions/create-user', {
  method: 'POST',
  headers: {
...
| --- | --- | --- | --- |
| `nombre` | string | yes | Contiene el nombre de un usuario. |
| `apellidos` | string | yes | Guarda los apellidos de un usuario. |
| `iniciales` | string | no | Almacena la primera letra del nombre y apellido de un usuario. |
| `email` | string | yes | Almacena el correo electrónico de un usuario. |
| `tkGrupo` | string | yes | Código token que corresponde a un grupo de usuarios al que pertenecerá el nuevo usuario. ¿Dónde obtengo el token? |
| `nivel` | number | yes | Código que define el nivel de privilegio de un usuario. - 1 Auditor. - 2 Gerente de ventas. - 3 Ejecutivo de ventas. |
| `verSistema` | number | no | Este código indica si el usuario tendrá permisos de administrador en el sistema, este valor es 1 cuando se desea crear un usuario de nivel "Administrador del Sistema" o "Gerente de ventas" - 0 No es administrador. - 1 Es administrador. Default: `0`. |
| `contrasenia1` | string | yes | Almacena la contraseña establecida para el usuario. |
| `contrasenia2` | string | yes | Se repite la contraseña establecida para el usuario. |
...
| `importar` | number | no | Código que indica si el usuario tiene permisos para realizar importaciones. - 1 No permitido. - 0 Permitido. Default: `0`. |

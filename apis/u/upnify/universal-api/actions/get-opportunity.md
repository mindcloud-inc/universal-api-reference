# Upnify: Get Opportunity

Retrieves an opportunity from Upnify.

```
GET https://connect.mindcloud.co/v1/universal/upnify/latest/actions/get-opportunity
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/upnify/latest/actions/get-opportunity?connectionId=$CONNECTION_ID&tkOportunidad=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/upnify/latest/actions/get-opportunity?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `tkOportunidad` | string | yes | Identificador unico de una oportunidad en el sistema, de la cual se requiere obtener los detalles. ¿Dónde obtengo el token? |
...
| `certeza` | number | Indica el valor numérico de una certeza en una oportunidad. |
| `certezaTexto` | string | Muestra el nombre del valor de la certeza. |
| `cierreEstimado` | date | Indica una fecha con la estimación para convertir una oportunidad en venta. |
...
| `cp100o` | string | Contiene el valor del centésimo campo personalizado para oportunidades, el cual es establecido por el usuario. |
| `cp1e` | string | Contiene el valor del primer campo personalizado para empresas, el cual es establecido por el usuario. |
| `cp1o` | string | Contiene el valor del primer campo personalizado para oportunidades, el cual es establecido por el usuario. |
...
| `descuentoGeneral` | number | Contiene una cantidad monetaria que representa un descuento general aplicado. |
...
| `fechaCreacion` | date | Contiene un formato compuesto de fecha y hora que indica cuando se creó la oportunidad. |
...
| `jsonSubtotales` | object | Contiene un arreglo que contiene informacion de la cotizacion. |
...
| `monedaSimbolo` | string | Contiene el símbolo de la moneda utilizada. |
...
| `oCatalogo1` | string | Primer catálogo personalizado, contiene el id del valor seleccionado de los catálogos adicionales para oportunidades, los cuales son elegidos por el usuario. |
| `oCatalogo2` | string | Segundo catálogo personalizado, contiene el id del valor seleccionado de los catálogos adicionales para oportunidades, los cuales son elegidos por el usuario. |
| `oCatalogo3` | string | Tercer catálogo personalizado, contiene el id del valor seleccionado de los catálogos adicionales para oportunidades, los cuales son elegidos por el usuario. |
...
| `pCatalogo2` | string | Segundo catálogo personalizado, contiene el id del valor seleccionado de los catálogos adicionales para prospectos, los cuales son elegidos por el usuario. |
| `pCatalogo3` | string | Tercer catálogo personalizado, contiene el id del valor seleccionado de los catálogos adicionales para prospectos, los cuales son elegidos por el usuario. |
| `perdida` | number | Indica si la oportunidad se encuentra perdida.  - 0 Oportunidad activa.  - 1 Oportuniad perdida. |
...
| `separadorDecimal` | string | El simbolo utilizado para separar los numeros decimales. |
| `separadorMil` | string | El simbolo utilizado para separar las milecimas de los numeros. |
...
| `tkCerteza` | string | Identificador unico de una certeza. |
...
| `tkMoneda` | string | Identificador unico de una moneda del sistema utilizada en la oportunidad. |
...
| `unicode` | string | Contiene el valor decimal de un tipo de moneda al utilizar una tabla unicode. |
...
| `vencida` | number | Indica si la oportunidad tiene un monto vencido.  - 0 Sin montos vencidos.  - 1 Montos vencidos. |

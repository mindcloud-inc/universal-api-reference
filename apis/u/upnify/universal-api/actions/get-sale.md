...
Retrieves a sale from Upnify.

```
GET https://connect.mindcloud.co/v1/universal/upnify/latest/actions/get-sale
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/upnify/latest/actions/get-sale?connectionId=$CONNECTION_ID&tkVenta=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/upnify/latest/actions/get-sale?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `tkVenta` | string | yes | Identificador unico de una venta en el sistema, de la cual se requiere obtener los detalles. ¿Dónde obtengo el token? |
...
| `monedaSimbolo` | string | Contiene el símbolo de la moneda. |
...
| `tkFase` | string | Identificador unico de a una fase de prospecto. |
...
| `tkUsuario` | string | Identificador de un usuario o ejecutivo del sistema. |
| `tkVenta` | string | Códigp token que corresponde a una venta registrada en el sistema. |
...
| `unicode` | number | Indica el valor decimal unicode que identifica al símbolo de moneda. |

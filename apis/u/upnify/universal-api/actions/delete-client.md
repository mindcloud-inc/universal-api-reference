# Upnify: Delete Client

Deletes an existing client from Upnify.

```
DELETE https://connect.mindcloud.co/v1/universal/upnify/latest/actions/delete-client
```
...
```bash
curl -X DELETE "https://connect.mindcloud.co/v1/universal/upnify/latest/actions/delete-client?connectionId=$CONNECTION_ID&tkCliente=string&tkRazonPerdida=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/upnify/latest/actions/delete-client?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `tkCliente` | string | yes | El código token que identifica a un cliente de manera única dentro del CRM, el cual se requiere descartar. ¿Dónde obtengo el token? |

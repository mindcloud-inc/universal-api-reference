# Upnify: List Opportunities

Retrieves opportunities from Upnify.

```
GET https://connect.mindcloud.co/v1/universal/upnify/latest/actions/list-opportunities
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/upnify/latest/actions/list-opportunities?connectionId=$CONNECTION_ID&limit=25&offset=0" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/upnify/latest/actions/list-opportunities?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `aprobacionEstado` | number | no | Indica el status de asignacion de contacto a compania. - 0 Aprobado. - 1 Pendiente. - 2 Rechazado. |
...
| `periodoCierre` | string | no | Permite filtrar los registros por fecha de cierre estimado. |
| `tkCerteza` | string | no | Permite filtrar los registros por una certeza de cierre en particular. |
...
| `monedaPorDefecto` | string | Almacena el código de la moneda que se tiene por defecto. |

# Usedesk Universal API Authentication

Every Usedesk API request through MindCloud uses a MindCloud API key and the `connectionId` of a Usedesk connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Usedesk connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Usedesk action](README.md#actions-24) with your API key and `connectionId`.

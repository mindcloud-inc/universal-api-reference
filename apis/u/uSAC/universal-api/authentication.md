# USAC Universal API Authentication

Every USAC API request through MindCloud uses a MindCloud API key and the `connectionId` of a USAC connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your USAC connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [USAC action](README.md#actions-4) with your API key and `connectionId`.

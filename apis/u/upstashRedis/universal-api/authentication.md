# Upstash Redis Universal API Authentication

Every Upstash Redis API request through MindCloud uses a MindCloud API key and the `connectionId` of a Upstash Redis connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Upstash Redis connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Upstash Redis action](README.md#actions-257) with your API key and `connectionId`.

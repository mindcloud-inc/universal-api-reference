...
```
GET https://connect.mindcloud.co/v1/universal/uON/latest/actions/list-leads
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/uON/latest/actions/list-leads?connectionId=$CONNECTION_ID&date_from=2026-05-07T12%3A00%3A00.000Z&date_to=2026-05-07T12%3A00%3A00.000Z" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/uON/latest/actions/list-leads?${params}`, {
  headers: {
...
Through the native U-ON API, this operation is `GET /leads/{date_from}/{date_to}/{page}.json` (base URL `https://api.u-on.ru/{key}`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-leads.md) for the provider-specific parameters and requirements.

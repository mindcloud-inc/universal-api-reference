...
Retrieves a tourist record from U-ON.

```
GET https://connect.mindcloud.co/v1/universal/uON/latest/actions/get-tourist
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/uON/latest/actions/get-tourist?connectionId=$CONNECTION_ID&id=1" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/uON/latest/actions/get-tourist?${params}`, {
  headers: {
...
Through the native U-ON API, this operation is `GET /user/{id}.json` (base URL `https://api.u-on.ru/{key}`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-tourist.md) for the provider-specific parameters and requirements.

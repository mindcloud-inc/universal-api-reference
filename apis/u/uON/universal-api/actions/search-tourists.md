# U-ON: Search Tourists

Finds tourists in U-ON by search criteria.

```
GET https://connect.mindcloud.co/v1/universal/uON/latest/actions/search-tourists
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/uON/latest/actions/search-tourists?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/uON/latest/actions/search-tourists?${params}`, {
  headers: {
...
| Key | Type | Required | Description |
| --- | --- | --- | --- |
| `passport` | string | no | Серия и номер гражданского паспорта / Passport number |
| `zagran_passport` | string | no | Серия и номер заграничного паспорта / Foreign passport number |
| `vkontakte` | string | no | Аккаунт в Vkontakte / Vkontakte account |
| `facebook` | string | no | Аккаунт в Facebook / Facebook account |
| `odnoklassniki` | string | no | Аккаунт в Odnoklassniki / Odnoklassniki account |
| `telegram` | string | no | Аккаунт в Telegram / Telegram account |
| `max` | string | no | Аккаунт в MAX / MAX account |
| `whatsapp` | string | no | Аккаунт в Whatsapp / Whatsapp account |
| `viber` | string | no | Аккаунт в Viber / Viber account |
| `instagram` | string | no | Аккаунт в Instagram / Instagram account |
| `label_ids` | string | no | ID меток через запятую / Labels ID (delimited by comma) |
| `part_match` | number | no | Полное совпадение (по-умолчанию) или частичное (=1) / Full match (by default) or part match (=1) |
...
Through the native U-ON API, this operation is `POST /user/search.json` (base URL `https://api.u-on.ru/{key}`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/search-tourists.md) for the provider-specific parameters and requirements.

# U-ON: List Requests by Tourist

Retrieves request records for a U-ON tourist.

```
GET https://connect.mindcloud.co/v1/universal/uON/latest/actions/list-requests-by-tourist
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/uON/latest/actions/list-requests-by-tourist?connectionId=$CONNECTION_ID&id=1" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/uON/latest/actions/list-requests-by-tourist?${params}`, {
  headers: {
...
Through the native U-ON API, this operation is `GET /request-by-tourist/{id}/{page}.json` (base URL `https://api.u-on.ru/{key}`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-requests-by-tourist.md) for the provider-specific parameters and requirements.

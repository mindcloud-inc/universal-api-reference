# U-ON: Search Requests

Finds requests in U-ON by search criteria.

```
GET https://connect.mindcloud.co/v1/universal/uON/latest/actions/search-requests
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/uON/latest/actions/search-requests?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/uON/latest/actions/search-requests?${params}`, {
  headers: {
...
| Key | Type | Required | Description |
| --- | --- | --- | --- |
| `date_lead_create_from` | date | no | Дата создания обращения (от), формат YYYY-mm-dd HH:ii / Create date of request (from) |
| `date_lead_create_to` | date | no | Дата создания обращения (до), формат YYYY-mm-dd HH:ii / Create date of request (till) |
| `date_create_from` | date | no | Дата создания заявки (от), формат YYYY-mm-dd HH:ii / Create date of request (from) |
| `date_create_to` | date | no | Дата создания заявки (до), формат YYYY-mm-dd HH:ii / Create date of request (till) |
| `date_begin_from` | date | no | Дата начала заявки (от), формат YYYY-mm-dd HH:ii / Start date of request (from) |
| `date_begin_to` | date | no | Дата начала заявки (до), формат YYYY-mm-dd HH:ii / Start date of request (till) |
| `date_end_from` | date | no | Дата окончания заявки (от), формат YYYY-mm-dd HH:ii / End date of request (from) |
| `date_end_to` | date | no | Дата окончания заявки (до), формат YYYY-mm-dd HH:ii / End date of request (till) |
| `r_id_system` | string | no | Системный ID заявки / Request's system ID |
| `r_id_internal` | string | no | Внутренний ID заявки / Request's internal ID |
| `reservation_number` | string | no | Номер брони / Request's reservation number |
...
| `tourist_ids` | string | no | ID туристов (через запятую) / Tourist IDs (delimited by comma) |
| `source_ids` | string | no | ID источников (через запятую) / Source IDs (delimited by comma) |
| `type_ids` | string | no | ID типов заявок (через запятую) / Request type IDs (delimited by comma) |
| `status_ids` | string | no | ID статусов (через запятую) / Status IDs (delimited by comma) |
| `pay_status_ids` | string | no | ID статусов по оплате (через запятую) / Pay status IDs (delimited by comma) |
| `service_type_ids` | string | no | ID типов услуг внутри заявки (через запятую) / Service type IDs inside request (delimited by comma) |
| `ext_fields[]` | array<string> | no | Массив дополнительных полей и их значений в формате [ID поля1 => [значение, значение, ...], ID поля2 => [значение, значение, ...]] / Array of extended fields with values in format [ID field1 => [value, value, ...], ID field2 => [value, value, ...]] |
| `calc_price_from` | number | no | Стоимость для клиента (от) / The cost for the client (from) |
| `calc_price_to` | number | no | Стоимость для клиента (до) / The cost for the client (till) |
| `calc_price_netto_from` | number | no | Себестоимость для клиента (от) / Netto cost for the client (from) |
| `calc_price_netto_to` | number | no | Себестоимость для клиента (до) / Netto cost for the client (till) |
| `calc_client_from` | number | no | Оплаты клиента (от) / Client payments (from) |
| `calc_client_to` | number | no | Оплаты клиента (до) / Client payments (till) |
| `calc_partner_from` | number | no | Оплаты партнерам (от) / Partner payments (from) |
| `calc_partner_to` | number | no | Оплаты партнерам (до) / Partner payments (till) |
...
Through the native U-ON API, this operation is `POST /request/search.json` (base URL `https://api.u-on.ru/{key}`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/search-requests.md) for the provider-specific parameters and requirements.

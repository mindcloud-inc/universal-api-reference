# USAJOBS: Search Jobs By Pay Grade Range

Finds jobs in USAJOBS by pay grade range.

```
GET https://connect.mindcloud.co/v1/universal/uSAJOBS/latest/actions/search-jobs-by-pay-grade-range
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/uSAJOBS/latest/actions/search-jobs-by-pay-grade-range?connectionId=$CONNECTION_ID&limit=25&offset=0" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/uSAJOBS/latest/actions/search-jobs-by-pay-grade-range?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `payGradeHigh` | string | no | Maximum GS-equivalent grade from 01 through 15. |
| `payGradeLow` | string | no | Minimum GS-equivalent grade from 01 through 15. |
...
Through the native USAJOBS API, this operation is `GET /api/Search` (base URL `https://data.usajobs.gov`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/search-jobs-by-pay-grade-range.md) for the provider-specific parameters and requirements.

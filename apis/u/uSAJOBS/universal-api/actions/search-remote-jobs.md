# USAJOBS: Search Remote Jobs

Finds remote job postings in USAJOBS.

```
GET https://connect.mindcloud.co/v1/universal/uSAJOBS/latest/actions/search-remote-jobs
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/uSAJOBS/latest/actions/search-remote-jobs?connectionId=$CONNECTION_ID&limit=25&offset=0" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/uSAJOBS/latest/actions/search-remote-jobs?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `remoteIndicator` | string | no | True returns only remote jobs; False excludes remote jobs. |
...
Through the native USAJOBS API, this operation is `GET /api/Search` (base URL `https://data.usajobs.gov`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/search-remote-jobs.md) for the provider-specific parameters and requirements.

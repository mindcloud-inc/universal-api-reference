# USAJOBS: Search Jobs By Salary Range

Finds jobs in USAJOBS by salary range.

```
GET https://connect.mindcloud.co/v1/universal/uSAJOBS/latest/actions/search-jobs-by-salary-range
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/uSAJOBS/latest/actions/search-jobs-by-salary-range?connectionId=$CONNECTION_ID&limit=25&offset=0" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/uSAJOBS/latest/actions/search-jobs-by-salary-range?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `remunerationMaximumAmount` | string | no | Maximum salary amount used for salary bucket matching. |
| `remunerationMinimumAmount` | string | no | Minimum salary amount used for salary bucket matching. |
...
| `matchedObjectDescriptor.userArea.details.agencyContactWebsite` | string |  |
...
| `matchedObjectDescriptor.userArea.details.benefitsUrl` | string |  |
...
| `matchedObjectDescriptor.userArea.details.mCOTags[]` | string |  |
...
Through the native USAJOBS API, this operation is `GET /api/Search` (base URL `https://data.usajobs.gov`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/search-jobs-by-salary-range.md) for the provider-specific parameters and requirements.

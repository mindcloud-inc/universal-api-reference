# USAJOBS: Search Jobs By Hiring Path

Finds jobs in USAJOBS by hiring path.

```
GET https://connect.mindcloud.co/v1/universal/uSAJOBS/latest/actions/search-jobs-by-hiring-path
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/uSAJOBS/latest/actions/search-jobs-by-hiring-path?connectionId=$CONNECTION_ID&limit=25&offset=0" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/uSAJOBS/latest/actions/search-jobs-by-hiring-path?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `hiringPath` | string | no | Hiring path code such as public or vet. |
...
Through the native USAJOBS API, this operation is `GET /api/Search` (base URL `https://data.usajobs.gov`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/search-jobs-by-hiring-path.md) for the provider-specific parameters and requirements.

# USAJOBS: Search Jobs By Location Radius

Finds jobs in USAJOBS within a location radius.

```
GET https://connect.mindcloud.co/v1/universal/uSAJOBS/latest/actions/search-jobs-by-location-radius
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/uSAJOBS/latest/actions/search-jobs-by-location-radius?connectionId=$CONNECTION_ID&limit=25&offset=0" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/uSAJOBS/latest/actions/search-jobs-by-location-radius?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `locationName` | string | no | Base location used with Radius. |
| `radius` | string | no | Radius to expand the location search. |
...
| `matchedObjectDescriptor.subAgency` | string |  |
...
| `matchedObjectDescriptor.userArea.details.subAgencyName` | string |  |
...
Through the native USAJOBS API, this operation is `GET /api/Search` (base URL `https://data.usajobs.gov`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/search-jobs-by-location-radius.md) for the provider-specific parameters and requirements.

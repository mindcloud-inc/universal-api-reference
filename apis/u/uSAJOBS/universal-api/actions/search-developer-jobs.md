# USAJOBS: Search Developer Jobs

Finds developer job postings in USAJOBS.

```
GET https://connect.mindcloud.co/v1/universal/uSAJOBS/latest/actions/search-developer-jobs
```
...
| --- | --- | --- | --- |
| `keyword` | string | no | Search term used to find matching job announcements. |
...
Through the native USAJOBS API, this operation is `GET /api/Search` (base URL `https://data.usajobs.gov`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/search-developer-jobs.md) for the provider-specific parameters and requirements.

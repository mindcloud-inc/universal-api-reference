# USAJOBS: Search Jobs By Candidate Eligibility

Finds jobs in USAJOBS by candidate eligibility.

```
GET https://connect.mindcloud.co/v1/universal/uSAJOBS/latest/actions/search-jobs-by-candidate-eligibility
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/uSAJOBS/latest/actions/search-jobs-by-candidate-eligibility?connectionId=$CONNECTION_ID&limit=25&offset=0" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/uSAJOBS/latest/actions/search-jobs-by-candidate-eligibility?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `whoMayApply` | string | no | Candidate designation such as Public. |
...
Through the native USAJOBS API, this operation is `GET /api/Search` (base URL `https://data.usajobs.gov`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/search-jobs-by-candidate-eligibility.md) for the provider-specific parameters and requirements.

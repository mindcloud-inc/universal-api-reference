# USAJOBS: Search Jobs By Position Title

Finds jobs in USAJOBS by position title.

```
GET https://connect.mindcloud.co/v1/universal/uSAJOBS/latest/actions/search-jobs-by-position-title
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/uSAJOBS/latest/actions/search-jobs-by-position-title?connectionId=$CONNECTION_ID&limit=25&offset=0" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/uSAJOBS/latest/actions/search-jobs-by-position-title?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `positionTitle` | string | no | Search by position title. |
...
Through the native USAJOBS API, this operation is `GET /api/Search` (base URL `https://data.usajobs.gov`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/search-jobs-by-position-title.md) for the provider-specific parameters and requirements.

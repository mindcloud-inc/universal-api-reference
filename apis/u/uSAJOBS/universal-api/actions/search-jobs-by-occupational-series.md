# USAJOBS: Search Jobs By Occupational Series

Finds jobs in USAJOBS by occupational series.

```
GET https://connect.mindcloud.co/v1/universal/uSAJOBS/latest/actions/search-jobs-by-occupational-series
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/uSAJOBS/latest/actions/search-jobs-by-occupational-series?connectionId=$CONNECTION_ID&limit=25&offset=0" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/uSAJOBS/latest/actions/search-jobs-by-occupational-series?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `jobCategoryCode` | string | no | Occupational series/job category code. |
...
Through the native USAJOBS API, this operation is `GET /api/Search` (base URL `https://data.usajobs.gov`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/search-jobs-by-occupational-series.md) for the provider-specific parameters and requirements.

# UK Check VAT Universal API Authentication

Every UK Check VAT API request through MindCloud uses a MindCloud API key and the `connectionId` of a UK Check VAT connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your UK Check VAT connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [UK Check VAT action](README.md#actions-2) with your API key and `connectionId`.

# UpGuard: Get Vendor Details

Retrieves details for a vendor in UpGuard.

```
GET https://connect.mindcloud.co/v1/universal/upGuard/latest/actions/get-vendor-details
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/upGuard/latest/actions/get-vendor-details?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/upGuard/latest/actions/get-vendor-details?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `hostname` | string | no | The hostname for which to return vendor data when no vendor ID is provided. |
...
Through the native UpGuard API, this operation is `GET /vendor` (base URL `https://cyber-risk.upguard.com/api/public`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-vendor-details.md) for the provider-specific parameters and requirements.

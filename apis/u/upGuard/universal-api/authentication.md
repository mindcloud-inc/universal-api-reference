# UpGuard Universal API Authentication

Every UpGuard API request through MindCloud uses a MindCloud API key and the `connectionId` of a UpGuard connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your UpGuard connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [UpGuard action](README.md#actions-40) with your API key and `connectionId`.

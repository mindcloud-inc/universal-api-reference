...
```
GET https://connect.mindcloud.co/v1/universal/nASAAPOD/latest/actions/get-astronomy-picture-of-the-day
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/nASAAPOD/latest/actions/get-astronomy-picture-of-the-day?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/nASAAPOD/latest/actions/get-astronomy-picture-of-the-day?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `date` | date | no | Optional APOD date in YYYY-MM-DD. Leave empty for today. |
...
Through the native NASA APOD API, this operation is `GET /planetary/apod` (base URL `https://api.nasa.gov`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-astronomy-picture-of-the-day.md) for the provider-specific parameters and requirements.

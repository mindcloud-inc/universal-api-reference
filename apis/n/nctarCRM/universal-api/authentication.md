# Néctar CRM Universal API Authentication

Every Néctar CRM API request through MindCloud uses a MindCloud API key and the `connectionId` of a Néctar CRM connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Néctar CRM connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Néctar CRM action](README.md#actions-21) with your API key and `connectionId`.

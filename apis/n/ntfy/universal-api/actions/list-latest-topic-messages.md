# ntfy: List Latest Topic Messages

Retrieves the latest message from an ntfy topic.

```
GET https://connect.mindcloud.co/v1/universal/ntfy/latest/actions/list-latest-topic-messages
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/ntfy/latest/actions/list-latest-topic-messages?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/ntfy/latest/actions/list-latest-topic-messages?${params}`, {
  headers: {
...
Through the native ntfy API, this operation is `GET /:topic/json` (base URL `https://ntfy.sh`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-latest-topic-messages.md) for the provider-specific parameters and requirements.

# ntfy: Publish Via GET

Publishes a message to an ntfy topic via GET.

```
POST https://connect.mindcloud.co/v1/universal/ntfy/latest/actions/publish-via-get
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/ntfy/latest/actions/publish-via-get" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/ntfy/latest/actions/publish-via-get', {
  method: 'POST',
  headers: {
...
Through the native ntfy API, this operation is `GET /:topic/publish` (base URL `https://ntfy.sh`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/publish-via-get.md) for the provider-specific parameters and requirements.

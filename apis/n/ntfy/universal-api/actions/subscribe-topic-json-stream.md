# ntfy: Subscribe Topic JSON Stream

Subscribes to an ntfy topic as a JSON stream.

```
GET https://connect.mindcloud.co/v1/universal/ntfy/latest/actions/subscribe-topic-json-stream
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/ntfy/latest/actions/subscribe-topic-json-stream?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/ntfy/latest/actions/subscribe-topic-json-stream?${params}`, {
  headers: {
...
Through the native ntfy API, this operation is `GET /:topic/json` (base URL `https://ntfy.sh`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/subscribe-topic-json-stream.md) for the provider-specific parameters and requirements.

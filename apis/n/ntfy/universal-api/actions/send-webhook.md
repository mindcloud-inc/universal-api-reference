...
Sends a GET webhook message to an ntfy topic.

```
POST https://connect.mindcloud.co/v1/universal/ntfy/latest/actions/send-webhook
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/ntfy/latest/actions/send-webhook" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/ntfy/latest/actions/send-webhook', {
  method: 'POST',
  headers: {
...
Through the native ntfy API, this operation is `GET /:topic/send` (base URL `https://ntfy.sh`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/send-webhook.md) for the provider-specific parameters and requirements.

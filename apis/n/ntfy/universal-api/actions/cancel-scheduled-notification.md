# ntfy: Cancel Scheduled Notification

Cancels a scheduled ntfy message by sequence ID.

```
DELETE https://connect.mindcloud.co/v1/universal/ntfy/latest/actions/cancel-scheduled-notification
```
...
```bash
curl -X DELETE "https://connect.mindcloud.co/v1/universal/ntfy/latest/actions/cancel-scheduled-notification?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/ntfy/latest/actions/cancel-scheduled-notification?${params}`, {
  headers: {
...
Through the native ntfy API, this operation is `DELETE /:topic/:sequence_id` (base URL `https://ntfy.sh`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/cancel-scheduled-notification.md) for the provider-specific parameters and requirements.

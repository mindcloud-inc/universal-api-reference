# ntfy: Trigger Webhook

Triggers an ntfy topic webhook via GET.

```
POST https://connect.mindcloud.co/v1/universal/ntfy/latest/actions/trigger-webhook
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/ntfy/latest/actions/trigger-webhook" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/ntfy/latest/actions/trigger-webhook', {
  method: 'POST',
  headers: {
...
Through the native ntfy API, this operation is `GET /:topic/trigger` (base URL `https://ntfy.sh`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/trigger-webhook.md) for the provider-specific parameters and requirements.

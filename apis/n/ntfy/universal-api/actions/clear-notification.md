# ntfy: Clear Notification

Clears an ntfy message by sequence ID.

```
DELETE https://connect.mindcloud.co/v1/universal/ntfy/latest/actions/clear-notification
```
...
```bash
curl -X DELETE "https://connect.mindcloud.co/v1/universal/ntfy/latest/actions/clear-notification?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/ntfy/latest/actions/clear-notification?${params}`, {
  headers: {
...
Through the native ntfy API, this operation is `PUT /:topic/:sequence_id/clear` (base URL `https://ntfy.sh`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/clear-notification.md) for the provider-specific parameters and requirements.

# NHTSA vPIC Universal API Authentication

Every NHTSA vPIC API request through MindCloud uses a MindCloud API key and the `connectionId` of a NHTSA vPIC connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your NHTSA vPIC connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [NHTSA vPIC action](README.md#actions-20) with your API key and `connectionId`.

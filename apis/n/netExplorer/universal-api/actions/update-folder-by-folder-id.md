...
```
PUT https://connect.mindcloud.co/v1/universal/netExplorer/latest/actions/update-folder-by-folder-id
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/netExplorer/latest/actions/update-folder-by-folder-id" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/netExplorer/latest/actions/update-folder-by-folder-id', {
  method: 'PUT',
  headers: {
...
Through the native NetExplorer API, this operation is `PUT /folder/:folderId` (base URL `{{credentials.platformBaseUrl}}/api`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-folder-by-folder-id.md) for the provider-specific parameters and requirements.

# NetExplorer: Update Provider



```
PUT https://connect.mindcloud.co/v1/universal/netExplorer/latest/actions/update-identity-provider-by-id
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/netExplorer/latest/actions/update-identity-provider-by-id" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/netExplorer/latest/actions/update-identity-provider-by-id', {
  method: 'PUT',
  headers: {
...
Through the native NetExplorer API, this operation is `PUT /identity/provider/:id` (base URL `{{credentials.platformBaseUrl}}/api`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-identity-provider-by-id.md) for the provider-specific parameters and requirements.

# NetExplorer: Get Log By Log ID



```
GET https://connect.mindcloud.co/v1/universal/netExplorer/latest/actions/get-log-by-log-id
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/netExplorer/latest/actions/get-log-by-log-id?connectionId=$CONNECTION_ID&logId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/netExplorer/latest/actions/get-log-by-log-id?${params}`, {
  headers: {
...
Through the native NetExplorer API, this operation is `GET /log/:logId` (base URL `{{credentials.platformBaseUrl}}/api`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-log-by-log-id.md) for the provider-specific parameters and requirements.

...
```
GET https://connect.mindcloud.co/v1/universal/netExplorer/latest/actions/get-workflow-instances
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/netExplorer/latest/actions/get-workflow-instances?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/netExplorer/latest/actions/get-workflow-instances?${params}`, {
  headers: {
...
Through the native NetExplorer API, this operation is `GET /workflow/instances` (base URL `{{credentials.platformBaseUrl}}/api`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-workflow-instances.md) for the provider-specific parameters and requirements.

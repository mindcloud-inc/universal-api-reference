# NetExplorer: Copy Template File



```
POST https://connect.mindcloud.co/v1/universal/netExplorer/latest/actions/create-template-file-copy
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/netExplorer/latest/actions/create-template-file-copy" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/netExplorer/latest/actions/create-template-file-copy', {
  method: 'POST',
  headers: {
...
Through the native NetExplorer API, this operation is `POST /template/file/copy` (base URL `{{credentials.platformBaseUrl}}/api`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-template-file-copy.md) for the provider-specific parameters and requirements.

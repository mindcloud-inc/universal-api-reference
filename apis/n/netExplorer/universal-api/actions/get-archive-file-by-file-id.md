# NetExplorer: Get File



```
GET https://connect.mindcloud.co/v1/universal/netExplorer/latest/actions/get-archive-file-by-file-id
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/netExplorer/latest/actions/get-archive-file-by-file-id?connectionId=$CONNECTION_ID&fileId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/netExplorer/latest/actions/get-archive-file-by-file-id?${params}`, {
  headers: {
...
Through the native NetExplorer API, this operation is `GET /archive/file/:fileId` (base URL `{{credentials.platformBaseUrl}}/api`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-archive-file-by-file-id.md) for the provider-specific parameters and requirements.

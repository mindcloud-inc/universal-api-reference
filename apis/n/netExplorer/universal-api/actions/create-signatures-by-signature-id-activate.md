# NetExplorer: Create Activate



```
POST https://connect.mindcloud.co/v1/universal/netExplorer/latest/actions/create-signatures-by-signature-id-activate
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/netExplorer/latest/actions/create-signatures-by-signature-id-activate" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/netExplorer/latest/actions/create-signatures-by-signature-id-activate', {
  method: 'POST',
  headers: {
...
Through the native NetExplorer API, this operation is `POST /signatures/:signatureId/activate` (base URL `{{credentials.platformBaseUrl}}/api`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-signatures-by-signature-id-activate.md) for the provider-specific parameters and requirements.

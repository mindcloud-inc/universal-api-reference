...
```
GET https://connect.mindcloud.co/v1/universal/netExplorer/latest/actions/get-file-activity
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/netExplorer/latest/actions/get-file-activity?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/netExplorer/latest/actions/get-file-activity?${params}`, {
  headers: {
...
Through the native NetExplorer API, this operation is `GET /file/activity` (base URL `{{credentials.platformBaseUrl}}/api`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-file-activity.md) for the provider-specific parameters and requirements.

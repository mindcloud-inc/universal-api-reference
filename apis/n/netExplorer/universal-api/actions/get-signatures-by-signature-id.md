...
```
GET https://connect.mindcloud.co/v1/universal/netExplorer/latest/actions/get-signatures-by-signature-id
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/netExplorer/latest/actions/get-signatures-by-signature-id?connectionId=$CONNECTION_ID&signatureId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/netExplorer/latest/actions/get-signatures-by-signature-id?${params}`, {
  headers: {
...
Through the native NetExplorer API, this operation is `GET /signatures/:signatureId` (base URL `{{credentials.platformBaseUrl}}/api`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-signatures-by-signature-id.md) for the provider-specific parameters and requirements.

...
```
GET https://connect.mindcloud.co/v1/universal/netExplorer/latest/actions/get-ulinks-emails
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/netExplorer/latest/actions/get-ulinks-emails?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/netExplorer/latest/actions/get-ulinks-emails?${params}`, {
  headers: {
...
Through the native NetExplorer API, this operation is `GET /ulinks/emails` (base URL `{{credentials.platformBaseUrl}}/api`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-ulinks-emails.md) for the provider-specific parameters and requirements.

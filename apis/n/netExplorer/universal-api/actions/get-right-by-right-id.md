# NetExplorer: Get Right By Right ID



```
GET https://connect.mindcloud.co/v1/universal/netExplorer/latest/actions/get-right-by-right-id
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/netExplorer/latest/actions/get-right-by-right-id?connectionId=$CONNECTION_ID&rightId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/netExplorer/latest/actions/get-right-by-right-id?${params}`, {
  headers: {
...
Through the native NetExplorer API, this operation is `GET /right/:rightId` (base URL `{{credentials.platformBaseUrl}}/api`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-right-by-right-id.md) for the provider-specific parameters and requirements.

# NetExplorer: Get Folder



```
GET https://connect.mindcloud.co/v1/universal/netExplorer/latest/actions/get-archive-folder-by-folder-id
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/netExplorer/latest/actions/get-archive-folder-by-folder-id?connectionId=$CONNECTION_ID&folderId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/netExplorer/latest/actions/get-archive-folder-by-folder-id?${params}`, {
  headers: {
...
Through the native NetExplorer API, this operation is `GET /archive/folder/:folderId` (base URL `{{credentials.platformBaseUrl}}/api`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-archive-folder-by-folder-id.md) for the provider-specific parameters and requirements.

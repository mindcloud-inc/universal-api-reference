...
```
GET https://connect.mindcloud.co/v1/universal/netExplorer/latest/actions/get-identity-provider-by-id
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/netExplorer/latest/actions/get-identity-provider-by-id?connectionId=$CONNECTION_ID&id=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/netExplorer/latest/actions/get-identity-provider-by-id?${params}`, {
  headers: {
...
Through the native NetExplorer API, this operation is `GET /identity/provider/:id` (base URL `{{credentials.platformBaseUrl}}/api`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-identity-provider-by-id.md) for the provider-specific parameters and requirements.

# Nutrient - PDF OCR Universal API Authentication

Every Nutrient - PDF OCR API request through MindCloud uses a MindCloud API key and the `connectionId` of a Nutrient - PDF OCR connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Nutrient - PDF OCR connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Nutrient - PDF OCR action](README.md#actions-1) with your API key and `connectionId`.

# NetSuite - Basic Universal API Pagination

Paginated list actions accept `limit` and `offset` as query parameters. MindCloud translates them into whatever pagination model NetSuite - Basic expects, so the request shape stays the same even when the native API uses pages or cursors.
...
## NetSuite - Basic actions that support pagination
...
- [List Non-Inventory Sale Items](actions/list-non-inventory-sale-items.md)
...
- [Test Connection](actions/test-connection.md)

# Nutrient - Convert to PDF Universal API Authentication

Every Nutrient - Convert to PDF API request through MindCloud uses a MindCloud API key and the `connectionId` of a Nutrient - Convert to PDF connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Nutrient - Convert to PDF connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Nutrient - Convert to PDF action](README.md#actions-6) with your API key and `connectionId`.

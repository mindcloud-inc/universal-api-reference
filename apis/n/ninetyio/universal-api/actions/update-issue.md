# Ninety.io: Update Issue

Updates an existing issue in Ninety.io.

```
PUT https://connect.mindcloud.co/v1/universal/ninetyio/latest/actions/update-issue
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/ninetyio/latest/actions/update-issue" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/ninetyio/latest/actions/update-issue', {
  method: 'PUT',
  headers: {
...
| `teamId` | string | no | Id of the team that owns the Issue |
| `interval` | string | no | Interval classification for the Issue |
| `description` | string | no | HTML description of the Issue |
| `priority` | number | no | Priority of the Issue: 0-5 |
| `completed` | boolean | no | Whether the Issue is completed |
...
Through the native Ninety.io API, this operation is `PATCH /v1/issues/:issueId` (base URL `https://api.public.ninety.io`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-issue.md) for the provider-specific parameters and requirements.

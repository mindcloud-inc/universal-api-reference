# Nationalize_io Universal API Authentication

Every Nationalize_io API request through MindCloud uses a MindCloud API key and the `connectionId` of a Nationalize_io connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Nationalize_io connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Nationalize_io action](README.md#actions-2) with your API key and `connectionId`.

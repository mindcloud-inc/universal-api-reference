# News API Universal API Authentication

Every News API API request through MindCloud uses a MindCloud API key and the `connectionId` of a News API connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your News API connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [News API action](README.md#actions-3) with your API key and `connectionId`.

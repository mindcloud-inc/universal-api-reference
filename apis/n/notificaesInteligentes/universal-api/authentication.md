# Notificações Inteligentes Universal API Authentication

Every Notificações Inteligentes API request through MindCloud uses a MindCloud API key and the `connectionId` of a Notificações Inteligentes connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Notificações Inteligentes connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Notificações Inteligentes action](README.md#actions-24) with your API key and `connectionId`.

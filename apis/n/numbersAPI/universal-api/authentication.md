# Numbers API Universal API Authentication

Every Numbers API API request through MindCloud uses a MindCloud API key and the `connectionId` of a Numbers API connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Numbers API connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Numbers API action](README.md#actions-5) with your API key and `connectionId`.

...
Retrieves a list of tags from Nicereply.

```
GET https://connect.mindcloud.co/v1/universal/nicereply/latest/actions/list-tags
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/nicereply/latest/actions/list-tags?connectionId=$CONNECTION_ID&limit=25&offset=0" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/nicereply/latest/actions/list-tags?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `sortOrder` | string | no | Sort tags by created_at using asc or desc. Default: `desc`. |

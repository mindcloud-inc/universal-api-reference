# Nicereply: Get Rating Values Settings

Retrieves rating value settings from Nicereply.

```
GET https://connect.mindcloud.co/v1/universal/nicereply/latest/actions/get-rating-values-settings
```
...
Through the native Nicereply API, this operation is `GET /rating-values` (base URL `https://api.nicereply.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-rating-values-settings.md) for the provider-specific parameters and requirements.

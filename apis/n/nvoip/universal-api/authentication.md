# Nvoip Universal API Authentication

Every Nvoip API request through MindCloud uses a MindCloud API key and the `connectionId` of a Nvoip connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Nvoip connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Nvoip action](README.md#actions-11) with your API key and `connectionId`.

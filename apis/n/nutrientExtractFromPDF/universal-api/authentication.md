# Nutrient - Extract from PDF Universal API Authentication

Every Nutrient - Extract from PDF API request through MindCloud uses a MindCloud API key and the `connectionId` of a Nutrient - Extract from PDF connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Nutrient - Extract from PDF connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Nutrient - Extract from PDF action](README.md#actions-5) with your API key and `connectionId`.

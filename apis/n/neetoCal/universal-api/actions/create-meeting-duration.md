# NeetoCal: Create Duration

Creates a new meeting duration in NeetoCal.

```
POST https://connect.mindcloud.co/v1/universal/neetoCal/latest/actions/create-meeting-duration
```
...
Through the native NeetoCal API, this operation is `POST /meetings/:meeting_sid/durations` (base URL `https://{{credentials.subdomain}}.neetocal.com/api/external/v2`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-meeting-duration.md) for the provider-specific parameters and requirements.

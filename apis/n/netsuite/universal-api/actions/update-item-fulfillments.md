# NetSuite - Advanced: Update Item Fulfillments
...
```
PUT https://connect.mindcloud.co/v1/universal/netsuite/latest/actions/update-item-fulfillments
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/netsuite/latest/actions/update-item-fulfillments" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/netsuite/latest/actions/update-item-fulfillments', {
  method: 'PUT',
  headers: {
...
Through the native NetSuite - Advanced API, this operation is `POST https://{{credentials.accountId}}.restlets.api.netsuite.com/app/site/hosting/restlet.nl` (base URL `https://{{credentials.accountId}}.suitetalk.api.netsuite.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-item-fulfillments.md) for the provider-specific parameters and requirements.

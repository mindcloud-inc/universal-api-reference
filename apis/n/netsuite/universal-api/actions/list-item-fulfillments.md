# NetSuite - Advanced: List Item Fulfillments
...
```
GET https://connect.mindcloud.co/v1/universal/netsuite/latest/actions/list-item-fulfillments
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/netsuite/latest/actions/list-item-fulfillments?connectionId=$CONNECTION_ID&limit=25&offset=0" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/netsuite/latest/actions/list-item-fulfillments?${params}`, {
  headers: {
...
Through the native NetSuite - Advanced API, this operation is `POST https://{{credentials.accountId}}.restlets.api.netsuite.com/app/site/hosting/restlet.nl` (base URL `https://{{credentials.accountId}}.suitetalk.api.netsuite.com`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/list-item-fulfillments.md) for the provider-specific parameters and requirements.

...
```
GET https://connect.mindcloud.co/v1/universal/ninox/latest/actions/download-file
```
...
Through the native Ninox API, this operation is `GET teams/:teamId/databases/:dbId/tables/:tableId/records/:recordId/files/:file` (base URL `https://api.ninox.com/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/download-file.md) for the provider-specific parameters and requirements.

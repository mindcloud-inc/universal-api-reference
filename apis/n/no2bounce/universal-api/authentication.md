# no2bounce Universal API Authentication

Every no2bounce API request through MindCloud uses a MindCloud API key and the `connectionId` of a no2bounce connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your no2bounce connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [no2bounce action](README.md#actions-2) with your API key and `connectionId`.

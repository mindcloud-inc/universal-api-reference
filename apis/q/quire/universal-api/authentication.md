# Quire Universal API Authentication

Every Quire API request through MindCloud uses a MindCloud API key and the `connectionId` of a Quire connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Quire connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Quire action](README.md#actions-21) with your API key and `connectionId`.

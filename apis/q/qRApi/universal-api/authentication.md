# QR Api Universal API Authentication

Every QR Api API request through MindCloud uses a MindCloud API key and the `connectionId` of a QR Api connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your QR Api connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [QR Api action](README.md#actions-8) with your API key and `connectionId`.

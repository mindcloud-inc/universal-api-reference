# Qualiobee: Create Learner

Creates a new learner in Qualiobee.

```
POST https://connect.mindcloud.co/v1/universal/qualiobee/latest/actions/create-learner
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/qualiobee/latest/actions/create-learner" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/qualiobee/latest/actions/create-learner', {
  method: 'POST',
  headers: {
...
Through the native Qualiobee API, this operation is `POST /:organizationUuid/learner` (base URL `https://app.qualiobee.fr/api`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-learner.md) for the provider-specific parameters and requirements.

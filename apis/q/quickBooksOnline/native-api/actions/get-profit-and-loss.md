...
| `accounting_method` | query | `list` | no | Accounting basis to use for the report, such as Cash or Accrual. Accepted values: `Accrual`, `Cash`. |
| `summarize_column_by` | query | `list` | no | Column grouping for the report, such as Total, Month, or Customers. Accepted values: `Classes`, `Customers`, `Days`, `Departments`, `Month`, `Total`, `Vendors`, `Week`. |

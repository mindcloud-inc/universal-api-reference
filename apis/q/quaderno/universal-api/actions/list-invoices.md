# Quaderno: List Invoices

Retrieves invoices from Quaderno.

```
GET https://connect.mindcloud.co/v1/universal/quaderno/latest/actions/list-invoices
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/quaderno/latest/actions/list-invoices?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/quaderno/latest/actions/list-invoices?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `contact` | string | no | Filter invoices by contact ID. |
| `date` | string | no | Filter invoices by issue date range. |
| `processorId` | string | no | Filter invoices by processor ID. |
| `q` | string | no | Filter invoices by invoice number, customer name, or PO number. |
| `state` | string | no | Filter invoices by state. |
...
Through the native Quaderno API, this operation is `GET /invoices` (base URL `https://sandbox-quadernoapp.com/api`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-invoices.md) for the provider-specific parameters and requirements.

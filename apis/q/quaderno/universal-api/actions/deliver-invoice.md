# Quaderno: Deliver Invoice

Delivers an invoice to the customer by email in Quaderno.

```
GET https://connect.mindcloud.co/v1/universal/quaderno/latest/actions/deliver-invoice
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/quaderno/latest/actions/deliver-invoice?connectionId=$CONNECTION_ID&id=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/quaderno/latest/actions/deliver-invoice?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `id` | string | yes | The ID of the invoice to deliver. |
...
Through the native Quaderno API, this operation is `GET /invoices/:id/deliver` (base URL `https://sandbox-quadernoapp.com/api`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/deliver-invoice.md) for the provider-specific parameters and requirements.

# Qntrl Universal API Authentication

Every Qntrl API request through MindCloud uses a MindCloud API key and the `connectionId` of a Qntrl connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Qntrl connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Qntrl action](README.md#actions-42) with your API key and `connectionId`.

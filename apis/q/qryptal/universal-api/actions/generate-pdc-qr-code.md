# Qryptal: Generate PDC QR Code



```
POST https://connect.mindcloud.co/v1/universal/qryptal/latest/actions/generate-pdc-qr-code
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/qryptal/latest/actions/generate-pdc-qr-code" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/qryptal/latest/actions/generate-pdc-qr-code', {
  method: 'POST',
  headers: {
...
Through the native Qryptal API, this operation is `POST /` (base URL `https://api2test.qryptal.com/v2/Vqodes/v2/Vqodes/`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/generate-pdc-qr-code.md) for the provider-specific parameters and requirements.

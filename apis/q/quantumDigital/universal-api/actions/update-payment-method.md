# Quantum Digital: Update Payment Method



```
PUT https://connect.mindcloud.co/v1/universal/quantumDigital/latest/actions/update-payment-method
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/quantumDigital/latest/actions/update-payment-method" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/quantumDigital/latest/actions/update-payment-method', {
  method: 'PUT',
  headers: {
...
| --- | --- | --- |
| `success` | boolean | Whether the payment method was updated successfully. |

## Native endpoint

Through the native Quantum Digital API, this operation is `PUT /devplatform/billing/:dashboardAccountId/paymentmethods` (base URL `https://api.quantumdigital.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-payment-method.md) for the provider-specific parameters and requirements.

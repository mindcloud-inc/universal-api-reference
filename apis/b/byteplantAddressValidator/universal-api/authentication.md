# Byteplant Address Validator Universal API Authentication

Every Byteplant Address Validator API request through MindCloud uses a MindCloud API key and the `connectionId` of a Byteplant Address Validator connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Byteplant Address Validator connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Byteplant Address Validator action](README.md#actions-4) with your API key and `connectionId`.

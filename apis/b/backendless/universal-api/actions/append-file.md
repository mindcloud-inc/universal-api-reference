# Backendless: Append File

Appends content to a file in Backendless.

```
PUT https://connect.mindcloud.co/v1/universal/backendless/latest/actions/append-file
```
...
Through the native Backendless API, this operation is `PUT /{{credentials.applicationId}}/{{credentials.apiKey}}/files/append/{path}` (base URL `{{credentials.apiUrl}}`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/append-file.md) for the provider-specific parameters and requirements.

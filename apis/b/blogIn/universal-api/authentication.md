# BlogIn Universal API Authentication

Every BlogIn API request through MindCloud uses a MindCloud API key and the `connectionId` of a BlogIn connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your BlogIn connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [BlogIn action](README.md#actions-30) with your API key and `connectionId`.

# BILL Payables & Receivables: Create Record

Creates a record in Bill.com.

```
POST https://connect.mindcloud.co/v1/universal/billcom/latest/actions/create-record
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/billcom/latest/actions/create-record" \
  -H "Authorization: Bearer $MINDCLOUD_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
  "connectionId": "$CONNECTION_ID",
  "recordType": "PurchaseOrder.json"
}'
```

```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/billcom/latest/actions/create-record', {
  method: 'POST',
  headers: {
...
| --- | --- | --- | --- |
| `recordType` | string | yes | Example: `PurchaseOrder.json`. |
...
Through the native BILL Payables & Receivables API, this operation is `POST Crud/Create/:recordType` (base URL `https://api.bill.com/api/v2`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/create-record.md) for the provider-specific parameters and requirements.

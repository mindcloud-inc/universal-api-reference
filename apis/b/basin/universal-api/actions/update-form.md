...
```
PUT https://connect.mindcloud.co/v1/universal/basin/latest/actions/update-form
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/basin/latest/actions/update-form" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/basin/latest/actions/update-form', {
  method: 'PUT',
  headers: {
...
| `form` | object | no | Form fields to update. |
| `form.name` | string | no | New form name. |
| `form.project_id` | number | no | Project that owns the form. |

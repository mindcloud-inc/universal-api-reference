# Bumpups: Condition Assessment

Creates a condition assessment from a video in Bumpups.

```
POST https://connect.mindcloud.co/v1/universal/bumpups/latest/actions/condition-assessment
```
...
Through the native Bumpups API, this operation is `POST /chat` (base URL `https://api.bumpups.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/condition-assessment.md) for the provider-specific parameters and requirements.

...
```
GET https://connect.mindcloud.co/v1/universal/belong/latest/actions/list-hubs
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/belong/latest/actions/list-hubs?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/belong/latest/actions/list-hubs?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `hubType` | list | no | One of: `Group`, `NFT Collection`. |
| `status` | list | no | One of: `Incomplete`, `Published`. |
| `source` | list | no | One of: `API`, `App`, `External`, `MCP`. |
| `private` | boolean | no |  |
| `ownerId` | string | no | Example: `69d45f6c6e0cc96c27ed7be4`. |
| `memberId` | string | no | Example: `69d45f6c6e0cc96c27ed7be4`. |
| `search` | string | no | Example: `community`. |
| `sort` | list | no | One of: `Created At`, `Updated At`. |
| `order` | list | no | One of: `Ascending`, `Descending`. |
| `fields[]` | array<string> | no | Accepts multiple values in one string, delimited by `,`. |
...
Through the native Belong API, this operation is `GET /hubs` (base URL `https://api.belong.net/api/v3`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-hubs.md) for the provider-specific parameters and requirements.

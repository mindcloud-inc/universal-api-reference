# Buildkite Universal API Authentication

Every Buildkite API request through MindCloud uses a MindCloud API key and the `connectionId` of a Buildkite connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Buildkite connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Buildkite action](README.md#actions-34) with your API key and `connectionId`.

# boomApp Connect Universal API Authentication

Every boomApp Connect API request through MindCloud uses a MindCloud API key and the `connectionId` of a boomApp Connect connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your boomApp Connect connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [boomApp Connect action](README.md#actions-9) with your API key and `connectionId`.

# boomApp Connect: Send SMS From Custom Number

Creates an SMS message from a custom number in boomApp Connect.

```
POST https://connect.mindcloud.co/v1/universal/boomAppConnect/latest/actions/send-sms-from-custom-number
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/boomAppConnect/latest/actions/send-sms-from-custom-number" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/boomAppConnect/latest/actions/send-sms-from-custom-number', {
  method: 'POST',
  headers: {
...
| --- | --- | --- | --- |
| `from` | string | yes | Numeric sending ID associated to the message. Required for successful runtime submission. Example: `447890123456`. |
...
Through the native boomApp Connect API, this operation is `POST /v1/flat/sms3` (base URL `https://direct-api.apps.boomcomms.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/send-sms-from-custom-number.md) for the provider-specific parameters and requirements.

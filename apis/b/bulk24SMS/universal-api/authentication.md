# Bulk24SMS Universal API Authentication

Every Bulk24SMS API request through MindCloud uses a MindCloud API key and the `connectionId` of a Bulk24SMS connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Bulk24SMS connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Bulk24SMS action](README.md#actions-1) with your API key and `connectionId`.

# Browse AI: List Robot Tasks

Retrieves robot tasks from Browse AI.

```
GET https://connect.mindcloud.co/v1/universal/browseAI/latest/actions/list-robot-tasks
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/browseAI/latest/actions/list-robot-tasks?connectionId=$CONNECTION_ID&robotId=c3689adb-50aa-44af-b265-a7e0d4e5846e" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/browseAI/latest/actions/list-robot-tasks?${params}`, {
  headers: {
...
| `status` | list | no | Task status One of: `failed`, `in-progress`, `successful`. Example: `successful`. |
| `robotBulkRunId` | string | no | filter the result based on robot bulk run ID Example: `f6fb62b6-f06a-4bf7-a623-c6a35c2e70b0`. |
| `includeRetried` | boolean | no | by passing false you can exclude the retried tasks Example: `false`. |
| `fromDate` | number | no | From task creation date and time in the form of a Unix timestamp Example: `1678795867879`. |
| `toDate` | number | no | To task creation date and time in the form of a Unix timestamp Example: `1678795867879`. |
...
| `pageSize` | number | no | Page size Example: `10`. |
| `sort` | string | no | A comma separated list of fields to sort by. Default sorting is ascending and prefixing field names with a hyphen '-' yields a descending order. Example: `-createdAt,finishedAt`. |
...
Through the native Browse AI API, this operation is `GET /robots/:robotId/tasks` (base URL `https://api.browse.ai/v2`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/list-robot-tasks.md) for the provider-specific parameters and requirements.

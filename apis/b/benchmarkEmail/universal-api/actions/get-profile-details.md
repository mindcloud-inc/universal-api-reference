# Benchmark Email: Get Profile Details

Retrieves client profile details from Benchmark Email.

```
GET https://connect.mindcloud.co/v1/universal/benchmarkEmail/latest/actions/get-profile-details
```
...
Through the native Benchmark Email API, this operation is `GET /Client/ProfileDetails` (base URL `https://clientapi.benchmarkemail.com`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-profile-details.md) for the provider-specific parameters and requirements.

# Benchmark Email Universal API Authentication

Every Benchmark Email API request through MindCloud uses a MindCloud API key and the `connectionId` of a Benchmark Email connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Benchmark Email connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Benchmark Email action](README.md#actions-24) with your API key and `connectionId`.

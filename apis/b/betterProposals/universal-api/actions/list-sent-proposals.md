# Better Proposals: List Sent Proposals

Retrieves sent proposals from Better Proposals.

```
GET https://connect.mindcloud.co/v1/universal/betterProposals/latest/actions/list-sent-proposals
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/betterProposals/latest/actions/list-sent-proposals?connectionId=$CONNECTION_ID&limit=25&offset=0" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/betterProposals/latest/actions/list-sent-proposals?${params}`, {
  headers: {
...
Through the native Better Proposals API, this operation is `GET /proposal/sent` (base URL `https://api.betterproposals.io`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/list-sent-proposals.md) for the provider-specific parameters and requirements.

# BigCommerce (B2B) Universal API Authentication

Every BigCommerce (B2B) API request through MindCloud uses a MindCloud API key and the `connectionId` of a BigCommerce (B2B) connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your BigCommerce (B2B) connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [BigCommerce (B2B) action](README.md#actions-14) with your API key and `connectionId`.

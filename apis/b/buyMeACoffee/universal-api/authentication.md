# Buy Me a Coffee Universal API Authentication

Every Buy Me a Coffee API request through MindCloud uses a MindCloud API key and the `connectionId` of a Buy Me a Coffee connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Buy Me a Coffee connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Buy Me a Coffee action](README.md#actions-6) with your API key and `connectionId`.

# Better Stack Uptime: Get Heartbeat

Retrieves a heartbeat from Better Stack Uptime.

```
GET https://connect.mindcloud.co/v1/universal/betterStackUptime/latest/actions/get-heartbeat
```
...
Through the native Better Stack Uptime API, this operation is `GET /v2/heartbeats/:heartbeat_id` (base URL `https://uptime.betterstack.com/api`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-heartbeat.md) for the provider-specific parameters and requirements.

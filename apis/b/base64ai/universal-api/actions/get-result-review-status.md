# Base64.ai: Get Result Review Status

Retrieves a result and review status from Base64.ai.

```
GET https://connect.mindcloud.co/v1/universal/base64ai/latest/actions/get-result-review-status
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/base64ai/latest/actions/get-result-review-status?connectionId=$CONNECTION_ID&resultUuid=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/base64ai/latest/actions/get-result-review-status?${params}`, {
  headers: {
...
Through the native Base64.ai API, this operation is `GET /api/result/:resultUuid` (base URL `https://base64.ai`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-result-review-status.md) for the provider-specific parameters and requirements.

# Base64.ai: Scan Document into Flow

Creates a document scan result in a Base64.ai flow.

```
POST https://connect.mindcloud.co/v1/universal/base64ai/latest/actions/scan-document-into-flow
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/base64ai/latest/actions/scan-document-into-flow" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/base64ai/latest/actions/scan-document-into-flow', {
  method: 'POST',
  headers: {
...
| --- | --- | --- | --- |
| `url` | string | yes | Public URL of the document to scan into the selected flow. |
| `flowId` | string | yes | Base64.ai flow ID to attach to the scan request header. |
...
Through the native Base64.ai API, this operation is `POST /api/scan` (base URL `https://base64.ai`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/scan-document-into-flow.md) for the provider-specific parameters and requirements.

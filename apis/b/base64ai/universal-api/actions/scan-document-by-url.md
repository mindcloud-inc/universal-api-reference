# Base64.ai: Scan Document by URL

Creates a document scan result in Base64.ai from a URL.

```
POST https://connect.mindcloud.co/v1/universal/base64ai/latest/actions/scan-document-by-url
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/base64ai/latest/actions/scan-document-by-url" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/base64ai/latest/actions/scan-document-by-url', {
  method: 'POST',
  headers: {
...
| --- | --- | --- | --- |
| `url` | string | yes | Public URL of the document to scan. |
...
Through the native Base64.ai API, this operation is `POST /api/scan` (base URL `https://base64.ai`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/scan-document-by-url.md) for the provider-specific parameters and requirements.

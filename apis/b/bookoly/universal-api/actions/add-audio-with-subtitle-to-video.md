# Bookoly: Add Audio With Subtitle To Video

Adds audio with subtitles to a video in Bookoly.

```
POST https://connect.mindcloud.co/v1/universal/bookoly/latest/actions/add-audio-with-subtitle-to-video
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/bookoly/latest/actions/add-audio-with-subtitle-to-video" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/bookoly/latest/actions/add-audio-with-subtitle-to-video', {
  method: 'POST',
  headers: {
...
Through the native Bookoly API, this operation is `POST /add-audio-with-subtitle-to-video` (base URL `https://bookoly.com/api/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/add-audio-with-subtitle-to-video.md) for the provider-specific parameters and requirements.

# Byteplant Email Validator Universal API Authentication

Every Byteplant Email Validator API request through MindCloud uses a MindCloud API key and the `connectionId` of a Byteplant Email Validator connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Byteplant Email Validator connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Byteplant Email Validator action](README.md#actions-2) with your API key and `connectionId`.

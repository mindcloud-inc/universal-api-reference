# BotHelp: Add Messenger Subscriber To Funnel

Adds a subscriber to a funnel by Facebook Messenger user ID in BotHelp.

```
PUT https://connect.mindcloud.co/v1/universal/botHelp/latest/actions/add-messenger-subscriber-to-funnel
```
...
Through the native BotHelp API, this operation is `POST /v2/subscribers/messenger/:messenger_user_id/funnel` (base URL `https://api.bothelp.io`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/add-messenger-subscriber-to-funnel.md) for the provider-specific parameters and requirements.

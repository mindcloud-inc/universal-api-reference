...
```
GET https://connect.mindcloud.co/v1/universal/booqable/latest/actions/list-items
```
...
| `filter` | object | no | Provider-specific item filters. |
...
Through the native Booqable API, this operation is `GET /items` (base URL `https://mindcloud.booqable.com/api/4`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/list-items.md) for the provider-specific parameters and requirements.

# Booqable: Search Customers

Finds customers in Booqable by search criteria.

```
GET https://connect.mindcloud.co/v1/universal/booqable/latest/actions/search-customers
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/booqable/latest/actions/search-customers?connectionId=$CONNECTION_ID&limit=25&offset=0" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/booqable/latest/actions/search-customers?${params}`, {
  headers: {
...
| `filter` | object | no | Advanced customer search filter object using Booqable's logical conditions syntax. |
...
Through the native Booqable API, this operation is `POST /customers/search` (base URL `https://mindcloud.booqable.com/api/4`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/search-customers.md) for the provider-specific parameters and requirements.

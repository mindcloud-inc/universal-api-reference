# Buttondown: Update Draft Email

Updates an existing draft email in Buttondown.

```
PUT https://connect.mindcloud.co/v1/universal/buttondown/latest/actions/update-draft-email
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/buttondown/latest/actions/update-draft-email" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/buttondown/latest/actions/update-draft-email', {
  method: 'PUT',
  headers: {
...
| `subject` | string | no | Updated draft email subject line. |
| `body` | string | no | Updated draft email body content. |
| `description` | string | no | Updated internal description for the draft email. |
| `slug` | string | no | Updated draft slug. |
| `canonical_url` | string | no | Updated canonical URL for the email. |
...
Through the native Buttondown API, this operation is `PATCH /emails/:id` (base URL `https://api.buttondown.com/v1`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-draft-email.md) for the provider-specific parameters and requirements.

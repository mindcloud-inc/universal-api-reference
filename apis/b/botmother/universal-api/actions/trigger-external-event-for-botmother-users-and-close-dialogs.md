# Botmother: Trigger External Event For Botmother Users And Close Dialogs

Triggers an external event in Botmother for users by bm_id and closes chats.

```
POST https://connect.mindcloud.co/v1/universal/botmother/latest/actions/trigger-external-event-for-botmother-users-and-close-dialogs
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/botmother/latest/actions/trigger-external-event-for-botmother-users-and-close-dialogs" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/botmother/latest/actions/trigger-external-event-for-botmother-users-and-close-dialogs', {
  method: 'POST',
  headers: {
...
| --- | --- | --- | --- |
| `usersBm[]` | array<string> | yes | Botmother bm_id values to trigger. |
...
Through the native Botmother API, this operation is `POST /` (base URL `{{credentials.eventUrl}}`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/trigger-external-event-for-botmother-users-and-close-dialogs.md) for the provider-specific parameters and requirements.

# Botmother: Trigger External Event For Botmother Users

Triggers an external event in Botmother for users by bm_id.

```
POST https://connect.mindcloud.co/v1/universal/botmother/latest/actions/trigger-external-event-for-botmother-users
```
...
Through the native Botmother API, this operation is `POST /` (base URL `{{credentials.eventUrl}}`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/trigger-external-event-for-botmother-users.md) for the provider-specific parameters and requirements.

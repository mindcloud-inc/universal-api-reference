# Botmother: Trigger External Event For Platform Users And Close Dialogs

Triggers an external event in Botmother for platform users and closes chats.

```
POST https://connect.mindcloud.co/v1/universal/botmother/latest/actions/trigger-external-event-for-platform-users-and-close-dialogs
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/botmother/latest/actions/trigger-external-event-for-platform-users-and-close-dialogs" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/botmother/latest/actions/trigger-external-event-for-platform-users-and-close-dialogs', {
  method: 'POST',
  headers: {
...
| --- | --- | --- | --- |
| `platform` | string | yes | Target platform: tg, fb, viber, vk, or any. |
| `users[]` | array<string> | yes | Platform user IDs to trigger. |
...
Through the native Botmother API, this operation is `POST /` (base URL `{{credentials.eventUrl}}`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/trigger-external-event-for-platform-users-and-close-dialogs.md) for the provider-specific parameters and requirements.

# Botmother: Trigger External Event For Platform Users

Triggers an external event in Botmother for platform users.

```
POST https://connect.mindcloud.co/v1/universal/botmother/latest/actions/trigger-external-event-for-platform-users
```
...
```bash
curl -X POST "https://connect.mindcloud.co/v1/universal/botmother/latest/actions/trigger-external-event-for-platform-users" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/botmother/latest/actions/trigger-external-event-for-platform-users', {
  method: 'POST',
  headers: {
...
Through the native Botmother API, this operation is `POST /` (base URL `{{credentials.eventUrl}}`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/trigger-external-event-for-platform-users.md) for the provider-specific parameters and requirements.

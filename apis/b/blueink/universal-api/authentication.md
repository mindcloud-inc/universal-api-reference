# Blueink Universal API Authentication

Every Blueink API request through MindCloud uses a MindCloud API key and the `connectionId` of a Blueink connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Blueink connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Blueink action](README.md#actions-24) with your API key and `connectionId`.

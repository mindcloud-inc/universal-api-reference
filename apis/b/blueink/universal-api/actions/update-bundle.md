# Blueink: Update Bundle

Updates an existing bundle in Blueink.

```
PUT https://connect.mindcloud.co/v1/universal/blueink/latest/actions/update-bundle
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/blueink/latest/actions/update-bundle" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/blueink/latest/actions/update-bundle', {
  method: 'PUT',
  headers: {
...
| --- | --- | --- | --- |
| `bundleSlug` | string | yes | Bundle slug to update. |
| `ccSender` | boolean | no | Whether to cc the sender when the bundle completes. |
...
Through the native Blueink API, this operation is `PATCH /bundles/:bundleSlug/` (base URL `https://api.blueink.com/api/v2`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/update-bundle.md) for the provider-specific parameters and requirements.

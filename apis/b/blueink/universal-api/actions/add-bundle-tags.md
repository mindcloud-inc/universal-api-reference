# Blueink: Add Bundle Tags

Adds tags to a Blueink bundle.

```
PUT https://connect.mindcloud.co/v1/universal/blueink/latest/actions/add-bundle-tags
```
...
| --- | --- | --- | --- |
| `bundleSlug` | string | yes | Bundle slug to add tags to. |
| `tags[]` | array<string> | yes | Tags to add to the bundle. |
...
Through the native Blueink API, this operation is `PUT /bundles/:bundleSlug/add_tags/` (base URL `https://api.blueink.com/api/v2`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/add-bundle-tags.md) for the provider-specific parameters and requirements.

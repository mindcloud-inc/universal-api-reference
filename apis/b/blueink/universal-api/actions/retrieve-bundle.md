# Blueink: Retrieve Bundle

Retrieves an existing bundle from Blueink.

```
GET https://connect.mindcloud.co/v1/universal/blueink/latest/actions/retrieve-bundle
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/blueink/latest/actions/retrieve-bundle?connectionId=$CONNECTION_ID&bundleSlug=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/blueink/latest/actions/retrieve-bundle?${params}`, {
  headers: {
...
| --- | --- | --- | --- |
| `bundleSlug` | string | yes | Bundle slug to retrieve. |
...
Through the native Blueink API, this operation is `GET /bundles/:bundleSlug/` (base URL `https://api.blueink.com/api/v2`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/retrieve-bundle.md) for the provider-specific parameters and requirements.

# Blueink: Cancel Bundle

Cancels an existing bundle in Blueink.

```
PUT https://connect.mindcloud.co/v1/universal/blueink/latest/actions/cancel-bundle
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/blueink/latest/actions/cancel-bundle" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/blueink/latest/actions/cancel-bundle', {
  method: 'PUT',
  headers: {
...
| --- | --- | --- | --- |
| `bundleSlug` | string | yes | Bundle slug to cancel. |
...
Through the native Blueink API, this operation is `PUT /bundles/:bundleSlug/cancel/` (base URL `https://api.blueink.com/api/v2`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/cancel-bundle.md) for the provider-specific parameters and requirements.

# Blueink: Remove Bundle Tags

Removes tags from a Blueink bundle.

```
PUT https://connect.mindcloud.co/v1/universal/blueink/latest/actions/remove-bundle-tags
```
...
```bash
curl -X PUT "https://connect.mindcloud.co/v1/universal/blueink/latest/actions/remove-bundle-tags" \
...
```js
const response = await fetch('https://connect.mindcloud.co/v1/universal/blueink/latest/actions/remove-bundle-tags', {
  method: 'PUT',
  headers: {
...
| --- | --- | --- | --- |
| `bundleSlug` | string | yes | Bundle slug to remove tags from. |
| `tags[]` | array<string> | yes | Tags to remove from the bundle. |
...
Through the native Blueink API, this operation is `PUT /bundles/:bundleSlug/remove_tags/` (base URL `https://api.blueink.com/api/v2`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/remove-bundle-tags.md) for the provider-specific parameters and requirements.

# BlockSurvey: Get Survey Cut Off Date

Retrieves a survey cut off date from BlockSurvey.

```
GET https://connect.mindcloud.co/v1/universal/blockSurvey/latest/actions/get-survey-cut-off-date
```
...
Through the native BlockSurvey API, this operation is `GET /v1/survey/cut_off_date` (base URL `https://api3.blocksurvey.io`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/get-survey-cut-off-date.md) for the provider-specific parameters and requirements.

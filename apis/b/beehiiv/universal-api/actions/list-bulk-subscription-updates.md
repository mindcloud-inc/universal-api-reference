# Beehiiv: List Bulk Subscription Updates

Retrieves bulk subscription updates from Beehiiv.

```
GET https://connect.mindcloud.co/v1/universal/beehiiv/latest/actions/list-bulk-subscription-updates
```
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/beehiiv/latest/actions/list-bulk-subscription-updates?${params}`, {
  headers: {
...
Through the native Beehiiv API, this operation is `GET /v2/publications/:publicationId/bulk_subscription_updates` (base URL `https://api.beehiiv.com`). The Universal API call above is translated to it by MindCloud, including authentication and pagination. See the [native action reference](../../native-api/actions/list-bulk-subscription-updates.md) for the provider-specific parameters and requirements.

# Beehiiv: Get Subscription by ID

Retrieves a subscription from Beehiiv by ID.

```
GET https://connect.mindcloud.co/v1/universal/beehiiv/latest/actions/get-subscription-by-id
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/beehiiv/latest/actions/get-subscription-by-id?connectionId=$CONNECTION_ID&publicationId=string&subscriptionId=string" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/beehiiv/latest/actions/get-subscription-by-id?${params}`, {
  headers: {

# Blip: Delete Answer from Intent

Deletes an answer from an intent in Blip.

```
DELETE https://connect.mindcloud.co/v1/universal/blip/latest/actions/delete-answer-from-intent
```
...
```bash
curl -X DELETE "https://connect.mindcloud.co/v1/universal/blip/latest/actions/delete-answer-from-intent?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/blip/latest/actions/delete-answer-from-intent?${params}`, {
  headers: {
...
Through the native Blip API, this operation is `POST /commands` (base URL `https://ae2bd556-b116-4f7a-a0d3-30a54ef5b9d7.http.msging.net`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/delete-answer-from-intent.md) for the provider-specific parameters and requirements.

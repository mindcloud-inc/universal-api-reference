...
Retrieves a ping response from Blip.

```
GET https://connect.mindcloud.co/v1/universal/blip/latest/actions/ping
```
...
```bash
curl -X GET "https://connect.mindcloud.co/v1/universal/blip/latest/actions/ping?connectionId=$CONNECTION_ID" \
...
const response = await fetch(`https://connect.mindcloud.co/v1/universal/blip/latest/actions/ping?${params}`, {
  headers: {
...
Through the native Blip API, this operation is `POST /commands` (base URL `https://ae2bd556-b116-4f7a-a0d3-30a54ef5b9d7.http.msging.net`). The Universal API call above is translated to it by MindCloud, including authentication. See the [native action reference](../../native-api/actions/ping.md) for the provider-specific parameters and requirements.

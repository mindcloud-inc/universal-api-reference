# Bitbucket Universal API Authentication

Every Bitbucket API request through MindCloud uses a MindCloud API key and the `connectionId` of a Bitbucket connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Bitbucket connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Bitbucket action](README.md#actions-52) with your API key and `connectionId`.

# Bump.sh Universal API Authentication

Every Bump.sh API request through MindCloud uses a MindCloud API key and the `connectionId` of a Bump.sh connection.
...
3. Open [Connections](https://app.mindcloud.co/credentials) and create your Bump.sh connection.
4. Close the connection modal, open the connection’s `•••` menu, and select **Copy Connection ID**.
5. Call any [Bump.sh action](README.md#actions-15) with your API key and `connectionId`.

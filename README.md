...
The API surface of **3,207 apps** — **75,906 endpoints** — documented in one uniform, greppable format. Every app gets the same treatment: authentication, pagination, filtering, sorting, argument conventions, response formats, and a mapping back to the vendor's native API. One file per endpoint.
...
- [`catalog.json`](catalog.json) — machine-readable catalog of all 3,207 apps, with per-letter shards under [`catalog/`](catalog).
...
- **Unified API / MCP** — if you'd rather call all 3,207 apps through one REST signature and auth model, that's [the hosted thing](https://mindcloud.co/docs/universal/rest/home/latest).

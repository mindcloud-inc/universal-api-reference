...
The API surface of **3,203 apps** — **75,773 endpoints** — documented in one uniform, greppable format. Every app gets the same treatment: authentication, pagination, filtering, sorting, argument conventions, response formats, and a mapping back to the vendor's native API. One file per endpoint.
...
- [`catalog.json`](catalog.json) — machine-readable catalog of all 3,203 apps, with per-letter shards under [`catalog/`](catalog).
